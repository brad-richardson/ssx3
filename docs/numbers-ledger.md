...
| M3 census (in progress) | Snow Jam update 3.08 / render 6.30 ms, Metro City 5.23 / 8.41, Ruthless Ridge 7.62 / 14.80, Intimidator 3.14 / 6.69 (medians, desktop 640x528, Metal validation off, 183 s legs, rode=true) | 09-18 | Mac, census player, host lease M3 (M1d -j4 build may overlap) | `local/research/M3/census/*/census.json`; provisional until M3's REPORT.md |
...
| Odin per-callback update/render split, stock | BLOCKED: trial binary c59d173e FIFO-panics deterministically at ~sample 55 in the race at stock clocks (lacks 309f639); pace binary never panics | 09-17 | Same, `--probe-quiet` | Same; needs a trial relink with the FIFO fix (M4 step 4) |
