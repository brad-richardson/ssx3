...
| --- | --- | --- | --- |
| **N** Android / Odin | **Brad's play build = F1 fold** (APK `c822a2b3`: RR1 rendering, race SFX, E57 VU1, sound on, his save, manual play). **Race 0.140×** (F1, 2 runs; was 0.117×); VU1 was 81 % of the frame before E57 (N11). Odin replays: `local/tooling/odin/odin_replay.py`. | NP1 profile + link/copy fixes; GB9 Part 2 descriptor-path replay; stripes; rider idle | — |
...
| **I** iOS | **F2: build `96e9f45` installed on Brad's iPhone (no launch) and iPad** (F1 content + 64-bit sign branches, sound HLE on guest time): RR1 race/menu rendering, race SFX, E57 VU1 speed, analog stick left / D-pad right, his save autoloads, manual play. Simulator + iPad reach the race. Audio gaps expected below full speed. iPad still an iPhone-compat window. | Brad listens on the phone; I28 iPad-native (low priority) | Brad's listen |
...
no Codex workers (quota). RV3 Fable review running. **Leases:** Odin back on the mini USB at 58 % (F2 Part 2, then NP1; USB drains under load, ~4 %/run); mini: four boot slots (speed runs take all four).
