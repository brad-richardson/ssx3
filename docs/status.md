...
**Pins:** PS2Recomp fork `ssx3` `f0d2d3c` (SS3: save-state holes closed — palette indices, GS transfer/vertex state, card dirs/timestamps, Android identity; VR3: VU0 static recompile + direct commit behind `PS2X_VU0_RECOMP`/`PS2X_VU0_DIRECT`, default off, canonical `vu0gen-ssx3`; BA1: Android `-O3`, +5.3 % Odin race; MT1: MTVU threaded VIF1/VU1/GIF unit behind `PS2X_MTVU` (+`PS2X_MTVU_LAG`), default off, det-identical; VR2 stage 4: VU1 block functions behind `PS2X_VU1_BLOCKS`, VB1 stop-path drain; canonical `vu1gen-ssx3` = set `d28e3fc6…`; VK2: present buffer-ownership ledger, RV5 blockers fixed; VK1: Android Vulkan present default on, 16:9 fill; HP1: trace formatting guard + minSdk 29; SS2: save states on Linux/libstdc++; VR2 stage 1: VU1 per-pair levers, Mac race 1.095×; SS1: save states, dev, default off; paraLLEl-GS fork `ssx3` `3d72467` = CLUT accessor + flush counters + palette-index/transfer/vertex state; devices still run `a3efbfe` builds. F5: VU1 stage B commit-at-issue, 4× SSAA/hi-res/pipelined/zero-copy knobs, post-race unwind fix, carve-trail raw-bits compare, TZ/x86 MXCSR; F4 + musttail chain fix: VU1 static recompile stage A, NP1 host fixes, vf0 read-only emitter, VU0 R ctor; F3: pacing, full brightness, pad v2, tap latch, V2/V3 unpack, rider vf0 fix; Android lambda build fix; ST1 progressive scanout, no stripes; F2: 64-bit sign branches + canonical codegen regen, sound HLE on guest time, coverage/INTC counters; F1 fold: RR1 race/menu rendering fixes, E57 VU1 speed, AU9 race SFX, N11 profileable + pin knob; I32 iOS controls; sound HLE, GS replay harness, widescreen, Turnip packaging) ·
...
| --- | --- | --- | --- |
| **N** Android / Odin | **Brad's play build = F7** (APK `825b436d`, fork `5d5c382`, `-O3`, full-screen Vulkan present; env `090cc981…` = his keys + `PS2X_MTVU=1 PS2X_MTVU_LAG=1 PS2X_VU1_BLOCKS=1` + core pins; LAG on: Brad 09-26). **Race 0.425×** (F7 E legs; 0.391× without LAG). Canonical play state `~/dev/ssx3-work/odin-play/`. | FS2 submit wait; VR4 SIMD FMAC; AP1 immersive mode | — |
...
| **I** iOS | **F7 build `a5e5940` on Brad's iPhone (no launch) and iPad**: MTVU + VU1 blocks on (bundled env), VU1 speed-ups, 4× SSAA + 1024×896 + pipelined + zero-copy, his save + manual play. | Brad's play; VU0 wiring in the next iOS build; I28 iPad layout | Brad's play |
