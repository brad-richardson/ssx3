...
| **A** Audio | Sound HLE is on fork `ssx3` but host output needs `PS2X_SOUND=1` (not default). AU8: the tag-1 music PCM is **planar** (first block → right); we read it interleaved, which scrambled the music. PCSX2's output equals planar tag-1 to 2 LSB. | AU8 prototype → fold (planar + channel order + 3→4 resample) and default sound on; then I29 rebuild for Brad | — |
