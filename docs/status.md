...
| --- | --- | --- | --- |
| **N** Android / Odin | **Brad's play build = F3** (APK `d5a94c27`, fork `ec2dbf1`: full brightness, rider fixed, pacing, race SFX, his save, manual play). **Race 0.139×**; VU1 dominates the frame (NP1). adb over Wi-Fi (paired, mDNS; the Odin lives on the wall charger). | F4 Odin pair (VU1 recompile + NP1 fixes); menu-side Turnip churn | — |
...
**PAUSED (Brad, 09-25 afternoon):** lanes wrap their current step, commit notes, release leases and stop; nothing new starts until Brad says go. Stopped: PF1 (mechanism named, fix unwritten), PX1 (gallery done, converter fix unvalidated), VB1 (commit-at-issue, 1.15× over stage A, bit-exact), HR1 (knobs + Mac zero-copy prototype + pipelined present; Mac speed holds and report left), LX1 1d (tick-94 = FP rounding: x86 rounds nearest where PS2/arm chop; clobberer unnamed), Q2 (interrupted at 69K context). F4 paused too: Mac race 1.40× over F3, APK still OOM on bytesize (job pool not attached on Android). Resume list: F4 Part 1b → push → Parts 2/3; then each lane from its notes; Q1 after Q2.
