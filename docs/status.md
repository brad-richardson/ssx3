...
**PAUSED (Brad, 09-25 afternoon):** lanes wrap their current step, commit notes, release leases and stop; nothing new starts until Brad says go. Stopped: PF1 (mechanism named, fix unwritten), PX1 (gallery done, converter fix unvalidated), VB1 (commit-at-issue, 1.15× over stage A, bit-exact), HR1 (knobs + Mac zero-copy prototype + pipelined present; Mac speed holds and report left), LX1 1d (tick-94 = FP rounding: x86 rounds nearest where PS2/arm chop; clobberer unnamed), Q2 (interrupted at 69K context). F4's APK build continues on bytesize only. Resume list: F4 Part 1b → push → Parts 2/3; then each lane from its notes; Q1 after Q2.
