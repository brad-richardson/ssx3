...
- [x] **E10 read (09-21) — PASS(iii), DROP preflight complete; query regen stopped at fresh APFS admission (249 MB short):**
      ...
      src/runner, 9,278 files), sidecar repair (174 proven-created companions
      removed, 0 generated bytes changed), suite 452/452, actual DROP handler
      (`0x426230`) returns to supplied RA with unchanged SP + preserved s0,
      fail-before `hasFunction(0x2c5140)=false` (absent rc 0 / present rc 1).
      Query row inserted @line 5087 then admission rejected (free 3,240,579,072
      vs required 3,489,660,928 — shortfall 249,081,856 exact) BEFORE generator
      spawn; CSV restored byte-exact (`b9aae747…`, 0 refs); scratch removed;
      0 boots, lease never claimed, fork `be0c9ee` unchanged, generated
      unstaged. Verified: admission math, suite tail, closure JSON
      (generator_started false, boots 0), fork HEAD, CSV sha, binding
      receipts, pgrep 1, lease absent. ~1 h. Next: E11 reclaims stale /tmp
      build trees → fresh admission → query regen → 1 guarded boot (E10
      NEXT-BRIEF + ordering repair). Ledger row added.
...
- [x] **T34 read (09-21) — PASS, Cross on Continue → game load → race intro → pre-race panel (ERRATUM: flap counts):**
      ...
      (TITLE poll01, Start ≤1.0 s, Menu @T+140.08, Zoe @T+180.04, Continue
      @T+208.78, Peak @T+237.51, Race @T+266.80, Snow Jam @T+297.92, MR park
      0.079–0.152) → ENTER Cross 535.6 ms @T+326.56 (pre = MR Continue,
      vs-mr 0.0791/0 PIL, viewed) → game load 18% (+1 s) → 92% → black frame
      (transition, not arrival) → race intro cinematic → pre-race panel
      (`Snow Jam - Race`, `X Continue`) by +32 s, 8 snaps / 98 s (pairwise
      0.024–0.395/p99 ≤18 — panel animation, widest arrival yet; vs-MR
      ~19.3–19.6, nearest ref SC ~19.1). Mid-session H10→H11 restart between
      staging and R1 (not by worker, staged shas re-verified); R1 full
      dmesg, 0 in-window restarts. EE 52 / IOP 155 sets identical to T33 R2
      (game load adds no new API). GATE ERRATUM: report claims 0 session
      flaps — the committed dmesg files contain 3 AcceptAsync kill lines
      (0/1/2 across H10/H11/H12), ALL outside the R1 window by uptime
      arithmetic (H11 @17.0 s = 27 s pre-R1; H12 both post-everything), so
      the zero-in-window conclusion stands but the counts were wrong; T35
      brief now requires exact counts + positions. Verified: ENTER walls
      exact, T34_DONE, MR-LIKE leg, head/tail 2000/2000, poll01 = T33 R2's
      + menupre = T33 R1's (full-sha), flap lines inspected. ~45 min. Next:
      T35 Cross on X Continue (T34 G1, settled-panel gate). Ledger row added.
...
- [x] **T28 read (09-20) — PASS, Single Event Cross: Select Character (Zoe) reached, stable 127 s:**
      ...
      run: park reproduced (3 frames bit-identical to T27 R3; TITLE poll01
      identical scores; Start ≤0.73 s) → Cross @T+135.59 on menu (pre = menu,
      0.0276/0 PIL, viewed) → Select Character by +1 s (Zoe slot 1, all stats
      1.0, viewed), 12 snaps / 127 s (pairwise ≤0.33/p99 ≤8; vs-menu ~10.15,
      vs-title ~12.5). 2 pre-session flaps + 1 VM restart (E→F after evidence
      home); run exit 0 in flap-free window, effects-verified + trace/NVM
      re-verified post-restart; NVM untouched. EE/IOP sets identical to T27 R3
      (52/155 — arrival adds NO new API); sceSdGetParam 412→616. Brief erratum
      (mine): cited `t27r3-menu01.jpg` doesn't exist — worker correctly used
      stab1 (future briefs cite real names). Verified: scope (0 outside),
      snap sizes + shas, bit-identities (poll01 + post15), SSD size + FULL sha
      re-run, slice shas, menu ref REGENERATED BY ME (size + sha exact),
      detector RE-RUN BY ME (0.1647/6/39 exact). Reference-state row: the trace
      IS the reference (arrival reached). ~35 min. Next: T29 (Cross on Zoe).
      Ledger row added.
- [x] **G9 read (09-20) — PASS, geometry probe: 85 px = screenshot aspect (both sides 512), no flag yields 597:**
      No new capture, no rebuild, read-only bytesize greps: PrivRegs decode
      (DISPLAY1 2561×448/MAG5, NTSC, EN1-only; stable + FIELD bit) → chain A
      derives PCSX2 internal 512×448 + sidecar u32(448×4/3)=597 (4:3 aspect
      resample); chain B derives replayer mode 512×448; G7 control reproduces
      640×480 + 640×448 + 712×240 unmodified. Flag sweep (4 replays, pre-
      computed predictions): {512, 570, 1024} (+1140 derived-not-run); 597
      unreachable (not a CRTC width) — no re-diff per exact-or-nothing rule.
      (i) SUPPORTED-as-dissolved; (ii) NOT SUPPORTED. Frontier redirect arrived
      mid-flight (accept aspect, stop 597 hunt, same-boundary dual replay —
      §4's ScreenshotSize=2 next action SUPERSEDED); this gate covers the
      brief-as-run. Hygiene: allocated caps tracked, G8 dir pristine (PPM shas
      re-verified), / unchanged, zero code copied. Verified: scope (0 outside),
      ps2xGS commit (text-only, pushed), dump-copy sha, derivation RE-RUN BY ME
      via committed g9-derive.py (597 + 512 + 570 MATCH), sweep PPMs present.
      Reference-state row: PCSX2 sidecar + embedded shot. ~40 min. Next: G10
      same-boundary dual replay. Ledger row added.
- [x] **E3b read (09-20/21) — PASS, order capture: 1-vs-4 RESOLVED (structural skip + post-read residue), E3-4 rows 1–7 answered:**
      G0 verified (same park post-K1); FAST coverage closed statically (0 FAST
      literals in 0x501400–0x50144F, scratchpad structurally Store-path,
      Font.cpp FAST unwired); taps R1–R4 + shared seq + binds (lib-only fork
      `b34b481`, pushed fork remote; suite 443→445, 2 new). ONE boot BOUND=span
      31 s (all other caps untouched; lease 31 s, zero waits): 1042 rows, 0
      torn, 0 suppressed, seq accounting exact (1040+40=1080). Order per frame:
      SPR-TO plants all flags → 20 reads (19 call + k19 skip; terminator
      a2=0x26/+0x10=0xffff; every pre-trio == plant afters) → stale guest
      writes → overwrite k8–19 → post-read rewrites (k0–3 → T26's exact
      4-nonzero: 0x5000/0x8000s, read-time 0). R2: 432 solo (all scratchpad,
      0 RAM, 0 FAST); R3: 4 spr-to only (aliases normalized, wrapped-split,
      deduped); R4: nbases=1, drift=0. E3-4: SPR overlap YES (to only),
      0x501420 NO, reads see planted values, 1-vs-4 = lifecycle stages, only
      the 4 spr-to in-window, no foreign reuse, post-K1 SAME. First-success
      PNGs captured (upload-16/17 byte-identical, K1 missed). Verdict: H_A+H_C
      CONFIRMED, H_B/H_D falsified; skip STRUCTURAL (no anomaly); proposes
      CLOSE G1, no SIF/CD/scheduler fixes (G3 open iff frontier names it).
      Verified: scope (0 outside), fork+remote, suite RE-RUN BY ME 445/445/0,
      artifacts (sizes/span/lease), rows 1042 + sha, PNG pair + FNV RECOMPUTED
      BY ME (`fd889dc5` exact). Reference-state row: K1 park + T26 watch (by
      reference). ~50 min. Next: frontier confirms G1 closure (CONFIRMED — see queue). Closure qualification: R1 brackets memory, not load operands (caller 363238 → load/branch 363258/36325c, not 362f84/88); no rerun. Ledger row added.
- [x] **G8 read (09-20) — PASS, post-first-draw replay: (i) SUPPORTED, (ii) NOT SUPPORTED (geometry + first-black gaps):**
      ...
      16 pal / 0 copies (project's own consume_flush_stats — ERRATUM per
      frontier: accumulated over BOTH passes, cold+warm; first=306=272+34;
      viability holds); LAST scanout =
...
- [x] **K1 read (09-20) — PASS, payload equivalent: 6 drops → 0, same park, P0 black frame verified:**
      ...
      and snapshot (unmuted-env receipt in k1-boot1.py; generic drops[] NOT
      silencing-independent per emitDropTo ordering — K1 counters are); suite
      439/439/0 → 443/443/0
