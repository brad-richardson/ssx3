...
- [ ] **Live panes:** k1 (item-3 fix CORRECTED per frontier steering, fork + P-lane lease, 6h) · t27 (press-on-title within dwell, bytesize, 4h) · od1 (Odin port readiness, synth path, 4h) · e3 (SPR/DMA/host-write audit, read-only, no boots, 4h).
- [ ] **Queue:** E3 boots PARKED behind K1 (frontier steering; follow-up = 1–2 guest frames if same park remains, not a census) · consolidated frontier doc + final recommendations pending · A-lane CLOSED (R1 settled the paradox) · P1ae CLOSED (writer init-only, not SIF) · E2b/E2c HELD · I8 audio-wall repro (iPad build this poll; iPhone when user confirms broader feedback) · G-lane (first game frames; MF1-s2 parked behind GX motion vectors) · mini day-one.
