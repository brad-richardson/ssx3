...
- [ ] **Live panes:** g9 (geometry-alignment probe, ps2xGS, 4h) · t28 (Single Event Cross → submenu, bytesize, 4h) · i8 (audio-wall device repro, fork worktree + iPhone, 6h) · mf2 (MF app-option map + toggle prototype, 6h) · ac1 (0x12C alarm-contract validation, read-only, 3h).
- [ ] **Queue:** G1 closure proposed by E3b (structural skip; frontier to confirm) · G3 0x501420 purpose (open iff frontier names it) · NO regen before 0x426230 disposition (P13b: drop selector or implement HLE first) · per-vsync scanout series (after G9 unblocks geometry) · rich post-loading dump (after comparison method unblocked) · M15-on-Odin arm (OD1 O1–O3; needs device lease + trial build) · K1 ret0-retirement (needs convergence boot) · P1ae still unjustified (SIF not eliminated) · I9 audio fix (after I8 diagnosis) · G-lane (strict = sensitivity surrogate; MF1-s2 parked; MF3 full integration after MF2 map) · E2b/E2c HELD · mini day-one.
