...
- [ ] iOS: gate the unconditional per-present `[ios-render]` stderr line behind a knob so iOS speed numbers are clean (IS1 numbers are provisional because of it).
- [ ] iPad full screen (IP1): the build is universal now; iPadOS 26 windowed mode restores a 590×820 pt window. Brad to maximize once and report whether the picture resizes cleanly; if not, or if a launch default is wanted, a follow-up requests full-screen geometry / sets scene size restrictions.
...
- [ ] 120 Hz simulation: RV6 (`docs/research/review-2026-09-26-astra-120hz.md`) recovered the timing model (app manager
      ...
      2 updates/VBlank but full-size steps (1.89×) and a 2× HUD clock. **Start here: `docs/research/ssx3-engine-timing.md`** (EN1: the frame end to end, GC↔PS2 address map, what's proven, open questions with what GC already settled). **TM4 done, 120 Hz paused (09-26):** rider step = vel×(tscale×1/60) with private 1/60 words (K1–K3) → exact
...
- [ ] **90 % Odin push (Brad 09-26; race now 0.425×, 0.442× with SIMD):** VR4 D1b+D2, GF1 (GS frontend thread), FS2 (Turnip),
      PT1 (profiler tool), CP2 (timeline after Turnip+SIMD), IS1 (iPad speed). Budget: ~18.5 ms/frame; the unit thread is ~21 ms.
