#!/bin/bash
# launch-opus.sh <ID> <prompt> — Claude worker pane (Opus; MODEL=claude-fable-5-1 for a Fable review)
...
sleep 4; herdr agent start "$name" --kind claude --pane "$pane" --timeout 120000 -- --model ${MODEL:-opus} --permission-mode auto >/dev/null || { echo "$ID start failed"; exit 1; }
