...
ssh "$REMOTE" "cd ~/$RROOT/PS2Recomp && git fetch origin ssx3 2>&1 | tail -1; git checkout --detach $SHA_IN 2>&1 | tail -1" || exit 1
...
[ -z "$(ssh "$REMOTE" "git -C ~/$RROOT/PS2Recomp status --short")" ] || { echo "remote fork tree dirty" >&2; exit 2; }
...
RVU1=$(ssh "$REMOTE" "(cd ~/$RROOT/vu1gen 2>/dev/null && sha256sum vu1_*.cpp | sha256sum | cut -d' ' -f1)" || true)
if [ "$RVU1" != "$VU1_MINI" ]; then
  echo "-- syncing vu1gen from mini"
  COPYFILE_DISABLE=1 tar -C "$VU1_DIR" -cf - . | ssh "$REMOTE" "mkdir -p ~/$RROOT/vu1gen && tar -C ~/$RROOT/vu1gen -xf -"
  RVU1=$(ssh "$REMOTE" "(cd ~/$RROOT/vu1gen && sha256sum vu1_*.cpp | sha256sum | cut -d' ' -f1)")
...
if [ "$RPGS" != "$PGS_PIN" ] || [ "$RGRAN" != "$GRANITE_PIN" ]; then
  echo "-- cloning parallel-gs pins on bradflix"
  ssh "$REMOTE" "rm -rf ~/$RROOT/parallel-gs && cd ~/$RROOT && git clone --recursive --branch ssx3 https://github.com/brad-richardson/parallel-gs.git parallel-gs 2>&1 | tail -1 && git -C ~/dev/ssx3-work/HS1/parallel-gs checkout --detach $PGS_PIN && git -C ~/dev/ssx3-work/HS1/parallel-gs/Granite checkout --detach $GRANITE_PIN" || exit 1
  RPGS=$(ssh "$REMOTE" "git -C ~/$RROOT/parallel-gs rev-parse HEAD")
  RGRAN=$(ssh "$REMOTE" "git -C ~/$RROOT/parallel-gs/Granite rev-parse HEAD")
  [ "$RPGS" = "$PGS_PIN" ] && [ "$RGRAN" = "$GRANITE_PIN" ] || { echo "PGS re-verify failed" >&2; exit 2; }
...
START=$SECONDS
ssh "$REMOTE" "docker run --rm --user 1000:1000 -e HOME=/work -e CCACHE_DIR=/ccache -e CCACHE_BASEDIR=/work -w /work -v ~/$RROOT:/work -v ~/$RCCACHE:/ccache $IMAGE cmake -S /work/PS2Recomp -B /work/$NAME -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_C_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DPS2X_GAME_CODEGEN_DIR=/work/codegen -DPS2X_VU1_RECOMP_DIR=/work/vu1gen -DPS2X_BUILD_TEST=ON -DPS2X_BUILD_STUDIO=OFF -DPS2X_ENABLE_DEBUG_UI=OFF -DPS2X_ENABLE_RUNTIME_LOGS=OFF -DPS2X_ENABLE_AGRESSIVE_LOGS=OFF -DPS2X_ENABLE_DIAG_TAPS=OFF -DPS2X_ENABLE_DET_HASH_TAP=$DET -DPS2X_GS_SHADOW_PARALLEL=ON -DPS2X_PARALLEL_GS_SOURCE_DIR=/work/parallel-gs -DPS2X_ENABLE_SCCACHE=OFF -DPS2X_RUNNER_UNITY_BUILD_BATCH_SIZE=$BATCH -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_C_FLAGS=-msse4.1 -DCMAKE_CXX_FLAGS=-msse4.1 > ~/$RROOT/$NAME-configure.log 2>&1; echo CONFIGURE_RC=\$?; grep -m1 'VU1 recomp' ~/$RROOT/$NAME-configure.log" || exit 1
