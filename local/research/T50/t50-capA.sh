#!/usr/bin/env bash
# T50 Capture A: T48 route verbatim -> Select Character (settled, Zoe) -> F1 savestate + 8-vsync dump + F8.
# Arms /tmp/t48-arm (T48 window) + /tmp/t49-arm (T49 re-trace) + /tmp/t50-arm (srcread watch).
...
D=$T4
if [ "${UNPATCHED:-0}" = "1" ]; then BIN=$G/pre-t48/pcsx2-qt; TAG=t50proof; else BIN=$G/pcsx2/build/bin/pcsx2-qt; TAG=t50a; fi
DAT=$G/dat-t48
...
if [ "${UNPATCHED:-0}" = "1" ]; then LOG=$T4/t50proof-poll.log; else LOG=$T4/t50a-poll.log; fi
...
fail() { echo "T50A_FAIL:$1"; stamp; exit 1; }
: > $LOG
plog() { echo "$@" | tee -a $LOG; }
snap() {
  local N=$1
...
if [ "${UNPATCHED:-0}" != "1" ]; then touch /tmp/t48-arm /tmp/t49-arm /tmp/t50-arm; ls -la /tmp/t48-arm /tmp/t49-arm /tmp/t50-arm; fi
pkill Xvfb || true
sleep 2
pgrep -a Xvfb || true
setsid nohup Xvfb :99 -screen 0 1280x1024x24 >/tmp/xvfb-t50a.log 2>&1 < /dev/null &
sleep 4
...
sleep 3; snap t50a-post3 || fail SNAP_POST3
read M3 P3 <<< $(score_band $T4/t50a-post3.ppm)
plog "post+3 titleband mean=$M3 p99=$P3 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 5; snap t50a-post8 || fail SNAP_POST8
read M8 P8 <<< $(score_band $T4/t50a-post8.ppm)
plog "post+8 titleband mean=$M8 p99=$P8"
sleep 7; snap t50a-post15 || fail SNAP_POST15
read M15 P15 <<< $(score_band $T4/t50a-post15.ppm)
plog "post+15 titleband mean=$M15 p99=$P15"
sleep 10; snap t50a-post25 || fail SNAP_POST25
read M25 P25 <<< $(score_band $T4/t50a-post25.ppm)
plog "post+25 titleband mean=$M25 p99=$P25"
W815=$(score_whole $T4/t50a-post8.ppm $T4/t50a-post15.ppm)
W1525=$(score_whole $T4/t50a-post15.ppm $T4/t50a-post25.ppm)
plog "whole815=$W815 whole1525=$W1525"
read MM25 MP25 <<< $(score_menu $T4/t50a-post25.ppm)
...
sleep 5
snap t50a-menupre || fail SNAP_MENUPRE
read MMP MMP_P <<< $(score_menu $T4/t50a-menupre.ppm)
plog "MENUPRE vs-menu mean=$MMP p99=$MMP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
gshot $TAG-menu
press K "MENU_CROSS"
sleep 1; snap t50a-mc-post1 || fail SNAP_MC_POST1
read M1 P1 <<< $(score_band $T4/t50a-mc-post1.ppm)
read MM1 MP1 <<< $(score_menu $T4/t50a-mc-post1.ppm)
read MS1 MSP1 <<< $(score_sc $T4/t50a-mc-post1.ppm)
plog "MC post+1 titleband mean=$M1 p99=$P1 vs-menu mean=$MM1 p99=$MP1 vs-sc mean=$MS1 p99=$MSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 2; snap t50a-mc-post3 || fail SNAP_MC_POST3
read M3 P3 <<< $(score_band $T4/t50a-mc-post3.ppm)
read MM3 MP3 <<< $(score_menu $T4/t50a-mc-post3.ppm)
read MS3 MSP3 <<< $(score_sc $T4/t50a-mc-post3.ppm)
plog "MC post+3 titleband mean=$M3 p99=$P3 vs-menu mean=$MM3 p99=$MP3 vs-sc mean=$MS3 p99=$MSP3"
sleep 5; snap t50a-mc-post8 || fail SNAP_MC_POST8
read M8 P8 <<< $(score_band $T4/t50a-mc-post8.ppm)
read MM8 MP8 <<< $(score_menu $T4/t50a-mc-post8.ppm)
read MS8 MSP8 <<< $(score_sc $T4/t50a-mc-post8.ppm)
plog "MC post+8 titleband mean=$M8 p99=$P8 vs-menu mean=$MM8 p99=$MP8 vs-sc mean=$MS8 p99=$MSP8"
sleep 7; snap t50a-mc-post15 || fail SNAP_MC_POST15
read M15 P15 <<< $(score_band $T4/t50a-mc-post15.ppm)
read MM15 MP15 <<< $(score_menu $T4/t50a-mc-post15.ppm)
read MS15 MSP15 <<< $(score_sc $T4/t50a-mc-post15.ppm)
plog "MC post+15 titleband mean=$M15 p99=$P15 vs-menu mean=$MM15 p99=$MP15 vs-sc mean=$MS15 p99=$MSP15"
H_PRE1=$(score_whole $T4/t50a-menupre.ppm $T4/t50a-mc-post1.ppm)
H_13=$(score_whole $T4/t50a-mc-post1.ppm $T4/t50a-mc-post3.ppm)
H_38=$(score_whole $T4/t50a-mc-post3.ppm $T4/t50a-mc-post8.ppm)
H_815=$(score_whole $T4/t50a-mc-post8.ppm $T4/t50a-mc-post15.ppm)
...
sleep 5
snap t50a-scpre || fail SNAP_SCPRE
read SCP SCP_P <<< $(score_sc $T4/t50a-scpre.ppm)
plog "SCPRE vs-sc mean=$SCP p99=$SCP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
if [ "${UNPATCHED:-0}" = "1" ]; then
  gshot $TAG-sc
  ls -la $T4/t50-shot-$TAG-sc.png $E
  kill $(cat $T4/pcsx2-t50.pid); sleep 10; pgrep -a pcsx2-qt || true
  stamp
  echo T50A_DONE
  exit 0
fi
...
cp "$SSD/$STATE_NEWEST" $T4/t50-sc-state || fail STATE_CP
ls -la $T4/t50-sc-state
