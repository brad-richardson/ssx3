#!/usr/bin/env python3
"""HR1 bounded boot: F3 driver + paraLLEl quality knobs (--ssaa/--hires/--ssaa-tex),
PS2X_FRAME_DUMP_ONCE_TICKS for det-mode frames (--dump-ticks), and per-thread CPU
samples with thread names (threadcpu, libproc) in the race window of speed boots.
I26-FAST to the race, paraLLEl GS backend.
...
Usage: f3_boot.py --mode speed|det --backend cpu|parallel --runner PATH --label NAME [--unpaced]
"""
import argparse
import hashlib
import json
...
WORK = Path('/Users/brad/dev/ssx3-work/HR1')
THREADCPU = WORK / 'bin' / 'threadcpu'
...
def main():
    ...
    ap.add_argument('--mode', choices=('speed', 'det', 'diag'), required=True)
    ...
    ap.add_argument('--ssaa', type=int, default=1, choices=(1, 2, 4, 8, 16))
    ap.add_argument('--hires', action='store_true', help='PS2X_PGS_HIRES_SCANOUT=1')
    ap.add_argument('--ssaa-tex', action='store_true', help='PS2X_PGS_SSAA_TEXTURES=1')
    ap.add_argument('--dump-ticks', default='', help='det: PS2X_FRAME_DUMP_ONCE_TICKS (three ticks)')
    ap.add_argument('--extra-env', default='', help='comma list K=V added verbatim')
    args = ap.parse_args()
    ...
    if args.ssaa != 1:
        env['PS2X_PGS_SSAA'] = str(args.ssaa)
    if args.hires:
        env['PS2X_PGS_HIRES_SCANOUT'] = '1'
    if args.ssaa_tex:
        env['PS2X_PGS_SSAA_TEXTURES'] = '1'
    ...
    if args.mode == 'det':
        ...
        env.update(PS2X_VSYNC_RATE_LOG='1', PS2X_SOUND='1',
                   PS2X_UNPACED='1',  # FP1: speed runs measure unpaced headroom
                   PS2X_THREAD_CPU_LOG='1')  # HR1: per-thread CPU on the 5 s cadence
        tick_re = RATE_TICK

    exclusive = args.mode == 'speed'
    held = os.environ.get('HR1_HELD_SLOT')
    if held:
        ...
        slot = claim('HR1-' + args.label, exclusive=exclusive)
        while slot is None:
            ...
            try:
                while True:
                    ...
                    if now - last_progress >= PROGRESS_CAP_S:
                        bound = 'progress_cap'
                        break
                    if (args.mode in ('speed', 'diag') and last_tick >= PS_START_TICK
                        ...
                        try:
                            ...
                            tc = subprocess.run([str(THREADCPU), str(proc.pid)],
                                                capture_output=True, timeout=15)
                            (lane / ('threads-%02d.txt' % ps_samples)).write_text(
                                'wall_s=%.3f tick=%d\n' % (now - t0, last_tick)
                                + tc.stdout.decode(errors='replace'))
                            ps_samples += 1
