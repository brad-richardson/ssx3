#!/usr/bin/env python3
"""BA1 Odin ABBA: base vs -O3 vs -O3+ThinLTO APKs (fork 173b31f + ba1 commits).
N12's launch.py (F5 lineage) repointed to BA1: Brad env pin a8d651a7 (play
env), OUT/SCR/DSCRAP/lease say BA1, --variant defaults to A (1x +
PRESENT_PIPELINE=1 = play knobs; the APK differs, never the env). Clean runs
only (no profiler); transport recorded per run with disconnect counting;
BACK only when the app is not focused.
Usage: launch.py --label R1 [--wall 600] [--stop-tick 4500]
...
Brad's play env (SHA a8d651a7...), mc0 SHAs match I31 pins, mc0-test empty.
Cool-down (thermal <=1, fixed 180 s wait) is done by cooldown.py BEFORE this
runs; this script only records the pre-launch thermal line.
"""
...
PKG = 'com.ps2x.runner'
LEASE_TOOL = os.path.expanduser('~/dev/ssx3/local/tooling/odin_lease.sh')


def lease_claim():
    # Atomic claim through odin_lease.sh (the pre-check above stays as a guard).
    r = subprocess.run(['bash', LEASE_TOOL, 'claim', 'BA1'], capture_output=True,
                       text=True, timeout=60)
    return r.returncode == 0, (r.stdout or '').strip()


def lease_release():
    r = subprocess.run(['bash', LEASE_TOOL, 'release', 'BA1'], capture_output=True,
                       text=True, timeout=60)
    return r.returncode == 0, (r.stdout or '').strip()
FILES = f'/storage/emulated/0/Android/data/{PKG}/files'
DSCRAP = '/data/local/tmp/ba1'
...
ap.add_argument('--variant', choices=('A', 'B'), default='A',
                help='BA1: A = 1x pipelined (play knobs; always A)')
a = ap.parse_args()

OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/BA1/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/BA1/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('BA1 ')):
...
log(f'CLAIM {out}')
if not ok:
    sys.exit(f'atomic lease claim failed: {out}')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            ok, out = lease_release()
            log(f'CLEANUP-RELEASE ok={ok} {out}')
    except Exception:
        pass
...
env = [f'# BA1 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
assert not any(x in e for e in env for x in ('DUMP', 'TRACE', 'CAPTURE', 'ORACLE')), 'dump/trace key in env'
