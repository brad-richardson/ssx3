#!/bin/bash
# I30: pinned, optimized iOS builds (sim + device) at fork d711506 (AU8 planar music fix); bundled env adds PS2X_SOUND=1; stage,
# sign, install. Adapted from I27C's script; deps from persistent ios-deps.
# No wait-for-idle: UP1 builds concurrently, so builds run under nice.
set -euo pipefail
W=/Users/brad/dev/ssx3-work/I30
...
ENVFILE="$REPO/local/research/I30/ps2x.env"
LOGS="$W/logs"
...
PIN=d7115063c59518fe3c7d13a79a2c9b75dc160fab
...
for step in "$@"; do echo "[i30 $(date +%H:%M:%S)] stage $step"; "$step"; done
