#!/bin/bash
# I30 iPad validation: install the I30 signed app, run I26-FAST with
# only PS2X_VSYNC_RATE_LOG=1 from the launcher (PS2X_SOUND=1 comes from
# the bundled ps2x.env), 4 screenshots, terminate.
set -u -o pipefail
IPAD=00008112-001224302184A01E
APPB=/Users/brad/dev/ssx3-work/I30/staged/ps2EntryRunner.app
APP=org.ps2x.ps2entryrunner
OUT=/Users/brad/dev/ssx3-work/I30/run-ipad
mkdir -p "$OUT"
cd "$OUT" || exit 1
...
trap cleanup EXIT
xcrun devicectl device process launch --device "$IPAD" --terminate-existing --environment-variables '{"PS2X_VSYNC_RATE_LOG":"1"}' --console "$APP" > console.log 2>&1 &
LP=$!
START=$(date +%s)
