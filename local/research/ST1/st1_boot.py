#!/usr/bin/env python3
"""ST1 bounded boot: F2 driver + 0.1 s change-gated snapshotter for consecutive-tick
frames. I26-FAST to the race; --backend cpu|parallel.
...
WORK = Path('/Users/brad/dev/ssx3-work/ST1')
...
def main():
    ...
    held = os.environ.get('ST1_HELD_SLOT')
    if held:
        ...
        slot = claim('ST1-' + args.label, exclusive=exclusive)
        while slot is None:
    ...
    def snapshotter(t0):
        ...
        while not stop_snap.wait(SNAP_PERIOD_S):
            ...
            try:
                ...
                if key == last_key:
                    continue  # unchanged since last copy
                last_key = key
                tag = 'snap-%06dt-%07.2fs' % (shared['tick'], el)
                shutil.copyfile(latest, snap_dir / (tag + '.png'))
                if latest_txt.exists():
                    shutil.copyfile(latest_txt, snap_dir / (tag + '.txt'))
            except OSError:
                pass
