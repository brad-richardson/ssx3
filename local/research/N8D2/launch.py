#!/usr/bin/env python3
"""N8D2 one-install/one-launch settled-race image probe; device writes use its lease."""
import atexit
import hashlib
import json
import os
...
D = '622c49b1'
PKG = 'com.ps2x.runner'
ROOT = Path('/Users/brad/dev/ssx3-work/N8D2')
FILES = f'/storage/emulated/0/Android/data/{PKG}/files'
DUMP = f'{FILES}/n8d2-frames'
APK = Path('/Users/brad/dev/ssx3-work/N8D1/app-release.apk')
...
PINS = {
    'apk': 'ece8b84ce3bce6bb85e42e21c432f6a6123ccae6e61dd91293e9da696fc63ae2',
...
def cleanup():
    ...
    if lease_claimed:
        ...
        try:
            adb('shell', "echo 'LEASE_FREE N8D2 done' > /data/local/tmp/mg/LEASE", timeout=30)
...
def preflight(allow_ours=False):
    ...
    free_or_ours = lease.startswith('LEASE_FREE') or (allow_ours and lease == 'N8D2 one-launch')
...
def capture(label, tick):
    remote = f'/data/local/tmp/n8d2-{label}.png'
...
def main():
    ...
    preflight()
    sh("echo 'N8D2 one-launch' > /data/local/tmp/mg/LEASE")
    lease_claimed = True
    event('LEASE claimed=' + sh('cat /data/local/tmp/mg/LEASE'))
    if sh('cat /data/local/tmp/mg/LEASE') != 'N8D2 one-launch':
    ...
    while True:
        ...
        if race_dumps and not race_image:
            ...
            race_image = True
            receipt['result'] = 'race dump and screen captured'
            event(f'STOP screen captured after race dump host_tick={host_tick}')
            break
        if tick >= 2100:
...
if receipt['result'] != 'race dump and screen captured':
    raise SystemExit(1)
