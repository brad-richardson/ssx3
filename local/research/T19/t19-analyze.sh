#!/usr/bin/env bash
# T19 trace analysis on the two-press emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t19-census.txt
python3 $D/t4-sample.py $E $D/t19-samples.txt
ls -la $D/t19-census.txt $D/t19-samples.txt
date -u
