#!/usr/bin/env bash
# E60 capture: T65/T48 closed-loop route, E60 data directory and audio/video recording.
set -x
...
fail() { echo "E60_FAIL:$1"; stamp; [ -f $T4/pcsx2-e60.pid ] && kill $(cat $T4/pcsx2-e60.pid) 2>/dev/null; [ -n "$XVFB_PID" ] && kill $XVFB_PID 2>/dev/null; sleep 5; exit 1; }
...
gshot() {
  ...
  cp "$SNAPS/$NEW" $T4/e60-shot-$LAB.png || fail GSHOT_CP_$LAB
  ls -la $T4/e60-shot-$LAB.png
}
leg() {
...
rm -f $T4/pcsx2-e60.pid $T4/pcsx2-vu0.bin $T4/arm
touch $T4/arm # E60 control: capture from boot
T_BOOT=$(date -u +%s)
echo "T_BOOT_WALL:$T_BOOT T_BOOT_UPTIME:$(cut -d. -f1 /proc/uptime)"
PCSX2_E60_TAP=$T4/pcsx2-vu0.bin nohup $BIN -nogui -slowboot -turbo -datapath $DAT -logfile $T4/logs/boot-$TAG.log -- "$R/inputs/SSX 3 (USA).iso" > $T4/logs/boot-$TAG.stdout 2>&1 &
echo $! > $T4/pcsx2-e60.pid
echo "PID:$!"
sleep 70
wallcheck
kill -0 $(cat $T4/pcsx2-e60.pid) || fail PCSX2_DIED_PRE_START
...
sleep 3 # T65
touch $T4/arm # already armed from boot
gshot $TAG-race
snap $TAG-race-live || fail SNAP_RACE_LIVE
# E60: bounded first race calls, 16 records maximum (4136 B each).
for W in $(seq 1 30); do
  wallcheck
  SZ=$(stat -c %s $T4/pcsx2-vu0.bin 2>/dev/null || echo 0)
  plog "E60_BYTES=$SZ wall=$(($(date -u +%s)-T_BOOT))"
  [ $SZ -ge 4136 ] && break
  sleep 2
done
[ $SZ -ge 4136 ] || plog "E60_NO_RACE_CALLS"
plog "CAPTURE_DONE bytes=$SZ"
kill $(cat $T4/pcsx2-e60.pid); sleep 5
