#!/usr/bin/env python3
"""E11-only oldest-first link-tree reclamation with a retained ownership gate."""
...
def main():
    ...
    assert json.loads((E/'checkpoint.json').read_text())['matching_generated_names_hashes']==9449
    ...
    if args.mode=='probe':
        ...
        # p10 is the first retired lane, while the active board has E11/G15/T36/I14.
        # Names and cwd are checked independently of saved current peer-pane claims.
    ...
    print('# E11 RECLAMATION TAIL COMPLETE '+args.mode+' '+args.tree)
