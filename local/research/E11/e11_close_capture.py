...
from e11_capture import size

E=Path(__file__).resolve().parent
assert os.environ.get('COPYFILE_DISABLE')=='1'
c=json.loads((E/'e11a-config.json').read_text())
paths={k:Path(v) for k,v in c['paths'].items()}
paths['function_log']=paths['function_log'].with_name('ps2_log-e11a-1.txt')
...
(E/'e11a-closed-caps.json').write_text(json.dumps(r,indent=2)+'\n')
with (E/'e11a-raw-tails.txt').open('w') as out:
    ...
    out.write('# E11 RAW TAILS COMPLETE\n')
print('closed logical',n,'allocated',a,'trace lines',lines)
print('# E11 CLOSED CAPS TAIL COMPLETE')
