#!/usr/bin/env python3
"""N12 Odin per-stage re-profile on the F5 play build (fork a3efbfe, APK 4ff81032).
F5's launch.py adapted: Brad env pin a8d651a7 (play env), OUT/SCR/DSCRAP/lease
repointed to N12, --variant defaults to A (1x + PRESENT_PIPELINE=1 = play knobs).
S1 clean (no profiler), S2 race profile (--profile-after-tick 2400 --profile-secs 30),
S3 menu profile (--profile-after-tick 636 --profile-secs 30). Profiler command is
N11's verbatim `simpleperf record -g --app`. Transport recorded per run with
disconnect counting; BACK only when the app is not focused.
...
ap.add_argument('--variant', choices=('A', 'B'), default='A',
                help='N12: A = 1x pipelined (play knobs; always A)')
a = ap.parse_args()

OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/N12/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/N12/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('N12 ')):
...
sh(f'echo "F5 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE N12 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# N12 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
sh("echo 'LEASE_FREE N12 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
