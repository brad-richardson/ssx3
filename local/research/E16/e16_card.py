#!/usr/bin/env python3
"""Join E16 query calls/returns to same-run constructor and UI-pointer guards."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('phase',choices=['a','b']);args=ap.parse_args();label='e16'+args.phase
    ...
    with (E/f'{label}-card.txt').open('w') as f:
        ...
        f.write('# E16 CARD TAIL COMPLETE: same-run objects and query truth/return joins\n')
    print(label,'queries',len(queries),'objects',[hex(x) for x in sorted(objects)],'targets',table['target_counts'])
    print('# E16 CARD TAIL COMPLETE')
