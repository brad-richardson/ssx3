...
label=sys.argv[1];dest=P/'e16-fixtures'/label;dest.mkdir(parents=True,exist_ok=False)
source_record=json.loads((E/'rebuilt-binaries-objects.json').read_text())['fixture_sources']
...
for a in args:
    ...
    out.append(a)
out+=['-c',str(E/'e16_binding_test.cpp'),'-o',str(dest/'test.o')]
...
link+=[str(dest/'test.o'),'-Wl,-e,_e16_binding_main','-Wl,-export_dynamic']
...
print(dest/'binding-test');print('# E16 ACTUAL OBJECT LINK TAIL COMPLETE')
