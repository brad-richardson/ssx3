#!/usr/bin/env python3
"""E16 progression capture driver. Explicit preflight, caps, owned lease cleanup."""
import argparse
import datetime
import hashlib
import json
...
from e16_frame import read_png, fnv32
from e16_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e16_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E16'
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E16 forbids lease wait')
        raise RuntimeError('Lease occupied; E16 tables and stops without waiting')
    ...
    assert manifest_path.exists(), 'fresh E16 build manifest required'
...
def main():
    ...
    assert not (EVIDENCE/'boot-attempt.json').exists(), 'E16 permits only one boot'
    assert not list(EVIDENCE.glob('e16?-result.json')), 'E16 permits only one boot'
    label='e16'+args.phase
    assert args.report_all, 'E16 requires REPORT_ALL=1'
