...
def main():
    ...
    folder=P/'e16-tooltmp'/a.label
    record=dict(label=a.label,utc=utc(),argv=argv,cwd=os.getcwd(),tmpdir=str(folder),before=sample(),caps=dict(wall=a.wall,wall_reserve=15,stdout=16*M,stdout_reserve=M,tmp=8*G,tmp_reserve=512*M,fixtures=G,fixture_reserve=64*M,poll_s=.25))
    save(a.label+'-bounded-start.json',record);admission(record['before'])
    folder.mkdir(parents=True,exist_ok=False)
    (folder/'E16-owner.json').write_text(json.dumps(dict(task='E16',label=a.label,created=record['utc'])))
        ...
        try:
            ...
            while p.poll() is None or sel.get_map():
                ...
                if elapsed>=next_sample:
                    row=dict(elapsed_s=elapsed,tmp_allocated=size(folder),fixture_allocated=size(P/'e16-fixtures'),stdout_seen=seen,stdout_kept=kept,**sample())
                    ...
                    if row['fixture_allocated']>=G-64*M:reason=reason or 'fixtures'
    ...
    print(json.dumps(record,indent=2));print('# E16 BOUNDED TOOL TAIL COMPLETE')
