...
def logged(argv,env,cwd,log,wall=45,scratch=None):
    ...
    return p.returncode,log.read_text(),dict(argv=list(map(str,argv)),cwd=str(cwd),env={k:v for k,v in env.items() if k.startswith('PS2X_') or k=='TMPDIR'},elapsed_s=time.monotonic()-t,stdout_bytes=seen,scratch_peak_allocated=peak)
...
def suite(label):
    ...
    assert rc==0 and 'Total Tests: 452' in txt and 'Failed: 0' in txt,record
    print('SUITE',label,'452/452')
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e16-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            assert rc==1 and 'result=FAIL-BEFORE' in txt and 'manualCallbackCalls=0' in txt and 'waitReason=6' in txt
            assert 'saved128=1 ram32MiB-unchanged=1' in txt
    ...
    print('# E16 VALIDATION TAIL COMPLETE')
    return result
