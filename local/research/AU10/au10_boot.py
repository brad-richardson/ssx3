#!/usr/bin/env python3
"""AU10 bounded boot: I26-FAST to the race, paraLLEl GS backend, det-hash.
...
slot (never exclusive). Wall 500 s / progress 120 s / log 16 MiB caps. Only
the recorded runner PID is signalled; the lease is released on every path.
--sound on|off toggles PS2X_SOUND only; both boots set PS2X_SND_LOG (cid0
counts), det-hash every tick, and frame dumps at 1090/2100/2390.

Usage: au10_boot.py --runner PATH --label NAME --sound on|off [--stop-tick N]
Results: ~/dev/ssx3-work/AU10/run/<label>/{boot.log,snd.log,frames/,result.json,trace.jsonl}
"""
import argparse
import hashlib
import json
...
WORK = Path('/Users/brad/dev/ssx3-work/AU10')
...
SND_TICK = re.compile(r'^tick cycle=\d+ ticks=(\d+) counter=0x[0-9a-f]+ cid0=(\d+) dmq=(\d+) done=(\d+) '
                      r'setdma=(\d+) tagbufs=(\d+) underruns=(\d+) overflows=(\d+)$', re.M)
...
def main():
    ...
    ap.add_argument('--sound', required=True, choices=('on', 'off'))
    ...
    env.update(PS2X_CD_IMAGE=str(ISO), PS2X_SKIP_MOVIE='1', PS2X_DETERMINISTIC='1',
               ...
               PS2X_DET_HASH_EVERY='1', PS2X_SND_LOG=str(snd_log),
               PS2X_FRAME_DUMP_DIR=str(frames_dir),
               PS2X_FRAME_DUMP_ONCE_TICKS='1090,2100,2390', COPYFILE_DISABLE='1')
    if args.sound == 'on':
        env['PS2X_SOUND'] = '1'

    slot = claim('AU10-' + args.label, exclusive=False)
    while slot is None:
        print('lease busy; retrying in 30 s', flush=True)
        time.sleep(30)
        slot = claim('AU10-' + args.label, exclusive=False)

    result = {'label': args.label, 'backend': 'parallel', 'sound': args.sound,
    ...
    try:
        ...
        snd_text = snd_log.read_text(errors='replace') if snd_log.exists() else ''
        snd_ticks = SND_TICK.findall(snd_text)
        result.update(bound=bound, elapsed_s=round(time.monotonic() - t0, 3),
                      last_tick=last_tick, log_bytes=log.stat().st_size,
                      snd_bytes=snd_log.stat().st_size if snd_log.exists() else 0,
                      snd_last_tick=list(map(int, snd_ticks[-1])) if snd_ticks else None,
                      frames=sorted(p.name for p in frames_dir.glob('*')) if frames_dir.exists() else [],
