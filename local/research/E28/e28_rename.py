"""Hex-safe mechanical rename into E28 + proof, per errata E23-E1.
...
E28 DIFFERENCE, declared: E28 draws from TWO source lanes, not one. The carried
...
normalized-diff proof maps THAT lane's token and `e28` to one placeholder.

Usage: e28_rename.py E25:e25_io.py:e28_io.py E24:e24_capture.py:e28_capture.py ...
"""
import json, re, sys
...
PROTECTED = ['PS2X_E21_PARSER_DIR', 'PS2X_E21_PROOF', '# E21 PARSER CLOSURE',
             ...
             # E28 ADDS: E25's snapshot path, the durable restore E28's
...
def rename_text(text, src_lane):
    ...
    out = text.replace(lo, 'e28').replace(up, 'E28')
...
def normalize(text, src_lane):
    ...
    return text.replace(lo, 'EXX').replace('e28', 'EXX') \
               .replace(up, 'EXX').replace('E28', 'EXX')
...
def main():
    ...
    (E / 'rename-proof-hexsafe.json').write_text(json.dumps(dict(
        tool='e28_rename.py', source_lanes=sorted({r['source_lane'] for r in rows}),
    ...
    print('# E28 RENAME TAIL COMPLETE')
