"""Pin all behavior-regression gates before spending the single E28 title allowance."""
import subprocess
from e28_common import *
# E28 CHANGE: this tool is the gate that AUTHORIZES the boot. The brief's
...
# E28 CHANGE 1: the regression receipts come from E25, which re-ran E24's
...
GATES=E.parent/'E25'
# E28's red flag is its OWN Mission 0 restore receipt: the instrument is
...
if not rst['all_ok']:
    ...
    print('# E28 PREPARE PROBE TAIL COMPLETE')
    raise SystemExit(0)
...
# E28 gate: the boot may not be spent before the complete-feed reference lands.
...
# The binaries are re-hashed HERE, a third time, against the pins E28's own
...
save('entry-preflight.json',dict(utc=utc(),cadence_cases=len(cad['rows']),cadence_branches=2,suite_rc=0,binding_rc=0,leaf_cases=24,consumer_cases=3,
    ...
    binaries_rehashed_by_E28=True,restore_path=rst['path_taken']))
save('e28a-build.json',dict(utc=utc(),fork_head=head,fork_status='?? ps2_log.txt\n',test_count=458,
    bin_size=ckpt['runner']['bytes'],bin_sha=ckpt['runner']['sha256'],
    test_size=ckpt['suite']['bytes'],test_sha=ckpt['suite']['sha256']))
print('All E28 regression gates pinned. Fresh T13 pre-claims and atomic lease remain mandatory.')
print('# E28 PREPARE PROBE TAIL COMPLETE')
