"""What E28 inherits, checked rather than asserted.

E28 spends E24's designed e24a boot on the re-baselined build. Three things it
...
check('no boot-attempt.json written by E28', not (E / 'boot-attempt.json').exists())

ok = all(c['ok'] for c in checks)
save('e28-readiness.json', dict(utc=utc(), boot_argv=boot_argv,
     capture_preflight_gate=gate, checks=checks, all_green=ok,
     note=('E28 spends no boot and claims no lease. These are read-only checks of what '
           'E28 inherits. Because the rebuilt runner is byte-identical to the binary E23 '
...
print('ALL GREEN:', ok)
print('# E28 E28 READINESS TAIL COMPLETE')
