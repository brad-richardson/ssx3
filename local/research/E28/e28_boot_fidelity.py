"""Prove -- not assert -- that the boot E28 spends IS the boot E24 designed.
...
mine = (E / 'e28_capture.py').read_text()
check('E28 driver == E24 driver under the hex-safe rename, byte for byte',
...
check('lease path identical (one P-lane lease, shared with every prior lane)',
      e24['lease'] == e28['lease'], lease=e28['lease'])

# The full watch list the driver builds, reproduced here exactly as main() does.
watches = [0x4a289c] + e28['display']
S = 0x61ba60
watches += [S + off for off in e28['offsets']]
...
check('lane wait log is E28-spelled (new lane log, not E24\'s)',
      e28['waits'].endswith('e28-waits.log'), waits=e28['waits'])
check('evidence directory is E28\'s', e28['evidence'].endswith('/E28'), evidence=e28['evidence'])

ok = all(c['ok'] for c in checks)
save('boot-fidelity.json', dict(utc=utc(), all_green=ok, checks=checks,
     label=dict(designed_as='e24a', spent_as='e28a',
                ...
                reason=('label-derived output names are not scientific content -- e22a and e23a '
                        ...
                        'byte accounting (e28-*, *e28*) honest'),
