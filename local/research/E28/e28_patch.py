"""E28's intentional tooling changes, applied idempotently and recorded as data.
...
E28 has TWO changes and no others. The CAPTURE DRIVER ITSELF IS NOT ONE OF
THEM: `e28_capture.py` is byte-identical to E26's under the hex-safe rename,
...
CHANGES = [
    dict(
        ...
        why=("E25 named its readiness receipt `e26-readiness.json` -- it wrote the readiness for "
             ...
             "mechanical e26->e28 carry rewrote `e28_restore_gate.py`'s reference to "
             "`E25/e28-readiness.json`, which does not exist: the Mission 0 restore gate would "
             ...
             "token is added to PROTECTED, and `e28_carry_audit.py` now resolves EVERY cross-lane "
        ...
        pairs=[("             'P1/e25-snapshot', 'e18-mpeg-link.tar']\n",
                ...
                "             # E28 ADDS (intentional change 1, recorded in tooling-changes.json):\n"
                ...
                "             # REAL PATH on disk, not a lane token, and the e26->e28 rename\n"
                "             # silently rewrote it to `E25/e28-readiness.json`, which does not\n"
                "             # exist: `e28_restore_gate.py` would have died on FileNotFoundError\n"
    ...
    dict(
        ...
        why=("E24's fidelity proof asserts the watch set is SHA-equal to E24's, which is right for "
             "a lane that changes nothing. E28's brief permits exactly one intentional change -- the "
             ...
             "as an EXACT PREFIX of E28's, with the 13 new entries appended last and disjoint from "
    ...
    dict(
        ...
        why=("E24's cost-model block inside `watch-set.json` carries only the e22a and e23a "
             ...
             "receipt instead, so E28's fit is four-point. Nothing else in the refit moves, and "
    ...
    dict(
        ...
        why=("E26's close tool writes `release_latency_ms=8` and `needed_sigkill=False` as LITERALS "
             ...
             "E28 they would report E26's boot in E28's audit. Both are computed from this lane's "
             "own `e28a-result.json` instead. Nothing else in the tool moves."),
...
save('tooling-changes.json', dict(utc=utc(), changes=rows,
     written_fresh=[dict(
         ...
         why=("E26's fix gate reads E26's OWN mission receipts (x-a-startcode.json, "
              ...
              "E28's. Carried mechanically it would read files this lane never produces. Written "
              "fresh against E28's own receipts and NOT claimed as a carry; it is therefore absent "
              "from rename-proof-hexsafe.json by design. E27 set the precedent with e27_common.py."),
         gate_decision_unchanged='STOP'),
         dict(file='e28_common.py', why='the lane common, written fresh exactly as E26 wrote its own'),
         dict(file='e28_bootstrap_rename.py', why='one-off: e26_rename.py hardcodes its own destination token and cannot rename itself'),
         dict(file='e28_carry_audit.py', why='new check for the rename-induced dangling-path class that CHANGE 1 instantiates'),
         dict(file='e28_slot.py', why='Mission 1 has no prior-lane equivalent'),
         dict(file='e28_watchset.py', why='the one permitted driver-behaviour change, with before/after receipts'),
         dict(file='e28_prereg.py', why='pre-registration of the armed set and the P1-P4 pass/fail bytes'),
         dict(file='e28_verdict.py', why='Mission 2 grading, against the pre-registered rows only')],
...
(E / 'tooling-diff.md').write_text('# E28 intentional tooling changes\n\n' + '\n'.join(diffs) +
