"""E28 close: refit E24's cost model with its third point, then the final audit."""
import subprocess
from e28_common import *

RUN = P / 'run'
result = json.loads((E / 'e28a-result.json').read_text())
...
save('cost-model-refit.json', dict(utc=utc(),
     ...
     slope_e23a_to_e28a=round(k23, 5),
     ...
     consequence=('The residual E24 declared -- an isolated write shorter than 121 B inside the '
                  ...
                  'that wants the residual closed can afford it. E28 does not re-spend a boot to '
...
boot_dirs = sorted(str(q) for q in RUN.glob('*e28a*'))
s = sample()
checks = dict(fork_triple_agree=fork_ok, binaries_unmoved=bins_ok,
              ...
              boots_spent_is_one=len(list(E.glob('e28?-result.json'))) == 1,
