#!/usr/bin/env python3
"""M37 c54 carrier coincidence: 701-mask geometry + row-list comparison (offline).

Usage: m37.py M16_DIR M15_DIR M34TSV M36TXT WORK_DIR EVID_DIR

Reads M16 per-shape triplets + loo.txt, the M15 triplet (baseline
guard only), m34-census.tsv (unnamed-universe + TSV guard), and
m36.txt (c54's 10 rows + the other-14 row lists to reproduce);
raw XFB dumps, 573440 B = 640x448 YUYV. Read-only inputs; receipt
text goes to stdout (redirect to WORK_DIR/m37.txt); PNG overlap
...
Tasks (see DESIGN.md, recorded before running):
  1: mask geometry (701 column profile + overlap tables + coverage)
  2: row-list comparison (c54 vs other 14 + value contrast + tail census)
...
C54_ROWS = [245, 246, 247, 252, 253, 258, 259, 278, 279, 283]
M36_EXTRAS = {
...
OTHER_COLS = [299, 311, 313, 316, 332, 336, 339, 620]
EXTRA_CELLS = {2: [311, 313, 332, 339], 3: [311, 316, 336, 339],
               700: [54, 299, 311, 313, 332, 620]}
M19_701_CELLS = [233, 20, 22, 1, 2, 2, 2, 367]  # s0 Y-resid in 701, cells 0-7
M22_701_TAIL_IN = 0
M22_701_BULK_IN = 367
...
def direction_split(v0b: bytes, midb: bytes, fullb: bytes, synth: bytes):
    """8-cell partition of residual bytes (M19 ids 0..7, verbatim).

    Returns dict with labels (int8 [N], -1 = non-residual).
    """
    a = np.frombuffer(v0b, np.uint8).astype(np.int16)
    m = np.frombuffer(midb, np.uint8).astype(np.int16)
    b = np.frombuffer(fullb, np.uint8).astype(np.int16)
    ...
    return {"labels": lab, "n": int(res.sum())}
...
def geom_core_lines(sets700, masks, unnamed):
    """Task 1 core (s0+700 only): 701 column profile + c54 overlaps + coverage.

    Determinism canon: depends only on s0+700 triplets + top-10 triplets.
    """
    L = []
    rm701 = masks[701]
    coln = rm701.sum(axis=0)
    # 1. column profile: every nonzero Y column + group summaries
    nz = [int(c) for c in range(W) if int(coln[c]) > 0]
    L.append(f"geom701 profile: nrem={int(rm701.sum())} (want 649) "
             f"ncols={len(nz)} zerocols={W - len(nz)}")
    for c in nz:
        rows = [int(r) for r in np.nonzero(rm701[:, c])[0]]
        L.append(f"geom701 col c={c} n={len(rows)} rows={rows}")
    named = [int(coln[c]) for c in NAMED8]
    un = [int(coln[c]) for c in unnamed]
    rest = int(rm701.sum()) - sum(named) - sum(un)
    L.append(f"geom701 groups: named8_n={sum(named)} "
             f"(cols>0={sum(1 for x in named if x > 0)}) "
             f"unnamed33_n={sum(un)} "
             f"(cols>0={sum(1 for x in un if x > 0)}) rest_n={rest}")
    # 2a. c54's 10 rows x 701 mask (per-row in/out)
    rQ = sets700["cells"][54]["rQ"]
    for r in rQ:
        L.append(f"overlap c54 r={r} c=54 in701={int(bool(rm701[r, 54]))}")
    nin = sum(1 for r in rQ if rm701[r, 54])
    L.append(f"overlap c54x701: in={nin}/{len(rQ)} (M36 want 10/10)")
    # 2b. c54's 10 rows x the other 9 carrier masks (per-row per-mask)
    for s in M16_TOP10:
        if s == 701:
            continue
        rm = masks[s]
        hits = [int(r) for r in rQ if rm[r, 54]]
        L.append(f"overlap c54xmask s={s}: in={len(hits)}/10 rows={hits}")
    # 3. coverage table: col 54 vs the 8 other-extra columns
    L.append(f"cover col54 maskrows={int(coln[54])}")
    for c in OTHER_COLS:
        L.append(f"cover other c={c} maskrows={int(coln[c])}")
    mx = max(int(coln[c]) for c in OTHER_COLS)
    ntouch = sum(1 for c in OTHER_COLS if int(coln[c]) > 0)
    L.append(f"cover summary: col54={int(coln[54])} othermax={mx} "
             f"othertouched={ntouch}/8 otherbare={8 - ntouch}/8")
    return L


def geom_others_lines(allsets, masks):
    """Task 1 others-overlap (needs s2/s3 sets): other 14 x 701 mask."""
    L = []
    rm701 = masks[701]
    pl = plane_of_byte()
    pr, pc = plane_coords()
    for Q in QLIST:
        for c in EXTRA_CELLS[Q]:
            if Q == 700 and c == 54:
                continue
            ex = allsets[Q]["cells"][c]["ex"]
            for o in np.nonzero(ex)[0]:
                o = int(o)
                inside = rm701[int(pr[o]), int(pc[o])] if int(pl[o]) == 0 \
                    else masks[701][:, 0::2][int(pr[o]), int(pc[o])]
                L.append(f"overlap other s{Q}c{c} r={int(pr[o])} "
                         f"in701={int(bool(inside))}")
    n_out = sum(1 for ln in L if ln.endswith("in701=0"))
    L.append(f"overlap othersx701: out={n_out}/{len(L)} (M36 want 14/14)")
    return L


def rowlist_lines(allsets):
    """Task 2.1: (s0 rows, Q rows) per extra-bearing cell (14 cells)."""
    L = []
    for Q in QLIST:
        for c in EXTRA_CELLS[Q]:
            ...
            L.append(f"rowlist s{Q}c{c}: s0rows={r0} s{Q}rows={rQ} "
                     f"miss={sorted(set(r0) - set(rQ))} "
                     f"extra={sorted(set(rQ) - set(r0))}")
    return L


def rowrecur_lines(allsets):
    """Task 2.1: row-number recurrence (c54 rows vs pooled other rows)."""
    c54 = set(allsets[700]["cells"][54]["rQ"])
    other = set()
    for Q in QLIST:
        for c in EXTRA_CELLS[Q]:
            if Q == 700 and c == 54:
                continue
            r0 = set(allsets[Q]["cells"][c]["r0"])
            rQ = set(allsets[Q]["cells"][c]["rQ"])
            other.update(rQ - r0)
    ov = sorted(c54 & other)
    L = [f"rowrecur c54rows={sorted(c54)} otherrows={sorted(other)} "
         f"overlap={ov} n={len(ov)}/10"]
    # per-shape other-row lists (same rows recurring across shapes?)
    for Q in QLIST:
        rows = set()
        for c in EXTRA_CELLS[Q]:
            if Q == 700 and c == 54:
            ...
            rQ = set(allsets[Q]["cells"][c]["rQ"])
            rows.update(rQ - r0)
        L.append(f"rowrecur s{Q}otherrows={sorted(rows)}")
    return L
...
def main():
    ...
    # ---- M36 24-site value pins (the brief's stop rule) ----
    print("== M36 24-site value pins (c54 10 + other 14 or stop) ==")
    ...
    for Q in QLIST:
        ...
        for (pQ, r, c), (wo, wad, wstat, wado, wgQ, wgO) in sorted(
            ...
            good = (in_ex and o == wo and adq == wad and ostat == wstat
                    and ado == wado and int(gQ[o]) == wgQ
                    and int(g0[o]) == wgO)
            pin_ok = pin_ok and good
            print(f"pin s{Q}c{c} r={r} o={o} (want {wo}) "
                  f"adQ={adq} (want {wad}) ostat={ostat} (want {wstat}) "
                  f"adO={ado} (want {wado}) gQ={int(gQ[o])} (want {wgQ}) "
                  f"gO={int(g0[o])} (want {wgO}) inex={in_ex} "
                  f"match={good}")
    # set-level: recomputed extras per (Q,cell) == pin rows exactly
    for Q in QLIST:
        for c in EXTRA_CELLS[Q]:
            got = sorted(int(pr[o]) for o in
                         np.nonzero(allsets[Q]["cells"][c]["ex"])[0])
            want = sorted(r for (pQ, r, cc) in M36_EXTRAS if pQ == Q
                          and cc == c)
            good = got == want
            pin_ok = pin_ok and good
            print(f"pinset s{Q}c{c}: extra rows={got} (want {want}) "
                  f"match={good}")
    print("24-site pins: " + ("OK" if pin_ok else
    ...
    # ---- Task 1: mask geometry (column profile + overlaps + coverage) ----
    print("== Task 1: 701-mask geometry (profile + overlaps + coverage) ==")
    t1 = time.time()
    canon = geom_core_lines(allsets[700], masks, UNNAMED)
    for ln in canon:
        print(ln)
    for ln in geom_others_lines(allsets, masks):
        print(ln)
    print(f"(Task-1 wall: {time.time() - t1:.1f}s)")

    # ---- Task 2.1: row lists + row recurrence (c54 vs other 14) ----
    print("== Task 2.1: row lists per extra cell + row recurrence ==")
    for ln in rowlist_lines(allsets):
        print(ln)
    for ln in rowrecur_lines(allsets):
        print(ln)

    # ---- Task 2.2: value contrast (c54 vs other 14, M36 side-by-side) ----
    print("== Task 2.2: value contrast (per-cell attrib + pooled) ==")
    for Q in QLIST:
        vQ, _, fullQ, _ = trips[Q]
        dQ = allsets[Q]["dQ"]
        ...
        for c in EXTRA_CELLS[Q]:
            ...
            la, _ = attrib_lines(f"s{Q}c{c}", ex, mi, shared, d0, mask0,
                                 dQ, allsets[Q]["maskQ"], v0, full0, vQ,
                                 fullQ)
            for ln in la:
                print(ln)
            ...
            for ln in dstats_lines(f"s{Q}c{c}", ex, mi, dQ, d0):
                print(ln)
    # pooled side-by-side: c54 (10) vs others (14, per-shape multiplicity)
    for nm, cells in (("c54", [(700, 54)]),
                      ("others", [(Q, c) for Q in QLIST
                                 for c in EXTRA_CELLS[Q]
                                 if not (Q == 700 and c == 54)])):
        n = nbulk = nad8 = npp = nmm = 0
        for Q, c in cells:
            ex = allsets[Q]["cells"][c]["ex"]
            dQ = allsets[Q]["dQ"]
            vQ, _, fullQ, _ = trips[Q]
            gQarr = (np.frombuffer(vQ, np.uint8).astype(np.int16)
                     - np.frombuffer(fullQ, np.uint8).astype(np.int16))
            for o in np.nonzero(ex)[0]:
                o = int(o)
                n += 1
                nbulk += int(bool(mask0[o]))
                nad8 += int(int(abs(int(dQ[o]))) == 8)
                npp += int(int(gQarr[o]) > 0 and int(g0[o]) > 0)
                nmm += int(int(gQarr[o]) < 0 and int(g0[o]) < 0)
        print(f"contrast {nm}: n={n} bulk={nbulk} noncell={n - nbulk} "
              f"adQ8={nad8} pp={npp} mm={nmm}")

    # ---- Task 2.3: 701-mask tail census (s0 + M19 row + all-764) ----
    print("== Task 2.3: 701-mask tail census ==")
    t0in = sum(1 for o in np.nonzero(t0 & (pl == 0))[0]
                 if masks[701][int(pr[o]), int(pc[o])])
    print(f"census701 s0: tail_in={t0in} (M22 want {M22_701_TAIL_IN})")
    ylab = y_cell_map(direction_split(v0, mid0, full0, b0)["labels"])
    inmask_lab = ylab[masks[701]]
    cells701 = [int((inmask_lab == c).sum()) for c in range(8)]
    print(f"census701 s0 cells={cells701} (M19 want {M19_701_CELLS}) "
          f"match={cells701 == M19_701_CELLS}")
    bulk0 = mask0 & (np.abs(d0) < 8)
    b0in = sum(1 for o in np.nonzero(bulk0 & (pl == 0))[0]
               if masks[701][int(pr[o]), int(pc[o])])
    print(f"census701 s0: bulk_in={b0in} (M22 want {M22_701_BULK_IN})")
    if (t0in != M22_701_TAIL_IN or b0in != M22_701_BULK_IN
            or cells701 != M19_701_CELLS):
        print("CENSUS MISMATCH vs M19/M22: STOP, tabled.")
        return
    # tail-in-mask for ALL 764 shapes (fresh loads)
    t1c = time.time()
    inmask_counts = {}
    for s in range(764):
        vv, mm, ff, bl = load_triplet(m16d, s)
        sp = interior_sites(vv, mm, ff, bl)
        t, _ = tail_bulk_masks(sp["mask"], sp["delta"])
        k = sum(1 for o in np.nonzero(t & (pl == 0))[0]
                if masks[701][int(pr[o]), int(pc[o])])
        inmask_counts[s] = (int(t.sum()), k)
    npos = sorted(s for s in inmask_counts if inmask_counts[s][1] > 0)
    ks = [inmask_counts[s][1] for s in range(764)]
    print(f"census701 all764: shapes_with_in={len(npos)} list={npos}")
    print(f"census701 all764: inmask min={min(ks)} med="
          f"{float(np.median(np.asarray(ks, dtype=np.float64))):.1f} "
          f"max={max(ks)}")
    for s in QLIST:
        print(f"census701 s{s}: tail={inmask_counts[s][0]} "
              f"inmask={inmask_counts[s][1]}")
    print(f"(census wall: {time.time() - t1c:.1f}s)")

    # ---- determinism re-run (Task 1 on s0+700, fresh loads) ----
    print("== determinism re-run (Task 1 on s0+700, second pass) ==")
    v0b, mid0b, full0b, b0b = load_triplet(m16d, 0)
    v700b, mid700b, full700b, b700b = load_triplet(m16d, 700)
    _, _, masks2, _ = compute_dec_masks(m16d, mid0b, b0b)
    sets2 = compute_sets_for(v0b, mid0b, full0b, b0b, v700b, mid700b,
                             full700b, b700b, SHAPES[700]["moved"])
    canon2 = geom_core_lines(sets2, masks2, UNNAMED)
    ...
    print(f"sets identical s700: c54rows={sets2['cells'][54]['rQ']} "
          f"(want {C54_ROWS})")

    # ---- PNG 701-overlap map (work dir; evidence copy iff rule meets) ----
    print("== PNG 701-overlap map ==")
    y0p, u0p, v0p = split_planes(np.frombuffer(mid0, np.uint8))
    rgbm = yuv_to_rgb(y0p, u0p, v0p)
    over = 0.35 * rgbm.astype(np.float32)
    over[masks[701]] = np.array([148, 163, 184])  # 701 mask slate
    im_s, _, _ = flat_to_planes(allsets[700]["shared"])
    over[im_s] = np.array([0, 255, 0])            # s700 shared green
    im_c54, _, _ = flat_to_planes(allsets[700]["cells"][54]["ex"])
    over[im_c54] = np.array([255, 255, 0])        # c54 extras yellow
    for Q in QLIST:
        for c in EXTRA_CELLS[Q]:
            if Q == 700 and c == 54:
                continue
            im_o, _, _ = flat_to_planes(allsets[Q]["cells"][c]["ex"])
            over[im_o] = np.array([255, 0, 0])    # other extras red
    p = workd / "m37-overlap-701.png"
    Image.fromarray(over.astype(np.uint8)).resize((320, 224),
                                                  Image.BILINEAR).save(p)
    print(f"png {p}: {p.stat().st_size} B (budget 5242880)")
    print("legend: slate=701 mask green=s700-shared yellow=c54-extras "
          "red=other-extras (s0 geometry; Y only)")

    # ---- falsification-bar measurements (values + thresholds) ----
    print("== falsification-bar measurements ==")
    rm701 = masks[701]
    coln = rm701.sum(axis=0)
    c54rows = allsets[700]["cells"][54]["rQ"]
    # H1/H2/H6: coverage of col 54 vs the 8 other-extra columns
    n54 = int(coln[54])
    nothers = [int(coln[c]) for c in OTHER_COLS]
    h1 = n54 > max(nothers)
    h2 = sum(1 for x in nothers if x > 0) >= 5
    h6 = sum(1 for x in nothers if x == 0) >= 5
    print(f"H1_GEOM_COVER: col54={n54} othermax={max(nothers)} "
          f"(bar col54>max; met={h1})")
    print(f"H2_MASK_SPAN: touched={sum(1 for x in nothers if x > 0)}/8 "
          f"(bar >=5; met={h2})")
    # H3: c54 rows inside any of the other 9 carrier masks
    h3hits = sorted({int(r) for s in M16_TOP10 if s != 701
                     for r in c54rows if masks[s][r, 54]})
    print(f"H3_C54_OTHER_CARRIER: rows={h3hits} n={len(h3hits)} "
          f"(bar >=1; met={len(h3hits) >= 1})")
    # H4: c54 row numbers recurring among pooled other-extra rows
    other_rows = set()
    for Q in QLIST:
        for c in EXTRA_CELLS[Q]:
            if Q == 700 and c == 54:
            ...
            r0 = set(allsets[Q]["cells"][c]["r0"])
            rQ = set(allsets[Q]["cells"][c]["rQ"])
            other_rows.update(rQ - r0)
    h4ov = sorted(set(c54rows) & other_rows)
    print(f"H4_ROW_RECURRENCE: overlap={h4ov} n={len(h4ov)} "
          f"(bar >=1; met={len(h4ov) >= 1})")
    # H5: shapes with >=1 tail byte in the 701 mask
    npos5 = sorted(s for s in inmask_counts if inmask_counts[s][1] > 0)
    print(f"H5_TAILWIDE: nshapes={len(npos5)} list={npos5} "
          f"(bar >=2; met={len(npos5) >= 2})")
    print(f"H6_OTHERS_BARE: bare={sum(1 for x in nothers if x == 0)}/8 "
          f"(bar >=5; met={h6})")
    # PNG rule inputs: H1 + other-extra sites outside the 701 mask
    n_out = 0
    n_oth = 0
    for Q in QLIST:
        for c in EXTRA_CELLS[Q]:
            ...
            for o in np.nonzero(allsets[Q]["cells"][c]["ex"])[0]:
                n_oth += 1
                n_out += int(not rm701[int(pr[o]), int(pc[o])])
    print(f"PNG rule: H1={h1} others_outside={n_out}/{n_oth} "
          f"(copy iff H1 and >=12/14)")
    print("N: 0 explained; tail stands (attribution, not explanation)")

    print(f"== total wall: {time.time() - t_start:.1f}s ==")
    print("tests: T1=mask-geometry T2=rowlists+contrast+census R=controls")
