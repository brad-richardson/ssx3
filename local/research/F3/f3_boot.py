#!/usr/bin/env python3
"""F3 bounded boot: F2 driver + --unpaced for det mode (F3 B2 needs det + unpaced).
...
WORK = Path('/Users/brad/dev/ssx3-work/F3')
...
def main():
    ...
    held = os.environ.get('F3_HELD_SLOT')
    if held:
        ...
        slot = claim('F3-' + args.label, exclusive=exclusive)
        while slot is None:
