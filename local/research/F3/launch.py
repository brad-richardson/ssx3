#!/usr/bin/env python3
"""F3 Odin speed runs: N11 launcher adapted for the F3 fold build (fork 0ed07c4).

Differences from N10: OUT text logs stay in git, binaries (PNG/perf.data) go
to scratch; mc0-test empty card (Brad's mc0 never touched); full F3 env with
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/F3/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/F3/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('F3-')):
...
sh(f'echo "F3 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE F3 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# F3 {a.label}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
sh("echo 'LEASE_FREE F3 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
