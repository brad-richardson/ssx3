#!/usr/bin/env bash
# T55 capture: SC-settled statefile boot, func_394ED0 call log whole boot.
# Foreground run (T51 §T51-6: no detached boots on bytesize). One held ssh.
# Like T54: free-gate files are touched for the shared T50/T51 gates; the T55
# hooks themselves are ungated (statefile boot lands SC-settled).
# Env: TAG=t55a (default).
set -x
...
TAG=${TAG:-t55a}
LOG=$T4/$TAG-poll.log
fail() { echo "T55_FAIL:$1"; date -u; exit 1; }
plog() { echo "$@" | tee -a $LOG; }
[ -x $BIN ] || fail NO_BIN
...
plog "COUNTS h394=$(grep -c 'h394 vsync=' $E || true) cap=$(grep -c 'T55_CAP' $E || true) ctag=$(grep -c 'ctag ' $E || true)"
