#!/usr/bin/env python3
"""M38 shared-site triple: cross-shape value/gap comparison (offline).

Usage: m38.py M16_DIR M15_DIR M34TSV M36TXT WORK_DIR EVID_DIR

Reads M16 per-shape triplets + loo.txt, the M15 triplet (baseline
guard only), m34-census.tsv (FULL TSV guard), and m36.txt (the 3
shared sites + per-shape gaps to reproduce); raw XFB dumps,
573440 B = 640x448 YUYV. Read-only inputs; receipt text goes to
stdout (redirect to WORK_DIR/m38.txt); PNG shared-site map to
WORK_DIR (evidence copy iff the DESIGN.md rule meets, decided
at report time).

Tasks (see DESIGN.md, recorded before running):
  1: shared-site recompute (per-site per-shape + divergence + signs)
  2: recurrence + standing context (rows/cols + standings + s700c313)
...
# Shared-site pins (M36 missing tables; the brief's stop rule):
# (r, c) -> (offset, |d_s0|, g_s0, missing-shape set, {Q: g_Q}).
SHARED_PINS = {
    (278, 314): (356468, 20, 42, frozenset((2, 3, 700)),
                 {2: 40, 3: 42, 700: 42}),
    (295, 332): (378264, 10, -22, frozenset((2, 3, 700)),
                 {2: -21, 3: -21, 700: -21}),
    (276, 313): (353906, 8, 17, frozenset((2, 3)), {2: 16, 3: 16}),
}
SHARED_ROWS = [276, 278, 295]
SHARED_COLS = [313, 314, 332]
...
def shared_canon_lines(allsets, trips, v0, full0):
    """Task 1 core: per-site per-shape table + divergence + sign tables.
    ...
    t0 = allsets[2]["t0"]
    d0 = allsets[2]["d0"]
    # 1. per-site per-shape rows
    for (r, c), (wo, wad0, wg0, missh, gaps) in sorted(SHARED_PINS.items()):
        o = off_of(r, c)
        in_T0 = bool(t0[o])
        ad0 = int(abs(int(d0[o]))) if in_T0 else None
        L.append(f"shared site rc=({r},{c}) o={o} (want {wo}) "
                 f"plane={PNAME[int(pl[o])]} inT0={in_T0} "
                 f"ad0={ad0} (want {wad0}) g0={int(g0[o])} "
                 f"(want {wg0}) misshapes={sorted(missh)}")
        for Q in QLIST:
            S = allsets[Q]
            in_TQ = bool(S["tQ"][o])
            in_mQ = bool(S["maskQ"][o])
            if in_T0 and not in_TQ:
                st = "missing"
            elif in_TQ:
                st = "tail"
            elif in_mQ:
                ...
                st = "noncell"
            adQ = int(abs(int(S["dQ"][o]))) if in_mQ else None
            gQv = int(gQ_of[Q][o])
            want_st = "missing" if Q in missh else "measured"
            want_g = gaps.get(Q, "measured")
            L.append(f"shared cell s{Q} rc=({r},{c}) status={st} "
                     f"(want {want_st}) "
                     f"adQ={adQ if adQ is not None else 'n/a'} "
                     f"gQ={gQv} (want {want_g}) "
                     f"sign={sgn(int(g0[o]))}{sgn(gQv)}")
        # 2. divergence: range over missing shapes (+ all-3 context)
        mg = [int(gQ_of[Q][o]) for Q in sorted(missh)]
        ag = [int(gQ_of[Q][o]) for Q in QLIST]
        L.append(f"shared div rc=({r},{c}) missgaps={mg} "
                 f"range={max(mg) - min(mg)} all3gaps={ag} "
                 f"all3range={max(ag) - min(ag)}")
    # 3. sign table: pooled missings recomputed + summary
    npp = nmm = 0
    for (Q, r, c) in sorted(M36_MISS):
        o = off_of(r, c)
        a, b = int(g0[o]), int(gQ_of[Q][o])
        npp += int(a > 0 and b > 0)
        nmm += int(a < 0 and b < 0)
        L.append(f"shared sign s{Q} rc=({r},{c}) g0={a} gQ={b} "
                 f"sign={sgn(a)}{sgn(b)}")
    L.append(f"shared sign pooled: pp={npp}/10 mm={nmm}/10 "
             f"(M36 want 7/3)")
    return L


def recur_lines(allsets):
    """Task 2.1: shared rows/cols at OTHER pooled extra/missing sites."""
    L = []
    _, pr, pc = plane_of_byte(), *plane_coords()
    sites = []
    for Q in QLIST:
        for c in SHAPES[Q]["moved"]:
            for o in np.nonzero(allsets[Q]["cells"][c]["ex"])[0]:
                sites.append((Q, "extra", int(pr[o]), int(pc[o])))
            for o in np.nonzero(allsets[Q]["cells"][c]["mi"])[0]:
                sites.append((Q, "miss", int(pr[o]), int(pc[o])))
    shared_rc = set(SHARED_PINS)
    for r in sorted(SHARED_ROWS):
        at = sorted((Q, k, cc) for (Q, k, rr, cc) in sites
                    if rr == r and (rr, cc) not in shared_rc)
        L.append(f"recur row r={r}: nothers={len(at)} sites={at}")
    for c in sorted(SHARED_COLS):
        at = sorted((Q, k, rr) for (Q, k, rr, cc) in sites
                    if cc == c and (rr, cc) not in shared_rc)
        L.append(f"recur col c={c}: nothers={len(at)} sites={at}")
    uniq_r = sorted({(rr, cc) for (_, _, rr, cc) in sites
                     if rr in SHARED_ROWS and (rr, cc) not in shared_rc})
    uniq_c = sorted({(rr, cc) for (_, _, rr, cc) in sites
                     if cc in SHARED_COLS and (rr, cc) not in shared_rc})
    L.append(f"recur rows unique-other={uniq_r}")
    L.append(f"recur cols unique-other={uniq_c}")
    return L


def standing_lines(allsets):
    """Task 2.2: standings — shared vs pooled missings vs extras."""
    L = []
    mask0 = allsets[2]["mask0"]
    # pooled missings: qstat on own Q
    n_non = n_bulk = 0
    for (Q, r, c) in sorted(M36_MISS):
        o = off_of(r, c)
        mQ = allsets[Q]["maskQ"]
        st = "bulk" if mQ[o] else "noncell"
        n_bulk += int(bool(mQ[o]))
        n_non += int(not mQ[o])
        in_mi = bool(allsets[Q]["cells"][c]["mi"][o])
        L.append(f"stand miss s{Q} rc=({r},{c}) qstat={st} inmi={in_mi}")
    L.append(f"stand miss pooled: bulk={n_bulk} noncell={n_non} "
             f"(M36 want 0/10)")
    # pooled extras: ostat on s0
    e_non = e_bulk = 0
    for (Q, r, c) in sorted(M36_EXTRAS):
        o = off_of(r, c)
        e_bulk += int(bool(mask0[o]))
        e_non += int(not mask0[o])
    L.append(f"stand extra pooled: bulk={e_bulk} noncell={e_non} "
             f"(M36 want 14/10)")
    # shared triple's own standing, compactly
    for (r, c) in sorted(SHARED_PINS):
        o = off_of(r, c)
        row = []
        for Q in QLIST:
            S = allsets[Q]
            if S["t0"][o] and not S["tQ"][o] and not S["maskQ"][o]:
                st = "missing/noncell"
            elif S["tQ"][o]:
                st = "tail"
            elif S["maskQ"][o]:
                ...
                st = "noncell"
            row.append(f"s{Q}={st}")
        L.append(f"stand shared rc=({r},{c}): " + " ".join(row))
    return L


def s700c313_lines(allsets):
    """Task 2.3: s700 row-list at c313 around r276."""
    L = []
    cell = allsets[700]["cells"][313]
    L.append(f"s700c313 s0rows={cell['r0']} s700rows={cell['rQ']} "
             f"miss={sorted(set(cell['r0']) - set(cell['rQ']))} "
             f"extra={sorted(set(cell['rQ']) - set(cell['r0']))}")
    t0 = allsets[700]["t0"]
    t700 = allsets[700]["tQ"]
    d700 = allsets[700]["dQ"]
    m700 = allsets[700]["maskQ"]
    for r in range(270, 283):
        o = off_of(r, 313)
        a = int(abs(int(d700[o]))) if m700[o] else None
        L.append(f"s700c313 win r={r} inT0={bool(t0[o])} "
                 f"inT700={bool(t700[o])} "
                 f"ad700={a if a is not None else 'n/a'}")
    return L
...
def main():
    ...
    # ---- M36 shared-site + missing + extra value pins (brief stop rule) ----
    print("== M36 value pins (3 shared + 10 miss + 24 extra or stop) ==")
    g0 = (np.frombuffer(v0, np.uint8).astype(np.int16)
          - np.frombuffer(full0, np.uint8).astype(np.int16))
    pin_ok = True
    for Q in QLIST:
    ...
    for Q in QLIST:
        for c in SHAPES[Q]["moved"]:
            ...
            print(f"pinset s{Q}c{c}: extra rows={gote} (want {wante}) "
                  f"match={good}")
    # shared-shape-set check: each shared (r,c) missing exactly on its shapes
    for (r, c), (_, _, _, missh, _) in sorted(SHARED_PINS.items()):
        o = off_of(r, c)
        got = sorted(Q for Q in QLIST
                     if bool(allsets[Q]["cells"][c]["mi"][o]))
        good = got == sorted(missh)
        pin_ok = pin_ok and good
        print(f"pinshared rc=({r},{c}): misshapes={got} "
              f"(want {sorted(missh)}) match={good}")
    print("value pins: " + ("OK" if pin_ok else
                            "MISMATCH vs M36: STOP, tabled."))
    if not pin_ok:
        return

    # ---- Task 1: shared-site recompute (canon for determinism) ----
    print("== Task 1: shared-site recompute ==")
    t1 = time.time()
    canon = shared_canon_lines(allsets, trips, v0, full0)
    for ln in canon:
        print(ln)
    print(f"(Task-1 wall: {time.time() - t1:.1f}s)")

    # ---- Task 2.1: row/col recurrence ----
    print("== Task 2.1: row/col recurrence ==")
    for ln in recur_lines(allsets):
        print(ln)

    # ---- Task 2.2: standing context ----
    print("== Task 2.2: standing context ==")
    for ln in standing_lines(allsets):
        print(ln)

    # ---- Task 2.3: s700 c313 row-list around r276 ----
    print("== Task 2.3: s700 c313 around r276 ==")
    for ln in s700c313_lines(allsets):
        print(ln)
    ...
    for Q in QLIST:
        ...
        allsets2[Q] = compute_sets_for(v0b, mid0b, full0b, b0b, vQb,
                                       midQb, fullQb, bQb,
                                       SHAPES[Q]["moved"])
    canon2 = shared_canon_lines(allsets2, trips2, v0b, full0b)
    ...
    o276 = off_of(276, 313)
    print(f"sets identical s700: (276,313) inT700="
          f"{bool(allsets2[700]['tQ'][o276])} "
          f"(want {bool(allsets[700]['tQ'][o276])})")

    # ---- PNG shared-site map (work dir; evidence copy iff rule meets) ----
    print("== PNG shared-site map ==")
    ...
    for Q in QLIST:
        for c in SHAPES[Q]["moved"]:
            im_e, _, _ = flat_to_planes(allsets[Q]["cells"][c]["ex"])
            over[im_e] = np.array([255, 0, 0])    # pooled extras red
            im_m, _, _ = flat_to_planes(allsets[Q]["cells"][c]["mi"])
            over[im_m] = np.array([255, 0, 255])  # other missings magenta
    for (r, c) in sorted(SHARED_PINS):
        o = off_of(r, c)
        col = (np.array([255, 255, 0]) if int(g0[o]) > 0
               else np.array([0, 255, 255]))      # shared ++ yellow / -- cyan
        over[r, c] = col
    p = workd / "m38-sharedmap.png"
    Image.fromarray(over.astype(np.uint8)).resize((320, 224),
                                                  Image.BILINEAR).save(p)
    print(f"png {p}: {p.stat().st_size} B (budget 5242880)")
    print("legend: green=s700-shared red=pooled-extras magenta=other-"
          "missings yellow=shared-++ cyan=shared--- (s0 geometry; Y only)")
    ...
    for Q in QLIST:
        ...
        gQ_of[Q] = (np.frombuffer(vQ, np.uint8).astype(np.int16)
                    - np.frombuffer(fullQ, np.uint8).astype(np.int16))
    # H1: gap-range 0 across missing shapes on >=2/3 shared sites
    h1_rng = {}
    for (r, c), (_, _, _, missh, _) in SHARED_PINS.items():
        o = off_of(r, c)
        mg = [int(gQ_of[Q][o]) for Q in sorted(missh)]
        h1_rng[(r, c)] = max(mg) - min(mg)
    h1 = sum(1 for v in h1_rng.values() if v == 0) >= 2
    print(f"H1_GAP_AGREE: ranges={ {k: v for k, v in sorted(h1_rng.items())} }"
          f" nagree0={sum(1 for v in h1_rng.values() if v == 0)}/3 "
          f"(bar >=2; met={h1})")
    # H2: >=1 shared site all-++ AND >=1 all--- (over missing shapes)
    def site_signs(r, c):
        o = off_of(r, c)
        _, _, _, missh, _ = SHARED_PINS[(r, c)]
        return [sgn(int(g0[o])) + sgn(int(gQ_of[Q][o]))
                for Q in sorted(missh)]
    h2_pp = [rc for rc in SHARED_PINS if set(site_signs(*rc)) == {"++"}]
    h2_mm = [rc for rc in SHARED_PINS if set(site_signs(*rc)) == {"--"}]
    h2 = len(h2_pp) >= 1 and len(h2_mm) >= 1
    print(f"H2_SHARED_SIGN_SPLIT: allpp={h2_pp} allmm={h2_mm} "
          f"(bar >=1 each; met={h2})")
    # H3/H4: recurrence of shared rows/cols at other pooled sites
    sites = []
    for Q in QLIST:
        for c in SHAPES[Q]["moved"]:
            for o in np.nonzero(allsets[Q]["cells"][c]["ex"])[0]:
                sites.append((int(pr[o]), int(pc[o])))
            for o in np.nonzero(allsets[Q]["cells"][c]["mi"])[0]:
                sites.append((int(pr[o]), int(pc[o])))
    shrc = set(SHARED_PINS)
    h3_rows = sorted({rr for (rr, cc) in sites
                      if rr in SHARED_ROWS and (rr, cc) not in shrc})
    h4_cols = sorted({cc for (rr, cc) in sites
                      if cc in SHARED_COLS and (rr, cc) not in shrc})
    print(f"H3_ROW_RECURRENCE: rows={h3_rows} (bar >=1; "
          f"met={len(h3_rows) >= 1})")
    print(f"H4_COL_RECURRENCE: cols={h4_cols} (bar >=1; "
          f"met={len(h4_cols) >= 1})")
    # H5: 10/10 pooled missings noncell on own Q
    h5_non = sum(1 for (Q, r, c) in M36_MISS
                 if not allsets[Q]["maskQ"][off_of(r, c)])
    print(f"H5_MISS_NONCELL: noncell={h5_non}/10 (bar 10/10; "
          f"met={h5_non == 10})")
    # H6: (276,313) tail on s700
    h6 = bool(allsets[700]["tQ"][o276])
    print(f"H6_S700_HOLDS: (276,313) inT700={h6} (bar True; met={h6})")
    print(f"PNG rule: H1={h1} H2={h2} (copy iff H1 and H2)")
    print("N: 0 explained; tail stands (attribution, not explanation)")

    print(f"== total wall: {time.time() - t_start:.1f}s ==")
    print("tests: T1=shared-recompute T2=recur+standing+s700c313 R=controls")
