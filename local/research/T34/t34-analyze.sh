#!/usr/bin/env bash
# T33 trace analysis on the SnowJam-Cross emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t34-census.txt
python3 $D/t4-sample.py $E $D/t34-samples.txt
ls -la $D/t34-census.txt $D/t34-samples.txt
date -u
