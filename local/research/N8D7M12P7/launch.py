#!/usr/bin/env python3
"""N8D7M12 Part 7: leased Odin diagnostic replay pair (run1, run2) of the staged stream.

Reviewed-SHA-held launcher for EXACTLY ONE diagnostic replay per --run
invocation (run1 then run2, one launch each) of the already-staged
...
reviewed SHA in a separate run gate (require --run run1|run2
...
RUN = None  # "run1" or "run2", set from --run before any device action
...
LEASE_TAG = None  # f"N8D7M12P7 {RUN} replay", set from --run before any device action
...
result = {"brief": "N8D7M12P7", "install_count": 0, "launch_count": 0,
...
def run():
    ...
    if len(sys.argv) != 5 or sys.argv[1] != "--run" \
            or sys.argv[2] not in ("run1", "run2") \
            or sys.argv[3] != "--released-sha":
        raise SystemExit("review hold: require --run run1|run2 "
                         "--released-sha <reviewed launch.py SHA-256>")
    RUN = sys.argv[2]
    SCRATCH = SCRATCH_BASE / RUN
    LEASE_TAG = f"N8D7M12P7 {RUN} replay"
    ...
    if (SCRATCH / "result.json").exists() or (SCRATCH / "driver.log").exists():
        raise SystemExit(f"one-run guard: N8D7M12P7 {RUN} receipt already exists")
    result["run"] = RUN
    ...
    stamp = int(time.time())
    ppm_dir = f"{FILES}/n8d7m12p7-{RUN}-frames-{stamp}"
    hashes_path = f"{FILES}/n8d7m12p7-{RUN}-{stamp}.hashes"
        ...
        if lease_claimed:
            ...
            try:
                ...
                if current == LEASE_TAG:
                    shell(f"echo 'LEASE_FREE N8D7M12P7 {RUN} done' > {LEASE}",
                          timeout=30)
