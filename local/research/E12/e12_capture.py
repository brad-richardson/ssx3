#!/usr/bin/env python3
"""E12 progression capture driver. Explicit preflight, caps, owned lease cleanup."""
import argparse
import datetime
import hashlib
import json
...
from e12_frame import read_png, fnv32
EVIDENCE = REPO / 'local/research/E12'
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E12 forbids lease wait')
        raise RuntimeError('Lease occupied; E12 tables and stops without waiting')
    ...
    assert manifest_path.exists(), 'fresh E12 build manifest required'
    ...
    assert entry['predicate_cases']==14 and entry['sibling_absent']
...
def main():
    ...
    assert not list(EVIDENCE.glob('e12?-result.json')), 'E12 permits only one boot'
