#!/usr/bin/env python3
"""Join E12 query calls/returns to same-run constructor and UI-pointer guards."""
import argparse,json
from collections import Counter
from pathlib import Path
from e12_mine import tap
E=Path(__file__).resolve().parent;RUN=Path('/Volumes/Extreme SSD/ps2recomp-spike/P1/run')
def main():
    ap=argparse.ArgumentParser();ap.add_argument('phase',choices=['a','b']);args=ap.parse_args();label='e12'+args.phase
    ...
    with (E/f'{label}-card.txt').open('w') as f:
        for key in ['constructors','ui_pointers','query_calls','query_returns','truth','target_counts','port_query_pairs','pending_writes','predicate_pairs','ui344_writes','unmatched_returns','unclosed_calls']:
            f.write(key+' '+json.dumps(table[key],sort_keys=True)+'\n')
        f.write('# E12 CARD TAIL COMPLETE: same-run objects and query truth/return joins\n')
    print(label,'queries',len(queries),'objects',[hex(x) for x in sorted(objects)],'targets',table['target_counts'])
    print('# E12 CARD TAIL COMPLETE')
