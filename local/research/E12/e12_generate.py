#!/usr/bin/env python3
"""E12-only APFS generation with fresh admission, bounds and complete manifests."""
...
OUT=Path('/tmp/ssx3-e12-predicate-codegen');TOOL=Path('/Volumes/Extreme SSD/ps2x-i10/host-recomp/ps2xRecomp/ps2_recomp')
...
def main():
    ...
    phase=sys.argv[1];assert phase=='entry','E12 authorizes predicate generation only';assert not (E/f'{phase}-generation.json').exists()
    ...
    assert sha(csv)=='41024f83a9e95408a48dbb91336db52b88f0b6fc7335a91eb6719479679f363d'
    ...
    assert 'sub_002C5358,' not in csv.read_text()
    ...
    OUT.mkdir(exist_ok=False);(OUT/'E12-owner.json').write_text(json.dumps(dict(evidence=str(E),pid=os.getpid(),utc=rec['utc'])))
    ...
    for name in ['register_functions.cpp','ps2_recompiled_functions.h','ps2_recompiled_stubs.h','sub_00426230_0x426230.cpp','sub_002C5140_0x2c5140.cpp','sub_002C5300_0x2c5300.cpp','sub_002C52D8_0x2c52d8.cpp']:
    ...
    predicate=(OUT/'sub_002C5300_0x2c5300.cpp').read_text()
    ...
    print('# E12 GENERATION TAIL COMPLETE phase='+phase)
