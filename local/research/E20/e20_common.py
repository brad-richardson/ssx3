...
NEWB = Path('/tmp/e20-mpeg-link/runtime')
B = Path(os.environ.get('E20_BUILD_DIR',str(B0)))
OUT = Path('/tmp/e20-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
...
def sample():
    paths = list(P.glob('e20-*')) + list(P.glob('._e20-*')) + list((P/'run').glob('*e20*'))
    # The shared function trace is charged even though its name lacks E20.
