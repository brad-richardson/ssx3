...
def observer(env,dest):
    library=os.environ.get('E20_OBSERVER_DYLIB')
    if library:
        directory=dest/'parser-observer';directory.mkdir(exist_ok=False)
        env.update(DYLD_INSERT_LIBRARIES=library,PS2X_E20_PARSER_DIR=str(directory))
    return env
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e20-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E20 requires E18's delivered behavior at baseline
        ...
        if '# E7 SHUTDOWN' in taps:
            from e20_closed_events import tap
    ...
    print('# E20 VALIDATION TAIL COMPLETE')
    return result
