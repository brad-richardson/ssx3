#!/usr/bin/env python3
"""E55D11 Part 1: ONE bounded Save/Load-selection boot (prepared, NOT run).
...
Route (PS2X_PAD_SCRIPT_CLOCK=vsync; guest ms = tick * 100000 / 5994):
  ...
  - ONE Square press at guest tick 820 (13680 ms), hold 150 ms (E55D10-proved
    Options opener; E55D10 post-press frames 919/944 show Options with Game
    Options highlighted and Save/Load four rows below).
  - FOUR separate Down pulses after Options settles: ticks 1000 / 1070 /
    1140 / 1210 (16683 / 17851 / 19019 / 20187 ms), hold 150 ms each.
    Every inter-input gap >= 60 guest ticks (180, 70, 70, 70).
  - Pre-Cross frame target F_PRE = 1290 (21522 ms): settled Save/Load-row
    highlight capture before Cross (80 ticks after D4).
  - ONE Cross press at guest tick 1360 (22689 ms), hold 150 ms (gap 150
    from D4, 70 after F_PRE target).
  - Post-Cross settled full-frame target F_POST = 1460 (24358 ms), 100
    ticks after Cross. Stop tick = 1460.
...
Fresh empty card roots only (refuses non-empty at start).

Stop only after tick >= 1460 AND a persisted snap PNG tagged >= 1460
...
Method (adapted from E55D10's proven e55d10_boot.py, read-only source):
  per-run cwd (~/dev/ssx3-work/E55D11/run/<label>, reserved for the released
...
Usage: e55d11_boot.py --label S1          (ONE boot only, after release)
       e55d11_boot.py --self-check        (pure helper tests, no boot/lease)
"""
import argparse
import hashlib
import json
...
WORK = Path('/Users/brad/dev/ssx3-work/E55D11')
...
# Planned guest ticks (I26-FAST spine + E55D10 observation: Options settled
# by ~919 with Game Options highlighted, Save/Load four rows below).
START_TICK = 636
SQUARE_TICK = 820
DOWN_TICKS = (1000, 1070, 1140, 1210)
PRE_CROSS_FRAME_TICK = 1290
CROSS_TICK = 1360
POST_FRAME_TICK = 1460
MIN_INPUT_GAP = 60
...
ROUTE = ','.join(
    ...
    ['%d:cross:150' % tick_to_ms(CROSS_TICK)]
)
...
def route_self_check():
    """Pure route/limit checks: START, one Square, four Downs, one Cross."""
    cases = []
    ...
    record('route_first_is_start_636',
           ...
           str(entries[0]))
    record('route_entry_count_7', len(entries) == 7, str(entries))
    ...
    record('route_exactly_four_downs', len(downs) == 4, str(downs))
    record('route_down_ticks_1000_1070_1140_1210',
           [e[0] for e in downs] == [tick_to_ms(t) for t in DOWN_TICKS],
    ...
    record('route_exactly_one_cross', len(crosses) == 1, str(crosses))
    record('route_cross_tick_1360',
           crosses and crosses[0][0] == tick_to_ms(CROSS_TICK),
           str(crosses[0] if crosses else None))
    record('route_cross_hold_150',
           crosses and crosses[0][2] == 150,
           str(crosses[0] if crosses else None))
    record('route_input_order_start_square_downs_cross',
    ...
    input_ticks = [START_TICK, SQUARE_TICK] + list(DOWN_TICKS) + [CROSS_TICK]
    gaps = [b - a for a, b in zip(input_ticks, input_ticks[1:])]
    record('route_all_input_gaps_ge_60', all(g >= MIN_INPUT_GAP for g in gaps),
           'gaps=%s' % gaps)
    record('route_pre_cross_frame_between_d4_and_cross',
           DOWN_TICKS[-1] < PRE_CROSS_FRAME_TICK < CROSS_TICK,
           'D4=%d Fpre=%d C=%d' % (DOWN_TICKS[-1], PRE_CROSS_FRAME_TICK, CROSS_TICK))
    record('route_post_frame_after_cross', POST_FRAME_TICK > CROSS_TICK,
           'C=%d Fpost=%d' % (CROSS_TICK, POST_FRAME_TICK))
    ...
    with tempfile.TemporaryDirectory(prefix='e55d11-lane-self-') as tmp:
        lane = Path(tmp) / 'S1'
    ...
    with tempfile.TemporaryDirectory(prefix='e55d11-proof-self-') as tmp:
        snap = Path(tmp) / 'snap'
        record('frame_proof_missing_dir', latest_frame_proof(snap) is None)
        snap.mkdir()
        record('frame_proof_empty', latest_frame_proof(snap) is None)
        (snap / 'snap-001459t-00010.00s.png').write_bytes(b'f1')
        (snap / 'snap-001460t-00011.00s.txt').write_bytes(b't')
        (snap / 'snap-001461t-00012.00s.png').write_bytes(b'f2')
        got = latest_frame_proof(snap)
        record('frame_proof_max_tick', got == (1461, 'snap-001461t-00012.00s.png'),
               str(got))
...
def main():
    ...
    result = {'label': args.label, 'stop_tick': STOP_TICK,
              ...
              'pre_cross_frame_tick': PRE_CROSS_FRAME_TICK,
              'cross_tick': CROSS_TICK, 'post_frame_tick': POST_FRAME_TICK,
            ...
            try:
                while True:
                    ...
                    if phase2_start is None:
                        ...
                        # Post-1460 full-frame proof: the 1 s snapshotter may
                        # not yet have copied F_POST when tick 1460 lands.
