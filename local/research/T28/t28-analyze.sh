#!/usr/bin/env bash
# T28 trace analysis on the menu-Cross emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t28-census.txt
python3 $D/t4-sample.py $E $D/t28-samples.txt
ls -la $D/t28-census.txt $D/t28-samples.txt
date -u
