...
def main():
    ...
    folder=W/'P1/e13-tooltmp'/a.label;folder.mkdir(parents=True,exist_ok=False)
    owner=dict(label=a.label,evidence=str(E),utc=datetime.datetime.now(datetime.timezone.utc).isoformat())
    (folder/'E13-owner.json').write_text(json.dumps(owner))
    ...
    assert json.loads((folder/'E13-owner.json').read_text())==owner
    ...
    print('# E13 BOUNDED TOOL TAIL COMPLETE')
