#!/usr/bin/env python3
"""E13 progression capture driver. Explicit preflight, caps, owned lease cleanup."""
import argparse
import datetime
import hashlib
import json
...
from e13_frame import read_png, fnv32
EVIDENCE = REPO / 'local/research/E13'
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E13 forbids lease wait')
        raise RuntimeError('Lease occupied; E13 tables and stops without waiting')
    ...
    assert manifest_path.exists(), 'fresh E13 build manifest required'
    ...
    assert entry['predicate_cases']==14 and entry['leaf_cases']==24 and entry['consumer_cases']==3 and entry['leaf_present']
...
def main():
    ...
    assert not list(EVIDENCE.glob('e13?-result.json')), 'E13 permits only one boot'
    label='e13'+args.phase
