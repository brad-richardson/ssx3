#!/usr/bin/env python3
"""Mine completed E13 captures only. Standard library; no guest execution."""
import argparse
...
def strict_live_window_tap(path):
    rows=[]
...
def main():
    ...
    with (EVIDENCE/f'{label}-watch-series.txt').open('w') as f:
        ...
        f.write('# E13 WATCH SERIES TAIL COMPLETE\n')
    ...
    if True:
        ...
        summary['observation_closure']=json.loads((EVIDENCE/'e13a-observation-closure.json').read_text())
        ...
        if not packets and (EVIDENCE/f'{label}-retained.json').exists():
        ...
        with (EVIDENCE/f'{label}-burst-series.txt').open('w') as f:
            ...
            f.write('# E13 BURST SERIES TAIL COMPLETE\n')
    ...
    print('# E13 MINER TAIL COMPLETE',path)
