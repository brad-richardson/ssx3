#!/usr/bin/env python3
"""E13-only APFS generation with fresh admission, bounds and complete manifests."""
...
OUT=Path('/tmp/ssx3-e13-leaf-codegen');TOOL=Path('/Volumes/Extreme SSD/ps2x-i10/host-recomp/ps2xRecomp/ps2_recomp')
...
def main():
    ...
    phase=sys.argv[1];assert phase=='entry','E13 authorizes leaf generation only';assert not (E/f'{phase}-generation.json').exists()
    gate=json.loads((E/'checkpoint.json').read_text());assert gate['binding_absent_rc']==0 and gate['binding_present_rc']==1
    ...
    assert sha(csv)=='0c6085744ab1a2afd60170a18ac261db02472441fa1d343ffd6756026e92d9f4'
    assert csv.read_text().count('sub_002C5140,0x2c5140,0x2c5168,0x28')==1
    assert csv.read_text().count('sub_002C5300,0x2c5300,0x2c5320,0x20')==1
    assert csv.read_text().count('sub_002C5358,0x2c5358,0x2c53b0,0x58')==1
    ...
    OUT.mkdir(exist_ok=False);(OUT/'E13-owner.json').write_text(json.dumps(dict(evidence=str(E),pid=os.getpid(),utc=rec['utc'])))
    ...
    for name in ['register_functions.cpp','ps2_recompiled_functions.h','ps2_recompiled_stubs.h','sub_00426230_0x426230.cpp','sub_002C5140_0x2c5140.cpp','sub_002C5300_0x2c5300.cpp','sub_002C52D8_0x2c52d8.cpp','sub_002C5338_0x2c5338.cpp','sub_002C5358_0x2c5358.cpp']:
    ...
    selected={hex(pc):[l for l in registry.splitlines() if l.rstrip().endswith(f'// 0x{pc:x}')] for pc in [0x426230,0x2c5140,0x2c5300,0x2c5358]}
    assert len(selected['0x426230'])==1 and 'sub_00426230_0x426230' in selected['0x426230'][0]
    assert len(selected['0x2c5140'])==1 and 'sub_002C5140_0x2c5140' in selected['0x2c5140'][0]
    assert len(selected['0x2c5300'])==1 and 'sub_002C5300_0x2c5300' in selected['0x2c5300'][0]
    assert len(selected['0x2c5358'])==1 and 'sub_002C5358_0x2c5358' in selected['0x2c5358'][0]
    predicate=(OUT/'sub_002C5358_0x2c5358.cpp').read_text()
    assert 'GuestBranchKind::DirectCall' not in predicate and 'GuestBranchKind::IndirectCall' not in predicate and 'ps2_stubs::' not in predicate
    assert predicate.count('dispatchGuestBranch')>=1 and 'GuestBranchKind::Return, "JR $ra"' in predicate
    ...
    print('# E13 GENERATION TAIL COMPLETE phase='+phase)
