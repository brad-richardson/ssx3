#!/usr/bin/env python3
"""VK1 Odin launcher: N12's launch.py (F5 lineage: play-env pin a8d651a7, mc0 pins,
...
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/VK1/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('VK1 ')):
...
sh(f'echo "VK1 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE VK1 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# VK1 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
sh("echo 'LEASE_FREE VK1 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
