#!/bin/bash
# VK1 close-out: put Brad's F5 play build back exactly (brief rule): F5 APK
...
case "$lease" in LEASE_FREE*|VK1\ *) ;; *) echo "lease held: $lease" >&2; exit 1;; esac
adb -s "$S" shell "echo 'VK1 $(date -u +%FT%TZ) restore' > /data/local/tmp/mg/LEASE"
trap 'adb -s "$S" shell "echo LEASE_FREE VK1 done > /data/local/tmp/mg/LEASE"' EXIT
...
adb -s "$S" shell "rm -rf /data/local/tmp/vk1 $FILES/vk1dump; rm -rf $FILES/mc0-test/*; ls -A $FILES/mc0-test | wc -l; am force-stop $PKG"
