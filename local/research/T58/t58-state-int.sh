...
leg() {
  ...
  while [ $TRY -lt 3 ]; do
    ...
    sleep 25; wallcheck; snap t58i-$LAB-t$TRY || fail SNAP_${LAB}
    read M P <<< $(score_ref $T4/t58i-$LAB-t$TRY.ppm $EXP)
...
for N in $(seq 1 70); do
  sleep 3; wallcheck
  snap t58i-a1-poll$N || fail SNAP_A1_POLL$N
  read M P <<< $(score_band $T4/t58i-a1-poll$N.ppm)
...
snap t58i-menu-pre || fail SNAP_MENU_PRE
read MM MP <<< $(score_ref $T4/t58i-menu-pre.ppm $REFMENU)
...
cp "$SSD/$STATE_NEWEST" $G/t58-pre-sc-state-int || fail STATE_CP
sha256sum $G/t58-pre-sc-state-int | tee -a $LOG
ls -la $G/t58-pre-sc-state-int
