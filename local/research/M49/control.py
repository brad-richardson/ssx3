#!/usr/bin/env python3
"""M49 control: synthetic truth with KNOWN single move (1 wipe + 1 dest).

Usage: control.py M16_DIR

Mid-level control (M36 C-VAL-style): s0 mid' with a known 9-site
wipe (c296's 9 rows, mimics 732) flipped tail->bulk, plus a known
2-site dest in ONE fresh column flipped bulk->tail (|d|=8).
Checks exact recovery of the wipe + dest + value tables + the
displacement assignment. Read-only inputs; receipt text to
...
from m49 import (assign_one, interior_sites, jaccard, load_dump,  # noqa: E402
                 off_of, plane_coords, plane_of_byte, priv_y_cols,
                 synth_w_bytes, tail_bulk_masks, y_col_rows)

WIPE = [(r, 296) for r in (24, 25, 26, 27, 28, 31, 32, 33, 34)]  # 732's 9


def main():
    m16d = Path(sys.argv[1])
    print("== C-MOVE setup (s0 mid' with known wipe + dest) ==")
    ...
    # dest col: c298 if it holds >=2 landable sites, else first
    # offset-order col with >=2 (M29 C-PART-EXTRA precedent)
    bycol = {}
    for o in cands:
        bycol.setdefault(int(pc[o]), []).append(o)
    destcol, destoffs, destdir = None, [], {}
    colorder = ([298] if 298 in bycol else []) + sorted(
        c for c in bycol if c != 298)
    for c in colorder:
        picks, dirs = [], {}
        for o in bycol[c]:
    ...
    print(f"dest col: c={destcol} (c298 preferred; fallback tabled) "
    ...
    pq, mq = tq & ~t0, t0 & ~tq
    jj = jaccard(tq, t0)
    want_j = "0.8942"  # 93/104 (732's counts exactly)
    print(f"tail'={int(tq.sum())} (want 95) J={jj:.4f} (want {want_j}) "
          f"cell-fixed={mask_fixed}")
    ok_sets = (mask_fixed and int(tq.sum()) == 95
               and f"{jj:.4f}" == want_j
               and set(int(o) for o in np.nonzero(pq)[0]) == set(destoffs)
               and set(int(o) for o in np.nonzero(mq)[0]) == set(woffs))
    print(f"sets-exact (priv=2 dest, miss=9 wipe): {ok_sets}")

    # wipe + dest row recovery
    ok_rows = True
    rq = y_col_rows(tq, 296, pl, pr, pc)
    ok = rq == []
    ok_rows = ok_rows and ok
    print(f"wipe c296: rows={rq} (want []) exact={ok}")
    ...
    # displacement recovery: the wipe col -> dest col
    pcols = priv_y_cols(pq, pl, pr, pc)
    a296 = assign_one(296, pcols)
    ok_disp = (a296["dest"] == destcol)
    print(f"disp c296->{a296['dest']} off={a296['off']} "
          f"rs={a296['rshift']} ms={a296['minshift']}")
    print(f"assign-exact (-> c{destcol}): {ok_disp}")
    ...
    print(f"wipe |d'|: {wipe_ads} (want [1]*9) exact={wipe_ads == [1] * 9}")
    ...
    print(f"dest s0-bulk: {dest_bulk0}/2; wipe mid'-bulk: {wipe_bulkq}/9")
    ...
    ok = (ok_sets and ok_rows and ok_disp and dest_ads == [8, 8]
          and wipe_ads == [1] * 9 and dest_bulk0 == 2 and wipe_bulkq == 9)
    print(f"C-MOVE pass: {ok}")
    print("tests: R=C-MOVE")
