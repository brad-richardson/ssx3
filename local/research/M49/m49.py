#!/usr/bin/env python3
"""M49 shape-732 c296->c298 move: dest-row listing + value table (offline).

Usage: m49.py M16_DIR M15_DIR M28TSV M34TSV WORK_DIR EVID_DIR
...
goes to stdout (redirect to WORK_DIR/m49.txt); PNG move map
to WORK_DIR (evidence copy iff the DESIGN.md rule meets, decided
at report time).

Tasks (see DESIGN.md, recorded before running):
  1: 732 wipe + dest reproduction (wipe + dest + displacement tables)
  2: full-set value table (wiped + dest + accounting tables)
...
S732 = {"tail": 95, "j": "0.8942", "moved": [296]}
DEST_PIN = {"rows": [26, 27], "adQ": [8, 14], "adO": [1, 6],
            "gQ": [-19, -33], "gO": [-5, -17],
            "offs": [33876, 35156]}
DISP_PIN = {296: {"dest": 298, "off": 2, "rshift": -1.5, "minshift": 2}}
S0U_PIN = {
...
def shared_lines(tag, kept, dQ, dA, v0Q, fullQ, v0A, fullA):
    """Kept-row value rows: both-frames |d| + gaps + signs (M48 helper).
...
def main():
    ...
    t_start = time.time()
    canon = []  # determinism canonical text (Task 1 on s0+732)

    print("== inputs (read-only) ==")
    guard_bins = [m16d / "m16-v0-s0000.bin", m16d / "m16-mid-s0000.bin",
                  m16d / "m16-full-s0000.bin",
                  m16d / "m16-v0-s0732.bin", m16d / "m16-mid-s0732.bin",
                  m16d / "m16-full-s0732.bin",
    ...
    if r0 != R0_M16 or r15 != R0_M15 or f0 != FNV_S0 or f15 != FNV_M15:
        print("MISMATCH vs M17-M48 baselines: STOP, tabled.")
        return
    if not carriers_ok:
    ...
    print("== 732 row guard ==")
    r732 = per[732]
    mv732 = sorted(c for c in NAMED8 if r732["cen"][c]["pres"] == 0)
    umv732 = sorted(c for c in UNNAMED if r732["ucen"][c]["pres"] == 0)
    print(f"s732: tail={r732['tail']} (want 95) J={r732['j']:.4f} "
          f"(want 0.8942) moved={mv732} (want [296])")
    print(f"s732 c296: n={r732['cen'][296]['n']} "
          f"ov={r732['cen'][296]['ov']} (want 0/0)")
    print(f"s732 unnamed moved: {umv732} (want [298])")
    print(f"s732 c298: n={r732['ucen'][298]['n']} "
          f"ov={r732['ucen'][298]['ov']} (want 6/4)")
    g732 = (r732["tail"] == 95 and f"{r732['j']:.4f}" == "0.8942"
            and mv732 == [296]
            and r732["cen"][296]["n"] == 0 and r732["cen"][296]["ov"] == 0
            and umv732 == [298]
            and r732["ucen"][298]["n"] == 6 and r732["ucen"][298]["ov"] == 4)
    print("732-row guard: " + ("OK" if g732 else "MISMATCH: STOP, tabled."))
    if not g732:
        return
    ...
    # ---- load triplets for 732 (Task 1/2 row + value tables) ----
    def load_sets(s):
        vv = load_dump(m16d / f"m16-v0-s{s:04d}.bin")
        mm = load_dump(m16d / f"m16-mid-s{s:04d}.bin")
        ff = load_dump(m16d / f"m16-full-s{s:04d}.bin")
    ...
    m732 = t0 & ~t732
    s732sh = t732 & t0
    print(f"s732 sets: priv={int(p732.sum())} miss={int(m732.sum())} "
          f"shared={int(s732sh.sum())} tail={int(t732.sum())}")

    # ---- displacement guard (M28 rule row) ----
    print("== 732 displacement guard (M28 row) ==")
    pcols732 = priv_y_cols(p732, pl, pr, pc)
    print(f"s732 private Y-cols: {sorted(pcols732.items())}")
    print(f"s732 private U/V sites: "
          f"{int(p732.sum()) - sum(len(v) for v in pcols732.values())}")
    disp_ok = True
    for c in (296,):
        a = assign_one(c, pcols732)
        w = DISP_PIN[c]
        ok = (a["dest"] == w["dest"] and a["off"] == w["off"]
              and a["rshift"] == w["rshift"] and a["minshift"] == w["minshift"]
              and a["dest"] not in NAMED8)
        print(f"move s732 c{c}->{a['dest']}: off={a['off']:+d} "
              f"(want {w['off']:+d}) rshift={fmt_f(a['rshift'])} "
              f"(want {fmt_f(w['rshift'])}) minshift={a['minshift']:+d} "
              f"(want {w['minshift']:+d}) fresh={a['dest'] not in NAMED8} "
              f"match={ok}")
    ...
    # ---- M44 dest-value guard ([26,27] + |d| + gaps) ----
    print("== 732 M44-value guard ==")
    a0 = np.frombuffer(v0, np.uint8).astype(np.int16)
    b0f = np.frombuffer(full0, np.uint8).astype(np.int16)
    aQ = np.frombuffer(q732["v0"], np.uint8).astype(np.int16)
    bQ = np.frombuffer(q732["full"], np.uint8).astype(np.int16)
    g0, gQ = a0 - b0f, aQ - bQ
    rows732_c298 = y_col_rows(t732, 298, pl, pr, pc)
    extra298 = sorted(set(rows732_c298) - set(S0U_PIN[298]))
    print(f"s732 c298 rows={rows732_c298} extra={extra298} "
          f"(want {DEST_PIN['rows']})")
    m44_ok = extra298 == DEST_PIN["rows"]
    for i, r in enumerate(DEST_PIN["rows"]):
        o = off_of(r, 298)
        adq = int(abs(int(q732["d"][o])))
        ...
        m44_ok = m44_ok and row_ok
    print("M44-value guard: "
          + ("OK" if m44_ok else "MISMATCH: STOP, tabled."))
    if not m44_ok:
        return

    # ---- Task 1.1: wipe table (c296) ----
    print("== Task 1.1: wipe table (c296) ==")
    for c in (296,):
        r0c = S0_ROWS[c]
        rqc = y_col_rows(t732, c, pl, pr, pc)
        missc = sorted(set(r0c) - set(rqc))
        print(f"wipe c{c}: s0-rows={r0c} s732-rows={rqc} miss={missc} "
              f"n/ov={r732['cen'][c]['n']}/{r732['cen'][c]['ov']}")
        canon.append(f"canon wipe c{c}: s0={r0c} q={rqc} miss={missc}")

    # ---- Task 1.2: dest table (c298 full row-list) ----
    print("== Task 1.2: dest table (c298 kept + extras) ==")
    r0c = S0U_PIN[298]
    rqc = rows732_c298
    keptc = sorted(set(r0c) & set(rqc))
    print(f"dest c298: s0-rows={r0c} s732-rows={rqc} kept={keptc} "
          f"extra={extra298} n/ov={r732['ucen'][298]['n']}/"
          f"{r732['ucen'][298]['ov']}")
    canon.append(f"canon dest c298: s0={r0c} q={rqc} kept={keptc} "
                 f"extra={extra298}")

    # ---- Task 1.3: displacement table (M28 rule row) ----
    print("== Task 1.3: displacement table (M28 rule row) ==")
    cands732 = {c: v for c, v in pcols732.items() if len(v) >= 2}
    print(f"s732 >=2-site candidates: {sorted(cands732.items())}")
    canon.append(f"canon disp pcols732={sorted(pcols732.items())}")
    for c in (296,):
        a = assign_one(c, pcols732)
        print(f"move s732 c{c}->{a['dest']}: off={a['off']:+d} "
              f"rshift={fmt_f(a['rshift'])} minshift={a['minshift']:+d} "
              f"npriv={a['npriv']} prows={cands732[a['dest']]} "
              f"fresh={a['dest'] not in NAMED8}")
        canon.append(f"canon disp c{c}: dest={a['dest']} off={a['off']} "
                     f"rs={fmt_f(a['rshift'])} ms={a['minshift']} "
                     f"status={a['status']}")
    eo298 = r732["ucen"][298]["miss"] == 0
    print(f"s732 c298 extra-only (miss==0): {eo298}")
    canon.append("canon disp c298: dest=None status=extra-only")

    # ---- Task 2.1: wiped-value table (9 rows, M36-style) ----
    print("== Task 2.1: wiped-value table (9 rows, M36-style) ==")
    empty = np.zeros(N, bool)
    wlines, wout = attrib_lines("wipe732", empty, m732, s732sh,
                                d0, mask0, q732["d"], q732["mask"],
                                v0, full0, q732["v0"], q732["full"])
    for ln in wlines:
        print(ln)
        if ln.startswith("wipe732 miss o="):
            canon.append("canon " + ln)
    for ln in gapsign_lines("wipe732", empty, m732,
                            v0, full0, q732["v0"], q732["full"]):
        print(ln)
        canon.append("canon " + ln)
    for ln in dstats_lines("wipe732", empty, m732, q732["d"], d0):
        print(ln)
        canon.append("canon " + ln)

    # ---- Task 2.2: dest-value table (extras + kept, M36-style) ----
    print("== Task 2.2: dest-value table (extras + kept) ==")
    dlines, dout = attrib_lines("dest732", p732, empty, s732sh,
                                d0, mask0, q732["d"], q732["mask"],
                                v0, full0, q732["v0"], q732["full"])
    for ln in dlines:
        print(ln)
        if ln.startswith("dest732 priv o="):
            canon.append("canon " + ln)
    for ln in gapsign_lines("dest732", p732, empty,
                            v0, full0, q732["v0"], q732["full"]):
        print(ln)
        canon.append("canon " + ln)
    for ln in dstats_lines("dest732", p732, empty, q732["d"], d0):
        print(ln)
        canon.append("canon " + ln)
    kept = np.zeros(N, bool)
    for r in keptc:
        o = off_of(r, 298)
        assert t732[o] and t0[o], f"kept c298 r{r}: not shared-tail?!"
        kept[o] = True
    for ln in shared_lines("keep732", kept, q732["d"], d0,
                           q732["v0"], q732["full"], v0, full0):
        print(ln)
        canon.append("canon " + ln)

    # ---- Task 2.3: accounting table (counts only, no assignment) ----
    print("== Task 2.3: accounting table (numbers only) ==")
    n_wipe = len(S0_ROWS[296]) - 0  # c296 0/0: all 9 wiped
    n_growth = len(extra298)
    n_kept = len(keptc)
    print(f"wiped rows: c296={n_wipe}")
    print(f"dest c298: growth=+{n_growth} kept={n_kept} "
          f"n0={len(S0U_PIN[298])} n={r732['ucen'][298]['n']}")
    print(f"full sets: priv={int(p732.sum())} miss={int(m732.sum())} "
          f"shared={int(s732sh.sum())} tailQ={int(t732.sum())} "
          f"tail0={nt0}")
    print(f"net tail delta: {int(t732.sum()) - nt0} "
          f"(growth-wiped={n_growth - n_wipe})")
    out_priv = sorted((int(pr[o]), int(pc[o]))
                     for o in np.nonzero(p732)[0] if int(pc[o]) != 298)
    out_miss = sorted((int(pr[o]), int(pc[o]))
                     for o in np.nonzero(m732)[0]
                     if int(pc[o]) != 296)
    print(f"privates outside c298: n={len(out_priv)} {out_priv}")
    print(f"missings outside c296: n={len(out_miss)} {out_miss}")
    print(f"counts admit full accounting: "
          f"miss==wiped({int(m732.sum())}=={n_wipe}) "
          f"priv==growth({int(p732.sum())}=={n_growth}) "
          f"outside==0({len(out_priv) + len(out_miss)}==0)")

    # ---- determinism re-run (Task 1 on s0+732, fresh loads) ----
    print("== determinism re-run (Task 1 on s0+732, second pass) ==")
    canon2 = []
    vv = load_dump(m16d / "m16-v0-s0732.bin")
    mm = load_dump(m16d / "m16-mid-s0732.bin")
    ff = load_dump(m16d / "m16-full-s0732.bin")
    bl = synth_w_bytes(vv, ff, 0.5)
    ...
    r0c = S0U_PIN[298]
    rqc = y_col_rows(tq, 298, pl, pr, pc)
    keptc2 = sorted(set(r0c) & set(rqc))
    extra2 = sorted(set(rqc) - set(r0c))
    canon2.append(f"canon dest c298: s0={r0c} q={rqc} kept={keptc2} "
                  f"extra={extra2}")
    pq2, mq2 = tq & ~t0, t0 & ~tq
    sh2 = tq & t0
    pcols2 = priv_y_cols(pq2, pl, pr, pc)
    canon2.append(f"canon disp pcols732={sorted(pcols2.items())}")
    for c in (296,):
        a = assign_one(c, pcols2)
        canon2.append(f"canon disp c{c}: dest={a['dest']} off={a['off']} "
                      f"rs={fmt_f(a['rshift'])} ms={a['minshift']} "
                      f"status={a['status']}")
    canon2.append("canon disp c298: dest=None status=extra-only")
    wlines2, _ = attrib_lines("wipe732", empty, mq2, sh2,
                              d0, mask0, spq["delta"], spq["mask"],
                              v0, full0, vv, ff)
    for ln in wlines2:
        if ln.startswith("wipe732 miss o="):
            canon2.append("canon " + ln)
    for ln in gapsign_lines("wipe732", empty, mq2, v0, full0, vv, ff):
        canon2.append("canon " + ln)
    for ln in dstats_lines("wipe732", empty, mq2, spq["delta"], d0):
        canon2.append("canon " + ln)
    dlines2, _ = attrib_lines("dest732", pq2, empty, sh2,
                              d0, mask0, spq["delta"], spq["mask"],
                              v0, full0, vv, ff)
    for ln in dlines2:
        if ln.startswith("dest732 priv o="):
            canon2.append("canon " + ln)
    for ln in gapsign_lines("dest732", pq2, empty, v0, full0, vv, ff):
        canon2.append("canon " + ln)
    for ln in dstats_lines("dest732", pq2, empty, spq["delta"], d0):
        canon2.append("canon " + ln)
    kept2 = np.zeros(N, bool)
    for r in keptc2:
        kept2[off_of(r, 298)] = True
    for ln in shared_lines("keep732", kept2, spq["delta"], d0,
                           vv, ff, v0, full0):
        canon2.append("canon " + ln)
    ...
    for r in keptc:
        img[r // 2, 298 // 2] = (0, 255, 0)
        px.setdefault((298 // 2, r // 2), []).append("kept")
    for r in S0_ROWS[296]:
        img[r // 2, 296 // 2] = (255, 0, 0)
        px.setdefault((296 // 2, r // 2), []).append("wipe296")
    for r in extra298:
        img[r // 2, 298 // 2] = (255, 255, 0)
        px.setdefault((298 // 2, r // 2), []).append("extra")
    p = workd / "m49-movemap.png"
    Image.fromarray(img).save(p)
    print(f"png {p}: {p.stat().st_size} B (budget 5242880)")
    print("legend: x=col//2 y=row//2; green=kept c298(4), red=wiped "
          "c296(9), yellow=dest extras(2)")
    ...
    print(f"H1_WIPENEAR: wiped near {wout['miss']['near']}/9 "
          f"({wout['miss']['near'] / 9:.4f}; bar >=0.50)")
    print(f"H2_WIPESET: wiped noncell {wout['miss']['non']}/9 "
          f"({wout['miss']['non'] / 9:.4f}; bar >=0.50)")
    print(f"H3_DESTNEAR: dest-extra near {dout['priv']['near']}/2 "
          f"({dout['priv']['near'] / 2:.4f}; bar >=0.50)")
    mm = sum(1 for o in np.nonzero(m732)[0]
             if int(g0[int(o)]) < 0 and int(gQ[int(o)]) < 0)
    print(f"H4_WIPEMM: wiped -- {mm}/9 ({mm / 9:.4f}; bar >=0.50)")
    wmed = float(np.median(np.asarray(
        sorted(int(abs(int(d0[int(o)]))) for o in np.nonzero(m732)[0]),
        dtype=np.float64)))
    emed = float(np.median(np.asarray(
        sorted(int(abs(int(q732["d"][int(o)])))
               for o in np.nonzero(p732)[0]),
        dtype=np.float64)))
    print(f"H5_VALMED: wiped med {wmed:.1f} vs dest-extra med {emed:.1f} "
          f"(meets iff strictly unequal)")
    a296 = assign_one(296, pcols732)
    print(f"H6_FRESHDEST: dest(c296)={a296['dest']} off={a296['off']} "
          f"fresh={a296['dest'] not in NAMED8} "
          f"(meets iff dest==298 AND off==+2 AND fresh)")
    print("N: 0 explained; tail stands (attribution, not explanation)")

    print(f"== total wall: {time.time() - t_start:.1f}s ==")
    print("tests: T1=single-repro T2=values+accounting R=controls")
