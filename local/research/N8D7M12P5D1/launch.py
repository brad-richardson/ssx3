#!/usr/bin/env python3
"""N8D7M12 Part 5D1: one leased Odin OFF-control replay of the staged stream.

Reviewed-SHA-held OFF launcher for EXACTLY ONE diagnostic replay of the
...
OFF vs ON (Part 5A): identical APK/stream/runner/Turnip/HAL pins,
PS2X_GS_REPLAY_ONDEVICE=1, parallel backend, step 50, PPM tick 2050.
Diff is exactly minus three measurement flags (absent, not =0):
PS2X_N8D7F_SELECTED_CAPTURE, PS2X_N8D7L_ORACLE, PS2X_N8D5_TILE_CAPTURE.
Selected/oracle/tile census markers are NOT required for OFF completion;
summary + 41 step-50 rows through tick2050 + GB4_FRAME tick=2050 +
replay ok markers=2050 + PPM/hashes pull + intact cleanup still are.

Device patterns based on N8D7M6 launch.py and Part 5A launch.py. Replay
enters before EE boot via PS2X_GS_REPLAY_ONDEVICE=1; no live GS capture
key, no pad script, no game thread. No device action may run before the
orchestrator releases the reviewed SHA in a separate run gate (require
--released-sha <launch.py SHA>). No ON or OFF graphics verdict here.
...
PKG = "com.ps2x.runner"
SCRATCH = Path("/Users/brad/dev/ssx3-work/N8D7M12P5D1")
...
# for logging only; OFF completion never requires these addresses.
...
# OFF: the three tick-2050 measurement flags are absent (unset, not =0).
REPLAY_ENV_KEYS = (
...
result = {"brief": "N8D7M12P5D1", "install_count": 0, "launch_count": 0,
...
def tile_vector(lines, name, words, pack):
    ...
    field or inconsistent continuation returns ``None``. Retained in OFF
    for logging only; OFF completion never requires any vector.
    """
...
def parse_controls(payload):
    ...
    Retained in OFF for logging only; OFF completion never requires
    oracle controls.
    """
    if payload is None:
        return None
...
def probe(lines):
    ...
    Retained in OFF for logging only; missing selected/oracle/tile
    lines are expected and never fail OFF completion.
    """
...
def census_gate(p):
    """OFF acceptance: selected/oracle/tile census is NOT required.

    Always passes so missing [n8d5b]/[n8d6a]/[n8d7f]/[n8d7l] markers,
    missing control=128/128, or missing 448/896-tile vectors can never
    falsely fail OFF. The OFF comparison fields (41 replay rows,
    tick-2050 present, PPM bytes) are flag-independent outputs. The
    probe above is retained for logging only.
    """
    del p
    return True
...
def run():
    ...
    if (SCRATCH / "result.json").exists() or (SCRATCH / "driver.log").exists():
        raise SystemExit("one-run guard: N8D7M12P5D1 receipt already exists")
    ...
    stamp = int(time.time())
    ppm_dir = f"{FILES}/n8d7m12p5d1-frames-{stamp}"
    hashes_path = f"{FILES}/n8d7m12p5d1-{stamp}.hashes"
    ...
    while True:
        ...
        # OFF: error gate uses replay-marker errors only; missing
        # selected/oracle/tile census never fails OFF.
        combined_errors = markers["errors"]
        if combined_errors:
    ...
    result["provisional_note"] = (
        ...
        "same-APK ON/OFF comparison; one OFF replay does not establish "
        "a graphics root cause or speed; no graphics verdict here")
        ...
        if lease_claimed:
            ...
            try:
                ...
                if current == LEASE_TAG:
                    shell(f"echo 'LEASE_FREE N8D7M12P5D1 done' > {LEASE}",
                          timeout=30)
