#!/bin/bash
# I25: build + stage + install the SSX 3 PS2 recomp for iOS.
#
...
FFMPEG="$W/ffmpeg-ios${SFX}-7.1.1"
SDL2="$W/sdl2-ios${SFX:--device}"
...
NICE="nice -n 10"
log() { echo "[i25 $(date +%H:%M:%S) $TARGET] $*"; }
...
stage_configure() {
  test "$(git -C "$FORK_WT" rev-parse --abbrev-ref HEAD)" = i25-ios
  test -d "$CODEGEN"
