#!/usr/bin/env python3
"""GB3 boot driver (from GB2): two-slot-lease-aware, wall-capped boot with frame snapshots.

Usage:
  python3 local/research/GB3/gb3_boot.py --label gb3a --wall 300 --script "..." --vq --pklog
  python3 local/research/GB3/gb3_boot.py --label gb3b --wall 300 --script "..." --vq --pklog --gs-queue 1 --csr-drain

GB3 additions: --vq-from/--vq-to/--vq-step (PS2X_VQ_FROM/TO/STEP), --vq-dump
(PS2X_VQ_DUMP_DIR = <run>/vqdump-<label>-1), --setcrt-legacy
(PS2X_GS_SETCRT_LEGACY=1), --backend (PS2X_GS_BACKEND), --env K=V (repeat).
...
RUN = os.path.join(WORK, "GB3-run")
RUNNER = os.path.join(WORK, "GB3", "build", "ps2xRuntime", "ps2EntryRunner")
...
def main():
    ...
    ap.add_argument("--vq-from", default=None)
    ap.add_argument("--vq-to", default=None)
    ap.add_argument("--vq-step", default=None)
    ap.add_argument("--vq-dump", action="store_true", help="PS2X_VQ_DUMP_DIR=<run>/vqdump-<label>-1")
    ap.add_argument("--setcrt-legacy", action="store_true", help="PS2X_GS_SETCRT_LEGACY=1 (pre-GB3 SetGsCrt)")
    ap.add_argument("--backend", default=None, help="PS2X_GS_BACKEND value (omit = unset)")
    ap.add_argument("--env", action="append", default=[], help="extra K=V env (repeatable)")
    args = ap.parse_args()
    ...
    for key, val in (("PS2X_VQ_FROM", args.vq_from), ("PS2X_VQ_TO", args.vq_to),
                     ("PS2X_VQ_STEP", args.vq_step), ("PS2X_GS_BACKEND", args.backend)):
        if val is not None:
            env[key] = str(val)
        else:
            env.pop(key, None)
    vqdump_dir = None
    if args.vq_dump:
        vqdump_dir = os.path.join(RUN, f"vqdump-{label}-1")
        os.makedirs(vqdump_dir, exist_ok=True)
        env["PS2X_VQ_DUMP_DIR"] = vqdump_dir
    else:
        env.pop("PS2X_VQ_DUMP_DIR", None)
    if args.setcrt_legacy:
        env["PS2X_GS_SETCRT_LEGACY"] = "1"
    else:
        env.pop("PS2X_GS_SETCRT_LEGACY", None)
    for kv in args.env:
        k, _, v = kv.partition("=")
        env[k] = v
    print(json.dumps({"event": "env", "gb3": {k: env[k] for k in sorted(env)
                                               if k.startswith(("PS2X_", "GRANITE_"))}}), flush=True)
