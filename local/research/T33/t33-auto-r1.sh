#!/usr/bin/env bash
# T33 (Cross on Snow Jam from the Select Event park): reproduce the T32 R1
...
# gate, Cross on Peak 1, SM-LIKE gate, Cross on Race), confirm the Select
# Event park (Snow Jam highlighted), then press ONE Cross (x Select) on
# Snow Jam and map what comes next.
# Bounded: <=3 park attempts, ONE SnowJam Cross (plus script-captured
# arrival series + stability). Unattended (~8 min; T32 ran 417 s, one more
...
# tag-crop-vs-SE-ref mean<2.0 (SE side <=0.14 PIL; SM side 14.14).
# Post-hoc "Cross acted on SE" bar: whole-vs-SE-ref mean<0.2 PIL
# (SE-SE <=0.063, SM >=0.507) + se-tag<2.0 + viewed snap.
TITLE_MEAN_MAX=2.0
TITLE_P99_MAX=10
STATIC_MEAN_MAX=1.0
MENU_MEAN_MAX=2.0
...
fail() { echo "T33_FAIL:$1"; stamp; exit 1; }
: > $LOG
plog() { echo "$@" | tee -a $LOG; }
...
sleep 2
pgrep -a Xvfb || true
setsid nohup Xvfb :99 -screen 0 1280x1024x24 >/tmp/xvfb-t33.log 2>&1 < /dev/null &
sleep 4
pgrep -a Xvfb || fail NO_XVFB
xdotool getdisplaygeometry || fail NO_DISPLAY
TS=$(date -u +%Y%m%dT%H%M%SZ)
if [ -f $E ]; then
  mv $E $D/dat/PCSX2/logs/emulog-pre-t33-$TS.txt
  ls -la $D/dat/PCSX2/logs/
fi
rm -f $D/pcsx2.pid
T_BOOT=$(date -u +%s)
echo "T_BOOT_WALL:$T_BOOT T_BOOT_UPTIME:$(cut -d. -f1 /proc/uptime)"
nohup $D/pcsx2/build/bin/pcsx2-qt -nogui -slowboot -turbo -datapath $D/dat -logfile $D/logs/boot-t33.log -- "$D/inputs/SSX 3 (USA).iso" > $D/logs/boot-t33.stdout 2>&1 &
echo $! > $D/pcsx2.pid
echo "PID:$!"
sleep 90
...
snap t33-start || fail SNAP_START
read SM SP <<< $(score_band $D/t33-start.ppm)
...
for A in 1 2 3; do
  ...
  snap t33-a$A-now || fail SNAP_A${A}_NOW
  read M P <<< $(score_band $D/t33-a$A-now.ppm)
  plog "ATTEMPT_$A now score mean=$M p99=$P"
  if is_title $M $P; then
    plog "ATTEMPT_$A TITLE-ALREADY-DETECTED (no Cross needed)"
    cp $D/t33-a$A-now.ppm $D/t33-a$A-pre.ppm
    cp $D/t33-a$A-now.jpg $D/t33-a$A-pre.jpg
    ...
    for N in 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 18; do
      sleep 2
      snap t33-a$A-poll$N || fail SNAP_A${A}_POLL$N
      read M P <<< $(score_band $D/t33-a$A-poll$N.ppm)
      plog "ATTEMPT_$A poll$N score mean=$M p99=$P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      if is_title $M $P; then
        plog "ATTEMPT_$A TITLE-DETECTED at poll$N"
        cp $D/t33-a$A-poll$N.ppm $D/t33-a$A-pre.ppm
        cp $D/t33-a$A-poll$N.jpg $D/t33-a$A-pre.jpg
        press Return "A${A}_START"
        DETECTED=1
        break
      fi
  ...
  # post-press series with scores (pre-press snap = t33-a$A-pre.*)
  sleep 3; snap t33-a$A-post3 || fail SNAP_A${A}_POST3
  read M3 P3 <<< $(score_band $D/t33-a$A-post3.ppm)
  plog "ATTEMPT_$A post+3 score mean=$M3 p99=$P3 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  sleep 5; snap t33-a$A-post8 || fail SNAP_A${A}_POST8
  read M8 P8 <<< $(score_band $D/t33-a$A-post8.ppm)
  plog "ATTEMPT_$A post+8 score mean=$M8 p99=$P8"
  sleep 7; snap t33-a$A-post15 || fail SNAP_A${A}_POST15
  read M15 P15 <<< $(score_band $D/t33-a$A-post15.ppm)
  plog "ATTEMPT_$A post+15 score mean=$M15 p99=$P15"
  sleep 10; snap t33-a$A-post25 || fail SNAP_A${A}_POST25
  read M25 P25 <<< $(score_band $D/t33-a$A-post25.ppm)
  plog "ATTEMPT_$A post+25 score mean=$M25 p99=$P25"
  # menu-like gate: +8/+15/+25 all non-title AND whole-static across them
  W815=$(score_whole $D/t33-a$A-post8.ppm $D/t33-a$A-post15.ppm)
  W1525=$(score_whole $D/t33-a$A-post15.ppm $D/t33-a$A-post25.ppm)
  plog "ATTEMPT_$A whole815=$W815 whole1525=$W1525"
  read MM25 MP25 <<< $(score_menu $D/t33-a$A-post25.ppm)
...
if [ $MENU_LIKE -eq 1 ]; then
  ...
  sleep 5
  snap t33-menupre || fail SNAP_MENUPRE
  read MMP MMP_P <<< $(score_menu $D/t33-menupre.ppm)
  plog "MENUPRE vs-menu mean=$MMP p99=$MMP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  read MPT MPT_P <<< $(score_band $D/t33-menupre.ppm)
  ...
  sleep 1; snap t33-mc-post1 || fail SNAP_MC_POST1
  read M1 P1 <<< $(score_band $D/t33-mc-post1.ppm)
  read MM1 MP1 <<< $(score_menu $D/t33-mc-post1.ppm)
  read MS1 MSP1 <<< $(score_sc $D/t33-mc-post1.ppm)
  plog "MC post+1 titleband mean=$M1 p99=$P1 vs-menu mean=$MM1 p99=$MP1 vs-sc mean=$MS1 p99=$MSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  sleep 2; snap t33-mc-post3 || fail SNAP_MC_POST3
  read M3 P3 <<< $(score_band $D/t33-mc-post3.ppm)
  read MM3 MP3 <<< $(score_menu $D/t33-mc-post3.ppm)
  read MS3 MSP3 <<< $(score_sc $D/t33-mc-post3.ppm)
  plog "MC post+3 titleband mean=$M3 p99=$P3 vs-menu mean=$MM3 p99=$MP3 vs-sc mean=$MS3 p99=$MSP3"
  sleep 5; snap t33-mc-post8 || fail SNAP_MC_POST8
  read M8 P8 <<< $(score_band $D/t33-mc-post8.ppm)
  read MM8 MP8 <<< $(score_menu $D/t33-mc-post8.ppm)
  read MS8 MSP8 <<< $(score_sc $D/t33-mc-post8.ppm)
  plog "MC post+8 titleband mean=$M8 p99=$P8 vs-menu mean=$MM8 p99=$MP8 vs-sc mean=$MS8 p99=$MSP8"
  sleep 7; snap t33-mc-post15 || fail SNAP_MC_POST15
  read M15 P15 <<< $(score_band $D/t33-mc-post15.ppm)
  read MM15 MP15 <<< $(score_menu $D/t33-mc-post15.ppm)
  read MS15 MSP15 <<< $(score_sc $D/t33-mc-post15.ppm)
  ...
  H_PRE1=$(score_whole $D/t33-menupre.ppm $D/t33-mc-post1.ppm)
  H_13=$(score_whole $D/t33-mc-post1.ppm $D/t33-mc-post3.ppm)
  H_38=$(score_whole $D/t33-mc-post3.ppm $D/t33-mc-post8.ppm)
  H_815=$(score_whole $D/t33-mc-post8.ppm $D/t33-mc-post15.ppm)
  ...
  if [ $SC_LIKE -eq 1 ]; then
    ...
    # dwell pressure.
    sleep 5
    snap t33-scpre || fail SNAP_SCPRE
    read SCP SCP_P <<< $(score_sc $D/t33-scpre.ppm)
    plog "SCPRE vs-sc mean=$SCP p99=$SCP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
    read SMP SMP_P <<< $(score_menu $D/t33-scpre.ppm)
    plog "SCPRE vs-menu mean=$SMP p99=$SMP_P"
    read SPT SPT_P <<< $(score_band $D/t33-scpre.ppm)
    ...
    sleep 1; snap t33-zc-post1 || fail SNAP_ZC_POST1
    read Z1 ZP1 <<< $(score_band $D/t33-zc-post1.ppm)
    read ZS1 ZSP1 <<< $(score_sc $D/t33-zc-post1.ppm)
    read ZZ1 ZZP1 <<< $(score_zc $D/t33-zc-post1.ppm)
    plog "ZC post+1 titleband mean=$Z1 p99=$ZP1 vs-sc mean=$ZS1 p99=$ZSP1 vs-zc mean=$ZZ1 p99=$ZZP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
    sleep 2; snap t33-zc-post3 || fail SNAP_ZC_POST3
    read Z3 ZP3 <<< $(score_band $D/t33-zc-post3.ppm)
    read ZS3 ZSP3 <<< $(score_sc $D/t33-zc-post3.ppm)
    read ZZ3 ZZP3 <<< $(score_zc $D/t33-zc-post3.ppm)
    plog "ZC post+3 titleband mean=$Z3 p99=$ZP3 vs-sc mean=$ZS3 p99=$ZSP3 vs-zc mean=$ZZ3 p99=$ZZP3"
    sleep 5; snap t33-zc-post8 || fail SNAP_ZC_POST8
    read Z8 ZP8 <<< $(score_band $D/t33-zc-post8.ppm)
    read ZS8 ZSP8 <<< $(score_sc $D/t33-zc-post8.ppm)
    read ZZ8 ZZP8 <<< $(score_zc $D/t33-zc-post8.ppm)
    ...
    ZH_PRE1=$(score_whole $D/t33-scpre.ppm $D/t33-zc-post1.ppm)
    ZH_13=$(score_whole $D/t33-zc-post1.ppm $D/t33-zc-post3.ppm)
    ZH_38=$(score_whole $D/t33-zc-post3.ppm $D/t33-zc-post8.ppm)
    ...
    if [ $ZC_LIKE -eq 1 ]; then
      ...
      sleep 5
      snap t33-ccpre || fail SNAP_CCPRE
      read CZP CZP_P <<< $(score_zc $D/t33-ccpre.ppm)
      plog "CCPRE vs-zc mean=$CZP p99=$CZP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      read CSP CSP_P <<< $(score_sc $D/t33-ccpre.ppm)
      plog "CCPRE vs-sc mean=$CSP p99=$CSP_P"
      read CPT CPT_P <<< $(score_band $D/t33-ccpre.ppm)
      ...
      # post-hoc local)
      sleep 1; snap t33-sp-post1 || fail SNAP_SP_POST1
      read S1 SP1 <<< $(score_band $D/t33-sp-post1.ppm)
      read SZ1 SZP1 <<< $(score_zc $D/t33-sp-post1.ppm)
      read SS1 SSP1 <<< $(score_sp $D/t33-sp-post1.ppm)
      plog "SP post+1 titleband mean=$S1 p99=$SP1 vs-zc mean=$SZ1 p99=$SZP1 vs-sp mean=$SS1 p99=$SSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      sleep 2; snap t33-sp-post3 || fail SNAP_SP_POST3
      read S3 SP3 <<< $(score_band $D/t33-sp-post3.ppm)
      read SZ3 SZP3 <<< $(score_zc $D/t33-sp-post3.ppm)
      read SS3 SSP3 <<< $(score_sp $D/t33-sp-post3.ppm)
      plog "SP post+3 titleband mean=$S3 p99=$SP3 vs-zc mean=$SZ3 p99=$SZP3 vs-sp mean=$SS3 p99=$SSP3"
      sleep 5; snap t33-sp-post8 || fail SNAP_SP_POST8
      read S8 SP8 <<< $(score_band $D/t33-sp-post8.ppm)
      read SZ8 SZP8 <<< $(score_zc $D/t33-sp-post8.ppm)
      read SS8 SSP8 <<< $(score_sp $D/t33-sp-post8.ppm)
      ...
      SPH_PRE1=$(score_whole $D/t33-ccpre.ppm $D/t33-sp-post1.ppm)
      SPH_13=$(score_whole $D/t33-sp-post1.ppm $D/t33-sp-post3.ppm)
      SPH_38=$(score_whole $D/t33-sp-post3.ppm $D/t33-sp-post8.ppm)
      ...
      if [ $SP_LIKE -eq 1 ]; then
        ...
        # dwell pressure.
        sleep 5
        snap t33-sppre || fail SNAP_SPPRE
        read SPP SPP_P <<< $(score_sp $D/t33-sppre.ppm)
        plog "SPPRE vs-sp mean=$SPP p99=$SPP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
        read SPZ SPZ_P <<< $(score_zc $D/t33-sppre.ppm)
        plog "SPPRE vs-zc mean=$SPZ p99=$SPZ_P"
        read SPT SPT_P <<< $(score_band $D/t33-sppre.ppm)
        ...
        # post-hoc local)
        sleep 1; snap t33-pc-post1 || fail SNAP_PC_POST1
        read P1 PP1 <<< $(score_band $D/t33-pc-post1.ppm)
        read PS1 PSP1 <<< $(score_sp $D/t33-pc-post1.ppm)
        read PX1 PXP1 <<< $(score_sm $D/t33-pc-post1.ppm)
        plog "PC post+1 titleband mean=$P1 p99=$PP1 vs-sp mean=$PS1 p99=$PSP1 vs-sm mean=$PX1 p99=$PXP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
        sleep 2; snap t33-pc-post3 || fail SNAP_PC_POST3
        read P3 PP3 <<< $(score_band $D/t33-pc-post3.ppm)
        read PS3 PSP3 <<< $(score_sp $D/t33-pc-post3.ppm)
        read PX3 PXP3 <<< $(score_sm $D/t33-pc-post3.ppm)
        plog "PC post+3 titleband mean=$P3 p99=$PP3 vs-sp mean=$PS3 p99=$PSP3 vs-sm mean=$PX3 p99=$PXP3"
        sleep 5; snap t33-pc-post8 || fail SNAP_PC_POST8
        read P8 PP8 <<< $(score_band $D/t33-pc-post8.ppm)
        read PS8 PSP8 <<< $(score_sp $D/t33-pc-post8.ppm)
        read PX8 PXP8 <<< $(score_sm $D/t33-pc-post8.ppm)
        ...
        PCH_PRE1=$(score_whole $D/t33-sppre.ppm $D/t33-pc-post1.ppm)
        PCH_13=$(score_whole $D/t33-pc-post1.ppm $D/t33-pc-post3.ppm)
        PCH_38=$(score_whole $D/t33-pc-post3.ppm $D/t33-pc-post8.ppm)
        ...
        if [ $SM_LIKE -eq 1 ]; then
          ...
          # dwell pressure.
          sleep 5
          snap t33-smpre || fail SNAP_SMPRE
          read SXP SXP_P <<< $(score_sm $D/t33-smpre.ppm)
          plog "SMPRE vs-sm mean=$SXP p99=$SXP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
          read SXSP SXSP_P <<< $(score_sp $D/t33-smpre.ppm)
          plog "SMPRE vs-sp mean=$SXSP p99=$SXSP_P"
          read SXPT SXPT_P <<< $(score_band $D/t33-smpre.ppm)
          ...
          sleep 1; snap t33-rc-post1 || fail SNAP_RC_POST1
          read R1 RP1 <<< $(score_band $D/t33-rc-post1.ppm)
          read RX1 RXP1 <<< $(score_sm $D/t33-rc-post1.ppm)
          read RT1 RTP1 <<< $(score_setag $D/t33-rc-post1.ppm)
          plog "RC post+1 titleband mean=$R1 p99=$RP1 vs-sm mean=$RX1 p99=$RXP1 se-tag mean=$RT1 p99=$RTP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
          sleep 2; snap t33-rc-post3 || fail SNAP_RC_POST3
          read R3 RP3 <<< $(score_band $D/t33-rc-post3.ppm)
          read RX3 RXP3 <<< $(score_sm $D/t33-rc-post3.ppm)
          read RT3 RTP3 <<< $(score_setag $D/t33-rc-post3.ppm)
          plog "RC post+3 titleband mean=$R3 p99=$RP3 vs-sm mean=$RX3 p99=$RXP3 se-tag mean=$RT3 p99=$RTP3"
          sleep 5; snap t33-rc-post8 || fail SNAP_RC_POST8
          read R8 RP8 <<< $(score_band $D/t33-rc-post8.ppm)
          read RX8 RXP8 <<< $(score_sm $D/t33-rc-post8.ppm)
          read RT8 RTP8 <<< $(score_setag $D/t33-rc-post8.ppm)
          read RS8 RSP8 <<< $(score_sp $D/t33-rc-post8.ppm)
          read RW8 RWP8 <<< $(score_se $D/t33-rc-post8.ppm)
          ...
          RCH_PRE1=$(score_whole $D/t33-smpre.ppm $D/t33-rc-post1.ppm)
          RCH_13=$(score_whole $D/t33-rc-post1.ppm $D/t33-rc-post3.ppm)
          RCH_38=$(score_whole $D/t33-rc-post3.ppm $D/t33-rc-post8.ppm)
          ...
          if [ $SE_LIKE -eq 1 ]; then
            # T33 press: ONE Cross (x Select) on the parked Select Event
            # (Snow Jam highlighted per T32 R1); arrival static 133 s, no
            # dwell pressure.
            sleep 5
            snap t33-sepre || fail SNAP_SEPRE
            read STP STP_P <<< $(score_setag $D/t33-sepre.ppm)
            plog "SEPRE se-tag mean=$STP p99=$STP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
            read SWP SWP_P <<< $(score_se $D/t33-sepre.ppm)
            plog "SEPRE vs-se mean=$SWP p99=$SWP_P"
            read SXP SXP_P <<< $(score_sm $D/t33-sepre.ppm)
            plog "SEPRE vs-sm mean=$SXP p99=$SXP_P"
            read SXSP SXSP_P <<< $(score_sp $D/t33-sepre.ppm)
            plog "SEPRE vs-sp mean=$SXSP p99=$SXSP_P"
            read SXPT SXPT_P <<< $(score_band $D/t33-sepre.ppm)
            plog "SEPRE vs-title mean=$SXPT p99=$SXPT_P"
            press K "SNOWJAM_CROSS"
            # arrival series: fast cadence early (transition hops), slow tail
            # (slow next-screen loads), each scored vs title-band, SE whole-ref
            # and SE tagline crop (vs-sm/vs-sp/vs-zc/vs-sc/vs-menu post-hoc
            # local)
            sleep 1; snap t33-sj-post1 || fail SNAP_SJ_POST1
            read J1 JP1 <<< $(score_band $D/t33-sj-post1.ppm)
            read JW1 JWP1 <<< $(score_se $D/t33-sj-post1.ppm)
            read JT1 JTP1 <<< $(score_setag $D/t33-sj-post1.ppm)
            plog "SJ post+1 titleband mean=$J1 p99=$JP1 vs-se mean=$JW1 p99=$JWP1 se-tag mean=$JT1 p99=$JTP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
            sleep 2; snap t33-sj-post3 || fail SNAP_SJ_POST3
            read J3 JP3 <<< $(score_band $D/t33-sj-post3.ppm)
            read JW3 JWP3 <<< $(score_se $D/t33-sj-post3.ppm)
            read JT3 JTP3 <<< $(score_setag $D/t33-sj-post3.ppm)
            plog "SJ post+3 titleband mean=$J3 p99=$JP3 vs-se mean=$JW3 p99=$JWP3 se-tag mean=$JT3 p99=$JTP3"
            sleep 5; snap t33-sj-post8 || fail SNAP_SJ_POST8
            read J8 JP8 <<< $(score_band $D/t33-sj-post8.ppm)
            read JW8 JWP8 <<< $(score_se $D/t33-sj-post8.ppm)
            read JT8 JTP8 <<< $(score_setag $D/t33-sj-post8.ppm)
            plog "SJ post+8 titleband mean=$J8 p99=$JP8 vs-se mean=$JW8 p99=$JWP8 se-tag mean=$JT8 p99=$JTP8"
            sleep 7; snap t33-sj-post15 || fail SNAP_SJ_POST15
            read J15 JP15 <<< $(score_band $D/t33-sj-post15.ppm)
            read JW15 JWP15 <<< $(score_se $D/t33-sj-post15.ppm)
            read JT15 JTP15 <<< $(score_setag $D/t33-sj-post15.ppm)
            plog "SJ post+15 titleband mean=$J15 p99=$JP15 vs-se mean=$JW15 p99=$JWP15 se-tag mean=$JT15 p99=$JTP15"
            sleep 10; snap t33-sj-post25 || fail SNAP_SJ_POST25
            read J25 JP25 <<< $(score_band $D/t33-sj-post25.ppm)
            read JW25 JWP25 <<< $(score_se $D/t33-sj-post25.ppm)
            read JT25 JTP25 <<< $(score_setag $D/t33-sj-post25.ppm)
            plog "SJ post+25 titleband mean=$J25 p99=$JP25 vs-se mean=$JW25 p99=$JWP25 se-tag mean=$JT25 p99=$JTP25"
            sleep 15; snap t33-sj-post40 || fail SNAP_SJ_POST40
            read J40 JP40 <<< $(score_band $D/t33-sj-post40.ppm)
            read JW40 JWP40 <<< $(score_se $D/t33-sj-post40.ppm)
            read JT40 JTP40 <<< $(score_setag $D/t33-sj-post40.ppm)
            plog "SJ post+40 titleband mean=$J40 p99=$JP40 vs-se mean=$JW40 p99=$JWP40 se-tag mean=$JT40 p99=$JTP40"
            # per-hop whole-frame diffs across the SnowJam transition chain
            SJH_PRE1=$(score_whole $D/t33-sepre.ppm $D/t33-sj-post1.ppm)
            SJH_13=$(score_whole $D/t33-sj-post1.ppm $D/t33-sj-post3.ppm)
            SJH_38=$(score_whole $D/t33-sj-post3.ppm $D/t33-sj-post8.ppm)
            SJH_815=$(score_whole $D/t33-sj-post8.ppm $D/t33-sj-post15.ppm)
            SJH_1525=$(score_whole $D/t33-sj-post15.ppm $D/t33-sj-post25.ppm)
            SJH_2540=$(score_whole $D/t33-sj-post25.ppm $D/t33-sj-post40.ppm)
            plog "SJ hops whole: pre-post1=$SJH_PRE1 p1-p3=$SJH_13 p3-p8=$SJH_38 p8-p15=$SJH_815 p15-p25=$SJH_1525 p25-p40=$SJH_2540"
            # arrival stability: 6 x 10 s (band + se-tag in-script; whole
            # vs-SE post-hoc local — keeps added scoring tight)
            for N in 1 2 3 4 5 6; do
              sleep 10; snap t33-sj-stab$N || fail SNAP_SJ_STAB$N
              read M P <<< $(score_band $D/t33-sj-stab$N.ppm)
              read MT MTP <<< $(score_setag $D/t33-sj-stab$N.ppm)
              plog "SJ_STAB$N titleband mean=$M p99=$P se-tag mean=$MT p99=$MTP MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
            done
          fi
        fi
      fi
