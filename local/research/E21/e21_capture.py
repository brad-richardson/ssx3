#!/usr/bin/env python3
"""E21 demand-watch capture driver. Fresh T13 pre-claims, caps, owned lease cleanup. One boot."""
import argparse
import datetime
import hashlib
import json
...
sys.path.insert(0, str(REPO / 'local/research/E21'))
from e21_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e21_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E21'
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E21 forbids lease wait')
        raise RuntimeError('Lease occupied; E21 tables and stops without waiting')
    ...
    manifest_path=EVIDENCE/'e21a-build.json'
    assert manifest_path.exists(), 'fresh E21 probe-gate manifest required'
    ...
    for path, size, expected in [(BIN,build['bin_size'],build['bin_sha']),(TEST,build['test_size'],build['test_sha']),
                                  ...
                                  (EVIDENCE/'parser/e21-parser-observer.dylib',None,json.loads((EVIDENCE/'parser-build.json').read_text())['observer']['sha256'])]:
...
def main():
    ...
    assert not (EVIDENCE/'boot-attempt.json').exists(), 'E21 permits only one boot'
    assert not list(EVIDENCE.glob('e21?-result.json')), 'E21 permits only one boot'
    label='e21'+args.phase
    assert args.report_all, 'E21 requires REPORT_ALL=1'
