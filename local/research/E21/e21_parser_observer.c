...
__attribute__((constructor)) static void initialize(void) {
    ...
    const char *dir=getenv("PS2X_E21_PARSER_DIR");if(!dir)return;
    char path[4096];
    ...
    startNs=now_ns();enabled=1;proof=getenv("PS2X_E21_PROOF")!=NULL;
...
void e21_parser_snapshot(E21ParserStats *out) {
    pthread_mutex_lock(&mutex);*out=stats;pthread_mutex_unlock(&mutex);
}
void e21_parser_mark(const char *label,uint64_t bytes) {
...
static void close_observation(const char *reason,int rc) {
    ...
    if(enabled && !closed) {
        ...
        char line[2048];
        int n=snprintf(line,sizeof(line),"# E21 PARSER CLOSURE source=API-interposer reason=%s rc=%d ns=%" PRIu64
