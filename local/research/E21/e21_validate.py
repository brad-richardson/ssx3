...
def observer(env,dest):
    library=os.environ.get('E21_OBSERVER_DYLIB')
    if library:
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e21-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E21 requires E18's delivered behavior at baseline
        ...
        if '# E7 SHUTDOWN' in taps:
            from e21_closed_events import tap
    ...
    print('# E21 VALIDATION TAIL COMPLETE')
    return result
