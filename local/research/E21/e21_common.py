...
NEWB = Path('/tmp/e21-mpeg-link/runtime')
B = Path(os.environ.get('E21_BUILD_DIR',str(B0)))
OUT = Path('/tmp/e21-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
...
def sample():
    paths = list(P.glob('e21-*')) + list(P.glob('._e21-*')) + list((P/'run').glob('*e21*'))
    # The shared function trace is charged even though its name lacks E21.
