...
check('no boot-attempt.json written by E25', not (E / 'boot-attempt.json').exists())

ok = all(c['ok'] for c in checks)
save('e26-readiness.json', dict(utc=utc(), boot_argv=boot_argv,
     capture_preflight_gate=gate, checks=checks, all_green=ok,
     note=('E25 spends no boot and claims no lease. These are read-only checks of what '
...
print('ALL GREEN:', ok)
print('# E25 E26 READINESS TAIL COMPLETE')
