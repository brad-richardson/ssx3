...
def observer(env,dest):
    library=os.environ.get('E25_OBSERVER_DYLIB')
    if library:
...
def suite(label):
    # E25 CHANGE 1: the suite binary lives in a PROTECTED build tree that the
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e25-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E25 requires E18's delivered behavior at baseline
    ...
    print('# E25 VALIDATION TAIL COMPLETE')
    return result
