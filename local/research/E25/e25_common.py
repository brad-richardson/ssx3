...
R = W / 'PS2Recomp'
# The three PROTECTED build trees E15-E23 ran on, all wiped by the host restart
# that opened E24 red. E25 REBUILDS exactly one of them -- B0, the E18 tree that
# holds the runner and the suite -- at its EXACT E18 path, because every
# downstream driver, boot argv and pin names that path. The other two are
# historical build trees for older lanes; they are inventoried as missing and
# tabled for the orchestrator, not rebuilt (internal cap 3 GiB, one tree).
B0 = Path('/tmp/e18-mpeg-link/runtime')
PROTECTED_BUILDS = [Path('/tmp/p1-link/runtime'), Path('/tmp/e17-map-link/runtime'), B0]
# E25 rebuilds IN PLACE at the E18 path: "same flags/toolchain/paths".
NEWB = B0
B = Path(os.environ.get('E25_BUILD_DIR', str(B0)))
OUT = Path('/tmp/e25-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
BASE_SHA = '3adc0478b6d2260acdd28a249466f2eef9a20176'
# E25 CONTRACT: internal reservation 3 GiB. E23's amendment A1 shrank it to
# 512 MiB because E23 ran no full build after its relink; E25 MUST run a full
# build (E18 measured 1,870,770,176 B allocated), so the reservation is
# re-expanded to the brief's 3 GiB and DECLARED IN `CONTRACT.md` up front --
# not amended mid-run. FLOOR and GUARD stay 2 GiB + 0.5 GiB, untouched.
IRES = 3*G
...
def sample():
    paths = list(P.glob('e25-*')) + list(P.glob('._e25-*')) + list((P/'run').glob('*e25*'))
    # The shared function trace is charged even though its name lacks E25.
