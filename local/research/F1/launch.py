#!/usr/bin/env python3
"""F1 Odin speed runs: N11 launcher adapted for the F1 fold build.

Differences from N10: OUT text logs stay in git, binaries (PNG/perf.data) go
to scratch; mc0-test empty card (Brad's mc0 never touched); full F1 env with
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/F1/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/F1/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('F1-')):
...
sh(f'echo "F1 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE F1 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
sh("echo 'LEASE_FREE F1 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
