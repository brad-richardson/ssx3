#!/usr/bin/env bash
# T51 Capture A (redirect): SC-settled statefile boot, ctag chain dump over
# [V,V+1] + T50 dmareg context + T51 st/sema/irq window. Foreground run.
set -x
...
TAG=t51b
LOG=$T4/$TAG-poll.log
fail() { echo "T51A_FAIL:$1"; date -u; exit 1; }
plog() { echo "$@" | tee -a $LOG; }
[ -x $BIN ] || fail NO_BIN
...
: > $LOG
date -u
rm -f /tmp/t48-arm /tmp/t48-dump-now /tmp/t49-arm /tmp/t50-arm /tmp/t50-free /tmp/t51-arm /tmp/t51-free /tmp/t51-watch
touch /tmp/t48-arm /tmp/t50-arm /tmp/t51-arm
printf "%s\n" 0x63d434 0x63dcb4 0x70a0b4 0x70a934 > /tmp/t51-watch
cat /tmp/t51-watch
...
plog "COUNTS ctag=$(grep -c 'ctag ' $E || true) dmareg=$(grep -c 'dmareg ' $E || true) st=$(grep -c 'st vsync=' $E || true) sema=$(grep -c 'sema vsync=' $E || true) irq=$(grep -c 'irq vsync=' $E || true)"
grep -m 2 "T51C_WINDOW" $E || true
grep -m 1 "T51C_CAP" $E || plog "NO_CTAG_CAP"
kill $(cat $T4/pcsx2-t51b.pid); sleep 5; pgrep -a pcsx2-qt || true
date -u
echo T51A_DONE
