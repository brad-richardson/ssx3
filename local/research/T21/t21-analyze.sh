#!/usr/bin/env bash
# T21 trace analysis on the three-press emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t21-census.txt
python3 $D/t4-sample.py $E $D/t21-samples.txt
ls -la $D/t21-census.txt $D/t21-samples.txt
date -u
