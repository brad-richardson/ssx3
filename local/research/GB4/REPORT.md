...
| Corrected direct replay vs live | Not run. The earlier sandboxed boot was void; the escalated replacement attempt stopped at lease refusal before starting a runner. | `~/dev/ssx3-work/GB4/run/boot-part2-wrapper.jsonl`, `run/boot-gb4cap2-1.log` document the void attempt; `run/boot-part2-escalated-wrapper.jsonl` documents the lease refusal. |
| Queue vs direct, queue repeat, drop-priv control | Pending direct/live gate. | No gate data yet |
| paraLLEl replay and live boot | Pending Part 1 gate. | No gate data yet |
...
The replacement runner records local-to-host bytes returned to the guest and the HLE `clearFramebufferContext` operation (a direct VRAM mutation at `sceGsSwapDBuffDc` that the original capture omitted). The replay checks returned bytes and applies each captured clear. Replay Present FNV now hashes only the visible rows, matching VQ's width and stride. The final runner's unchanged-value priv-store capture is included. Build 1 and suite passed (607/607); the new stream features await a live capture.
