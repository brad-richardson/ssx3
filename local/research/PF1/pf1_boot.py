#!/usr/bin/env python3
"""PF1 boot (copy of FR1's fr1_boot.py + --env/--mc-src/--unpaced): FR1-R1 post-race repro.

Mac, paraLLEl + PGS_HIER_BINNING=force, PS2X_SOUND=1, empty mc0,
PS2X_SKIP_MOVIE=1 (dev-only), vsync pad clock, one mini slot each.

Routes (guest ms, PS2X_PAD_SCRIPT_CLOCK=vsync); exact strings in
local/research/FR1/ROUTES-FR1.md:
  r1  I26-FAST menu prefix + Happiness downs + Rival-card taps, NO tail:
      the rider rides untouched after the card taps.
  r2  SJ1 R2 verbatim (Snow Jam default + X-tap every 10 s guest with
      down held between taps to ~358 s guest).
  r3  I26-FAST menu prefix + ONE Select-Event down (Metro-City) + card
      taps, NO tail: the rider rides untouched.

Env: PS2X_DETERMINISTIC=1, PS2X_VSYNC_RATE_LOG=1 (tick source),
PS2X_FRAME_DUMP_DIR (upload-latest pair) + 30 s snapshotter,
PS2X_COVERAGE_TICK, PS2X_MISSING_FUNCTION_POLICY=stop. A STOP file in the
run dir ends the boot cleanly (bound=stop_file) so the worker can stop on
observed end states.

Usage: fr1_boot.py --route r1|r2|r3 --runner PATH --label NAME --wall SEC
       --coverage-tick N [--stop-tick N]
"""
import argparse
import hashlib
import json
...
WORK = Path('/Users/brad/dev/ssx3-work/PF1')
...
MENU_DOWNS_HAPPINESS = '21205:down:150,21706:down:150,'  # Snow Jam -> Metro-City -> Happiness
MENU_DOWNS_METRO = '21205:down:150,'  # Snow Jam -> Metro-City
...
def race_tail():
    # X tap every 10 s guest (RECOVER/jump), down held otherwise, to ~358 s.
    parts = []
    for k in range(32):
        t = 38522 + k * 10000
...
ROUTE_R1 = MENU_PREFIX + MENU_DOWNS_HAPPINESS + MENU_SUFFIX
ROUTE_R2 = MENU_PREFIX + MENU_SUFFIX + race_tail()
ROUTE_R3 = MENU_PREFIX + MENU_DOWNS_METRO + MENU_SUFFIX

# Byte-check against ROUTES-FR1.md is done by the worker before booting
# (see REPORT.md); entry counts: R1 30, R2 92, R3 29.
...
def main():
    ...
    ap.add_argument('--env', action='append', default=[], help='extra KEY=VAL')
    ap.add_argument('--mc-src', default=None, help='copy this dir into mc0 (scratch only)')
    ap.add_argument('--log-cap-mb', type=int, default=16)
    ap.add_argument('--no-frames', action='store_true', help='no PS2X_FRAME_DUMP_DIR (PNG per present)')
    args = ap.parse_args()

    route = {'r1': ROUTE_R1, 'r2': ROUTE_R2, 'r3': ROUTE_R3}[args.route]
    ...
    env.update(PS2X_CD_IMAGE=str(ISO), PS2X_SKIP_MOVIE='1', PS2X_DETERMINISTIC='1',
               ...
               PS2X_UNPACED='1', COPYFILE_DISABLE='1')
    if args.no_frames:
        env.pop('PS2X_FRAME_DUMP_DIR')
    for kv in args.env:
        k, v = kv.split('=', 1)
        env[k] = v

    slot = claim('PF1-' + args.label, exclusive=False)
    while slot is None:
            ...
            try:
                while True:
                    ...
                    if stop_file.exists():
                        ...
                        break
                    if log.stat().st_size > args.log_cap_mb * 1024 * 1024:
                        bound = 'log_cap'
                        break
