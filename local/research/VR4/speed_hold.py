#!/usr/bin/env python3
"""VR4 (copy of VR2's): speed boots under one exclusive mini lease hold (VB1's holder, repointed
...
Candidates: base = F5 runner-clean (fork a3efbfe), cN = VR2 bin/runner-cN-clean.
...
WORK = '/Users/brad/dev/ssx3-work/VR4'
DRIVER = '/Users/brad/dev/ssx3/local/tooling/boot/ssx3_boot.py'
LOAD_MAX, HOLD_CAP, BOOT_EST = 8.0, 300, 80


# VR4 D1: d0 = vr4 runner with PS2X_VU1_FMAC_SIMD=OFF, d1 = ON (same source,
# same canonical images); both with PS2X_VU1_BLOCKS=1 (play settings) unless
# the candidate ends in "m" (+ PS2X_MTVU=1).
B = ['--env', 'PS2X_VU1_BLOCKS=1']
RUNNERS = {'d0': (WORK + '/bin/runner-d0-clean', B),
           'd1': (WORK + '/bin/runner-d1-clean', B),
           'd0m': (WORK + '/bin/runner-d0-clean', B + ['--env', 'PS2X_MTVU=1']),
           'd1m': (WORK + '/bin/runner-d1-clean', B + ['--env', 'PS2X_MTVU=1'])}
