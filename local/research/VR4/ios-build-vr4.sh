...
set -euo pipefail
# VR4: copy of ~/dev/ssx3-work/F6/ios/build-install.sh with PGS = the canonical
# paraLLEl checkout at 3d72467 (the fork tip f0d2d3c needs its SS3 accessors).
# VR3: IOS_W / IOS_FORK_WT override the build root / fork worktree (defaults unchanged).
W=${IOS_W:-/Users/brad/dev/ssx3-work/F6/ios}
D=/Users/brad/dev/ssx3-work/ios-deps
REPO=/Users/brad/dev/ssx3
FORK_WT=${IOS_FORK_WT:-/Users/brad/dev/ssx3-work/F6/PS2Recomp}
PGS=/Users/brad/dev/ssx3-work/parallel-gs-ssx3
RAYLIB="$D/raylib-src"
CODEGEN=/Users/brad/dev/ssx3-work/codegen-ssx3
VU1GEN=/Users/brad/dev/ssx3-work/vu1gen-ssx3
# VR3: generated VU0 images (used only with PS2X_VU0_RECOMP=1 at run time).
VU0GEN=/Users/brad/dev/ssx3-work/vu0gen-ssx3
MVK=/Users/brad/dev/ssx3-work/moltenvk-1.4.2/MoltenVK/MoltenVK/dynamic/MoltenVK.xcframework
I26="$REPO/local/research/I26"
ENVFILE="$REPO/local/research/F5/ps2x.env"
LOGS="$W/logs"
...
PIN=fb28d99aa1c41eaddb742dab49a4f6d5ae26cd94
RAYPIN=c1ab645ca298a2801097931d1079b10ff7eb9df8
PGSPIN=3d72467033ce6c4a8c7319e567880578aca39f6b
...
preflight() {
  ...
  shasum -a 256 "$VU1GEN"/vu1_*.cpp > "$LOGS/vu1-sha.txt"
  test "$(wc -l < "$LOGS/vu1-sha.txt" | tr -d ' ')" = 7
  shasum -a 256 "$VU0GEN"/vu0_*.cpp > "$LOGS/vu0-sha.txt"
  test "$(wc -l < "$LOGS/vu0-sha.txt" | tr -d ' ')" = 1
...
configure_one() { # $1 = sim|device
  ...
  env -u CC -u CXX cmake -G Xcode \
    ...
    -DPS2X_VU1_RECOMP_DIR="$VU1GEN" \
    -DPS2X_VU0_RECOMP_DIR="$VU0GEN" \
...
install_iphone() {
  ...
  grep -F "$IPHONE" "$LOGS/devices-install-iphone.txt" | grep -v unavailable | grep -E 'connected|available'
