#!/bin/bash
# VR4 copy of local/tooling/build/mac_build.sh with one addition: --cmake ARG
# (repeatable) appends a raw cmake -D option (e.g. -DPS2X_VU1_FMAC_SIMD=ON).
...
if [ "${#EXTRA[@]}" -gt 0 ]; then CONFIGURE+=("${EXTRA[@]}"); fi
