#!/usr/bin/env python3
"""E55D5 Part 1: pad-B discriminator comparator (A1 baseline vs B1).

Compares the E55D4 A1 baseline (saved `boot.log.gz`/`probe.log.gz`, read
read-only with bounded decompression into private scratch) against one fresh
B1 boot with the single predeclared route change `33517:cross:200` ->
`33517:square:200`. Requires on both sides: complete consecutive hashes
1..2053, well-formed shared-seq pad/getdir/mcread records and a persisted
complete probe line with vsync > 2053 as flush proof. Compares all hash
fields/payloads and identifies the first differing guest-write
seq/vsync/port/bytes plus the first differing hash tick.

Predeclared pulse window (guest ms -> tick: tick = ms * 5994 / 100000):
  33517 ms -> tick 2009.0; hold 200 ms -> ~12 ticks (2009..2021).
  PULSE_VSYNC_LO=2008 (-1 tick vsync boundary), PULSE_VSYNC_HI=2025
  (+4 scheduling tolerance past the hold end).

Classifier (predeclared):
  PASS pad_b_discriminated — paired first pad records with identical
    metadata (seq, vsync, ord, port, slot, addr, len, ok) and exactly
    differing `bytes` payload; first diff is `pad` family with vsync in
    [2008, 2025]; identical ordered records before it; no card manifest
    drift; no live pad row; first hash tick is None or >= the first probe
    vsync (a later hash divergence alone is never attributed to the pad
    without the ordered write chain; both first differences are preserved
    in the result).
  FAIL — a measured difference that is not the predeclared discriminator:
    probe_wrong_family (first diff is getdir/mcread/cap),
    probe_earlier_difference (pad diff with vsync < 2008),
    probe_late_difference (pad diff with vsync > 2025 but <= 2053),
    probe_metadata_difference (paired lines differ anywhere outside the
    `bytes` payload, including seq/vsync/ord/port/slot/addr/len/ok),
    probe_length_difference (one trace has an extra windowed line; prefix
    identical but counts differ),
    hash_without_probe (probe identical, hash differs),
    hash_earlier_than_probe (hash tick < first probe vsync),
    card_manifest_difference.
  OTHER — bounded/incomplete run, not a comparison:
    probe_missing_X, probe_truncated_X, probe_framing_X, hash_error_X,
    probe_cap_X, incomplete_trace, probe_unproven_X, live_pad_present,
    no_probe_difference (probe identical through 2053 and hash identical:
    the pulse did not reach a guest write by tick2053).

Usage: e55d5_compare.py <runA_baseline> <runB> [--json-out <path>]
                        [--hidutil-list <path>]
       e55d5_compare.py --self-check   (synthetic fixtures only, no boots)
Pure stdlib; reads inputs (plain or .gz, bounded), never boots, never
touches the lease, never writes into the input run dirs.
"""
import argparse
import gzip
import json
import re
import sys
import tempfile
from pathlib import Path

STOP_TICK = 2053
# Predeclared 33517 ms pulse window in vsync terms (see docstring).
PULSE_MS_START = 33517
PULSE_MS_HOLD = 200
PULSE_VSYNC_LO = 2008
PULSE_VSYNC_HI = 2025

HASH_RE = re.compile(
    r'\[det-hash:v1\] tick=(\d+) eeCycle=(\d+) rdram=([0-9a-f]{16}) '
    r'scratch=([0-9a-f]{16}) vu1Data=([0-9a-f]{16}) vu1Code=([0-9a-f]{16}) '
    r'combined=([0-9a-f]{16}) count=(\d+)\b')
HASH_ERROR_MARKERS = ('[det-hash] null', '[det-hash] line cap', '[det-hash] invalid')
PROBE_SEQ = re.compile(r'^(pad|getdir|mcread|cap) seq=(\d+)\b')
PROBE_ORD = re.compile(r'\bord=(\d+)\b')
PROBE_VSYNC = re.compile(r'vsync=(\d+)')
PROBE_PORT = re.compile(r'\bport=(\d+)\b')
HASH_FIELDS = ('tick', 'eeCycle', 'rdram', 'scratch', 'vu1Data', 'vu1Code', 'combined', 'count')

# Bounded read-only decompression of saved A1 gzip logs (live logs are
# ~375 KiB boot / ~580 KiB probe; caps are 4 MiB log / 16 MiB probe).
BOOT_GZ_CAP = 16 * 1024 * 1024
PROBE_GZ_CAP = 32 * 1024 * 1024

# Known internal Apple devices whose names contain 'Controller' but which are
# not game input (E55D4: AppleANS3CGv2Controller = internal NAND temp sensor,
# UsagePage 65280 vendor / Usage 5, Product 'NAND CH0 temp').
HID_KNOWN_INTERNAL = ('AppleANS3CGv2Controller',)
HID_GAME_USAGES = (4, 5, 8)  # HID Generic Desktop: Joystick / Game Pad / Multi-axis
HID_GAME_NAME = re.compile(r'\b(game\s*pad|joystick)\b', re.IGNORECASE)
HID_ROW_SPLIT = re.compile(r'\s{2,}')


def classify_hid(text):
    """Classify a full `hidutil list` text by row, not by blob substring.

    Each row is split into columns; UsagePage/Usage come from their columns
    and name matching uses the Class/Product/UserClass tail. A row is
    flagged only for an actual gamepad/joystick product, a Generic-Desktop
    game usage (page 1, usage 4/5/8), or an ambiguous non-allowlisted
    'controller' row. The known internal Apple storage controller is
    recorded, not flagged.
    """
    flagged = []
    known_internal = []
    rows = 0
    for raw in text.splitlines():
        row = raw.strip()
        if not row or row.startswith('Services:') or row.startswith('VendorID'):
            continue
        fields = HID_ROW_SPLIT.split(row)
        if len(fields) < 6:
            continue
        rows += 1
        try:
            usage = (int(fields[3]), int(fields[4]))
        except ValueError:
            usage = (None, None)
        tail = ' '.join(fields[7:]) if len(fields) > 7 else row
        if any(name in row for name in HID_KNOWN_INTERNAL):
            known_internal.append(row[:160])
            continue
        if HID_GAME_NAME.search(tail):
            flagged.append(('game_name', row[:160]))
        elif usage[0] == 1 and usage[1] in HID_GAME_USAGES:
            flagged.append(('game_usage', row[:160]))
        elif 'controller' in tail.lower():
            flagged.append(('ambiguous_controller', row[:160]))
    return {'flag': bool(flagged), 'flagged': flagged,
            'known_internal': known_internal, 'rows': rows}


def split_persisted(data):
    """Split probe bytes into (complete_lines, truncated).

    Only newline-terminated lines count as persisted; a trailing chunk
    without '\\n' is a partial write (or torn tail) and is excluded.
    """
    if not data:
        return ([], False)
    if data.endswith(b'\n'):
        return (data[:-1].split(b'\n'), False)
    chunks = data.split(b'\n')
    return (chunks[:-1], True)


def read_text_bounded(path, cap):
    """Read a plain text file (str)."""
    data = Path(path).read_bytes()
    if len(data) > cap:
        raise ValueError('plain file %s exceeds bound %d (%d)' % (path, cap, len(data)))
    return data.decode('utf-8', errors='replace')


def read_gz_text_bounded(path, cap):
    """Read a .gz file read-only with a bounded output cap."""
    out = bytearray()
    with gzip.open(path, 'rb') as f:
        while True:
            chunk = f.read(1 << 20)
            if not chunk:
                break
            if isinstance(chunk, str):
                chunk = chunk.encode('utf-8', errors='replace')
            out.extend(chunk)
            if len(out) > cap:
                raise ValueError('gz file %s exceeds bound %d' % (path, cap))
    return bytes(out).decode('utf-8', errors='replace')


def read_probe_bytes_bounded(path, cap):
    out = bytearray()
    if str(path).endswith('.gz'):
        with gzip.open(path, 'rb') as f:
            while True:
                chunk = f.read(1 << 20)
                if not chunk:
                    break
                if isinstance(chunk, str):
                    chunk = chunk.encode('utf-8', errors='replace')
                out.extend(chunk)
                if len(out) > cap:
                    raise ValueError('gz file %s exceeds bound %d' % (path, cap))
        return bytes(out)
    data = Path(path).read_bytes()
    if len(data) > cap:
        raise ValueError('plain file %s exceeds bound %d (%d)' % (path, cap, len(data)))
    return data


def resolve_boot_text(run_dir):
    run_dir = Path(run_dir)
    plain, gz = run_dir / 'boot.log', run_dir / 'boot.log.gz'
    if plain.exists():
        return read_text_bounded(plain, BOOT_GZ_CAP)
    if gz.exists():
        return read_gz_text_bounded(gz, BOOT_GZ_CAP)
    raise FileNotFoundError('neither boot.log nor boot.log.gz in %s' % run_dir)


def resolve_probe_bytes(run_dir, result):
    run_dir = Path(run_dir)
    candidates = []
    pf = result.get('probe_file')
    if pf:
        candidates.append(Path(pf))
        if not str(pf).endswith('.gz'):
            candidates.append(Path(str(pf) + '.gz'))
    candidates.append(run_dir / 'probe.log')
    candidates.append(run_dir / 'probe.log.gz')
    for cand in candidates:
        if cand.exists():
            return read_probe_bytes_bounded(cand, PROBE_GZ_CAP)
    return None


def parse_hashes(data):
    rows = []
    for match in HASH_RE.finditer(data):
        tick = int(match.group(1))
        rows.append((tick,) + tuple(
            int(match.group(i)) if i in (2, 8) else match.group(i) for i in range(2, 9)))
    errors = [marker for marker in HASH_ERROR_MARKERS if marker in data]
    return rows, errors


def hash_consecutive(rows):
    return [row[0] for row in rows] == list(range(1, len(rows) + 1))


def hash_prefix_ok(rows):
    """Exact consecutive prefix 1..STOP_TICK; later rows allowed (phase 2)."""
    return (len(rows) >= STOP_TICK
            and [row[0] for row in rows[:STOP_TICK]] == list(range(1, STOP_TICK + 1)))


def parse_probe(data):
    """Parse persisted probe bytes; framing validated on complete lines."""
    complete, truncated = split_persisted(data)
    lines = [c.decode('utf-8', errors='replace') for c in complete]
    families = []
    seqs = []
    ords = {'pad': [], 'getdir': [], 'mcread': []}
    vsyncs = []
    capped = False
    malformed = []
    for lineno, line in enumerate(lines, 1):
        match = PROBE_SEQ.match(line)
        vmatch = PROBE_VSYNC.search(line)
        if not match or not vmatch:
            malformed.append(lineno)
            continue
        family, seq = match.group(1), int(match.group(2))
        families.append(family)
        seqs.append(seq)
        vsyncs.append(int(vmatch.group(1)))
        if family == 'cap':
            capped = True
        else:
            omatch = PROBE_ORD.search(line)
            ords[family].append(int(omatch.group(1)) if omatch else None)
    framing_ok = (not malformed and seqs == list(range(1, len(seqs) + 1))
                  and all(vals == list(range(1, len(vals) + 1))
                          and all(v is not None for v in vals)
                          for vals in ords.values()))
    proof_vsync = max(vsyncs) if vsyncs else None
    windowed = [(seq, line) for seq, fam, vsync, line
                in zip(seqs, families, vsyncs, lines)
                if fam != 'cap' and vsync <= STOP_TICK]
    return {'lines': lines, 'truncated': truncated, 'framing_ok': framing_ok,
            'malformed': malformed, 'capped': capped,
            'proof_vsync': proof_vsync, 'windowed': windowed,
            'families': families, 'vsyncs': vsyncs, 'seqs': seqs}


def parse_probe_line(line):
    """Extract family/seq/vsync/port from one windowed probe line."""
    m = PROBE_SEQ.match(line)
    v = PROBE_VSYNC.search(line)
    p = PROBE_PORT.search(line)
    if not m or not v:
        return None
    return {'family': m.group(1), 'seq': int(m.group(2)),
            'vsync': int(v.group(1)),
            'port': int(p.group(1)) if p else None}


def split_bytes_payload(line):
    """Split a probe line into (metadata_head, bytes_payload).

    The head is everything before the first `bytes=` marker (it carries
    seq/vsync/ord/port/slot/addr/len/ok and family); the payload is the
    hex after it (possibly empty). Returns (None, None) if no marker.
    """
    head, sep, payload = line.partition('bytes=')
    if not sep:
        return (None, None)
    return (head, payload)


def load_run(run_dir):
    run_dir = Path(run_dir)
    result = json.loads((run_dir / 'result.json').read_text())
    boot = resolve_boot_text(run_dir)
    probe = resolve_probe_bytes(run_dir, result)
    return result, boot, probe


def summarize(result, boot, probe_data):
    rows, errors = parse_hashes(boot)
    probe = parse_probe(probe_data) if probe_data is not None else None
    return {
        'bound': result.get('bound'),
        'hash_rows': len(rows),
        'hash_consecutive_from_1': hash_consecutive(rows) if rows else False,
        'hash_errors': errors,
        'last_hash_tick': rows[-1][0] if rows else 0,
        'log_bytes': result.get('log_bytes'),
        'probe_lines': len(probe['lines']) if probe else 0,
        'probe_windowed_lines': len(probe['windowed']) if probe else 0,
        'probe_framing_ok': probe['framing_ok'] if probe else False,
        'probe_truncated': probe['truncated'] if probe else None,
        'probe_capped': probe['capped'] if probe else None,
        'probe_malformed': probe['malformed'] if probe else None,
        'probe_proof_vsync': probe['proof_vsync'] if probe else None,
        'probe_bytes': result.get('probe_bytes'),
        'probe_missing': probe is None,
        'card_initial_sha256': result.get('card_initial_sha256'),
        'card_final_sha256': result.get('card_final_sha256'),
        'gamepad_like': bool((result.get('hidutil') or {}).get('gamepad_like')),
    }


def first_hash_diff(rows_a, rows_b):
    for tick, row_a, row_b in zip(range(1, STOP_TICK + 1),
                                  rows_a[:STOP_TICK], rows_b[:STOP_TICK]):
        if row_a != row_b:
            fields = [name for name, va, vb in zip(HASH_FIELDS, row_a, row_b) if va != vb]
            return {'differ': True, 'first_hash_tick': tick, 'fields': fields,
                    'row_a': list(row_a), 'row_b': list(row_b)}
    return {'differ': False, 'first_hash_tick': None}


def first_probe_diff(win_a, win_b):
    for (seq_a, line_a), (seq_b, line_b) in zip(win_a, win_b):
        if line_a != line_b:
            return {'differ': True, 'reason': 'probe_difference',
                    'first_probe_seq': seq_a, 'line_a': line_a[:512],
                    'line_b': line_b[:512]}
    if len(win_a) != len(win_b):
        longer = win_a if len(win_a) > len(win_b) else win_b
        return {'differ': True, 'reason': 'probe_length_difference',
                'first_probe_seq': longer[min(len(win_a), len(win_b))][0],
                'lines_a': len(win_a), 'lines_b': len(win_b)}
    return {'differ': False, 'first_probe_seq': None}


def compare(run_a, run_b, hid_text=None):
    res_a, boot_a, probe_a = load_run(run_a)
    res_b, boot_b, probe_b = load_run(run_b)
    out = {'run_a': str(run_a), 'run_b': str(run_b),
           'pulse_ms_start': PULSE_MS_START, 'pulse_ms_hold': PULSE_MS_HOLD,
           'pulse_vsync_lo': PULSE_VSYNC_LO, 'pulse_vsync_hi': PULSE_VSYNC_HI,
           'summary_a': summarize(res_a, boot_a, probe_a),
           'summary_b': summarize(res_b, boot_b, probe_b)}

    def other(reason, **extra):
        out.update({'verdict': 'OTHER', 'reason': reason})
        out.update(extra)
        return out

    parsed_a = parse_probe(probe_a) if probe_a is not None else None
    parsed_b = parse_probe(probe_b) if probe_b is not None else None
    for tag, summ in (('A', out['summary_a']), ('B', out['summary_b'])):
        if summ['probe_missing']:
            return other('probe_missing_%s' % tag)
    assert parsed_a is not None and parsed_b is not None
    parsed = {'A': parsed_a, 'B': parsed_b}
    for tag in ('A', 'B'):
        summ = out['summary_' + tag.lower()]
        if parsed[tag]['truncated']:
            return other('probe_truncated_%s' % tag,
                         persisted_lines=len(parsed[tag]['lines']))
        if not parsed[tag]['framing_ok']:
            return other('probe_framing_%s' % tag,
                         malformed=parsed[tag]['malformed'])
        if summ['hash_errors']:
            return other('hash_error_%s' % tag, markers=summ['hash_errors'])
        if parsed[tag]['capped']:
            return other('probe_cap_%s' % tag)

    rows_a, _ = parse_hashes(boot_a)
    rows_b, _ = parse_hashes(boot_b)
    complete_a = res_a.get('bound') == 'target' and hash_prefix_ok(rows_a)
    complete_b = res_b.get('bound') == 'target' and hash_prefix_ok(rows_b)
    if not (complete_a and complete_b):
        return other('incomplete_trace',
                     complete_a=complete_a, complete_b=complete_b,
                     rows_a=len(rows_a), rows_b=len(rows_b),
                     bound_a=res_a.get('bound'), bound_b=res_b.get('bound'))

    for tag, parsed in (('A', parsed_a), ('B', parsed_b)):
        if parsed['proof_vsync'] is None or parsed['proof_vsync'] <= STOP_TICK:
            return other('probe_unproven_%s' % tag,
                         proof_vsync=parsed['proof_vsync'])

    hash_diff = first_hash_diff(rows_a, rows_b)
    probe_diff = first_probe_diff(parsed_a['windowed'], parsed_b['windowed'])

    # Preserve both first differences for every post-gate outcome.
    both = {'first_hash_tick': hash_diff['first_hash_tick'],
            'first_probe_seq': probe_diff['first_probe_seq']}
    if hash_diff['differ']:
        both.update({k: v for k, v in hash_diff.items()
                     if k not in ('differ', 'first_hash_tick')})
    if probe_diff['differ']:
        both.update({k: v for k, v in probe_diff.items()
                     if k not in ('differ', 'reason', 'first_probe_seq')})

    for key in ('card_initial_files', 'card_final_files',
                'card_initial_sha256', 'card_final_sha256'):
        if res_a.get(key) != res_b.get(key):
            out.update({'verdict': 'FAIL', 'reason': 'card_manifest_difference',
                        'key': key})
            out.update(both)
            return out

    if hid_text is not None:
        hid_cls = classify_hid(hid_text)
        out['hid_classification'] = {
            'flag': hid_cls['flag'], 'flagged': hid_cls['flagged'],
            'known_internal': hid_cls['known_internal'], 'rows': hid_cls['rows']}
        if hid_cls['flag']:
            out.update({'verdict': 'OTHER', 'reason': 'live_pad_present'})
            out.update(both)
            return out
    elif out['summary_a']['gamepad_like'] or out['summary_b']['gamepad_like']:
        out.update({'verdict': 'OTHER', 'reason': 'live_pad_present',
                    'note': 'legacy substring flag; no full hidutil list adjudicated',
                    'hidutil_a': res_a.get('hidutil'), 'hidutil_b': res_b.get('hidutil')})
        out.update(both)
        return out

    # B discriminator below the card/HID gates.
    if not probe_diff['differ'] and not hash_diff['differ']:
        out.update({'verdict': 'OTHER', 'reason': 'no_probe_difference'})
        out.update(both)
        return out
    if not probe_diff['differ'] and hash_diff['differ']:
        # Ordered write chain is missing: never attribute a later hash
        # divergence to the pad alone.
        out.update({'verdict': 'FAIL', 'reason': 'hash_without_probe'})
        out.update(both)
        return out

    # Probe differs: locate the first differing guest write. PASS requires a
    # paired first pad record with identical metadata (seq, vsync, ord,
    # port, slot, addr, len, ok) and exactly the `bytes` payload differing.
    # Length-only (extra windowed line) or any metadata drift is FAIL.
    assert probe_diff['differ']
    if probe_diff.get('reason') == 'probe_length_difference':
        longer = parsed_a['windowed'] if len(parsed_a['windowed']) > len(parsed_b['windowed']) \
            else parsed_b['windowed']
        first_seq, first_line = longer[min(len(parsed_a['windowed']), len(parsed_b['windowed']))]
        info = parse_probe_line(first_line)
        if info is None:
            out.update({'verdict': 'FAIL', 'reason': 'probe_malformed_first_diff'})
            out.update(both)
            return out
        out['first_probe_vsync'] = info['vsync']
        out['first_probe_family'] = info['family']
        out['first_probe_port'] = info['port']
        out.update(both)
        out.update({'verdict': 'FAIL', 'reason': 'probe_length_difference'})
        return out
    info_a = parse_probe_line(probe_diff['line_a'])
    info_b = parse_probe_line(probe_diff['line_b'])
    if info_a is None or info_b is None:
        out.update({'verdict': 'FAIL', 'reason': 'probe_malformed_first_diff'})
        out.update(both)
        return out
    head_a, pay_a = split_bytes_payload(probe_diff['line_a'])
    head_b, pay_b = split_bytes_payload(probe_diff['line_b'])
    out['first_probe_vsync'] = info_b['vsync']
    out['first_probe_family'] = info_b['family']
    out['first_probe_port'] = info_b['port']
    out.update(both)
    if head_a is None or head_b is None:
        out.update({'verdict': 'FAIL', 'reason': 'probe_malformed_first_diff'})
        return out
    if (info_a['seq'] != info_b['seq'] or info_a['vsync'] != info_b['vsync']
            or head_a != head_b):
        out.update({'verdict': 'FAIL', 'reason': 'probe_metadata_difference'})
        return out
    if pay_a == pay_b:
        # Heads identical yet lines differ should be impossible; treat as
        # metadata-class failure rather than a bytes discriminator.
        out.update({'verdict': 'FAIL', 'reason': 'probe_metadata_difference'})
        return out
    info = info_b
    if info['family'] != 'pad':
        out.update({'verdict': 'FAIL', 'reason': 'probe_wrong_family'})
        return out
    if info['vsync'] < PULSE_VSYNC_LO:
        out.update({'verdict': 'FAIL', 'reason': 'probe_earlier_difference'})
        return out
    if info['vsync'] > PULSE_VSYNC_HI:
        out.update({'verdict': 'FAIL', 'reason': 'probe_late_difference'})
        return out
    if hash_diff['differ'] and hash_diff['first_hash_tick'] < info['vsync']:
        out.update({'verdict': 'FAIL', 'reason': 'hash_earlier_than_probe'})
        return out
    out.update({'verdict': 'PASS', 'reason': 'pad_b_discriminated'})
    return out


PAD_OK = ('pad seq=%d vsync=%d ord=%d port=%d slot=0 addr=0x12345678 len=32 ok=1 bytes=%s')
GETDIR_EMPTY = ('getdir seq=2 vsync=11 ord=1 port=0 slot=0 addr=0x10000000 '
                'entries=0 max=16 len=0 ok=0 reason=empty bytes=')
MCREAD_OK = ('mcread seq=%d vsync=%d ord=%d fd=3 addr=0x10010000 req=64 len=64 '
             'ok=1 err=- bytes=%s')


def _pad(seq, vsync, ord, payload, port=0):
    return PAD_OK % (seq, vsync, ord, port, payload)


def _pulse_probe(payload_at_2010='ab' * 32, extra_2054=True):
    """Small synthetic window around the pulse: seqs 1..5 with proof > 2053."""
    lines = [
        _pad(1, 2005, 1, 'ab' * 32),
        _pad(2, 2009, 2, 'ab' * 32),
        _pad(3, 2010, 3, payload_at_2010),
        _pad(4, 2020, 4, 'ab' * 32),
    ]
    if extra_2054:
        lines.append(_pad(5, 2054, 5, 'ef' * 32))
    return lines


def _write_fake_run(parent, label, ticks=STOP_TICK, hash_mut=None,
                    probe_lines=None, raw_probe=None, drop_probe=False,
                    bound='target', manifest=None, gamepad=False,
                    gzip_logs=False):
    lane = Path(parent) / label
    lane.mkdir(parents=True)
    boot_lines = []
    for tick in range(1, ticks + 1):
        row = {'tick': tick, 'eeCycle': 1000 + tick, 'rdram': 'a' * 16,
               'scratch': 'b' * 16, 'vu1Data': 'c' * 16, 'vu1Code': 'd' * 16,
               'combined': 'e' * 16, 'count': tick * 3}
        if hash_mut and tick == hash_mut[0]:
            row[hash_mut[1]] = hash_mut[2]
        boot_lines.append(
            '[det-hash:v1] tick={tick} eeCycle={eeCycle} rdram={rdram} '
            'scratch={scratch} vu1Data={vu1Data} vu1Code={vu1Code} '
            'combined={combined} count={count}'.format(**row))
    boot_text = '\n'.join(boot_lines) + '\n'
    if gzip_logs:
        (lane / 'boot.log.gz').write_bytes(gzip.compress(boot_text.encode('utf-8')))
    else:
        (lane / 'boot.log').write_text(boot_text)
    if probe_lines is None:
        probe_lines = _pulse_probe()
    if raw_probe is not None:
        probe_bytes = bytes(raw_probe)
    else:
        probe_bytes = ('\n'.join(probe_lines) + '\n').encode()
    if not drop_probe:
        if gzip_logs:
            (lane / 'probe.log.gz').write_bytes(gzip.compress(probe_bytes))
        else:
            (lane / 'probe.log').write_bytes(probe_bytes)
    files = manifest if manifest is not None else {'mc0': [], 'mc1': []}
    result = {'label': label, 'bound': bound, 'stop_tick': STOP_TICK,
              'log_bytes': len(boot_text),
              'probe_bytes': 0 if drop_probe else len(probe_bytes),
              'probe_file': str(lane / 'probe.log'),
              'card_initial_files': files, 'card_final_files': files,
              'card_initial_sha256': 'x', 'card_final_sha256': 'x',
              'hidutil': {'gamepad_like': gamepad}}
    (lane / 'result.json').write_text(json.dumps(result) + '\n')
    return lane


def self_check():
    cases = []

    def check(name, verdict, reason=None, extra=None, **kwargs):
        with tempfile.TemporaryDirectory(prefix='e55d5-self-') as tmp:
            lanes = kwargs.pop('lanes')
            hid = kwargs.pop('hid_text', None)
            got = compare(*lanes(tmp), hid_text=hid)
        ok = got['verdict'] == verdict and (reason is None or got['reason'] == reason)
        if extra:
            ok = ok and all(got.get(k) == v for k, v in extra.items())
        cases.append((name, ok, got['verdict'], got['reason']))
        return ok

    def identical_prefix(tmp):
        # Byte-identical ordered prefix through the window + proof, no hash
        # diff: the pulse did not reach a guest write -> OTHER.
        return (_write_fake_run(tmp, 'A'), _write_fake_run(tmp, 'B'))

    def unchanged_no_event(tmp):
        # Same as above with gzipped logs (A1 on disk is gzipped): still OTHER.
        return (_write_fake_run(tmp, 'A', gzip_logs=True),
                _write_fake_run(tmp, 'B', gzip_logs=True))

    def expected_pad_change(tmp):
        bad = _pulse_probe(payload_at_2010='ce' + 'ab' * 31)
        return (_write_fake_run(tmp, 'A'),
                _write_fake_run(tmp, 'B', probe_lines=bad))

    def both_first_differences(tmp):
        # Pad diff in window (seq 3, vsync 2010) plus a later hash diff at
        # tick 2015: PASS with both first-difference fields preserved.
        bad = _pulse_probe(payload_at_2010='ce' + 'ab' * 31)
        return (_write_fake_run(tmp, 'A'),
                _write_fake_run(tmp, 'B', hash_mut=(2015, 'combined', 'f' * 16),
                                probe_lines=bad))

    def earlier_difference(tmp):
        lines = _pulse_probe()
        lines[0] = _pad(1, 2005, 1, 'ff' * 32)
        return (_write_fake_run(tmp, 'A'),
                _write_fake_run(tmp, 'B', probe_lines=lines))

    def wrong_family(tmp):
        lines_a = [_pad(1, 2005, 1, 'ab' * 32),
                   _pad(2, 2009, 2, 'ab' * 32),
                   MCREAD_OK % (3, 2010, 1, 'cd' * 64),
                   _pad(4, 2020, 3, 'ab' * 32),
                   _pad(5, 2054, 4, 'ef' * 32)]
        lines_b = [_pad(1, 2005, 1, 'ab' * 32),
                   _pad(2, 2009, 2, 'ab' * 32),
                   MCREAD_OK % (3, 2010, 1, 'ce' + 'cd' * 63),
                   _pad(4, 2020, 3, 'ab' * 32),
                   _pad(5, 2054, 4, 'ef' * 32)]
        return (_write_fake_run(tmp, 'A', probe_lines=lines_a),
                _write_fake_run(tmp, 'B', probe_lines=lines_b))

    def late_difference(tmp):
        lines_a = _pulse_probe()
        lines_a[3] = _pad(4, 2040, 4, 'ab' * 32)
        lines_b = _pulse_probe()
        lines_b[3] = _pad(4, 2040, 4, 'ff' * 32)
        return (_write_fake_run(tmp, 'A', probe_lines=lines_a),
                _write_fake_run(tmp, 'B', probe_lines=lines_b))

    def metadata_only(tmp):
        # Same bytes payload at seq 3 / vsync 2010, but port metadata
        # differs (0 vs 1): framing stays valid, must be FAIL metadata.
        lines_b = _pulse_probe()
        lines_b[2] = _pad(3, 2010, 3, 'ab' * 32, port=1)
        return (_write_fake_run(tmp, 'A'),
                _write_fake_run(tmp, 'B', probe_lines=lines_b))

    def length_only(tmp):
        # Shared 4-line windowed prefix identical; B carries one extra
        # windowed pad line (vsync 2025) before the proof line.
        lines_b = [
            _pad(1, 2005, 1, 'ab' * 32),
            _pad(2, 2009, 2, 'ab' * 32),
            _pad(3, 2010, 3, 'ab' * 32),
            _pad(4, 2020, 4, 'ab' * 32),
            _pad(5, 2025, 5, 'ab' * 32),
            _pad(6, 2054, 6, 'ef' * 32),
        ]
        return (_write_fake_run(tmp, 'A'),
                _write_fake_run(tmp, 'B', probe_lines=lines_b))

    def hash_without_probe(tmp):
        return (_write_fake_run(tmp, 'A'),
                _write_fake_run(tmp, 'B', hash_mut=(1500, 'combined', 'f' * 16)))

    def hash_earlier_than_probe(tmp):
        bad = _pulse_probe(payload_at_2010='ce' + 'ab' * 31)
        return (_write_fake_run(tmp, 'A'),
                _write_fake_run(tmp, 'B', hash_mut=(2005, 'combined', 'f' * 16),
                                probe_lines=bad))

    def truncated_probe(tmp):
        raw = ('\n'.join(_pulse_probe())).encode()  # no trailing newline
        return (_write_fake_run(tmp, 'A', raw_probe=raw, bound='flush_unproven'),
                _write_fake_run(tmp, 'B'))

    def capped(tmp):
        lane_a = _write_fake_run(tmp, 'A')
        with open(lane_a / 'probe.log', 'ab') as f:
            f.write(b'cap seq=6 vsync=2055 bytes=100 msg=byte-cap-reached\n')
        return (lane_a, _write_fake_run(tmp, 'B'))

    def unproven(tmp):
        return (_write_fake_run(tmp, 'A', probe_lines=_pulse_probe(extra_2054=False),
                                bound='target'),
                _write_fake_run(tmp, 'B'))

    all_ok = True
    all_ok &= check('identical_prefix', 'OTHER', 'no_probe_difference',
                    extra={'first_hash_tick': None, 'first_probe_seq': None},
                    lanes=identical_prefix)
    all_ok &= check('unchanged_no_event', 'OTHER', 'no_probe_difference',
                    lanes=unchanged_no_event)
    all_ok &= check('expected_pad_change', 'PASS', 'pad_b_discriminated',
                    extra={'first_probe_seq': 3, 'first_probe_vsync': 2010,
                           'first_hash_tick': None},
                    lanes=expected_pad_change)
    all_ok &= check('both_first_differences', 'PASS', 'pad_b_discriminated',
                    extra={'first_probe_seq': 3, 'first_hash_tick': 2015},
                    lanes=both_first_differences)
    all_ok &= check('earlier_difference', 'FAIL', 'probe_earlier_difference',
                    extra={'first_probe_seq': 1}, lanes=earlier_difference)
    all_ok &= check('wrong_family', 'FAIL', 'probe_wrong_family',
                    extra={'first_probe_seq': 3}, lanes=wrong_family)
    all_ok &= check('late_difference', 'FAIL', 'probe_late_difference',
                    extra={'first_probe_seq': 4}, lanes=late_difference)
    all_ok &= check('metadata_only', 'FAIL', 'probe_metadata_difference',
                    extra={'first_probe_seq': 3, 'first_probe_vsync': 2010},
                    lanes=metadata_only)
    all_ok &= check('length_only', 'FAIL', 'probe_length_difference',
                    extra={'first_probe_seq': 5}, lanes=length_only)
    all_ok &= check('hash_without_probe', 'FAIL', 'hash_without_probe',
                    extra={'first_probe_seq': None}, lanes=hash_without_probe)
    all_ok &= check('hash_earlier_than_probe', 'FAIL', 'hash_earlier_than_probe',
                    extra={'first_probe_seq': 3, 'first_hash_tick': 2005},
                    lanes=hash_earlier_than_probe)
    all_ok &= check('truncated_probe', 'OTHER', 'probe_truncated_A',
                    lanes=truncated_probe)
    all_ok &= check('capped_probe', 'OTHER', 'probe_cap_A', lanes=capped)
    all_ok &= check('unproven_flush', 'OTHER', 'probe_unproven_A', lanes=unproven)
    for name, ok, verdict, reason in cases:
        print('%s %s verdict=%s reason=%s' % ('ok' if ok else 'MISMATCH', name, verdict, reason))
    return 0 if all_ok else 1


HID_HEADER = ('Services:\nVendorID ProductID LocationID UsagePage Usage '
              'RegistryID Transport Class Product UserClass Built-In')
HID_SMC_ROW = ('0x0  0x0  0x54503962  65280  5  0x100000c3a  (null)  '
               'AppleSMCKeysEndpoint  PMU tdie9  (null)  1')
HID_ANS_ROW = ('0x0  0x0  0x544e306e  65280  5  0x100000cb2  (null)  '
               'AppleANS3CGv2Controller  NAND CH0 temp  (null)  1')
HID_PAD_ROW = ('0x57e  0x2a0  0x12345678  1  5  0x100000d00  USB  '
               'IOUSBHostHIDDevice  Wireless Gamepad  Gamepad  0')
HID_USAGE_ROW = ('0x1234  0x5678  0x1234567a  1  4  0x100000d02  USB  '
                 'IOUSBHostHIDDevice  Vendor Input Stick  (null)  0')
HID_AMB_ROW = ('0x45e  0x28e  0x12345679  65280  3  0x100000d01  USB  '
               'IOUSBHostHIDDevice  Xbox 360 Controller  (null)  0')
HID_INTERNAL_ONLY = '\n'.join([HID_HEADER, HID_SMC_ROW, HID_ANS_ROW]) + '\n'


def self_check_hid():
    """Synthetic controls: internal ANS3 vs real gamepad vs ambiguous."""
    cases = []

    def record(name, ok, detail=''):
        cases.append(ok)
        print('%s %s %s' % ('ok' if ok else 'MISMATCH', name, detail))

    internal = classify_hid(HID_INTERNAL_ONLY)
    record('hid_internal_clean',
           not internal['flag'] and len(internal['known_internal']) == 1,
           'flag=%r known=%d' % (internal['flag'], len(internal['known_internal'])))
    gamepad = classify_hid(HID_INTERNAL_ONLY + HID_PAD_ROW + '\n')
    record('hid_real_gamepad_flagged',
           gamepad['flag'] and gamepad['flagged'][0][0] == 'game_name',
           str([k for k, _ in gamepad['flagged']]))
    usage = classify_hid(HID_INTERNAL_ONLY + HID_USAGE_ROW + '\n')
    record('hid_game_usage_flagged',
           usage['flag'] and usage['flagged'][0][0] == 'game_usage',
           str([k for k, _ in usage['flagged']]))
    ambig = classify_hid(HID_INTERNAL_ONLY + HID_AMB_ROW + '\n')
    record('hid_ambiguous_flagged',
           ambig['flag'] and ambig['flagged'][0][0] == 'ambiguous_controller',
           str([k for k, _ in ambig['flagged']]))

    def hid_pair(tmp, hid_text):
        return (_write_fake_run(tmp, 'A', gamepad=True),
                _write_fake_run(tmp, 'B', gamepad=True), hid_text)

    with tempfile.TemporaryDirectory(prefix='e55d5-hid-self-') as tmp:
        lane_a, lane_b, text = hid_pair(tmp, HID_INTERNAL_ONLY)
        got = compare(lane_a, lane_b, hid_text=text)
        # Identical probes + clean HID -> OTHER no-event (not PASS): the
        # pulse left no ordered write, so there is nothing to discriminate.
        record('hid_internal_end_to_end_other',
               got['verdict'] == 'OTHER' and got['reason'] == 'no_probe_difference',
               got['reason'])
    with tempfile.TemporaryDirectory(prefix='e55d5-hid-self-') as tmp:
        lane_a, lane_b, text = hid_pair(tmp, HID_INTERNAL_ONLY + HID_PAD_ROW + '\n')
        got = compare(lane_a, lane_b, hid_text=text)
        record('hid_gamepad_end_to_end_other',
               got['verdict'] == 'OTHER' and got['reason'] == 'live_pad_present',
               got['reason'])
    with tempfile.TemporaryDirectory(prefix='e55d5-hid-self-') as tmp:
        lane_a = _write_fake_run(tmp, 'A', gamepad=True)
        lane_b = _write_fake_run(tmp, 'B', gamepad=True)
        got = compare(lane_a, lane_b)
        record('hid_legacy_flag_other',
               got['verdict'] == 'OTHER' and got['reason'] == 'live_pad_present',
               got['reason'])
    return 0 if all(cases) else 1


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('run_a', nargs='?')
    ap.add_argument('run_b', nargs='?')
    ap.add_argument('--json-out', default=None)
    ap.add_argument('--hidutil-list', default=None,
                    help='full `hidutil list` text adjudicating the live-pad gate')
    ap.add_argument('--self-check', action='store_true')
    args = ap.parse_args()
    if args.self_check:
        rc = self_check()
        return rc or self_check_hid()
    if not args.run_a or not args.run_b:
        ap.error('need two run dirs (or --self-check)')
    hid_text = Path(args.hidutil_list).read_text(errors='replace') \
        if args.hidutil_list else None
    out = compare(args.run_a, args.run_b, hid_text=hid_text)
    text = json.dumps(out, indent=2) + '\n'
    if args.json_out:
        Path(args.json_out).write_text(text)
    print(text, end='')
    return 0 if out['verdict'] == 'PASS' else 1


if __name__ == '__main__':
    sys.exit(main())
