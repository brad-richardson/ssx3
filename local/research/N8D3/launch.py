#!/usr/bin/env python3
"""N8D3 one-install/one-launch settled-race image probe; device writes use its lease."""
import atexit
import hashlib
import json
import os
...
D = '622c49b1'
PKG = 'com.ps2x.runner'
ROOT = Path('/Users/brad/dev/ssx3-work/N8D3')
FILES = f'/storage/emulated/0/Android/data/{PKG}/files'
DUMP = f'{FILES}/n8d3-frames'
...
def cleanup():
    ...
    if lease_claimed:
        ...
        try:
            adb('shell', "echo 'LEASE_FREE N8D3 done' > /data/local/tmp/mg/LEASE", timeout=30)
...
def preflight(allow_ours=False):
    ...
    free_or_ours = lease.startswith('LEASE_FREE') or (allow_ours and lease == 'N8D3 one-launch')
...
def capture(label, tick):
    remote = f'/data/local/tmp/n8d3-{label}.png'
...
def pull_stage_files():
    ...
    receipt['result'] = 'race stages captured'


def main():
    global lease_claimed, force_stopped, logger, log_file, launch_start
    preflight()
    sh("echo 'N8D3 one-launch' > /data/local/tmp/mg/LEASE")
    lease_claimed = True
    event('LEASE claimed=' + sh('cat /data/local/tmp/mg/LEASE'))
    if sh('cat /data/local/tmp/mg/LEASE') != 'N8D3 one-launch':
    ...
    while True:
        ...
        if race_dumps and not race_image:
            ...
            race_image = True
            receipt['result'] = 'race frontend dump triggered'
            event(f'STOP frontend dump at host_tick={host_tick}')
            break
        if tick >= 2100:
...
if receipt['result'] != 'race stages captured':
    raise SystemExit(1)
