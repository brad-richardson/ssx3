#!/usr/bin/env python3
"""N8D6C: one leased Odin install and launch after orchestrator review."""
...
SERIAL = '622c49b1'
PKG = 'com.ps2x.runner'
SCRATCH = Path('/Users/brad/dev/ssx3-work/N8D6C')
APK = Path('/Users/brad/dev/ssx3-work/N8D6B/app-release.apk')
FILES = f'/storage/emulated/0/Android/data/{PKG}/files'
LEASE = '/data/local/tmp/mg/LEASE'
LOG_CAP = 16 * 1024 * 1024
PINS = {
    'apk': '6839a0a48ed1b55affb6e6bd952a269ba3736200255bb6628270bb11a874a611',
    'runner': '4e6c056dc48683ab725e16792748b83a122421d865f523d6d37c4bbb27fa0efd',
...
result = {'brief': 'N8D6C', 'install_count': 0, 'launch_count': 0,
...
def preflight(ours=False):
    ...
    free_lease = lease.startswith('LEASE_FREE') or (ours and lease == 'N8D6C one-launch')
...
def check_apk():
    ...
    record('APK and packaged runner/Turnip/HAL SHA pairs match N8D6B pins')
...
def probe(lines):
    ...
    out['vectors'] = {kind: tile_vector(lines, kind) for kind in ('sampled', 'raw')}
    out['errors'] = [line for line in relevant + stage_lines if 'ERROR' in line
                     or 'alignment=OTHER' in line]
...
def classify(p, frame):
    ...
    stages = p['stages']
    vectors = p['vectors']
    if set(stages) != {'circuit1', 'pre_deinterlace_merged', 'final'} \
            or not all(vectors.values()):
        return 'OTHER'
    ...
    if p['errors'] or (int(tick), int(fbp), pmode.lower(), int(width), int(height)) \
            ...
            or any((vector['occupied'], vector['active']) !=
                   (int(p[kind][1]), int(p[kind][2])) for kind, vector in vectors.items()) \
            or vectors['sampled']['values'] != vectors['raw']['values']:
        return 'OTHER'
    if circuit1[4] <= 100 and merged[4] <= 100 and final[4] <= 100:
        return 'A'
    if circuit1[4] >= 250 and merged[4] <= 100 and final[4] <= 100:
        return 'B'
    if circuit1[4] >= 250 and merged[4] >= 250 and final[4] <= 100:
        return 'C'
    return 'OTHER'
...
def run():
    ...
    if (SCRATCH / 'result.json').exists() or (SCRATCH / 'driver.log').exists():
        raise SystemExit('one-run guard: N8D6C receipt already exists')
    result['script_sha'] = script_sha
    record(f'SCRIPT_SHA {script_sha}')
    check_apk()
    preflight()
    shell(f"echo 'N8D6C one-launch' > {LEASE}")
    lease_claimed = True
    if shell(f'cat {LEASE}') != 'N8D6C one-launch':
        raise RuntimeError('lease claim did not persist')
    record('LEASE claimed N8D6C one-launch')
    ...
    dump = f'{FILES}/n8d6c-frames-{int(time.time())}'
    ...
    while True:
        ...
        if all(selected_probe.get(k) for k in ('alignment', 'control', 'sampled', 'raw')) \
            ...
            stop_reason = 'first complete stage and vector receipt with frontend dump'
            break
        if tick >= 2100:
    ...
    if stop_reason == 'first complete stage and vector receipt with frontend dump':
...
if __name__ == '__main__':
    try:
        ...
        if lease_claimed:
            ...
            try:
                shell(f"echo 'LEASE_FREE N8D6C done' > {LEASE}")
