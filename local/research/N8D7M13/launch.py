#!/usr/bin/env python3
"""N8D7M13: leased Odin diagnostic STEP=1 replay (step1) of the staged stream.

Reviewed-SHA-held launcher for EXACTLY ONE diagnostic replay per --run
invocation (step1, one launch) of the already-staged
1,100,696,462-byte N8D7M6 stream on the Odin, using gated APK
da9a41a8e808fe31d509489c60b3927587c367866c6c2c6582009c35de6ef262 with runner
e5a3302c..11af1, Turnip 717812c3..1ac29d, HAL 1b49d27c..fc387.

Device patterns based on N8D7M12P5A launch.py. Replay enters before EE boot via
...
reviewed SHA in a separate run gate (require --run step1
--released-sha <launch.py SHA>).

Source-grounded log syntax (shared core, fork n8d7m12-app @ 4fa0df1):
...
SCRATCH = SCRATCH_BASE  # rebound to SCRATCH_BASE / RUN once --run parses
RUN = None  # "step1", set from --run before any device action
APK = Path("/Users/brad/dev/ssx3-work/N8D7M12P6M6/app-release.apk")
APK_SIZE = 153753116
...
LEASE_TAG = None  # f"N8D7M13 {RUN} replay", set from --run before any device action
...
result = {"brief": "N8D7M13", "install_count": 0, "launch_count": 0,
...
def check_apk():
    ...
    with ZipFile(APK) as archive:
        ...
        for marker in (b"PS2X_GS_REPLAY_ONDEVICE", b"PS2X_GS_REPLAY_CAPTURE",
                       ...
                       b"PS2X_GS_REPLAY_PKTSEQ", b"GB4_PKTSEQ tick="):
...
PKTSEQ_PATTERN = re.compile(
    r"GB4_PKTSEQ tick=(\d+) seq=([0-9a-fA-F]{16}) commands=(\d+)")


def pktseq_rows(lines):
    """Extract ordered worker-consumption rows; None unless exact 2050.

    Returns a list of (tick, seq, commands) for every GB4_PKTSEQ line,
    or None unless the ticks are exactly 1..2050 in order (2050 rows),
    ...
    for line in lines:
        ...
        if match:
            rows.append((int(match.group(1)), match.group(2).lower(),
                         int(match.group(3))))
    if [tick for tick, _, _ in rows] != list(range(1, 2051)):
        return None
    return rows


REPLAY_PATTERN = re.compile(
    r"GB4_REPLAY tick=(\d+) vram=([0-9a-fA-F]{8}) "
    r"priv=([0-9a-fA-F]{8}) present=([0-9a-fA-F]{8})")
...
def run():
    ...
    if len(sys.argv) != 5 or sys.argv[1] != "--run" \
            or sys.argv[2] != "step1" \
            or sys.argv[3] != "--released-sha":
        raise SystemExit("review hold: require --run step1 "
                         "--released-sha <reviewed launch.py SHA-256>")
    RUN = sys.argv[2]
    SCRATCH = SCRATCH_BASE / RUN
    LEASE_TAG = f"N8D7M13 {RUN} replay"
    script_sha = file_sha(Path(__file__))
    if sys.argv[4] != script_sha:
    ...
    if (SCRATCH / "result.json").exists() or (SCRATCH / "driver.log").exists():
        raise SystemExit(f"one-run guard: N8D7M13 {RUN} receipt already exists")
    result["run"] = RUN
    result["script_sha"] = script_sha
    record(f"SCRIPT_SHA {script_sha} run={RUN}")
    check_apk()
    check_stream_local()
    preflight()
    ...
    stamp = int(time.time())
    ppm_dir = f"{FILES}/n8d7m13-{RUN}-frames-{stamp}"
    hashes_path = f"{FILES}/n8d7m13-{RUN}-{stamp}.hashes"
    ...
    record("REPLAY rows=2050 ticks=1..2050")
        ...
        if lease_claimed:
            ...
            try:
                ...
                if current == LEASE_TAG:
                    shell(f"echo 'LEASE_FREE N8D7M13 {RUN} done' > {LEASE}",
                          timeout=30)
