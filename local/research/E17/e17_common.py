...
R = W / 'PS2Recomp'
B0 = Path('/tmp/p1-link/runtime')
NEWB = Path('/tmp/e17-map-link/runtime')
B = Path(os.environ.get('E17_BUILD_DIR',str(NEWB)))
OUT = Path('/tmp/ssx3-e17-absorb-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
BASE_SHA = '784f2b3e668ffa7a238936be415dc2e03d6c283c'
...
def sample():
    paths = list(P.glob('e17-*')) + list(P.glob('._e17-*')) + list((P/'run').glob('*e17*'))
    # The shared function trace is charged even though its name lacks E17.
    ...
    if allocation_receipt.exists():
        ...
        roots=[R/'ps2xRuntime/src/runner',R/'.git',R/'games/ssx3']
        for root in roots:
...
def bound(s):
    if s.get('codegen_allocated',0)>=G+256*M-128*M:return 'codegen_allocation'
    if OUT.exists() and size(OUT,False)>=G-128*M:return 'codegen_logical'
    if s['internal_allocated'] >= 6*G-512*M: return 'internal_allocation'
    if s['internal_free'] <= 2*G+512*M: return 'internal_floor'
    if s['ssd_allocated'] >= 36*G-512*M: return 'ssd_allocation'
    if s['ssd_free'] <= 2*G+512*M: return 'ssd_floor'
def admission(s):
    assert not bound(s), s
    assert s['internal_free'] >= 2*G+512*M+max(0,6*G-s['internal_allocated']), s
    assert s['ssd_free'] >= 2*G+512*M+max(0,36*G-s['ssd_allocated']), s
