...
authored=list(E.glob('e17_*.py'))+[E/'e17_binding_test.cpp']+list(E.glob('*.md'))
for p in authored:
    data=p.read_text()
...
tails={}
for name,marker in [('REPORT.md','E17 REPORT TAIL COMPLETE'),('CONTRACT.md','E17 CONTRACT TAIL COMPLETE'),
                    ('COMMANDS.md','E17 COMMANDS TAIL COMPLETE'),('NEXT-BRIEF.md','E17 NEXT-BRIEF TAIL COMPLETE'),
...
assert all(f'E17 REPORT CHUNK {i} COMPLETE' in report for i in range(1,5))
...
assert stage['only_e17'] and stage['main_git_positive_growth']<32*1024**2
for p in E.rglob('*'):
...
lines=['# E17 complete tail receipt','','| Document | Lines | Bytes | SHA256 |','|---|---:|---:|---|']
...
lines+=['','| Report tail, verbatim |','|---|',f'| {tails["REPORT.md"]["last_line"]} |','',
         '**E17 TAIL RECEIPT COMPLETE — all four report chunks and the final tail are present; no omitted final section.**']
...
print('# E17 HYGIENE TAIL COMPLETE')
