...
save('e17a-lifetime.json',dict(intervals=intervals,guarded_card_event_count=len(guarded),guarded_card_event_sequences=guarded,aligned_triggers=aligns,shutdown=tail))
print('dynamic lifetimes',len(intervals),'guarded card events',len(guarded),'alignments',len(aligns))
print('# E17 LIFETIME TAIL COMPLETE — post-reuse stores remain raw')
