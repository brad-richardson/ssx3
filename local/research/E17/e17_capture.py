#!/usr/bin/env python3
"""E17 progression capture driver. Explicit preflight, caps, owned lease cleanup."""
import argparse
import datetime
import hashlib
import json
...
from e17_frame import read_png, fnv32
from e17_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e17_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E17'
W = Path('/Volumes/Extreme SSD/ps2recomp-spike')
FORK = W / 'PS2Recomp'
RUN = W / 'P1/run'
BIN = Path('/tmp/e17-map-link/runtime/ps2xRuntime/ps2EntryRunner')
TEST = Path('/tmp/e17-map-link/runtime/ps2xTest/ps2x_tests')
ELF = W / 'P1/cd/SLUS_207.72'
LEASE = Path('/tmp/ssx3-p-lane-lease')
WAITS = RUN / 'e17-waits.log'
...
MIB = 1024 * 1024
ALLOCATED_CAPS=dict(boot_log=256*MIB,trace=96*MIB,function_log=1024*MIB,e4_dir=64*MIB,park_dir=128*MIB,frames_dir=64*MIB,e7_dir=64*MIB,aggregate=1536*MIB)
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E17 forbids lease wait')
        raise RuntimeError('Lease occupied; E17 tables and stops without waiting')
    ...
    assert manifest_path.exists(), 'fresh E17 build manifest required'
    ...
    assert entry['predicate_cases']==14 and entry['leaf_cases']==24 and entry['consumer_cases']==3 and entry['leaf_present'] and entry['mpeg_fail_before_cases']==2
    ...
    suite_rc,suite_text,suite_env = run_suite(label+'-preclaim')
    suite=dict(rc=suite_rc,stdout=suite_text,stderr='')
    record['suite_environment']=suite_env
    (EVIDENCE/f'{label}-suite.txt').write_text(suite_text)
...
def size(path, allocated=False):
    # ExFAT metadata may live beside a root file/directory, outside its tree.
    path=Path(path)
...
def main():
    ...
    assert not (EVIDENCE/'boot-attempt.json').exists(), 'E17 permits only one boot'
    assert not list(EVIDENCE.glob('e17?-result.json')), 'E17 permits only one boot'
    label='e17'+args.phase
    assert args.report_all, 'E17 requires REPORT_ALL=1'
    ...
    try:
        with (EVIDENCE/f'{label}-liveness.txt').open('w') as live, paths['boot_log'].open('wb') as log:
            ...
            result['pid']=p.pid
            result['boot_utc']=utc()
            trace_pos=0;lines=0;span_at=None;last_emit=-5;last_frame=0
            while p.poll() is None:
                ...
                sample=dict(resources=resources,elapsed_s=round(elapsed,3),trace_lines=lines,bytes=counts,allocated=allocated,ssd_free=shutil.disk_usage(W).free,ssd_free_delta=free_before-shutil.disk_usage(W).free)
                if elapsed-last_emit>=5:
                    emit(sample);last_emit=elapsed
                # Aligned host images are retained in-process with exact metadata.
                # Avoid expensive PNG decoding inside the cap monitor.
