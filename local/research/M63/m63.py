#!/usr/bin/env python3
"""M63 interval-head bulk pair: row-323/324 neighborhood census (offline).

Usage: m63.py M16_DIR M15_DIR M62TXT WORK_DIR EVID_DIR

Reads M16 per-shape triplets + loo.txt, the M15 triplet, and the M62
receipt (pair + interval tables to reproduce); raw XFB dumps, 573440 B =
640x448 YUYV. Read-only inputs; receipt text goes to stdout (redirect
to WORK_DIR/m63.txt); PNG neighborhood map to WORK_DIR (evidence copy
iff the DESIGN.md rule meets, decided at report time).

Tasks (see DESIGN.md, recorded before running):
  1: pair reproduction (head pair + row-323/324 slices, all 33 cols)
  2: neighborhood census (bulk-neighbor + delta + decile tables)
...
M63_ROWS = (323, 324)  # neighborhood rows, inclusive
M63_NAMED = (310, 311, 312, 313, 314, 315)  # brief's neighbor set
# Head-bulk pins: (r,c) -> (kind, delta, dec, band) on m15.
M63_HEAD = {(323, 311): ("bulk", 3, 0, 2),
            (324, 311): ("bulk", 1, 0, 2),
            (323, 312): ("noncell", None, 3, 2),
            (324, 312): ("noncell", None, 3, 2)}
...
def parse_m62_target(txt: str):
    """Match targets from m62.txt (union + margins + runs + spans +
    gaps + 48 interval probes for 311/312).

    NOTE (disclosed, M61 precedent): m62.txt carries no per-site
    pair lines, so c311/c312 site sets guard vs DESIGN pins only.
    Likewise no seat d-profile lines: dhists guard vs DESIGN pins.
    """
    ...
    intervals = {}
    for m in re.finditer(r"^interval (m15|s0) c=(\d+) r=(\d+) "
                         r"kind=(\S+) delta=(\S+) dec=(\d+) band=(\d+)",
                         txt, re.M):
        frame, c, r = m.group(1), int(m.group(2)), int(m.group(3))
        dl = m.group(5)
        intervals[(frame, c, r)] = (m.group(4),
                                    None if dl == "None" else int(dl),
                                    int(m.group(6)), int(m.group(7)))
    if len(intervals) != 48:
        return None
    return {"union": union, "cols": cols, "runs": runs, "spans": spans,
            "gaps": gaps, "intervals": intervals}
...
def row_slice(row, cols, status):
    """Per-column probes at one fixed row over a column list.

    row: int; cols: list of Y-columns; status: {(r,c): {kind,
    delta, dec, band}}. Returns {col: {kind, delta, dec, band}}
    sorted by col. Pure function of (row, cols, status) for
    control reuse.
    """
    out = {}
    for col in sorted(int(c) for c in cols):
        out[col] = dict(status[(int(row), col)])
    return out


def probe_d(r, col_rows):
    """Nearest-tail-row distance of probe row r on one column.

    col_rows: sorted m15 tail-Y rows of the column (may be
    empty). Returns (d, lo, hi) with d = min |r-x| (None when
    empty), lo = max tail row <= r (or None), hi = min tail
    row >= r (or None). Pure function of (row, rows).
    """
    rows = sorted(int(x) for x in col_rows)
    if not rows:
        return None, None, None
    r = int(r)
    lo = max([x for x in rows if x <= r], default=None)
    hi = min([x for x in rows if x >= r], default=None)
    return min(abs(r - x) for x in rows), lo, hi
...
def task1_neighborhood_lines(t0, t15, pr, pc, pl, cols0, cols15,
                              dec_s0, masks, d0, d15, mask0,
                              mask15, tgt):
    """Task 1 receipt lines: margins + head pair + row slices +
    48-probe recheck."""
    L = []
    canon = []
    ...
    for col in M62_PAIR_COLS:
        ...
        L.append(f"pair margin c={col}: s0_n={s0n} m15_n={b['n']} "
                 ...
                 f"class={b['uclass']} "
                 f"m62txt_match={m_txt} design_match={m_des}")
        canon.append(L[-1])
    ...
    canon.append(L[-1])
    # head pair: full value rows for rows 323-324 on c311/c312
    # (probe-d = nearest-m15-tail-row distance on the same column).
    for (r, c) in [(323, 311), (324, 311), (323, 312), (324, 312)]:
        st = cell_status(r, c, mask15, d15, dec_s0)
        pd, plo, phi = probe_d(r, cols15.get(c, {"rows": []})["rows"])
        want = M63_HEAD[(r, c)]
        m = ((st["kind"], st["delta"], st["dec"], st["band"]) == want)
        L.append(f"headpair: m15 c={c} r={r} kind={st['kind']} "
                 f"delta={_fmt_delta(st['delta'])} dec={st['dec']} "
                 f"band={st['band']} d={pd} lo={plo} hi={phi} "
                 f"design_match={m}")
        canon.append(L[-1])
    # runs + setguards
    ...
    L.append(f"pair setguard c311: design_match={m311d} "
             f"(m62.txt carries no per-site lines, disclosed)")
    canon.append(L[-1])
    L.append(f"pair setguard c312: design_match={m312d} "
             f"(m62.txt carries no per-site lines, disclosed)")
    canon.append(L[-1])
    ...
    for col, rows in ((311, rowsA), (312, rowsB)):
        ...
        L.append(f"gap overlap c={col}: rows_in_{ilo}_{ihi}={ov} "
                 f"(want [])")
        canon.append(L[-1])
    # 48-probe recheck: rows 323-334 x pair cols x both frames
    # vs m62.txt interval lines (H2 guard input).
    nre = 0
    for tag, mask, dd in (("m15", mask15, d15), ("s0", mask0, d0)):
        for c in M62_PAIR_COLS:
            for r in range(ilo, ihi + 1):
                st = cell_status(r, c, mask, dd, dec_s0)
                want = tgt["intervals"].get((tag, c, r))
                m = (want is not None
                     and (st["kind"], st["delta"], st["dec"],
                          st["band"]) == want)
                nre += m
                L.append(f"recheck {tag} c={c} r={r} kind={st['kind']} "
                         f"delta={_fmt_delta(st['delta'])} dec={st['dec']} "
                         f"band={st['band']} m62txt_match={m}")
                canon.append(L[-1])
    L.append(f"recheck summary: match48={nre}/48")
    canon.append(L[-1])
    # neighborhood probes: rows 323-324 x all 33 union cols,
    # m15 primary + s0 auxiliary (132 probes).
    rlo, rhi = M63_ROWS
    m15stat, s0stat = {}, {}
    for c in allcols:
        for r in range(rlo, rhi + 1):
            ...
            m15stat[(r, c)] = st
            pd, plo, phi = probe_d(
                r, cols15.get(c, {"rows": []})["rows"])
            L.append(f"neighbor m15 c={c} r={r} kind={st['kind']} "
                     f"delta={_fmt_delta(st['delta'])} dec={st['dec']} "
                     f"band={st['band']} d={pd} lo={plo} hi={phi}")
            canon.append(L[-1])
    for c in allcols:
        for r in range(rlo, rhi + 1):
            ...
            pd, plo, phi = probe_d(
                r, cols15.get(c, {"rows": []})["rows"])
            L.append(f"neighbor s0 c={c} r={r} kind={st['kind']} "
                     f"delta={_fmt_delta(st['delta'])} dec={st['dec']} "
                     f"band={st['band']} d={pd} lo={plo} hi={phi} (aux)")
            canon.append(L[-1])
    m15cen = interval_census(rlo, rhi, allcols, m15stat)
    s0cen = interval_census(rlo, rhi, allcols, s0stat)
    for tag, cen in (("m15", m15cen), ("s0", s0cen)):
        for col in allcols:
            b = cen[col]
            L.append(f"neighborhood census {tag} c={col}: "
                     f"bulk={b['counts']['bulk']} "
                     f"noncell={b['counts']['noncell']} "
                     f"tail={b['counts']['tail']} n=2 "
                     f"decspan={b['decspan']}")
            canon.append(L[-1])
    for tag, stat in (("m15", m15stat), ("s0", s0stat)):
        for r in range(rlo, rhi + 1):
            sl = row_slice(r, allcols, stat)
            bk = sorted(c for c in allcols
                        if sl[c]["kind"] == "bulk")
            tl = sorted(c for c in allcols
                        if sl[c]["kind"] == "tail")
            nc = sum(1 for c in allcols
                     if sl[c]["kind"] == "noncell")
            L.append(f"rowslice {tag} r={r}: bulkcols={bk} "
                     f"tailcols={tl} noncell_n={nc} ncols={len(allcols)}")
            canon.append(L[-1])
    info = {"urows": urows, "srows": srows, "drows": drows,
            ...
            "allcols": allcols, "recheck48": nre}
    return L, canon, info


def task2_neighbor_lines(mask0, mask15, d0, d15, dec_s0, info1):
    """Task 2 receipt lines: bulk-neighbor + delta + decile tables."""
    L = []
    canon = []
    rlo, rhi = M63_ROWS
    m15cen = info1["m15cen"]
    s0cen = info1["s0cen"]
    allcols = info1["allcols"]
    # bulk-neighbor tables (m15 primary, s0 auxiliary): bulk rows
    # + counts at 323-324 per union column.
    for tag, cen in (("m15", m15cen), ("s0", s0cen)):
        for col in allcols:
            b = cen[col]["bulk"]
            k = cen[col]["counts"]
            s = (" ".join(f"{r}:{d:+d}/dec{dc}" for (r, d, dc) in b)
                 if b else "NONE")
            L.append(f"bulkneighbor {tag} c={col}: n={len(b)} "
                     f"rows=[{s}] counts={k['bulk']}/{k['noncell']}/"
                     f"{k['tail']}")
            canon.append(L[-1])
    # delta tables: every bulk/tail probe at 323-324 (col,row,delta,
    # sign) per frame — one line each.
    for tag, cen in (("m15", m15cen), ("s0", s0cen)):
        hits = []
        for col in allcols:
            for (r, d, _dc) in cen[col]["bulk"]:
                hits.append((col, r, d, "bulk"))
            for (r, d, _dc) in cen[col]["tail"]:
                hits.append((col, r, d, "tail"))
        hits.sort()
        s = (" ".join(f"c{c}r{r}:{d:+d}/{k}" for (c, r, d, k) in hits)
             if hits else "NONE")
        L.append(f"neighdelta {tag}: n={len(hits)} rows=[{s}]")
        canon.append(L[-1])
    # decile tables: per-column deciles at 323-324 (row order) +
    # named-neighbor gradient rows.
    for tag, cen in (("m15", m15cen), ("s0", s0cen)):
        for col in allcols:
            b = cen[col]
            dh = " ".join(f"d{d}:{b['dechist'][d]}"
                          for d in sorted(b["dechist"]))
            L.append(f"neighdec {tag} c={col}: "
                     f"roworder={b['deciles']} decspan={b['decspan']} "
                     f"dechist=[{dh}]")
            canon.append(L[-1])
    for tag, cen in (("m15", m15cen), ("s0", s0cen)):
        for r, ix in ((rlo, 0), (rhi, 1)):
            g = " ".join(f"c{c}:d{cen[c]['deciles'][ix]}"
                         for c in M63_NAMED)
            L.append(f"neighdecgrad {tag} r={r}: named=[{g}]")
            canon.append(L[-1])
    # neighbor-activity summary: union cols with bulk/tail at
    # 323-324 per frame (the pre-registered PNG split input).
    for tag, cen in (("m15", m15cen), ("s0", s0cen)):
        bk = sorted(c for c in allcols if cen[c]["counts"]["bulk"])
        tl = sorted(c for c in allcols if cen[c]["counts"]["tail"])
        L.append(f"neighboractivity {tag}: bulkcols={bk} "
                 f"tailcols={tl}")
        canon.append(L[-1])
    info = {"m15cen": m15cen, "s0cen": s0cen}
    return L, canon, info


def main():
    m16d, m15d, m62t, workd, evidd = (Path(a) for a in sys.argv[1:6])
    ...
    m62b = m62t.read_bytes()
    m62sha = hashlib.sha256(m62b).hexdigest()
    print(f"input {m62t}: bytes={len(m62b)} sha256={m62sha}")
    print(f"m62.txt sha prefix={m62sha[:8]}..{m62sha[-8:]} (tabled)")
    tgt = parse_m62_target(m62b.decode(errors="replace"))
    if tgt is None:
        print("m62.txt UNION/MARGIN/RUNS/SPAN/GAP/INTERVAL LINES NOT FOUND: STOP, tabled.")
        return
    print(f"m62.txt target union: ncols={len(tgt['union'])} "
          f"cols={tgt['union']}")
    for col in sorted(tgt["cols"]):
        b = tgt["cols"][col]
        print(f"m62.txt target margin c={col}: s0_n={b['s0_n']} "
              f"m15_n={b['m15_n']} pos={b['pos']} neg={b['neg']} "
              f"min={b['min']} max={b['max']} sum={b['sum']} "
              f"class={b['class']}")
    print(f"m62.txt target intervals: n={len(tgt['intervals'])} (want 48)")
    for col in sorted(tgt["runs"]):
        print(f"m62.txt target runs c={col}: nruns={tgt['runs'][col][0]} "
              f"runs=[{tgt['runs'][col][1]}]")
    for col in sorted(tgt["spans"]):
        b = tgt["spans"][col]
        print(f"m62.txt target span c={col}: m15_n={b['m15_n']} "
              f"rows={b['rowmin']}-{b['rowmax']} rowspan={b['rowspan']} "
              f"decspan={b['decspan']} dechist={b['dechist']}")
    for col in sorted(tgt["gaps"]):
        b = tgt["gaps"][col]
        print(f"m62.txt target gap c={col}: rows={b['rows']} "
    ...
    print("== Task 1: pair reproduction (head pair + row slices) ==")
    t1, canon1, pair1 = task1_neighborhood_lines(t0, t15, pr, pc, pl,
                                                 cols0, cols15, dec_s0,
                                                 masks, d0, d15, mask0,
                                                 mask15, tgt)
    for ln in t1:
    ...
    ok311 = c311set == M61_C311
    ok312 = c312set == M61_C312
    print(f"c311-guard: set_match={ok311} (design pins; "
          f"m62.txt carries no per-site lines)")
    print(f"c312-guard: set_match={ok312} (design pins; "
          f"m62.txt carries no per-site lines)")
    oks = True
    for col in M62_PAIR_COLS:
        s = pair1["srows"][col]
        ...
        m = (s["n"] == wt["m15_n"]
             ...
             and p["dhist"] == want_d[0]
             and p["na"] == want_d[1]
             and p["dmax"] == want_d[2]
             and p["dmode"] == want_d[3])
    ...
    print(f"interval-overlap-guard (no site rows in {ilo}-{ihi}): "
          f"match={oko}")
    okr48 = pair1["recheck48"] == 48
    print(f"recheck48-guard (48/48 interval probes vs m62.txt): "
          f"match={okr48}")
    if not (oku and okc and ok311 and ok312 and oks and okr and okg
            and oko and okr48):
        print("M62 UNION/MARGIN/SETS/SPAN/RUNS/GAP/OVERLAP/RECHECK MISMATCH: STOP, tabled.")
        return

    print("== Task 2: neighborhood census (bulk + delta + deciles) ==")
    t2b, canon2, census = task2_neighbor_lines(mask0, mask15, d0, d15,
                                               dec_s0, pair1)
    for ln in t2b:
        print(ln)
    ...
    m15cen = pair1["m15cen"]
    s0cen = pair1["s0cen"]
    print(f"bars: head_match={okc and ok311 and ok312} "
          f"recheck48={pair1['recheck48']} "
          f"m15bulk={{c: {dict((c, m15cen[c]['counts']['bulk']) for c in pair1['allcols'])}}} "
          f"s0bulk={{c: {dict((c, s0cen[c]['counts']['bulk']) for c in pair1['allcols'])}}}")
    ...
    _, canon1b, _pair1b = task1_neighborhood_lines(t0b, t15b, pr, pc, pl,
    ...
    print("== PNG neighborhood map (m15 tail-Y pair class + row overlay) ==")
    ...
    for (r, c) in m15sites:
        ...
        if pri[col3] >= pri.get(cur, 0):
            px[x, y] = col3
    rlo, rhi = M63_ROWS
    for c in pair1["allcols"]:
        for r in range(rlo, rhi + 1):
            x, y = c // 2, r // 2
            kind = pair1["m15stat"][(r, c)]["kind"]
            if kind == "bulk":
                px[x, y] = (0, 255, 255)  # cyan: neighborhood bulk
            elif kind == "tail":
                px[x, y] = (255, 0, 255)  # magenta: neighborhood tail
            else:
                px[x, y] = (0, 0, 139)  # dark blue: neighborhood noncell
    pngp = workd / "m63-neighbormap.png"
    img.save(pngp)
    nact = sorted(c for c in pair1["allcols"] if c != 311
                  and (m15cen[c]["counts"]["bulk"]
                       or m15cen[c]["counts"]["tail"]))
    print(f"neighbormap: {pngp} bytes={pngp.stat().st_size} "
          f"(evidence iff any non-311 union col bulk/tail at 323-324; "
          f"active={nact} split={bool(nact)})")
