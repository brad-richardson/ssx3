#!/usr/bin/env python3
"""N7: per-phase guest vsyncs/s from [vsync-rate] lines (logcat -v epoch) + SF present rates.
Usage: phases.py <launch dir>. Phase tick boundaries come from the I26-FAST route anchors
...
PH = [('title/startup', 0, 636), ('main menu', 636, 766),
      ('Select Character', 766, 884), ('Setup Character / Peak', 884, 1099),
      ('Select Mode / Event / My Rules', 1099, 1440),
      ('loading + Rival card', 1440, 1714), ('race', 1714, 10**9)]
...
wa = wall_at(1714)
if wa:
    print('race per-5s:', ' '.join(f'{(t1-t0)/(w1-w0):.1f}' for (w0,t0),(w1,t1) in zip(pts,pts[1:]) if t0 >= 1714))
