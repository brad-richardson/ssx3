#!/usr/bin/env python3
"""CP1 Odin launcher: F7's launch.py (F5 lineage: play-env pin ef9f94e1, mc0 pins,
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/CP1/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/CP1/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('CP1 ')):
...
if lease.startswith('CP1 '):
    CLAIMED['v'] = True  # outer hold (or a stale CP1 lease); we own it
else:
    r = subprocess.run(['bash', LEASESH, 'claim', 'CP1'], capture_output=True, text=True, timeout=60)
    if r.returncode != 0:
...
def lease_release():
    subprocess.run(['bash', LEASESH, 'release', 'CP1'], capture_output=True, timeout=60)
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['bash', LEASESH, 'release', 'CP1'],
                           capture_output=True, timeout=60)
    except Exception:
        pass
...
env = [f'# CP1 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
