#!/bin/sh
# G31: configure + build (b1)/(b2) state-probe parallel-gs-replayer for Android arm64.
# NEW build dir (all older dirs untouched). Host builds -j2.
# Recipe = G22/G26/G28/G29/G30 recipe (NON-sanitizer: no ANDROID_STL override, no
...
# (retained) + G31 in-vsync() state-dump hunk in gs/gs_interface.cpp
# (GSInterface::vsync, post-call, logcat only).
# Receipts: configure exit 0, build exit 0 [458/458], binary size/sha/BuildID
# in REPORT.md, G31: strings x1/x2, G30: x1/x1, G29: x1/x2, G28: x1, G26: x1.
set -e
export COPYFILE_DISABLE=1
CLONE="/Volumes/Extreme SSD/parallel-gs-g7"
BUILD="/Volumes/Extreme SSD/parallel-gs-g31-android-build"
