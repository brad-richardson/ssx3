#!/usr/bin/env bash
# AU9 race capture (from AU8 au8_race_cap.sh): same route, + AU9 SND/SPU2 hooks in $T4/cap.
...
stamp
pgrep -af 'GradleWrapperMain|ninja|clang|pcsx2-qt' | grep -v pgrep > $T4/precapture-jobs.txt || true # AU9: the idle Gradle daemon is not a job
...
rm -rf $T4/pcsx2-au4.pid $T4/pcsx2-tag1.bin $T4/spu2-tap.bin $T4/cap; mkdir -p $T4/cap
T_BOOT=$(date -u +%s)
echo "T_BOOT_WALL:$T_BOOT T_BOOT_UPTIME:$(cut -d. -f1 /proc/uptime)"
mkdir -p $DAT $T4/logs; PCSX2_AU4_CAPTURE=1 PCSX2_AU8_DIR=$T4 PCSX2_AU9_DIR=$T4/cap nohup $BIN -nogui -slowboot -turbo -datapath $DAT -logfile $T4/logs/boot-$TAG.log -- "$R/inputs/SSX 3 (USA).iso" > $T4/logs/boot-$TAG.stdout 2>&1 &
...
for W in $(seq 1 150); do
  ...
  [ $N -ge $((SC_RECORDS + 21000)) ] && break
  sleep 2
done
[ $N -ge $((SC_RECORDS + 21000)) ] || fail TOO_SHORT
