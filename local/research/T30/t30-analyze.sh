#!/usr/bin/env bash
# T30 trace analysis on the Continue-Cross emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t30-census.txt
python3 $D/t4-sample.py $E $D/t30-samples.txt
ls -la $D/t30-census.txt $D/t30-samples.txt
date -u
