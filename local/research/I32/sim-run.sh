#!/bin/bash
# I30 Simulator run: bundled ps2x.env sets PS2X_SOUND=1, so NO launch-env
# sound override; only the vsync rate log comes from the launcher.
set -u -o pipefail
...
trap cleanup EXIT
while [ -z "$SLOT" ]; do
  if ! SLOT=$(python3 /Users/brad/dev/ssx3/local/tooling/p_lane_lease.py claim I32-sim 2>/dev/null); then
    SLOT=''
...
START=$(date +%s)
for target in 10 30 35 100 160; do
