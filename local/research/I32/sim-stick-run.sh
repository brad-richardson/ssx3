#!/bin/bash
# I32 run 2 (stick proof): auto-route OFF (empty PS2X_PAD_SCRIPT from the
# launcher, I26's mechanism), DEV-ONLY PS2X_VPAD_TEST_STICK="1,0" injected.
# Expect: [vpad] test stick lx=1.000 ly=0.000 and
# [vpad] stick pad bytes lx=0xff ly=0x80 in console.log.
set -u -o pipefail
...
trap cleanup EXIT
while [ -z "$SLOT" ]; do
  if ! SLOT=$(python3 /Users/brad/dev/ssx3/local/tooling/p_lane_lease.py claim I32-stick 2>/dev/null); then
    SLOT=''
...
xcrun simctl terminate "$SIM" "$APP" >/dev/null 2>&1 || true
SIMCTL_CHILD_PS2X_VSYNC_RATE_LOG=1 SIMCTL_CHILD_PS2X_VPAD_TEST_STICK="1,0" SIMCTL_CHILD_PS2X_PAD_SCRIPT= xcrun simctl launch --console-pty "$SIM" "$APP" > "$OUT/console.log" 2>&1 &
LP=$!
...
for target in 20; do
  while [ $(( $(date +%s) - START )) -lt "$target" ]; do
    ...
    if [ "$elapsed" -ge 40 ]; then echo 'wall cap reached' >> "$OUT/run.txt"; exit 1; fi
...
echo "elapsed=$(( $(date +%s) - START ))s stick_proof_done=yes" >> "$OUT/run.txt"
