#!/usr/bin/env python3
"""RP1 bounded boot (from F4's driver): RP1 codegen clone pins, --route i26|fr1r1,
--dump-every N (PS2X_FRAME_DUMP_EVERY), --dump-ticks a,b,c (PS2X_FRAME_DUMP_ONCE_TICKS), --wall.
I26-FAST to the race, paraLLEl GS backend. Builds use the UNPROMOTED F4
codegen (~/dev/ssx3-work/F4/codegen); register_functions.cpp is identical
to canonical, so the changed file sub_003FE828 is pinned too.
...
WORK = Path('/Users/brad/dev/ssx3-work/RP1')
ELF = Path('/Users/brad/dev/ssx3-work/E32-inputs/cd/SLUS_207.72')
ISO = Path('/Users/brad/dev/ssx3-work/E32-inputs/SSX 3 (USA).iso')
CODEGEN = Path('/Users/brad/dev/ssx3-work/RP1/codegen/register_functions.cpp')
CODEGEN_VF0 = Path('/Users/brad/dev/ssx3-work/RP1/codegen/sub_003FE828_0x3fe828.cpp')
...
def main():
    ...
    ap.add_argument('--route', choices=('i26', 'fr1r1'), default='i26')
    ...
    if args.mode == 'det':
        ...
        if args.dump_every:
            env['PS2X_FRAME_DUMP_EVERY'] = str(args.dump_every)
        if args.dump_ticks:
    ...
    held = os.environ.get('RP1_HELD_SLOT')
    if held:
        ...
        slot = claim('RP1-' + args.label, exclusive=exclusive)
        while slot is None:
    ...
    try:
        ...
        with log.open('wb') as out:
            ...
            th = None
            if args.mode == 'det' and not args.no_snap and not args.dump_every and not args.dump_ticks:
