#!/usr/bin/env bash
# T29 trace analysis on the Zoe-Cross emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t29-census.txt
python3 $D/t4-sample.py $E $D/t29-samples.txt
ls -la $D/t29-census.txt $D/t29-samples.txt
date -u
