#!/usr/bin/env python3
"""N8D5E: one leased Odin install and launch after orchestrator review."""
...
SERIAL = '622c49b1'
PKG = 'com.ps2x.runner'
SCRATCH = Path('/Users/brad/dev/ssx3-work/N8D5E')
APK = Path('/Users/brad/dev/ssx3-work/N8D5D/app-release.apk')
FILES = f'/storage/emulated/0/Android/data/{PKG}/files'
LEASE = '/data/local/tmp/mg/LEASE'
LOG_CAP = 16 * 1024 * 1024
PINS = {
    'apk': 'a6a0c3793e31310fc8794e7a0d6b6cc282e5060d59cfe116b919236b9f50612a',
    'runner': '6a93af327b3cb1c5ebd9b3b81dc68ae1eb5e259633c9c61b78a82ba06428d162',
...
result = {'brief': 'N8D5E', 'install_count': 0, 'launch_count': 0,
...
def preflight(ours=False):
    ...
    free_lease = lease.startswith('LEASE_FREE') or (ours and lease == 'N8D5E one-launch')
...
def check_apk():
    ...
    record('APK and packaged runner/Turnip SHA pairs match N8D5D pins')
...
def run():
    ...
    if (SCRATCH / 'result.json').exists() or (SCRATCH / 'driver.log').exists():
        raise SystemExit('one-run guard: N8D5E receipt already exists')
    result['script_sha'] = script_sha
    record(f'SCRIPT_SHA {script_sha}')
    check_apk()
    preflight()
    shell(f"echo 'N8D5E one-launch' > {LEASE}")
    lease_claimed = True
    if shell(f'cat {LEASE}') != 'N8D5E one-launch':
        raise RuntimeError('lease claim did not persist')
    record('LEASE claimed N8D5E one-launch')
    ...
    dump = f'{FILES}/n8d5e-frames-{int(time.time())}'
...
if __name__ == '__main__':
    try:
        ...
        if lease_claimed:
            ...
            try:
                shell(f"echo 'LEASE_FREE N8D5E done' > {LEASE}")
