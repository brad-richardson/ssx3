#!/usr/bin/env python3
"""Pin E9 configs, active generated sources/bindings and executable identity."""
...
def main():
    ...
    sources=list((R/'ps2xRuntime/src/runner').glob('*.cpp'))+list((R/'ps2xRuntime/include').glob('ps2_recompiled_*.h'))
    ...
    selected={hex(pc):[line for line in bindings if line.rstrip().endswith(f'// 0x{pc:x}')]
              for pc in [0x2c50e0,0x2c5140,0x426230]}
    ...
    for p in sources:
        if any(s in p.name for s in ['002C50E0','002C5140','004261F0','00426230']):
