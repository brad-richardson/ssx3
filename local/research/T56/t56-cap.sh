#!/usr/bin/env bash
# T56 capture: SC-settled statefile boot, scratchpad writer watch whole boot.
# Foreground run (T51 §T51-6: no detached boots on bytesize). One held ssh.
# Free-gate files are touched for the shared T50/T51 gates; the T56 watch
# itself is ungated (statefile boot lands SC-settled).
# Env: TAG=t56a (default). WATCH2_SRC=path to install as /tmp/t56-watch2
# (capture 2 only; capture 1 ensures the file is absent).
set -x
...
TAG=${TAG:-t56a}
LOG=$T4/$TAG-poll.log
fail() { echo "T56_FAIL:$1"; date -u; exit 1; }
plog() { echo "$@" | tee -a $LOG; }
[ -x $BIN ] || fail NO_BIN
...
if [ -n "${WATCH2_SRC:-}" ]; then
  cp "$WATCH2_SRC" /tmp/t56-watch2 || fail WATCH2_CP
  cat /tmp/t56-watch2 | tee -a $LOG
fi
...
plog "COUNTS spw=$(grep -c 'spw vsync=' $E || true) cap=$(grep -c 'T56_CAP' $E || true) armed=$(grep -c 'T56W_ARMED' $E || true) ctag=$(grep -c 'ctag ' $E || true)"
