#!/usr/bin/env bash
# T31 (Cross on Peak 1 from the Select Peak park): reproduce the T30 R1
...
# SC-LIKE gate, Cross on Zoe, ZC-LIKE gate, Cross on Continue), confirm
# the Select Peak park, then press ONE Cross (x Select) on Peak 1 and map
# what comes next.
# Bounded: <=3 park attempts, ONE Peak Cross (plus script-captured
...
fail() { echo "T31_FAIL:$1"; stamp; exit 1; }
: > $LOG
plog() { echo "$@" | tee -a $LOG; }
...
sleep 2
pgrep -a Xvfb || true
setsid nohup Xvfb :99 -screen 0 1280x1024x24 >/tmp/xvfb-t31.log 2>&1 < /dev/null &
sleep 4
pgrep -a Xvfb || fail NO_XVFB
xdotool getdisplaygeometry || fail NO_DISPLAY
TS=$(date -u +%Y%m%dT%H%M%SZ)
if [ -f $E ]; then
  mv $E $D/dat/PCSX2/logs/emulog-pre-t31-$TS.txt
  ls -la $D/dat/PCSX2/logs/
fi
rm -f $D/pcsx2.pid
T_BOOT=$(date -u +%s)
echo "T_BOOT_WALL:$T_BOOT T_BOOT_UPTIME:$(cut -d. -f1 /proc/uptime)"
nohup $D/pcsx2/build/bin/pcsx2-qt -nogui -slowboot -turbo -datapath $D/dat -logfile $D/logs/boot-t31.log -- "$D/inputs/SSX 3 (USA).iso" > $D/logs/boot-t31.stdout 2>&1 &
echo $! > $D/pcsx2.pid
echo "PID:$!"
sleep 90
...
snap t31-start || fail SNAP_START
read SM SP <<< $(score_band $D/t31-start.ppm)
...
for A in 1 2 3; do
  ...
  snap t31-a$A-now || fail SNAP_A${A}_NOW
  read M P <<< $(score_band $D/t31-a$A-now.ppm)
  plog "ATTEMPT_$A now score mean=$M p99=$P"
  if is_title $M $P; then
    plog "ATTEMPT_$A TITLE-ALREADY-DETECTED (no Cross needed)"
    cp $D/t31-a$A-now.ppm $D/t31-a$A-pre.ppm
    cp $D/t31-a$A-now.jpg $D/t31-a$A-pre.jpg
    ...
    for N in 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 18; do
      sleep 2
      snap t31-a$A-poll$N || fail SNAP_A${A}_POLL$N
      read M P <<< $(score_band $D/t31-a$A-poll$N.ppm)
      plog "ATTEMPT_$A poll$N score mean=$M p99=$P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      if is_title $M $P; then
        plog "ATTEMPT_$A TITLE-DETECTED at poll$N"
        cp $D/t31-a$A-poll$N.ppm $D/t31-a$A-pre.ppm
        cp $D/t31-a$A-poll$N.jpg $D/t31-a$A-pre.jpg
        press Return "A${A}_START"
        DETECTED=1
        break
      fi
  ...
  # post-press series with scores (pre-press snap = t31-a$A-pre.*)
  sleep 3; snap t31-a$A-post3 || fail SNAP_A${A}_POST3
  read M3 P3 <<< $(score_band $D/t31-a$A-post3.ppm)
  plog "ATTEMPT_$A post+3 score mean=$M3 p99=$P3 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  sleep 5; snap t31-a$A-post8 || fail SNAP_A${A}_POST8
  read M8 P8 <<< $(score_band $D/t31-a$A-post8.ppm)
  plog "ATTEMPT_$A post+8 score mean=$M8 p99=$P8"
  sleep 7; snap t31-a$A-post15 || fail SNAP_A${A}_POST15
  read M15 P15 <<< $(score_band $D/t31-a$A-post15.ppm)
  plog "ATTEMPT_$A post+15 score mean=$M15 p99=$P15"
  sleep 10; snap t31-a$A-post25 || fail SNAP_A${A}_POST25
  read M25 P25 <<< $(score_band $D/t31-a$A-post25.ppm)
  plog "ATTEMPT_$A post+25 score mean=$M25 p99=$P25"
  # menu-like gate: +8/+15/+25 all non-title AND whole-static across them
  W815=$(score_whole $D/t31-a$A-post8.ppm $D/t31-a$A-post15.ppm)
  W1525=$(score_whole $D/t31-a$A-post15.ppm $D/t31-a$A-post25.ppm)
  plog "ATTEMPT_$A whole815=$W815 whole1525=$W1525"
  read MM25 MP25 <<< $(score_menu $D/t31-a$A-post25.ppm)
...
if [ $MENU_LIKE -eq 1 ]; then
  ...
  sleep 5
  snap t31-menupre || fail SNAP_MENUPRE
  read MMP MMP_P <<< $(score_menu $D/t31-menupre.ppm)
  plog "MENUPRE vs-menu mean=$MMP p99=$MMP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  read MPT MPT_P <<< $(score_band $D/t31-menupre.ppm)
  ...
  sleep 1; snap t31-mc-post1 || fail SNAP_MC_POST1
  read M1 P1 <<< $(score_band $D/t31-mc-post1.ppm)
  read MM1 MP1 <<< $(score_menu $D/t31-mc-post1.ppm)
  read MS1 MSP1 <<< $(score_sc $D/t31-mc-post1.ppm)
  plog "MC post+1 titleband mean=$M1 p99=$P1 vs-menu mean=$MM1 p99=$MP1 vs-sc mean=$MS1 p99=$MSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  sleep 2; snap t31-mc-post3 || fail SNAP_MC_POST3
  read M3 P3 <<< $(score_band $D/t31-mc-post3.ppm)
  read MM3 MP3 <<< $(score_menu $D/t31-mc-post3.ppm)
  read MS3 MSP3 <<< $(score_sc $D/t31-mc-post3.ppm)
  plog "MC post+3 titleband mean=$M3 p99=$P3 vs-menu mean=$MM3 p99=$MP3 vs-sc mean=$MS3 p99=$MSP3"
  sleep 5; snap t31-mc-post8 || fail SNAP_MC_POST8
  read M8 P8 <<< $(score_band $D/t31-mc-post8.ppm)
  read MM8 MP8 <<< $(score_menu $D/t31-mc-post8.ppm)
  read MS8 MSP8 <<< $(score_sc $D/t31-mc-post8.ppm)
  plog "MC post+8 titleband mean=$M8 p99=$P8 vs-menu mean=$MM8 p99=$MP8 vs-sc mean=$MS8 p99=$MSP8"
  sleep 7; snap t31-mc-post15 || fail SNAP_MC_POST15
  read M15 P15 <<< $(score_band $D/t31-mc-post15.ppm)
  read MM15 MP15 <<< $(score_menu $D/t31-mc-post15.ppm)
  read MS15 MSP15 <<< $(score_sc $D/t31-mc-post15.ppm)
  ...
  H_PRE1=$(score_whole $D/t31-menupre.ppm $D/t31-mc-post1.ppm)
  H_13=$(score_whole $D/t31-mc-post1.ppm $D/t31-mc-post3.ppm)
  H_38=$(score_whole $D/t31-mc-post3.ppm $D/t31-mc-post8.ppm)
  H_815=$(score_whole $D/t31-mc-post8.ppm $D/t31-mc-post15.ppm)
  ...
  if [ $SC_LIKE -eq 1 ]; then
    ...
    # dwell pressure.
    sleep 5
    snap t31-scpre || fail SNAP_SCPRE
    read SCP SCP_P <<< $(score_sc $D/t31-scpre.ppm)
    plog "SCPRE vs-sc mean=$SCP p99=$SCP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
    read SMP SMP_P <<< $(score_menu $D/t31-scpre.ppm)
    plog "SCPRE vs-menu mean=$SMP p99=$SMP_P"
    read SPT SPT_P <<< $(score_band $D/t31-scpre.ppm)
    ...
    sleep 1; snap t31-zc-post1 || fail SNAP_ZC_POST1
    read Z1 ZP1 <<< $(score_band $D/t31-zc-post1.ppm)
    read ZS1 ZSP1 <<< $(score_sc $D/t31-zc-post1.ppm)
    read ZZ1 ZZP1 <<< $(score_zc $D/t31-zc-post1.ppm)
    plog "ZC post+1 titleband mean=$Z1 p99=$ZP1 vs-sc mean=$ZS1 p99=$ZSP1 vs-zc mean=$ZZ1 p99=$ZZP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
    sleep 2; snap t31-zc-post3 || fail SNAP_ZC_POST3
    read Z3 ZP3 <<< $(score_band $D/t31-zc-post3.ppm)
    read ZS3 ZSP3 <<< $(score_sc $D/t31-zc-post3.ppm)
    read ZZ3 ZZP3 <<< $(score_zc $D/t31-zc-post3.ppm)
    plog "ZC post+3 titleband mean=$Z3 p99=$ZP3 vs-sc mean=$ZS3 p99=$ZSP3 vs-zc mean=$ZZ3 p99=$ZZP3"
    sleep 5; snap t31-zc-post8 || fail SNAP_ZC_POST8
    read Z8 ZP8 <<< $(score_band $D/t31-zc-post8.ppm)
    read ZS8 ZSP8 <<< $(score_sc $D/t31-zc-post8.ppm)
    read ZZ8 ZZP8 <<< $(score_zc $D/t31-zc-post8.ppm)
    ...
    ZH_PRE1=$(score_whole $D/t31-scpre.ppm $D/t31-zc-post1.ppm)
    ZH_13=$(score_whole $D/t31-zc-post1.ppm $D/t31-zc-post3.ppm)
    ZH_38=$(score_whole $D/t31-zc-post3.ppm $D/t31-zc-post8.ppm)
    ...
    if [ $ZC_LIKE -eq 1 ]; then
      ...
      sleep 5
      snap t31-ccpre || fail SNAP_CCPRE
      read CZP CZP_P <<< $(score_zc $D/t31-ccpre.ppm)
      plog "CCPRE vs-zc mean=$CZP p99=$CZP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      read CSP CSP_P <<< $(score_sc $D/t31-ccpre.ppm)
      plog "CCPRE vs-sc mean=$CSP p99=$CSP_P"
      read CPT CPT_P <<< $(score_band $D/t31-ccpre.ppm)
      ...
      # post-hoc local)
      sleep 1; snap t31-sp-post1 || fail SNAP_SP_POST1
      read S1 SP1 <<< $(score_band $D/t31-sp-post1.ppm)
      read SZ1 SZP1 <<< $(score_zc $D/t31-sp-post1.ppm)
      read SS1 SSP1 <<< $(score_sp $D/t31-sp-post1.ppm)
      plog "SP post+1 titleband mean=$S1 p99=$SP1 vs-zc mean=$SZ1 p99=$SZP1 vs-sp mean=$SS1 p99=$SSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      sleep 2; snap t31-sp-post3 || fail SNAP_SP_POST3
      read S3 SP3 <<< $(score_band $D/t31-sp-post3.ppm)
      read SZ3 SZP3 <<< $(score_zc $D/t31-sp-post3.ppm)
      read SS3 SSP3 <<< $(score_sp $D/t31-sp-post3.ppm)
      plog "SP post+3 titleband mean=$S3 p99=$SP3 vs-zc mean=$SZ3 p99=$SZP3 vs-sp mean=$SS3 p99=$SSP3"
      sleep 5; snap t31-sp-post8 || fail SNAP_SP_POST8
      read S8 SP8 <<< $(score_band $D/t31-sp-post8.ppm)
      read SZ8 SZP8 <<< $(score_zc $D/t31-sp-post8.ppm)
      read SS8 SSP8 <<< $(score_sp $D/t31-sp-post8.ppm)
      ...
      SPH_PRE1=$(score_whole $D/t31-ccpre.ppm $D/t31-sp-post1.ppm)
      SPH_13=$(score_whole $D/t31-sp-post1.ppm $D/t31-sp-post3.ppm)
      SPH_38=$(score_whole $D/t31-sp-post3.ppm $D/t31-sp-post8.ppm)
      ...
      if [ $SP_LIKE -eq 1 ]; then
        # T31 press: ONE Cross (x Select) on the parked Select Peak
        # (Peak 1 highlighted per T30 R1); arrival static 131 s, no
        # dwell pressure.
        sleep 5
        snap t31-sppre || fail SNAP_SPPRE
        read SPP SPP_P <<< $(score_sp $D/t31-sppre.ppm)
        plog "SPPRE vs-sp mean=$SPP p99=$SPP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
        read SPZ SPZ_P <<< $(score_zc $D/t31-sppre.ppm)
        plog "SPPRE vs-zc mean=$SPZ p99=$SPZ_P"
        read SPT SPT_P <<< $(score_band $D/t31-sppre.ppm)
        plog "SPPRE vs-title mean=$SPT p99=$SPT_P"
        press K "PEAK_CROSS"
        # arrival series: fast cadence early (transition hops), slow tail
        # (slow next-screen loads), each scored vs title-band, SP-ref and
        # ZC-ref (vs-sc/vs-menu post-hoc local)
        sleep 1; snap t31-pc-post1 || fail SNAP_PC_POST1
        read P1 PP1 <<< $(score_band $D/t31-pc-post1.ppm)
        read PS1 PSP1 <<< $(score_sp $D/t31-pc-post1.ppm)
        read PZ1 PZP1 <<< $(score_zc $D/t31-pc-post1.ppm)
        plog "PC post+1 titleband mean=$P1 p99=$PP1 vs-sp mean=$PS1 p99=$PSP1 vs-zc mean=$PZ1 p99=$PZP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
        sleep 2; snap t31-pc-post3 || fail SNAP_PC_POST3
        read P3 PP3 <<< $(score_band $D/t31-pc-post3.ppm)
        read PS3 PSP3 <<< $(score_sp $D/t31-pc-post3.ppm)
        read PZ3 PZP3 <<< $(score_zc $D/t31-pc-post3.ppm)
        plog "PC post+3 titleband mean=$P3 p99=$PP3 vs-sp mean=$PS3 p99=$PSP3 vs-zc mean=$PZ3 p99=$PZP3"
        sleep 5; snap t31-pc-post8 || fail SNAP_PC_POST8
        read P8 PP8 <<< $(score_band $D/t31-pc-post8.ppm)
        read PS8 PSP8 <<< $(score_sp $D/t31-pc-post8.ppm)
        read PZ8 PZP8 <<< $(score_zc $D/t31-pc-post8.ppm)
        plog "PC post+8 titleband mean=$P8 p99=$PP8 vs-sp mean=$PS8 p99=$PSP8 vs-zc mean=$PZ8 p99=$PZP8"
        sleep 7; snap t31-pc-post15 || fail SNAP_PC_POST15
        read P15 PP15 <<< $(score_band $D/t31-pc-post15.ppm)
        read PS15 PSP15 <<< $(score_sp $D/t31-pc-post15.ppm)
        read PZ15 PZP15 <<< $(score_zc $D/t31-pc-post15.ppm)
        plog "PC post+15 titleband mean=$P15 p99=$PP15 vs-sp mean=$PS15 p99=$PSP15 vs-zc mean=$PZ15 p99=$PZP15"
        sleep 10; snap t31-pc-post25 || fail SNAP_PC_POST25
        read P25 PP25 <<< $(score_band $D/t31-pc-post25.ppm)
        read PS25 PSP25 <<< $(score_sp $D/t31-pc-post25.ppm)
        read PZ25 PZP25 <<< $(score_zc $D/t31-pc-post25.ppm)
        plog "PC post+25 titleband mean=$P25 p99=$PP25 vs-sp mean=$PS25 p99=$PSP25 vs-zc mean=$PZ25 p99=$PZP25"
        sleep 15; snap t31-pc-post40 || fail SNAP_PC_POST40
        read P40 PP40 <<< $(score_band $D/t31-pc-post40.ppm)
        read PS40 PSP40 <<< $(score_sp $D/t31-pc-post40.ppm)
        read PZ40 PZP40 <<< $(score_zc $D/t31-pc-post40.ppm)
        plog "PC post+40 titleband mean=$P40 p99=$PP40 vs-sp mean=$PS40 p99=$PSP40 vs-zc mean=$PZ40 p99=$PZP40"
        # per-hop whole-frame diffs across the Peak transition chain
        PCH_PRE1=$(score_whole $D/t31-sppre.ppm $D/t31-pc-post1.ppm)
        PCH_13=$(score_whole $D/t31-pc-post1.ppm $D/t31-pc-post3.ppm)
        PCH_38=$(score_whole $D/t31-pc-post3.ppm $D/t31-pc-post8.ppm)
        PCH_815=$(score_whole $D/t31-pc-post8.ppm $D/t31-pc-post15.ppm)
        PCH_1525=$(score_whole $D/t31-pc-post15.ppm $D/t31-pc-post25.ppm)
        PCH_2540=$(score_whole $D/t31-pc-post25.ppm $D/t31-pc-post40.ppm)
        plog "PC hops whole: pre-post1=$PCH_PRE1 p1-p3=$PCH_13 p3-p8=$PCH_38 p8-p15=$PCH_815 p15-p25=$PCH_1525 p25-p40=$PCH_2540"
        # arrival stability: 6 x 10 s
        for N in 1 2 3 4 5 6; do
          sleep 10; snap t31-pc-stab$N || fail SNAP_PC_STAB$N
          read M P <<< $(score_band $D/t31-pc-stab$N.ppm)
          read MS MSP <<< $(score_sp $D/t31-pc-stab$N.ppm)
          plog "PC_STAB$N titleband mean=$M p99=$P vs-sp mean=$MS p99=$MSP MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
        done
      fi
    fi
  fi
