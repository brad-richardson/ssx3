#!/usr/bin/env bash
# T40 (sub-second exposure series around the 1 s hold: ONE D-pad Left HOLD
# on the live race + BLIND back-to-back dense series, scored post-hoc):
# reproduce the T39 R1 chain (fresh boot, NVM skips to attract, Cross
...
# THE DELIBERATE CHANGE (the ONLY one vs T39): the dense phase goes BLIND
# — snaps captured back-to-back at ~1 s wall cadence (sleep 1, NO per-snap
# scoring calls, NO in-script per-hop diffs), then scored post-hoc from the
# fetched snaps with t40-cropdiff.py / t40-track.py. T36-T39's ~3 s
# exposure gaps (1 s sleep + ~2 s in-script scoring per snap) railed SCPS
# at +-40 AND +-120 and flooded RDC; sub-second exposures test whether the
# railing stops and a gated RDC pair can straddle the hold. All
# chain/arrival phases keep their in-script gates (they need them); only
# the dense phase goes blind. Each blind snap logs an ns-precision wall
# stamp to the poll log so post-hoc analysis recovers exact exposure gaps.
...
# capture shortens the T39 R1 562 s shape — delta tabled with full dmesg
# coverage).
...
# (T40 reuses the T36 LIVE calibration, T35 R1 receipts: panel->countdown departure hop
...
fail() { echo "T40_FAIL:$1"; stamp; exit 1; }
: > $LOG
plog() { echo "$@" | tee -a $LOG; }
...
bsnap() {
  # $1=snap-name $2=label: BLIND snap for the dense phase (T40 deliberate
...
sleep 2
pgrep -a Xvfb || true
setsid nohup Xvfb :99 -screen 0 1280x1024x24 >/tmp/xvfb-t40.log 2>&1 < /dev/null &
sleep 4
pgrep -a Xvfb || fail NO_XVFB
xdotool getdisplaygeometry || fail NO_DISPLAY
TS=$(date -u +%Y%m%dT%H%M%SZ)
if [ -f $E ]; then
  mv $E $D/dat/PCSX2/logs/emulog-pre-t40-$TS.txt
  ls -la $D/dat/PCSX2/logs/
fi
rm -f $D/pcsx2.pid
T_BOOT=$(date -u +%s)
echo "T_BOOT_WALL:$T_BOOT T_BOOT_UPTIME:$(cut -d. -f1 /proc/uptime)"
nohup $D/pcsx2/build/bin/pcsx2-qt -nogui -slowboot -turbo -datapath $D/dat -logfile $D/logs/boot-t40.log -- "$D/inputs/SSX 3 (USA).iso" > $D/logs/boot-t40.stdout 2>&1 &
echo $! > $D/pcsx2.pid
echo "PID:$!"
sleep 90
...
snap t40-start || fail SNAP_START
read SM SP <<< $(score_band $D/t40-start.ppm)
...
for A in 1 2 3; do
  ...
  snap t40-a$A-now || fail SNAP_A${A}_NOW
  read M P <<< $(score_band $D/t40-a$A-now.ppm)
  plog "ATTEMPT_$A now score mean=$M p99=$P"
  if is_title $M $P; then
    plog "ATTEMPT_$A TITLE-ALREADY-DETECTED (no Cross needed)"
    cp $D/t40-a$A-now.ppm $D/t40-a$A-pre.ppm
    cp $D/t40-a$A-now.jpg $D/t40-a$A-pre.jpg
    ...
    for N in 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 18; do
      sleep 2
      snap t40-a$A-poll$N || fail SNAP_A${A}_POLL$N
      read M P <<< $(score_band $D/t40-a$A-poll$N.ppm)
      plog "ATTEMPT_$A poll$N score mean=$M p99=$P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      if is_title $M $P; then
        plog "ATTEMPT_$A TITLE-DETECTED at poll$N"
        cp $D/t40-a$A-poll$N.ppm $D/t40-a$A-pre.ppm
        cp $D/t40-a$A-poll$N.jpg $D/t40-a$A-pre.jpg
        press Return "A${A}_START"
        DETECTED=1
        break
      fi
  ...
  # post-press series with scores (pre-press snap = t40-a$A-pre.*)
  sleep 3; snap t40-a$A-post3 || fail SNAP_A${A}_POST3
  read M3 P3 <<< $(score_band $D/t40-a$A-post3.ppm)
  plog "ATTEMPT_$A post+3 score mean=$M3 p99=$P3 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  sleep 5; snap t40-a$A-post8 || fail SNAP_A${A}_POST8
  read M8 P8 <<< $(score_band $D/t40-a$A-post8.ppm)
  plog "ATTEMPT_$A post+8 score mean=$M8 p99=$P8"
  sleep 7; snap t40-a$A-post15 || fail SNAP_A${A}_POST15
  read M15 P15 <<< $(score_band $D/t40-a$A-post15.ppm)
  plog "ATTEMPT_$A post+15 score mean=$M15 p99=$P15"
  sleep 10; snap t40-a$A-post25 || fail SNAP_A${A}_POST25
  read M25 P25 <<< $(score_band $D/t40-a$A-post25.ppm)
  plog "ATTEMPT_$A post+25 score mean=$M25 p99=$P25"
  # menu-like gate: +8/+15/+25 all non-title AND whole-static across them
  W815=$(score_whole $D/t40-a$A-post8.ppm $D/t40-a$A-post15.ppm)
  W1525=$(score_whole $D/t40-a$A-post15.ppm $D/t40-a$A-post25.ppm)
  plog "ATTEMPT_$A whole815=$W815 whole1525=$W1525"
  read MM25 MP25 <<< $(score_menu $D/t40-a$A-post25.ppm)
...
if [ $MENU_LIKE -eq 1 ]; then
  ...
  sleep 5
  snap t40-menupre || fail SNAP_MENUPRE
  read MMP MMP_P <<< $(score_menu $D/t40-menupre.ppm)
  plog "MENUPRE vs-menu mean=$MMP p99=$MMP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  read MPT MPT_P <<< $(score_band $D/t40-menupre.ppm)
  ...
  sleep 1; snap t40-mc-post1 || fail SNAP_MC_POST1
  read M1 P1 <<< $(score_band $D/t40-mc-post1.ppm)
  read MM1 MP1 <<< $(score_menu $D/t40-mc-post1.ppm)
  read MS1 MSP1 <<< $(score_sc $D/t40-mc-post1.ppm)
  plog "MC post+1 titleband mean=$M1 p99=$P1 vs-menu mean=$MM1 p99=$MP1 vs-sc mean=$MS1 p99=$MSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  sleep 2; snap t40-mc-post3 || fail SNAP_MC_POST3
  read M3 P3 <<< $(score_band $D/t40-mc-post3.ppm)
  read MM3 MP3 <<< $(score_menu $D/t40-mc-post3.ppm)
  read MS3 MSP3 <<< $(score_sc $D/t40-mc-post3.ppm)
  plog "MC post+3 titleband mean=$M3 p99=$P3 vs-menu mean=$MM3 p99=$MP3 vs-sc mean=$MS3 p99=$MSP3"
  sleep 5; snap t40-mc-post8 || fail SNAP_MC_POST8
  read M8 P8 <<< $(score_band $D/t40-mc-post8.ppm)
  read MM8 MP8 <<< $(score_menu $D/t40-mc-post8.ppm)
  read MS8 MSP8 <<< $(score_sc $D/t40-mc-post8.ppm)
  plog "MC post+8 titleband mean=$M8 p99=$P8 vs-menu mean=$MM8 p99=$MP8 vs-sc mean=$MS8 p99=$MSP8"
  sleep 7; snap t40-mc-post15 || fail SNAP_MC_POST15
  read M15 P15 <<< $(score_band $D/t40-mc-post15.ppm)
  read MM15 MP15 <<< $(score_menu $D/t40-mc-post15.ppm)
  read MS15 MSP15 <<< $(score_sc $D/t40-mc-post15.ppm)
  ...
  H_PRE1=$(score_whole $D/t40-menupre.ppm $D/t40-mc-post1.ppm)
  H_13=$(score_whole $D/t40-mc-post1.ppm $D/t40-mc-post3.ppm)
  H_38=$(score_whole $D/t40-mc-post3.ppm $D/t40-mc-post8.ppm)
  H_815=$(score_whole $D/t40-mc-post8.ppm $D/t40-mc-post15.ppm)
  ...
  if [ $SC_LIKE -eq 1 ]; then
    ...
    # dwell pressure.
    sleep 5
    snap t40-scpre || fail SNAP_SCPRE
    read SCP SCP_P <<< $(score_sc $D/t40-scpre.ppm)
    plog "SCPRE vs-sc mean=$SCP p99=$SCP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
    read SMP SMP_P <<< $(score_menu $D/t40-scpre.ppm)
    plog "SCPRE vs-menu mean=$SMP p99=$SMP_P"
    read SPT SPT_P <<< $(score_band $D/t40-scpre.ppm)
    ...
    sleep 1; snap t40-zc-post1 || fail SNAP_ZC_POST1
    read Z1 ZP1 <<< $(score_band $D/t40-zc-post1.ppm)
    read ZS1 ZSP1 <<< $(score_sc $D/t40-zc-post1.ppm)
    read ZZ1 ZZP1 <<< $(score_zc $D/t40-zc-post1.ppm)
    plog "ZC post+1 titleband mean=$Z1 p99=$ZP1 vs-sc mean=$ZS1 p99=$ZSP1 vs-zc mean=$ZZ1 p99=$ZZP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
    sleep 2; snap t40-zc-post3 || fail SNAP_ZC_POST3
    read Z3 ZP3 <<< $(score_band $D/t40-zc-post3.ppm)
    read ZS3 ZSP3 <<< $(score_sc $D/t40-zc-post3.ppm)
    read ZZ3 ZZP3 <<< $(score_zc $D/t40-zc-post3.ppm)
    plog "ZC post+3 titleband mean=$Z3 p99=$ZP3 vs-sc mean=$ZS3 p99=$ZSP3 vs-zc mean=$ZZ3 p99=$ZZP3"
    sleep 5; snap t40-zc-post8 || fail SNAP_ZC_POST8
    read Z8 ZP8 <<< $(score_band $D/t40-zc-post8.ppm)
    read ZS8 ZSP8 <<< $(score_sc $D/t40-zc-post8.ppm)
    read ZZ8 ZZP8 <<< $(score_zc $D/t40-zc-post8.ppm)
    ...
    ZH_PRE1=$(score_whole $D/t40-scpre.ppm $D/t40-zc-post1.ppm)
    ZH_13=$(score_whole $D/t40-zc-post1.ppm $D/t40-zc-post3.ppm)
    ZH_38=$(score_whole $D/t40-zc-post3.ppm $D/t40-zc-post8.ppm)
    ...
    if [ $ZC_LIKE -eq 1 ]; then
      ...
      sleep 5
      snap t40-ccpre || fail SNAP_CCPRE
      read CZP CZP_P <<< $(score_zc $D/t40-ccpre.ppm)
      plog "CCPRE vs-zc mean=$CZP p99=$CZP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      read CSP CSP_P <<< $(score_sc $D/t40-ccpre.ppm)
      plog "CCPRE vs-sc mean=$CSP p99=$CSP_P"
      read CPT CPT_P <<< $(score_band $D/t40-ccpre.ppm)
      ...
      # post-hoc local)
      sleep 1; snap t40-sp-post1 || fail SNAP_SP_POST1
      read S1 SP1 <<< $(score_band $D/t40-sp-post1.ppm)
      read SZ1 SZP1 <<< $(score_zc $D/t40-sp-post1.ppm)
      read SS1 SSP1 <<< $(score_sp $D/t40-sp-post1.ppm)
      plog "SP post+1 titleband mean=$S1 p99=$SP1 vs-zc mean=$SZ1 p99=$SZP1 vs-sp mean=$SS1 p99=$SSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
      sleep 2; snap t40-sp-post3 || fail SNAP_SP_POST3
      read S3 SP3 <<< $(score_band $D/t40-sp-post3.ppm)
      read SZ3 SZP3 <<< $(score_zc $D/t40-sp-post3.ppm)
      read SS3 SSP3 <<< $(score_sp $D/t40-sp-post3.ppm)
      plog "SP post+3 titleband mean=$S3 p99=$SP3 vs-zc mean=$SZ3 p99=$SZP3 vs-sp mean=$SS3 p99=$SSP3"
      sleep 5; snap t40-sp-post8 || fail SNAP_SP_POST8
      read S8 SP8 <<< $(score_band $D/t40-sp-post8.ppm)
      read SZ8 SZP8 <<< $(score_zc $D/t40-sp-post8.ppm)
      read SS8 SSP8 <<< $(score_sp $D/t40-sp-post8.ppm)
      ...
      SPH_PRE1=$(score_whole $D/t40-ccpre.ppm $D/t40-sp-post1.ppm)
      SPH_13=$(score_whole $D/t40-sp-post1.ppm $D/t40-sp-post3.ppm)
      SPH_38=$(score_whole $D/t40-sp-post3.ppm $D/t40-sp-post8.ppm)
      ...
      if [ $SP_LIKE -eq 1 ]; then
        ...
        # dwell pressure.
        sleep 5
        snap t40-sppre || fail SNAP_SPPRE
        read SPP SPP_P <<< $(score_sp $D/t40-sppre.ppm)
        plog "SPPRE vs-sp mean=$SPP p99=$SPP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
        read SPZ SPZ_P <<< $(score_zc $D/t40-sppre.ppm)
        plog "SPPRE vs-zc mean=$SPZ p99=$SPZ_P"
        read SPT SPT_P <<< $(score_band $D/t40-sppre.ppm)
        ...
        # post-hoc local)
        sleep 1; snap t40-pc-post1 || fail SNAP_PC_POST1
        read P1 PP1 <<< $(score_band $D/t40-pc-post1.ppm)
        read PS1 PSP1 <<< $(score_sp $D/t40-pc-post1.ppm)
        read PX1 PXP1 <<< $(score_sm $D/t40-pc-post1.ppm)
        plog "PC post+1 titleband mean=$P1 p99=$PP1 vs-sp mean=$PS1 p99=$PSP1 vs-sm mean=$PX1 p99=$PXP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
        sleep 2; snap t40-pc-post3 || fail SNAP_PC_POST3
        read P3 PP3 <<< $(score_band $D/t40-pc-post3.ppm)
        read PS3 PSP3 <<< $(score_sp $D/t40-pc-post3.ppm)
        read PX3 PXP3 <<< $(score_sm $D/t40-pc-post3.ppm)
        plog "PC post+3 titleband mean=$P3 p99=$PP3 vs-sp mean=$PS3 p99=$PSP3 vs-sm mean=$PX3 p99=$PXP3"
        sleep 5; snap t40-pc-post8 || fail SNAP_PC_POST8
        read P8 PP8 <<< $(score_band $D/t40-pc-post8.ppm)
        read PS8 PSP8 <<< $(score_sp $D/t40-pc-post8.ppm)
        read PX8 PXP8 <<< $(score_sm $D/t40-pc-post8.ppm)
        ...
        PCH_PRE1=$(score_whole $D/t40-sppre.ppm $D/t40-pc-post1.ppm)
        PCH_13=$(score_whole $D/t40-pc-post1.ppm $D/t40-pc-post3.ppm)
        PCH_38=$(score_whole $D/t40-pc-post3.ppm $D/t40-pc-post8.ppm)
        ...
        if [ $SM_LIKE -eq 1 ]; then
          ...
          # dwell pressure.
          sleep 5
          snap t40-smpre || fail SNAP_SMPRE
          read SXP SXP_P <<< $(score_sm $D/t40-smpre.ppm)
          plog "SMPRE vs-sm mean=$SXP p99=$SXP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
          read SXST SXST_P <<< $(score_setag $D/t40-smpre.ppm)
          plog "SMPRE se-tag mean=$SXST p99=$SXST_P"
          read SXSP SXSP_P <<< $(score_sp $D/t40-smpre.ppm)
          plog "SMPRE vs-sp mean=$SXSP p99=$SXSP_P"
          read SXPT SXPT_P <<< $(score_band $D/t40-smpre.ppm)
          ...
          sleep 1; snap t40-rc-post1 || fail SNAP_RC_POST1
          read R1 RP1 <<< $(score_band $D/t40-rc-post1.ppm)
          read RX1 RXP1 <<< $(score_sm $D/t40-rc-post1.ppm)
          read RT1 RTP1 <<< $(score_setag $D/t40-rc-post1.ppm)
          plog "RC post+1 titleband mean=$R1 p99=$RP1 vs-sm mean=$RX1 p99=$RXP1 se-tag mean=$RT1 p99=$RTP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
          sleep 2; snap t40-rc-post3 || fail SNAP_RC_POST3
          read R3 RP3 <<< $(score_band $D/t40-rc-post3.ppm)
          read RX3 RXP3 <<< $(score_sm $D/t40-rc-post3.ppm)
          read RT3 RTP3 <<< $(score_setag $D/t40-rc-post3.ppm)
          plog "RC post+3 titleband mean=$R3 p99=$RP3 vs-sm mean=$RX3 p99=$RXP3 se-tag mean=$RT3 p99=$RTP3"
          sleep 5; snap t40-rc-post8 || fail SNAP_RC_POST8
          read R8 RP8 <<< $(score_band $D/t40-rc-post8.ppm)
          read RX8 RXP8 <<< $(score_sm $D/t40-rc-post8.ppm)
          read RT8 RTP8 <<< $(score_setag $D/t40-rc-post8.ppm)
          read RS8 RSP8 <<< $(score_sp $D/t40-rc-post8.ppm)
          read RW8 RWP8 <<< $(score_se $D/t40-rc-post8.ppm)
          ...
          RCH_PRE1=$(score_whole $D/t40-smpre.ppm $D/t40-rc-post1.ppm)
          RCH_13=$(score_whole $D/t40-rc-post1.ppm $D/t40-rc-post3.ppm)
          RCH_38=$(score_whole $D/t40-rc-post3.ppm $D/t40-rc-post8.ppm)
          ...
          if [ $SE_LIKE -eq 1 ]; then
            ...
            # no dwell pressure.
            sleep 5
            snap t40-sepre || fail SNAP_SEPRE
            read STP STP_P <<< $(score_setag $D/t40-sepre.ppm)
            plog "SEPRE se-tag mean=$STP p99=$STP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
            read SWP SWP_P <<< $(score_se $D/t40-sepre.ppm)
            plog "SEPRE vs-se mean=$SWP p99=$SWP_P"
            read SXP SXP_P <<< $(score_sm $D/t40-sepre.ppm)
            plog "SEPRE vs-sm mean=$SXP p99=$SXP_P"
            read SXSP SXSP_P <<< $(score_sp $D/t40-sepre.ppm)
            plog "SEPRE vs-sp mean=$SXSP p99=$SXSP_P"
            read SXPT SXPT_P <<< $(score_band $D/t40-sepre.ppm)
            ...
            sleep 1; snap t40-sj-post1 || fail SNAP_SJ_POST1
            read J1 JP1 <<< $(score_band $D/t40-sj-post1.ppm)
            read JW1 JWP1 <<< $(score_se $D/t40-sj-post1.ppm)
            read JR1 JRP1 <<< $(score_mr $D/t40-sj-post1.ppm)
            plog "SJ post+1 titleband mean=$J1 p99=$JP1 vs-se mean=$JW1 p99=$JWP1 vs-mr mean=$JR1 p99=$JRP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
            sleep 2; snap t40-sj-post3 || fail SNAP_SJ_POST3
            read J3 JP3 <<< $(score_band $D/t40-sj-post3.ppm)
            read JW3 JWP3 <<< $(score_se $D/t40-sj-post3.ppm)
            read JR3 JRP3 <<< $(score_mr $D/t40-sj-post3.ppm)
            plog "SJ post+3 titleband mean=$J3 p99=$JP3 vs-se mean=$JW3 p99=$JWP3 vs-mr mean=$JR3 p99=$JRP3"
            sleep 5; snap t40-sj-post8 || fail SNAP_SJ_POST8
            read J8 JP8 <<< $(score_band $D/t40-sj-post8.ppm)
            read JW8 JWP8 <<< $(score_se $D/t40-sj-post8.ppm)
            read JR8 JRP8 <<< $(score_mr $D/t40-sj-post8.ppm)
            ...
            SJH_PRE1=$(score_whole $D/t40-sepre.ppm $D/t40-sj-post1.ppm)
            SJH_13=$(score_whole $D/t40-sj-post1.ppm $D/t40-sj-post3.ppm)
            SJH_38=$(score_whole $D/t40-sj-post3.ppm $D/t40-sj-post8.ppm)
            ...
            if [ $MR_LIKE -eq 1 ]; then
              ...
              sleep 5
              snap t40-mrpre || fail SNAP_MRPRE
              read RMP RMP_P <<< $(score_mr $D/t40-mrpre.ppm)
              plog "MRPRE vs-mr mean=$RMP p99=$RMP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
              read RMS RMS_P <<< $(score_se $D/t40-mrpre.ppm)
              plog "MRPRE vs-se mean=$RMS p99=$RMS_P"
              read RMPT RMPT_P <<< $(score_band $D/t40-mrpre.ppm)
              ...
              sleep 1; snap t40-mr-post1 || fail SNAP_MR_POST1
              read K1 KP1 <<< $(score_band $D/t40-mr-post1.ppm)
              read KR1 KRP1 <<< $(score_mr $D/t40-mr-post1.ppm)
              read KU1 KUP1 <<< $(score_pp $D/t40-mr-post1.ppm)
              plog "MR post+1 titleband mean=$K1 p99=$KP1 vs-mr mean=$KR1 p99=$KRP1 vs-pp mean=$KU1 p99=$KUP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
              sleep 2; snap t40-mr-post3 || fail SNAP_MR_POST3
              read K3 KP3 <<< $(score_band $D/t40-mr-post3.ppm)
              read KR3 KRP3 <<< $(score_mr $D/t40-mr-post3.ppm)
              read KU3 KUP3 <<< $(score_pp $D/t40-mr-post3.ppm)
              plog "MR post+3 titleband mean=$K3 p99=$KP3 vs-mr mean=$KR3 p99=$KRP3 vs-pp mean=$KU3 p99=$KUP3"
              sleep 5; snap t40-mr-post8 || fail SNAP_MR_POST8
              read K8 KP8 <<< $(score_band $D/t40-mr-post8.ppm)
              read KR8 KRP8 <<< $(score_mr $D/t40-mr-post8.ppm)
              read KU8 KUP8 <<< $(score_pp $D/t40-mr-post8.ppm)
              plog "MR post+8 titleband mean=$K8 p99=$KP8 vs-mr mean=$KR8 p99=$KRP8 vs-pp mean=$KU8 p99=$KUP8"
              sleep 7; snap t40-mr-post15 || fail SNAP_MR_POST15
              read K15 KP15 <<< $(score_band $D/t40-mr-post15.ppm)
              read KR15 KRP15 <<< $(score_mr $D/t40-mr-post15.ppm)
              read KU15 KUP15 <<< $(score_pp $D/t40-mr-post15.ppm)
              plog "MR post+15 titleband mean=$K15 p99=$KP15 vs-mr mean=$KR15 p99=$KRP15 vs-pp mean=$KU15 p99=$KUP15"
              sleep 10; snap t40-mr-post25 || fail SNAP_MR_POST25
              read K25 KP25 <<< $(score_band $D/t40-mr-post25.ppm)
              read KR25 KRP25 <<< $(score_mr $D/t40-mr-post25.ppm)
              read KU25 KUP25 <<< $(score_pp $D/t40-mr-post25.ppm)
              plog "MR post+25 titleband mean=$K25 p99=$KP25 vs-mr mean=$KR25 p99=$KRP25 vs-pp mean=$KU25 p99=$KUP25"
              sleep 15; snap t40-mr-post40 || fail SNAP_MR_POST40
              read K40 KP40 <<< $(score_band $D/t40-mr-post40.ppm)
              read KR40 KRP40 <<< $(score_mr $D/t40-mr-post40.ppm)
              read KU40 KUP40 <<< $(score_pp $D/t40-mr-post40.ppm)
              ...
              MRH_PRE1=$(score_whole $D/t40-mrpre.ppm $D/t40-mr-post1.ppm)
              MRH_13=$(score_whole $D/t40-mr-post1.ppm $D/t40-mr-post3.ppm)
              MRH_38=$(score_whole $D/t40-mr-post3.ppm $D/t40-mr-post8.ppm)
              MRH_815=$(score_whole $D/t40-mr-post8.ppm $D/t40-mr-post15.ppm)
              MRH_1525=$(score_whole $D/t40-mr-post15.ppm $D/t40-mr-post25.ppm)
              MRH_2540=$(score_whole $D/t40-mr-post25.ppm $D/t40-mr-post40.ppm)
              ...
              for N in 1 2; do
                sleep 10; snap t40-mr-stab$N || fail SNAP_MR_STAB$N
                read M P <<< $(score_band $D/t40-mr-stab$N.ppm)
                read MT MTP <<< $(score_mr $D/t40-mr-stab$N.ppm)
                read MU MUP <<< $(score_pp $D/t40-mr-stab$N.ppm)
                plog "MR_STAB$N titleband mean=$M p99=$P vs-mr mean=$MT p99=$MTP vs-pp mean=$MU p99=$MUP MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
              done
              MRH_40S1=$(score_whole $D/t40-mr-post40.ppm $D/t40-mr-stab1.ppm)
              MRH_S12=$(score_whole $D/t40-mr-stab1.ppm $D/t40-mr-stab2.ppm)
              plog "MR panel hops whole: post40-stab1=$MRH_40S1 stab1-stab2=$MRH_S12"
              read MS2 MSP2 <<< $(score_mr $D/t40-mr-stab2.ppm)
              ...
              if [ $PP_LIKE -eq 1 ]; then
                ...
                sleep 5
                snap t40-pppre || fail SNAP_PPPRE
                read PUP PUP_P <<< $(score_pp $D/t40-pppre.ppm)
                plog "PPPRE vs-pp mean=$PUP p99=$PUP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
                read PMP PMP_P <<< $(score_mr $D/t40-pppre.ppm)
                plog "PPPRE vs-mr mean=$PMP p99=$PMP_P"
                read PPT PPT_P <<< $(score_band $D/t40-pppre.ppm)
                ...
                sleep 1; snap t40-x-post1 || fail SNAP_X_POST1
                read X1 XP1 <<< $(score_band $D/t40-x-post1.ppm)
                read XU1 XUP1 <<< $(score_pp $D/t40-x-post1.ppm)
                plog "X post+1 titleband mean=$X1 p99=$XP1 vs-pp mean=$XU1 p99=$XUP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
                sleep 2; snap t40-x-post3 || fail SNAP_X_POST3
                read X3 XP3 <<< $(score_band $D/t40-x-post3.ppm)
                read XU3 XUP3 <<< $(score_pp $D/t40-x-post3.ppm)
                plog "X post+3 titleband mean=$X3 p99=$XP3 vs-pp mean=$XU3 p99=$XUP3"
                sleep 5; snap t40-x-post8 || fail SNAP_X_POST8
                read X8 XP8 <<< $(score_band $D/t40-x-post8.ppm)
                read XU8 XUP8 <<< $(score_pp $D/t40-x-post8.ppm)
                plog "X post+8 titleband mean=$X8 p99=$XP8 vs-pp mean=$XU8 p99=$XUP8"
                sleep 7; snap t40-x-post15 || fail SNAP_X_POST15
                read X15 XP15 <<< $(score_band $D/t40-x-post15.ppm)
                read XU15 XUP15 <<< $(score_pp $D/t40-x-post15.ppm)
                plog "X post+15 titleband mean=$X15 p99=$XP15 vs-pp mean=$XU15 p99=$XUP15"
                sleep 10; snap t40-x-post25 || fail SNAP_X_POST25
                read X25 XP25 <<< $(score_band $D/t40-x-post25.ppm)
                read XU25 XUP25 <<< $(score_pp $D/t40-x-post25.ppm)
                plog "X post+25 titleband mean=$X25 p99=$XP25 vs-pp mean=$XU25 p99=$XUP25"
                sleep 15; snap t40-x-post40 || fail SNAP_X_POST40
                read X40 XP40 <<< $(score_band $D/t40-x-post40.ppm)
                read XU40 XUP40 <<< $(score_pp $D/t40-x-post40.ppm)
                ...
                XH_PRE1=$(score_whole $D/t40-pppre.ppm $D/t40-x-post1.ppm)
                XH_13=$(score_whole $D/t40-x-post1.ppm $D/t40-x-post3.ppm)
                XH_38=$(score_whole $D/t40-x-post3.ppm $D/t40-x-post8.ppm)
                XH_815=$(score_whole $D/t40-x-post8.ppm $D/t40-x-post15.ppm)
                XH_1525=$(score_whole $D/t40-x-post15.ppm $D/t40-x-post25.ppm)
                XH_2540=$(score_whole $D/t40-x-post25.ppm $D/t40-x-post40.ppm)
                ...
                if [ $LIVE_LIKE -eq 1 ]; then
                  # T40 hold: ONE 1 s D-pad Left HOLD (T38's input class) on
                  # live gameplay, with a PRE-hold pair (no input between) +
                  # DENSE post-hold series (10 snaps back-to-back). THE
                  # DELIBERATE CHANGE vs T39: the whole dense phase goes
                  # BLIND — bsnap captures with ns wall stamps and NO
                  # scoring calls (no score_band/score_pp/score_whole
                  # in-script anywhere below); HUD + per-hop whole diffs +
                  # rider tracking are all scored post-hoc from the fetched
                  # snaps with t40-cropdiff.py / t40-track.py.
                  sleep 5
                  bsnap t40-npre1 NPRE1
                  sleep 1
                  bsnap t40-npre2 NPRE2
                  press_hold Left "HOLD_LEFT"
                  # dense post-hold series: 10 snaps back-to-back at ~1 s
                  # wall cadence (sleep 1, NO per-snap scoring)
                  sleep 1; bsnap t40-d-post1 D_POST1
                  sleep 1; bsnap t40-d-post2 D_POST2
                  sleep 1; bsnap t40-d-post3 D_POST3
                  sleep 1; bsnap t40-d-post4 D_POST4
                  sleep 1; bsnap t40-d-post5 D_POST5
                  sleep 1; bsnap t40-d-post6 D_POST6
                  sleep 1; bsnap t40-d-post7 D_POST7
                  sleep 1; bsnap t40-d-post8 D_POST8
                  sleep 1; bsnap t40-d-post9 D_POST9
                  sleep 1; bsnap t40-d-post10 D_POST10
