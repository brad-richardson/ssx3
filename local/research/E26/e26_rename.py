"""Hex-safe mechanical rename into E26 + proof, per errata E23-E1.
...
E26 DIFFERENCE, declared: E26 draws from TWO source lanes, not one. The carried
...
normalized-diff proof maps THAT lane's token and `e26` to one placeholder.

Usage: e26_rename.py E25:e25_io.py:e26_io.py E24:e24_capture.py:e26_capture.py ...
"""
import json, re, sys
...
PROTECTED = ['PS2X_E21_PARSER_DIR', 'PS2X_E21_PROOF', '# E21 PARSER CLOSURE',
             ...
             # E26 ADDS: E25's snapshot path, the durable restore E26's
...
def rename_text(text, src_lane):
    ...
    out = text.replace(lo, 'e26').replace(up, 'E26')
...
def normalize(text, src_lane):
    ...
    return text.replace(lo, 'EXX').replace('e26', 'EXX') \
               .replace(up, 'EXX').replace('E26', 'EXX')
...
def main():
    ...
    (E / 'rename-proof-hexsafe.json').write_text(json.dumps(dict(
        tool='e26_rename.py', source_lanes=sorted({r['source_lane'] for r in rows}),
    ...
    print('# E26 RENAME TAIL COMPLETE')
