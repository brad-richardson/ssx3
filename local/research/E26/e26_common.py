...
# 2152e5ad..., both byte-equal to the lost pins). E26 BUILDS NOTHING: it spends
# the boot on the binary already standing at this exact path, which is the very
# binary E23 booted. The other two trees stay absent and are NOT rebuilt --
# E25 proved them pre-E18 output, irreproducible at this fork.
B0 = Path('/tmp/e18-mpeg-link/runtime')
PROTECTED_BUILDS = [Path('/tmp/p1-link/runtime'), Path('/tmp/e17-map-link/runtime'), B0]
NEWB = B0
B = Path(os.environ.get('E26_BUILD_DIR', str(B0)))
OUT = Path('/tmp/e26-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
BASE_SHA = '3adc0478b6d2260acdd28a249466f2eef9a20176'
# E26 CONTRACT: internal reservation 3 GiB, the brief's declared value (E25's,
# for a lane that may have to build). E26's Mission 0 restore path (a) found the
# tree intact, so no build runs -- the reservation is declared, not consumed.
# FLOOR and GUARD stay 2 GiB + 0.5 GiB, untouched.
IRES = 3*G
...
def sample():
    paths = list(P.glob('e26-*')) + list(P.glob('._e26-*')) + list((P/'run').glob('*e26*'))
    # The shared function trace is charged even though its name lacks E26.
