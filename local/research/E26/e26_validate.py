...
def observer(env,dest):
    library=os.environ.get('E26_OBSERVER_DYLIB')
    if library:
        directory=dest/'parser-observer';directory.mkdir(exist_ok=False)
        env.update(DYLD_INSERT_LIBRARIES=library,PS2X_E21_PARSER_DIR=str(directory))
    return env
...
def suite(label):
    # E26 CHANGE 1: the suite binary lives in a PROTECTED build tree that the
    ...
    env={k:v for k,v in os.environ.items() if not k.startswith(('PS2X_','DYLD_'))};env['TMPDIR']=str(scratch)+'/'
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e26-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E26 requires E18's delivered behavior at baseline
        ...
        if '# E7 SHUTDOWN' in taps:
            from e26_closed_events import tap
    ...
    print('# E26 VALIDATION TAIL COMPLETE')
    return result
