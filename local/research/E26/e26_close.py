"""E26 close: refit E24's cost model with its third point, then the final audit."""
import subprocess
from e26_common import *

RUN = P / 'run'
result = json.loads((E / 'e26a-result.json').read_text())
...
pts.append(dict(boot='e26a', watches=result['watch_entries'],
...
save('cost-model-refit.json', dict(utc=utc(),
     ...
     slope_e23a_to_e26a=round(k23, 5),
     ...
     consequence=('The residual E24 declared -- an isolated write shorter than 121 B inside the '
                  ...
                  'that wants the residual closed can afford it. E26 does not re-spend a boot to '
...
boot_dirs = sorted(str(q) for q in RUN.glob('*e26a*'))
s = sample()
checks = dict(fork_triple_agree=fork_ok, binaries_unmoved=bins_ok,
              ...
              boots_spent_is_one=len(list(E.glob('e26?-result.json'))) == 1,
