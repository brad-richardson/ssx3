#!/usr/bin/env python3
"""E26 demand-watch capture driver. Fresh T13 pre-claims, caps, owned lease cleanup. One boot."""
import argparse
import datetime
import hashlib
import json
...
sys.path.insert(0, str(REPO / 'local/research/E26'))
from e26_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e26_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E26'
...
# E26 step 4: the guest producer/source structures the E7 tap named on this exact
...
# E26 CHANGE 2: the TIERED extended watch set (BOOT-DESIGN.md, watch-set.json).
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E26 forbids lease wait')
        raise RuntimeError('Lease occupied; E26 tables and stops without waiting')
    ...
    manifest_path=EVIDENCE/'e26a-build.json'
    assert manifest_path.exists(), 'fresh E26 probe-gate manifest required'
    ...
    # E26 CHANGE 1: PRESENCE first. e23a's driver went straight to .stat() and
    # would raise FileNotFoundError deep inside the loop; E26 opened to exactly
    ...
    if absent:
        raise RuntimeError('required binaries absent; E26 tables and stops '
...
def main():
    ...
    assert not (EVIDENCE/'boot-attempt.json').exists(), 'E26 permits only one boot'
    assert not list(EVIDENCE.glob('e26?-result.json')), 'E26 permits only one boot'
    label='e26'+args.phase
    assert args.report_all, 'E26 requires REPORT_ALL=1'
    ...
    for key in inherited:
        del env[key]
    # E26 CHANGE 3: PS2X_DIAG_SEMA / _S0 are already compiled into the proven
    ...
    assert not any('stdbuf' in x for x in boot_argv), 'E26 forbids stdbuf in the boot argv'
    ...
    try:
        with (EVIDENCE/f'{label}-liveness.txt').open('w') as live, paths['boot_log'].open('wb') as log:
            ...
            # E26 CHANGE 4: record span-complete on EVERY path, not only when a
