#!/usr/bin/env bash
# T33 trace analysis on the X-Continue-Cross emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t38-census.txt
python3 $D/t4-sample.py $E $D/t38-samples.txt
ls -la $D/t38-census.txt $D/t38-samples.txt
date -u
