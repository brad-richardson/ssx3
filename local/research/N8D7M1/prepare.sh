#!/usr/bin/env bash
set -euo pipefail
root=/home/brad/n8d7m1
base=/home/brad/n8d7h
test ! -e "$root"
...
echo "== diff PS2Recomp vs N8D7H (expect empty) =="
...
echo "== diff parallel-gs vs N8D7H (expect empty) =="
diff -rq --exclude=.cxx --exclude=build --exclude=.gradle \
  "$base/parallel-gs" "$root/parallel-gs" \
...
echo "== diff jniLibs vs N8D7H (expect empty) =="
