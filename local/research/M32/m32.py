#!/usr/bin/env python3
"""M32 upper-K confirmation: windowed census at K in {233,234,366,367} (offline).

Usage: m32.py M16_DIR M15_DIR M28_DIR M31_DIR WORK_DIR EVID_DIR

Reads M16 per-shape triplets + loo.txt, the M15 triplet (baseline
guard only), m28.txt + m28-census.tsv (exact-match guard), and
m31.txt + m31-census-K1.tsv (K=1 reproduction target) +
m31-census-K100.tsv (K=100 diff baseline); raw XFB dumps,
573440 B = 640x448 YUYV. Read-only inputs; receipt text goes to
stdout (redirect to WORK_DIR/m32.txt); windowed census TSVs + PNG
cell map to WORK_DIR (evidence copies iff the DESIGN.md rules meet,
decided at report time).

Tasks (see DESIGN.md, recorded before running):
  1: K=1 gate + windowed re-run per sweep K + off-span + full ordered diffs
  2: per-site admission + standing response + extras re-enumeration + joins
...
KS_SWEEP = [233, 234, 366, 367]  # FIXED K list (DESIGN.md, no additions)
KS_RUN = [1, 233, 234, 366, 367]  # sweep Ks + K=1 gate recompute
KS_DIFF = [100, 233, 234, 366, 367]  # full ordered diff list (100 via M31 TSV)
M28_MOVERS = [9, 22, 708, 709, 710, 711, 731, 732, 733, 734]
M31_K1_MOVERS = [708, 709, 710, 711, 731, 732, 733, 734]
WANT_K1 = {  # (shape, col) -> (nK, ovK, standing, dest, off)
...
def main():
    m16d, m15d, m28d, m31d, workd, evidd = (Path(a) for a in sys.argv[1:7])
    workd.mkdir(parents=True, exist_ok=True)
    evidd.mkdir(parents=True, exist_ok=True)
    t_start = time.time()
    canon = []  # determinism canonical text (Task 1 K=233 on s0+9+22)
    ...
    for p in (m28d / "m28.txt", m28d / "m28-census.tsv",
              m31d / "m31.txt", m31d / "m31-census-K1.tsv",
              m31d / "m31-census-K100.tsv"):
        b = p.read_bytes()
        print(f"input {p}: bytes={len(b)} "
              f"sha256={hashlib.sha256(b).hexdigest()}")
    ...
    tsv100 = parse_m28_tsv(m31d / "m31-census-K100.tsv")
    print(f"m31 K100 TSV parsed shapes: n={len(tsv100)} (want 764)")
    ...
    if r0 != R0_M16 or r15 != R0_M15 or f0 != FNV_S0 or f15 != FNV_M15:
        print("MISMATCH vs M17-M31 baselines: STOP, tabled.")
        return
    if not carriers_ok:
    ...
    # ---- window geometry per computed K (mismatch EXPECTED at K=1 c340/c342) ----
    ...
    for K in KS_RUN:
        ...
        (workd / f"m32-census-K{K}.tsv").write_text("\n".join(tsvw) + "\n")
        print(f"K={K}: windowed TSV: rows={len(tsvw) - 1} "
              f"bytes={(workd / f'm32-census-K{K}.tsv').stat().st_size} "
              f"sha256={hashlib.sha256((workd / f'm32-census-K{K}.tsv').read_bytes()).hexdigest()}")
    ...
    # ---- K=1 gate vs M31 (movers/cells/standings + TSV bytes or stop) ----
    print("== K=1 gate vs M31 (or table the mismatch and stop) ==")
    k1_ok = True
    if wres[1]["movers"] != M31_K1_MOVERS:
        k1_ok = False
    print(f"K=1 movers: n={len(wres[1]['movers'])} shapes={wres[1]['movers']} "
          f"want={M31_K1_MOVERS} match={wres[1]['movers'] == M31_K1_MOVERS}")
    k1cells = {(s, c): (wres[1]["wcen"][s][c]["nK"],
                        wres[1]["wcen"][s][c]["ovK"], st, d, o)
               for (s, c, st, d, o, _, _) in wres[1]["moves"]}
    print(f"K=1 moved cells: n={len(k1cells)} (want 13)")
    if len(k1cells) != 13 or set(k1cells) != set(WANT_K1):
        k1_ok = False
        print(f"K=1 cell-set mismatch: extra={sorted(set(k1cells) - set(WANT_K1))} "
              f"missing={sorted(set(WANT_K1) - set(k1cells))}")
    for (s, c), (wn, wo, wst, wd, woff) in sorted(WANT_K1.items()):
        got = k1cells.get((s, c), "(ABSENT)")
        good = got == (wn, wo, wst, wd, woff)
        if not good:
            k1_ok = False
        print(f"K=1 cell s{s} c{c}: got={got} want={(wn, wo, wst, wd, woff)} "
              f"match={good}")
    k1sha = hashlib.sha256((workd / "m32-census-K1.tsv").read_bytes()).hexdigest()
    m31k1sha = hashlib.sha256((m31d / "m31-census-K1.tsv").read_bytes()).hexdigest()
    print(f"K=1 TSV sha: recomputed={k1sha}")
    print(f"K=1 TSV sha: M31-target ={m31k1sha} identical={k1sha == m31k1sha}")
    if k1sha != m31k1sha:
        k1_ok = False
    print("K=1 gate: " + ("OK" if k1_ok else
                          "MISMATCH vs M31: STOP, tabled."))
    if not k1_ok:
        return

    # ---- M31 K100 TSV cross-check (diff baseline) ----
    print("== M31 K100 TSV cross-check (sha vs K=1 recompute) ==")
    tj_bad100 = sum(1 for s in range(764)
                    if tsv100[s]["tail"] != per[s]["tail"]
                    or tsv100[s]["j"] != f"{per[s]['j']:.4f}")
    print(f"K100 TSV tail/J vs recompute: bad={tj_bad100} (want 0)")
    m31k100sha = hashlib.sha256((m31d / "m31-census-K100.tsv").read_bytes()).hexdigest()
    print(f"M31 K100 TSV sha == K1 recompute sha: {m31k100sha == k1sha}")
    k100pres_bad = sum(1 for s in range(764) for c in NAMED8
                       if tsv100[s]["cen"][c]["pres"]
                       != wres[1]["wcen"][s][c]["presK"])
    print(f"K100 TSV pres vs K=1 recompute presK: bad-cells={k100pres_bad}")
    ...
    # ---- Task 2.1: per-site admission + standing response + extras re-enum ----
    print("== Task 2.1: per-site admission (far extras x computed K) ==")
    for (s, c, r) in FAR_EXTRAS:
        for K in KS_RUN:
            isin = r in wres[K]["wcen"][s][c]["win"]
            print(f"far-extra s{s} c{c} r{r} (d{span_dist(r, c):+d}): "
                  f"K={K} inwindow={isin} "
                  f"offspan={(c, r) in wres[K]['off'][s]}")
    print("== Task 2.1: standing response (s9-c321 / s22-c296 per K) ==")
    for (s, c) in ((9, 321), (22, 296)):
        for K in KS_RUN:
            wc = wres[K]["wcen"][s][c]
            st = ("present" if wc["presK"] == 1
                  else standing_of_K(c, wc, per[s]["pcols"]))
            print(f"K={K} s{s}-c{c}: nK={wc['nK']}/ovK={wc['ovK']} "
    ...
    # ---- Task 2.2: bracket legs recap + min-K table ----
    print("== Task 2.2: bracket legs recap (first flip past K=100) ==")
    first_flip = None
    for (ra, rb) in zip(KS_DIFF, KS_DIFF[1:]):
        nflip = sum(1 for s in range(764) for c in NAMED8
                    if pres_of(ra, s, c) != pres_of(rb, s, c))
        ma, mb = movers_of(ra), movers_of(rb)
        print(f"bracket leg {ra}->{rb}: flipping cells={nflip} "
              f"mover-flips={len(ma ^ mb)}")
        if nflip and first_flip is None:
            first_flip = (ra, rb)
    print(f"first real-data flip past K=100: {first_flip}")
    ...
    # ---- determinism re-run (Task 1 K=233 windowed on s0+9+22) ----
    print("== determinism re-run (Task 1 K=233 on s0+9+22, second pass) ==")
    ...
    def canon_lines(t0m):
        ...
        for s in (0, 9, 22):
            ...
            wc = wcensus_of_shape(colrows, 233)
    ...
    print(f"sets identical: wmoved233-s0={per[0]['wmoved233']} "
          f"wmoved233-s9={per[9]['wmoved233']} "
          f"wmoved233-s22={per[22]['wmoved233']}")

    # ---- PNG K-sweep cell map ----
    print("== PNG K-sweep cell map ==")
    rules = [100, 233, 234, 366, 367]
    ...
    p = workd / "m32-cellmap.png"
    Image.fromarray(arr).resize((80, 100), Image.NEAREST).save(p)
    print(f"png {p}: {p.stat().st_size} B (budget 5242880)")
    print("legend: x=M28-mover x named-col (80 cells), "
          "rows=100,233,234,366,367; green=present magenta=moved")

    # ---- falsification-bar measurements (values + thresholds) ----
    print("== falsification-bar measurements ==")
    print(f"B1_K1REPRO: gate={'met' if k1_ok else 'STOPPED'} "
          f"(8 movers / 13 cells / standings + TSV bytes)")
    off233 = set()
    for s in range(764):
        off233 |= {(s, c, r) for (c, r) in wres[233]["off"][s]}
    want233 = {(9, 321, 266), (22, 296, 400), (22, 296, 401)}
    print(f"B2_OFF233: K=233 off-span {len(off233)} sites == far-3: "
          f"{off233 == want233} (bar True)")
    off234 = set()
    for s in range(764):
        off234 |= {(s, c, r) for (c, r) in wres[234]["off"][s]}
    want234 = {(22, 296, 400), (22, 296, 401)}
    print(f"B3_ADMIT234: K=234 off-span {len(off234)} sites == r400+r401: "
          f"{off234 == want234} (bar True)")
    off366 = set()
    for s in range(764):
        off366 |= {(s, c, r) for (c, r) in wres[366]["off"][s]}
    want366 = {(22, 296, 401)}
    print(f"B4_ADMIT366: K=366 off-span {len(off366)} sites == r401-only: "
          f"{off366 == want366} (bar True)")
    off367 = set()
    for s in range(764):
        off367 |= {(s, c, r) for (c, r) in wres[367]["off"][s]}
    print(f"B5_ALLIN367: K=367 off-span {len(off367)} sites == empty: "
          f"{off367 == set()} (bar True); sites={sorted(off367)[:8]}")
    s9rows = []
    for K in KS_RUN:
        wc = wres[K]["wcen"][9][321]
        st = ("present" if wc["presK"] == 1
              else standing_of_K(321, wc, per[9]["pcols"]))
        s9rows.append((K, wc["nK"], wc["ovK"], st))
    s9want = [(1, 10, 10, "present"), (233, 10, 10, "present"),
              (234, 11, 10, "none(extra-only)"),
              (366, 11, 10, "none(extra-only)"),
              (367, 11, 10, "none(extra-only)")]
    print(f"B6_S9STAND: rows={s9rows} want={s9want} match={s9rows == s9want} "
          f"(bar True)")
    s22rows = []
    for K in KS_RUN:
        wc = wres[K]["wcen"][22][296]
        st = ("present" if wc["presK"] == 1
              else standing_of_K(296, wc, per[22]["pcols"]))
        s22rows.append((K, wc["nK"], wc["ovK"], st))
    s22want = [(1, 9, 9, "present"), (233, 9, 9, "present"),
               (234, 9, 9, "present"),
               (366, 10, 9, "none(extra-only)"),
               (367, 11, 9, "none(extra-only)")]
    print(f"B7_S22STAND: rows={s22rows} want={s22want} match={s22rows == s22want} "
          f"(bar True)")
    for (ra, rb) in zip(KS_DIFF, KS_DIFF[1:]):
        flips = [(s, c) for s in range(764) for c in NAMED8
                 if pres_of(ra, s, c) != pres_of(rb, s, c)]
        ma, mb = movers_of(ra), movers_of(rb)
        print(f"B8_KDIFFS {ra}->{rb}: flipping-cells={len(flips)} {flips[:6]} "
              f"mover-flips={len(ma ^ mb)} "
              f"only-{ra}={sorted(ma - mb)} only-{rb}={sorted(mb - ma)} "
              f"(bar 100->233:0/0 233->234:1/1 234->366:1/1 366->367:0/0)")
    for K in KS_RUN:
        wm = wres[K]["movers"]
        nb = sum(1 for s in wm if per[s]["j"] < 0.95)
        print(f"B9_BELOW95 K={K}: windowed movers below-0.95 {nb}/{len(wm)} "
              f"({nb / len(wm) if wm else 0:.4f}; bar >=0.50)")
    for K in KS_RUN:
        nmi = sum(1 for (s, c, r) in MISS_ROWS if span_dist(r, c) <= K)
        print(f"B10_MISSIN K={K}: missing rows in-window {nmi}/4 (bar 4/4)")
    print("N: 0 explained; tail stands (attribution, not explanation)")

    print(f"== total wall: {time.time() - t_start:.1f}s ==")
    print("tests: T1=bracket+offspan+diffs T2=admission+joins R=controls")
