#!/usr/bin/env python3
"""E57 bounded boot: I26-FAST to the race, deterministic, empty cards.
...
Usage: e57_boot.py --mode hash|speed|profile --runner PATH --label NAME [--stop-tick N]
"""
import argparse
import hashlib
import json
...
WORK = Path('/Users/brad/dev/ssx3-work/NP1/macgate')
...
def main():
    ...
    ap.add_argument('--queue', action='store_true',
                    help='NP1: PS2X_GS_QUEUE=1 (CPU backend on the worker; exercises enqueue/batch)')
    args = ap.parse_args()
    ...
    slot = claim('NP1-' + args.label, exclusive=exclusive)
    while slot is None:
