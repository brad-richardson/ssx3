#!/usr/bin/env python3
"""NP1 Odin race call-graph profile on the F1 APK (RV3 rank 2, Part 1).

N11/F1 launcher with the profile step switched to `simpleperf record
--call-graph fp` (brief: fp first, dwarf if fp unwinds are shallow).
OUT text logs stay in git, binaries (PNG/perf.data) go to scratch;
mc0-test empty card (Brad's mc0 never touched); full NP1 env with
PS2X_SOUND=1 written per run, Brad's env (SHA 9fb46f85...) restored after;
gpubusy sampler every 5 s; gated atexit (force-stop/restore/release
only what this run claimed/pushed/launched).

Usage: launch.py --label P1 [--wall 600] --profile-after-tick 2400 --profile-secs 30
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/NP1/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/NP1/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('NP1 ')):
...
sh(f'echo "NP1 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE NP1 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# NP1 {a.label}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
sh("echo 'LEASE_FREE NP1 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
