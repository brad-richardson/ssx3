#!/usr/bin/env python3
"""E18 progression capture driver. Explicit preflight, caps, owned lease cleanup."""
import argparse
import datetime
import hashlib
import json
...
from e18_frame import read_png, fnv32
from e18_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e18_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E18'
...
CAPS = dict(wall_s=90, terminate_reserve_s=15, progress_lines=1000000,
            observation_tick_stop=540, source_window_end=603,
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E18 forbids lease wait')
        raise RuntimeError('Lease occupied; E18 tables and stops without waiting')
    ...
    assert manifest_path.exists(), 'fresh E18 build manifest required'
...
def main():
    ...
    assert not (EVIDENCE/'boot-attempt.json').exists(), 'E18 permits only one boot'
    assert not list(EVIDENCE.glob('e18?-result.json')), 'E18 permits only one boot'
    label='e18'+args.phase
    assert args.report_all, 'E18 requires REPORT_ALL=1'
    ...
    try:
        with (EVIDENCE/f'{label}-liveness.txt').open('w') as live, paths['boot_log'].open('wb') as log:
            ...
            while p.poll() is None:
                ...
                if bound is None and observation_tick is not None and observation_tick>=CAPS['observation_tick_stop']: bound='observation_window_progress'
