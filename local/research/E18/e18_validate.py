...
def suite(label):
    ...
    count=452 if label=='baseline' else 458
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e18-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if '# E7 SHUTDOWN' in taps:
            from e18_closed_events import tap
    ...
    print('# E18 VALIDATION TAIL COMPLETE')
    return result
