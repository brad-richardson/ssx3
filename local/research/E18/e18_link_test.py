...
label=sys.argv[1];dest=P/'e18-fixtures'/label;dest.mkdir(parents=True,exist_ok=False)
source_record=[pin(E/name) for name in ['e18_binding_test.cpp','e17_binding_test.cpp','e16_binding_test.cpp','e15_binding_test.cpp','prior_binding_test.cpp']]
...
for a in args:
    ...
    out.append(a)
out+=['-c',str(E/'e18_binding_test.cpp'),'-o',str(dest/'test.o')]
...
link+=[str(dest/'test.o'),'-Wl,-e,_e18_binding_main','-Wl,-export_dynamic']
...
print(dest/'binding-test');print('# E18 ACTUAL OBJECT LINK TAIL COMPLETE')
