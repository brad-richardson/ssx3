...
R = W / 'PS2Recomp'
B0 = Path('/tmp/e17-map-link/runtime')
NEWB = Path('/tmp/e18-mpeg-link/runtime')
B = Path(os.environ.get('E18_BUILD_DIR',str(NEWB)))
OUT = Path('/tmp/e18-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
BASE_SHA = 'e63f1616320d12c3f213899af6fba03fee3eafb2'
...
def sample():
    paths = list(P.glob('e18-*')) + list(P.glob('._e18-*')) + list((P/'run').glob('*e18*'))
    # The shared function trace is charged even though its name lacks E18.
