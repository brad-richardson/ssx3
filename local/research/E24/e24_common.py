...
R = W / 'PS2Recomp'
# The three PROTECTED build trees E15-E23 all ran on. They live under /tmp on
# the internal volume. E24 opened after a host restart that wiped /tmp; their
# absence is the lane's red gate and is receipted in `env-audit.json`. The
# constants stay declared so every downstream check names the exact path it
# wanted rather than silently substituting another build.
B0 = Path('/tmp/e18-mpeg-link/runtime')
PROTECTED_BUILDS = [Path('/tmp/p1-link/runtime'), Path('/tmp/e17-map-link/runtime'), B0]
NEWB = Path('/tmp/e24-mpeg-link/runtime')
B = Path(os.environ.get('E24_BUILD_DIR', str(B0)))
OUT = Path('/tmp/e24-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
BASE_SHA = '3adc0478b6d2260acdd28a249466f2eef9a20176'
# E23 CONTRACT AMENDMENT A1 carried: internal reservation 512 MiB, FLOOR and
# GUARD unchanged at 2 GiB + 0.5 GiB. E24 runs no build step at all, so the
# reservation that A1 shrank is not re-expanded here.
IRES = 512*M
...
def sample():
    paths = list(P.glob('e24-*')) + list(P.glob('._e24-*')) + list((P/'run').glob('*e24*'))
    # The shared function trace is charged even though its name lacks E24.
