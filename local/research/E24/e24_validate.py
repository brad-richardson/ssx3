...
def observer(env,dest):
    library=os.environ.get('E24_OBSERVER_DYLIB')
    if library:
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e24-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E24 requires E18's delivered behavior at baseline
        ...
        if '# E7 SHUTDOWN' in taps:
            from e24_closed_events import tap
    ...
    print('# E24 VALIDATION TAIL COMPLETE')
    return result
