...
BATCH=32
REPO=/Users/brad/dev/ssx3  # VR3 copy of VR2's copy (private export, VU0 images added)
...
VU1_DIR=${VR2_VU1_DIR:-$HOME/dev/ssx3-work/vu1gen-ssx3}
RVU1DIR=${VR2_RVU1DIR:-vu1gen}
# VR3: generated VU0 images (PS2X_VU0_RECOMP_DIR); VR3_VU0_DIR="" builds without.
VU0_DIR=${VR3_VU0_DIR-$HOME/dev/ssx3-work/vu0gen-vr3}
RVU0DIR=${VR3_RVU0DIR:-vu0gen-vr3}
...
# VR2 copy: a private export (PS2Recomp-vr3, git archive of the SHA, no .git),
# so another lane's checkout in the shared PS2Recomp cannot change sources mid-build.
ssh "$REMOTE" "cd ~/$RROOT/PS2Recomp && git fetch origin ssx3 2>&1 | tail -1; rm -rf ../PS2Recomp-vr3 && mkdir ../PS2Recomp-vr3 && git archive $SHA_IN | tar -C ../PS2Recomp-vr3 -xf -" || exit 1
...
if [ -n "$VU0_DIR" ]; then
  ...
  RVU0=$(ssh "$REMOTE" "(cd ~/$RROOT/$RVU0DIR 2>/dev/null && sha256sum vu0_*.cpp | sha256sum | cut -d' ' -f1)" || true)
  if [ "$RVU0" != "$VU0_MINI" ]; then
    echo "-- syncing vu0gen from mini (vu0_*.cpp only)"
    (cd "$VU0_DIR" && COPYFILE_DISABLE=1 tar -cf - vu0_*.cpp) | ssh "$REMOTE" "rm -rf ~/$RROOT/$RVU0DIR && mkdir -p ~/$RROOT/$RVU0DIR && tar -C ~/$RROOT/$RVU0DIR -xf -"
    RVU0=$(ssh "$REMOTE" "(cd ~/$RROOT/$RVU0DIR && sha256sum vu0_*.cpp | sha256sum | cut -d' ' -f1)")
    [ "$RVU0" = "$VU0_MINI" ] || { echo "vu0gen re-verify failed" >&2; exit 2; }
  else echo "-- vu0gen OK"; fi
  VU0_FLAG="-DPS2X_VU0_RECOMP_DIR=/work/$RVU0DIR"
fi
...
START=$SECONDS
ssh "$REMOTE" "docker run --rm --user 1000:1000 -e HOME=/work -e CCACHE_DIR=/ccache -e CCACHE_BASEDIR=/work -w /work -v ~/$RROOT:/work -v ~/$RCCACHE:/ccache $IMAGE cmake -S /work/PS2Recomp-vr3 -B /work/$NAME -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_C_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER_LAUNCHER=ccache -DPS2X_GAME_CODEGEN_DIR=/work/codegen -DPS2X_VU1_RECOMP_DIR=/work/$RVU1DIR $VU0_FLAG -DPS2X_BUILD_TEST=ON -DPS2X_BUILD_STUDIO=OFF -DPS2X_ENABLE_DEBUG_UI=OFF -DPS2X_ENABLE_RUNTIME_LOGS=OFF -DPS2X_ENABLE_AGRESSIVE_LOGS=OFF -DPS2X_ENABLE_DIAG_TAPS=OFF -DPS2X_ENABLE_DET_HASH_TAP=$DET -DPS2X_GS_SHADOW_PARALLEL=ON -DPS2X_PARALLEL_GS_SOURCE_DIR=/work/parallel-gs -DPS2X_ENABLE_SCCACHE=OFF -DPS2X_RUNNER_UNITY_BUILD_BATCH_SIZE=$BATCH -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_C_FLAGS=-msse4.1 -DCMAKE_CXX_FLAGS=-msse4.1 > ~/$RROOT/$NAME-configure.log 2>&1; echo CONFIGURE_RC=\$?; grep -E -m2 'VU[01] recomp' ~/$RROOT/$NAME-configure.log" || exit 1
