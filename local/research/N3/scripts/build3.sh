#!/bin/bash
# N3 build3: arm64-only rebuild (shim + bypass + pad) + bars, foreground
N2=~/n2; LOG=$N2/logs
...
echo "BUILD3_START $(date -u +%FT%TZ) branch=$(git -C $N2/PS2Recomp rev-parse --abbrev-ref HEAD) head=$(git -C $N2/PS2Recomp rev-parse --short HEAD)"
./gradlew assembleRelease -Pps2xBootElf=/storage/emulated/0/Android/data/com.ps2x.runner/files/SLUS_207.72 -Pps2xGameCodegenDir=/home/brad/n2/c17-codegen/codegen-output --max-workers=4 --console=plain >"$LOG/build3-assembleRelease.log" 2>&1
EXIT=$?
echo "$EXIT" > "$LOG/build3-exit.txt"
echo "BUILD3_END $(date -u +%FT%TZ) EXIT=$EXIT"
if [ "$EXIT" != "0" ]; then
  tail -40 "$LOG/build3-assembleRelease.log"
  exit $EXIT
fi
# --- bars ---
...
for s in PS2X_SKIP_MOVIE DEV-SKIP-MOVIE PS2X_PAD_SCRIPT ps2x.env 'ps2x.env: set ' ps2x SLUS_207.72; do
