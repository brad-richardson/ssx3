#!/usr/bin/env python3
"""M8 replay-fidelity table from probe JSONLs (superset of M7 analyze.py).

Usage: analyze.py LABEL=path/to/probe.jsonl [LABEL=path ...]
Prints M7's tables unchanged, then M7 sections, then M8 sections: m8_meta
lines, per-slot census tables (writes, reads, draws-affected), xdiff
distribution rows for delta runs.
