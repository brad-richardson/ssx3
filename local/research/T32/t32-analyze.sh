#!/usr/bin/env bash
# T32 trace analysis on the Race-Cross emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t32-census.txt
python3 $D/t4-sample.py $E $D/t32-samples.txt
ls -la $D/t32-census.txt $D/t32-samples.txt
date -u
