#!/usr/bin/env python3
"""Compress/move only closed E8-owned captures, verify hashes, retain one copy."""
import argparse
import gzip
import hashlib
import json
...
def main():
    ...
    rows=[]
    canonical={}
    for old in sorted(EVIDENCE.glob('e8*-retained.json')):
    ...
    for src in sorted((RUN/f'{label}-1').iterdir()):
        ...
        if src.suffix=='.bin':
            ...
            prior=EVIDENCE.parent/'E7/raw'/f'vram-{sha}.bin.gz'
            dest=baseline if digest(baseline,True)==sha else prior if prior.exists() else EVIDENCE/'raw'/f'vram-{sha}.bin.gz'
            keep(src,dest,True)
    ...
    print('# E8 RETENTION TAIL COMPLETE; every removed raw verified against its canonical content hash')
