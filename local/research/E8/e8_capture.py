#!/usr/bin/env python3
"""E8 progression capture driver. Explicit preflight, caps, owned lease cleanup."""
import argparse
import datetime
import hashlib
import json
...
from e8_frame import read_png, fnv32
EVIDENCE = REPO / 'local/research/E8'
...
WAITS = RUN / 'e8-waits.log'
BIN_SHA = '4f02b144a8de09622f66aa81533097d5b7777eaf63ffab3abe80f8e73fafa290'
TEST_SHA = '9a93097032fd668084c96a6d25caa158a2ce9349ae8a788baf0d701e094f52bf'
...
CAPS = dict(wall_s=600, terminate_reserve_s=15, progress_lines=1000000,
            ...
            e7_dir=8*MIB,
            aggregate=1280*MIB, poll_s=0.25, grace_s=10)
...
def preflight(label):
    ...
    build=json.loads(manifest_path.read_text()) if manifest_path.exists() else dict(
        fork_head='4acc59ffebc99561ea2b4fe8e1aa1bebbc3075f2',bin_sha=BIN_SHA,bin_size=161313936,
        test_sha=TEST_SHA,test_size=5624728,test_count=452)
...
def main():
    ...
    assert not (EVIDENCE/f'e8{args.phase}-result.json').exists(), 'phase already used'
    label='e8'+args.phase
    ...
    return 0 if result['rc']==0 and result.get('release_utc') else 2
