#!/usr/bin/env python3
"""N7 Odin launch: I26-FAST vsync-clock route, PS2X_VSYNC_RATE_LOG=1, optional
...
ap.add_argument('--scap-ticks', default='875,2000,3000,4400')
ap.add_argument('--scap-every', type=float, default=9999.0)
ap.add_argument('--min-battery', type=int, default=20)
a = ap.parse_args()

OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/N7/logs/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('N7 ')):
...
sh(f'echo "N7 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
...
def _force_stop():
    ...
    subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE N7 done' > /data/local/tmp/mg/LEASE"], capture_output=True, timeout=30)


atexit.register(_force_stop)
signal.signal(signal.SIGTERM, lambda *_: sys.exit('SIGTERM'))
env = [f'# N7 {a.label}', f'PS2X_CD_IMAGE={FILES}/SSX3.iso', 'PS2X_SKIP_MOVIE=1',
