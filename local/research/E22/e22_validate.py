...
def observer(env,dest):
    library=os.environ.get('E22_OBSERVER_DYLIB')
    if library:
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e22-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E22 requires E18's delivered behavior at baseline
        ...
        if '# E7 SHUTDOWN' in taps:
            from e22_closed_events import tap
    ...
    print('# E22 VALIDATION TAIL COMPLETE')
    return result
