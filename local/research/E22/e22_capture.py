#!/usr/bin/env python3
"""E22 demand-watch capture driver. Fresh T13 pre-claims, caps, owned lease cleanup. One boot."""
import argparse
import datetime
import hashlib
import json
...
sys.path.insert(0, str(REPO / 'local/research/E22'))
from e22_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e22_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E22'
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E22 forbids lease wait')
        raise RuntimeError('Lease occupied; E22 tables and stops without waiting')
    ...
    manifest_path=EVIDENCE/'e22a-build.json'
    assert manifest_path.exists(), 'fresh E22 probe-gate manifest required'
...
def main():
    ...
    assert not (EVIDENCE/'boot-attempt.json').exists(), 'E22 permits only one boot'
    assert not list(EVIDENCE.glob('e22?-result.json')), 'E22 permits only one boot'
    label='e22'+args.phase
    assert args.report_all, 'E22 requires REPORT_ALL=1'
    ...
    assert not any('stdbuf' in x for x in boot_argv), 'E22 forbids stdbuf in the boot argv'
