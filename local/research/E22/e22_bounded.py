...
def main():
    ...
    (folder/'E22-owner.json').write_text(json.dumps(dict(task='E22',label=a.label,created=record['utc'])))
        ...
        try:
            ...
            while p.poll() is None or sel.get_map():
                ...
                if elapsed>=next_sample:
                    row=dict(elapsed_s=elapsed,tmp_allocated=size(folder),fixture_allocated=size(P/'e22-fixtures'),stdout_seen=seen,stdout_kept=kept,**sample())
    ...
    print(json.dumps(record,indent=2));print('# E22 BOUNDED TOOL TAIL COMPLETE')
