...
NEWB = Path('/tmp/e22-mpeg-link/runtime')
B = Path(os.environ.get('E22_BUILD_DIR',str(B0)))
OUT = Path('/tmp/e22-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
...
def sample():
    paths = list(P.glob('e22-*')) + list(P.glob('._e22-*')) + list((P/'run').glob('*e22*'))
    # The shared function trace is charged even though its name lacks E22.
...
def bound(s):
    ...
    if s['internal_allocated'] >= 3*G-256*M: return 'internal_allocation'
...
def admission(s):
    ...
    assert s['internal_free'] >= 2*G+512*M+max(0,3*G-s['internal_allocated']), s
