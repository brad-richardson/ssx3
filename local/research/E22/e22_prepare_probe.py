"""Pin all behavior-regression gates before spending the single E22 title allowance."""
import subprocess
...
save('e22a-build.json',dict(utc=utc(),fork_head=head,fork_status='?? ps2_log.txt\n',test_count=458,
    bin_size=ckpt['runner']['bytes'],bin_sha=ckpt['runner']['sha256'],
    test_size=ckpt['suite']['bytes'],test_sha=ckpt['suite']['sha256']))
print('All E22 regression gates pinned. Fresh T13 pre-claims and atomic lease remain mandatory.')
print('# E22 PREPARE PROBE TAIL COMPLETE')
