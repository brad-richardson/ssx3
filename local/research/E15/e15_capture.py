#!/usr/bin/env python3
"""E15 progression capture driver. Explicit preflight, caps, owned lease cleanup."""
import argparse
import datetime
import hashlib
import json
...
from e15_frame import read_png, fnv32
from e15_bounded import sample as resource_sample, resource_bound
from e15_suite import run as run_suite
EVIDENCE = REPO / 'local/research/E15'
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E15 forbids lease wait')
        raise RuntimeError('Lease occupied; E15 tables and stops without waiting')
    ...
    assert manifest_path.exists(), 'fresh E15 build manifest required'
    ...
    assert record['resource_admission']['internal_free'] >= 1879048192
    assert record['resource_admission']['ssd_free'] >= 8858370048-record['resource_admission']['ssd_allocated']
    record['wait_tails'] = {}
...
def main():
    ...
    assert not list(EVIDENCE.glob('e15?-result.json')), 'E15 permits only one boot'
    label='e15'+args.phase
    assert args.report_all, 'E15 requires REPORT_ALL=1'
