#!/usr/bin/env python3
"""Join E15 query calls/returns to same-run constructor and UI-pointer guards."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('phase',choices=['a','b']);args=ap.parse_args();label='e15'+args.phase
    result=json.loads((E/f'{label}-result.json').read_text());assert result.get('release_utc')
    events,tail=tap(RUN/f'{label}-join/e7-events.txt',allow_missing_footer=True)
    ...
    with (E/f'{label}-card.txt').open('w') as f:
        ...
        f.write('# E15 CARD TAIL COMPLETE: same-run objects and query truth/return joins\n')
    print(label,'queries',len(queries),'objects',[hex(x) for x in sorted(objects)],'targets',table['target_counts'])
    print('# E15 CARD TAIL COMPLETE')
