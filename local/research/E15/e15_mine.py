#!/usr/bin/env python3
"""Join this E15 run's dynamic registration/request, guards, packets and images."""
import hashlib,json,re,struct
from collections import Counter
from pathlib import Path
from e15_io import read_capture,resolve
from e15_closed_events import tap,fields
from e15_packet import packet
from e15_frame import surface,census,field_surface,read_png,write_png,fnv32
...
def main():
    ...
    data=read_capture(RUN/f'{LABEL}-join/e7-events.txt');events,tail=tap(RUN/f'{LABEL}-join/e7-events.txt',allow_missing_footer=True)
    ...
    mpeg=dict(dynamic_mpeg=hex(obj),create=create,registration=register,request=request,wait=wait,existing_private_state_receipt=waiting,
        ...
        actual_callback_dispatches=0,valid_no_input_returns=0,add_bs_dispatches=0,
        ...
        limits=['Selection inside the private callback map is not tapped; eligibility follows the pinned selector source. Scheduler selection and target dispatch are independently observed.',
                ...
                'The required source shutdown footer is absent. Observed pairs and independent function/park logs do not substitute for final source counters or truncation flags.'])
    dump('e15a-mpeg.json',mpeg)
    ...
    dump('e15a-graphics.json',graphics)
    ...
    dump('e15a-observation-closure.json',audit)
    ...
    print('# E15 JOIN MINER TAIL COMPLETE')
