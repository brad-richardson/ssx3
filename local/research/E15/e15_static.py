#!/usr/bin/env python3
"""E15 read-only ELF/OUT miner, inherited from trusted EF decoder; standard library only, never boots or builds.
...
class Elf:
    ...
    def decode(self, pc):
        ...
        if s is None:
            ...
            elif op == 0 and (w&63) in (0,8,9,0x21,0x24,0x25,0x2d):
                rd,shift,fn=(w>>11)&31,(w>>6)&31,w&63
                if fn==0: s=f'sll ${REG[rd]}, ${REG[rt]}, {shift}'
                elif fn==8: s=f'jr ${REG[rs]}'
                elif fn==9: s=f'jalr ${REG[rd]}, ${REG[rs]}'
                else: s=f'{ {0x21:"addu",0x24:"and",0x25:"or",0x2d:"daddu"}[fn]} ${REG[rd]}, ${REG[rs]}, ${REG[rt]}'
            elif op in (9,0xd,0xf):
                if op==9: s=f'addiu ${REG[rt]}, ${REG[rs]}, {simm:#x}'
                elif op==0xd:s=f'ori ${REG[rt]}, ${REG[rs]}, {imm:#x}'
                else:s=f'lui ${REG[rt]}, {imm:#x}'
            elif op in (2, 3):
...
def main():
    ...
    print('# E15 STATIC TAIL COMPLETE')
