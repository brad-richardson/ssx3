...
plog "COUNTS app=$(grep -c 'app vsync=' $E || true) tpl=$(grep -c 'tpl vsync=' $E || true) tplrearm=$(grep -c 'tplrearm vsync=' $E || true) t60cap=$(grep -c 'T60_CAP' $E || true) ebw=$(grep -c 'ebw vsync=' $E || true) ebwend=$(grep -c 'ebwend vsync=' $E || true) ebwlast=$(grep -c 'ebwlast vsync=' $E || true) cap=$(grep -c 'T58_CAP' $E || true) caplast=$(grep -c 'T58_CAPLAST' $E || true) vu0call=$(grep -c 'vu0call vsync=' $E || true) vif0op=$(grep -c 'vif0op vsync=' $E || true) t57cap=$(grep -c 'T57_CAP' $E || true) ctag=$(grep -c 'ctag ' $E || true) spw=$(grep -c 'spw vsync=' $E || true)"
