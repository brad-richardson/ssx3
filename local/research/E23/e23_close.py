...
# E23's own deliverables; Q1/Q2 are E21 receipts consumed, not re-derived.
assert (E/'CADENCE.md').exists() and (E/'cadence.json').exists() and (E/'BOOT-DESIGN.md').exists()
assert (E/'e23a-boot.json').exists() and (E/'AMENDMENT-A1.md').exists() and (E/'hex-audit.json').exists()
assert (E.parent/'E22/X-HUNT.md').exists() and (E.parent/'E21/Q2-AUDIT.md').exists()
cad=json.loads((E/'cadence.json').read_text())
assert all(r['cadence']['producerFirings']==1 for r in cad['rows']),'reference cadence receipt missing'
boot=json.loads((E/'e23a-boot.json').read_text())
assert boot['producer_watch_hits_after_park']==0 and boot['producer_watch_hits']==29
assert json.loads((E/'e23a-result.json').read_text()).get('release_utc')
...
save('final-audit.json',dict(utc=utc(),fork=BASE_SHA,fork_commands=receipts,fork_source_changes=0,fork_commits=0,
    ...
    cadence_fixture=pin(P/'e23-fixtures/complete/binding-test'),cadence_cases=len(cad['rows']),
    observer=pin(E/'parser/e21-parser-observer.dylib'),observer_rebuilt=False,
    title_boots=1,lease_claims=1,observed_boot=True,cadence_reference=True,
    amendment='A1',errata=['E23-E1'],resources=resources))
print('Final audit: fork unchanged;9457 generated and1725 protected hashes agree;1 boot/1 claim/0 fork commits.')
print('# E23 FINAL AUDIT TAIL COMPLETE')
