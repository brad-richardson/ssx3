...
def observer(env,dest):
    library=os.environ.get('E23_OBSERVER_DYLIB')
    if library:
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e23-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E23 requires E18's delivered behavior at baseline
    ...
    print('# E23 VALIDATION TAIL COMPLETE')
    return result
