"""Pin all behavior-regression gates before spending the single E23 title allowance."""
import subprocess
...
save('e23a-build.json',dict(utc=utc(),fork_head=head,fork_status='?? ps2_log.txt\n',test_count=458,
    bin_size=ckpt['runner']['bytes'],bin_sha=ckpt['runner']['sha256'],
    test_size=ckpt['suite']['bytes'],test_sha=ckpt['suite']['sha256']))
print('All E23 regression gates pinned. Fresh T13 pre-claims and atomic lease remain mandatory.')
print('# E23 PREPARE PROBE TAIL COMPLETE')
