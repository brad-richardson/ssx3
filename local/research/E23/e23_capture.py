#!/usr/bin/env python3
"""E23 demand-watch capture driver. Fresh T13 pre-claims, caps, owned lease cleanup. One boot."""
import argparse
import datetime
import hashlib
import json
...
sys.path.insert(0, str(REPO / 'local/research/E23'))
from e23_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e23_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E23'
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E23 forbids lease wait')
        raise RuntimeError('Lease occupied; E23 tables and stops without waiting')
    ...
    manifest_path=EVIDENCE/'e23a-build.json'
    assert manifest_path.exists(), 'fresh E23 probe-gate manifest required'
...
def main():
    ...
    assert not (EVIDENCE/'boot-attempt.json').exists(), 'E23 permits only one boot'
    assert not list(EVIDENCE.glob('e23?-result.json')), 'E23 permits only one boot'
    label='e23'+args.phase
    assert args.report_all, 'E23 requires REPORT_ALL=1'
    ...
    assert not any('stdbuf' in x for x in boot_argv), 'E23 forbids stdbuf in the boot argv'
