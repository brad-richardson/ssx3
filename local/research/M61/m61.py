#!/usr/bin/env python3
"""M61 c311-vs-c312 per-site decile/delta join: runner-up vs split (offline).

Usage: m61.py M16_DIR M15_DIR M60TXT WORK_DIR EVID_DIR

Reads M16 per-shape triplets + loo.txt, the M15 triplet, and the M60
receipt (span/decile tables to reproduce); raw XFB dumps, 573440 B =
640x448 YUYV. Read-only inputs; receipt text goes to stdout (redirect
to WORK_DIR/m61.txt); PNG pair map to WORK_DIR (evidence copy
iff the DESIGN.md rule meets, decided at report time).

Tasks (see DESIGN.md, recorded before running):
  1: pair reproduction (site-29 + decpair + missing)
  2: separator census (rowgeom + runs + seats)
...
# M60 pins: m15 tail-Y column-sign table + pair site detail
...
M60_UNION_COLS = sorted(M60_COLSIGN)
# c312 sites: {(row, col): (delta, dec)} x23, row order (M60 colsign pins).
M60_C312 = {(316, 312): (11, 7), (317, 312): (20, 5),
            ...
            (379, 312): (10, 3)}
# c311 sites: {(row, col): (delta, dec)} x6, row order (M58/M60 pins).
...
def parse_m60_pair(txt: str):
    """Pair match targets from m60.txt (unanim + c312 sites/runs +
    span + decmix + dprof for 311/312)."""
    m_u = re.search(r"^unanim union: ncols=(\d+) cols=\[([^\]]*)\]",
                    txt, re.M)
    if not m_u:
        return None
    ...
    cols = {}
    for col in (311, 312):
        m = re.search(rf"^unanim c={col}: s0_n=(\d+) m15_n=(\d+) "
    ...
    c312 = {}
    for m in re.finditer(r"^c312 site: m15 c=312 r=(\d+) delta=([+-]?\d+) "
                         r"sign=(\S+) dec=(\d+) d=(\S+) lo=(\S+) hi=(\S+)",
                         txt, re.M):
        c312[(int(m.group(1)), 312)] = (int(m.group(2)), int(m.group(4)))
    if len(c312) != 23:
        return None
    m_r = re.search(r"^c312 runs: nruns=(\d+) runs=\[([^\]]*)\]", txt, re.M)
    if not m_r:
        return None
    spans = {}
    for col in (311, 312):
        m = re.search(rf"^span c={col}: m15_n=(\d+) "
    ...
    mixes = {}
    for col in (311, 312):
        m = re.search(rf"^decmix c={col}: class=(\S+) m15_n=(\d+) "
                      r"ndec=(\d+) hist=\[([^\]]*)\] maxshare=(\S+)",
                      txt, re.M)
        if not m:
            return None
        dh = _parse_dechist(m.group(4))
        if dh is None:
            return None
        mixes[col] = {"class": m.group(1), "m15_n": int(m.group(2)),
                      "ndec": int(m.group(3)), "hist": dh}
    dprofs = {}
    for col in (311, 312):
        m = re.search(rf"^dprof c={col}: class=(\S+) m15_n=(\d+) "
                      r"dhist=\[([^\]]*)\] na=(\d+) dmax=(\S+) "
                      r"dmode=(\S+)",
                      txt, re.M)
        if not m:
            return None
        dh = _parse_dhist(m.group(3))
        if dh is None:
            return None
        dprofs[col] = {"class": m.group(1), "m15_n": int(m.group(2)),
                       "dhist": dh, "na": int(m.group(4)),
                       "dmax": None if m.group(5) == "None" else int(m.group(5)),
                       "dmode": None if m.group(6) == "None" else int(m.group(6))}
    return {"union": union, "cols": cols, "c312": c312,
            "runs": m_r.group(2), "spans": spans, "mixes": mixes,
            "dprofs": dprofs}
...
def task1_pair_lines(t0, t15, pr, pc, pl, cols0, cols15, dec_s0,
                     masks, d0, d15, mask0, mask15, tgt):
    """Task 1 receipt lines: site-29 + decpair + missing."""
    L = []
    canon = []
    ...
    # pair margins (311/312) vs m60.txt + DESIGN
    for col in M61_PAIR_COLS:
        s0n = len(cols0.get(col, {"rows": []})["rows"])
        b = urows[col]
        want_txt = tgt["cols"].get(col)
        want_des = M60_COLSIGN.get(col)
        ...
        L.append(f"pair margin c={col}: s0_n={s0n} m15_n={b['n']} "
                 ...
                 f"class={b['uclass']} "
                 f"m60txt_match={m_txt} design_match={m_des}")
        canon.append(L[-1])
    # site-29: all 29 sites (col,row) with seats + d
    ...
    for (r, c) in c311rows + c312rows:
        ...
        L.append(f"pair site: m15 c={c} r={r} delta={dd:+d} sign={sg} "
        ...
        canon.append(L[-1])
    # decpair: same-decile cross-column pairs d0-d9
    dp = decpair_rows(c311rows, c312rows, dec_s0, deltamap)
    for dci in range(10):
        b = dp[dci]
        wa, wb = M61_DEPAIR[dci]
        m = b["A_deltas"] == wa and b["B_deltas"] == wb
        astr = ",".join(f"{x:+d}" for x in b["A_deltas"]) or "none"
        bstr = ",".join(f"{x:+d}" for x in b["B_deltas"]) or "none"
        L.append(f"decpair d{dci}: A_n={b['A_n']} A=[{astr}] "
                 f"B_n={b['B_n']} B=[{bstr}] design_match={m}")
        canon.append(L[-1])
    # missing: d4/d7 on c312 vs absence on c311 + cell probes
    for dci, wantB in ((4, [(363, -8)]), (7, [(316, 11)])):
        b = dp[dci]
        L.append(f"missing d{dci}: A_n={b['A_n']} B_n={b['B_n']} "
                 f"(want 0-vs-1)")
        canon.append(L[-1])
        for (r, wantd) in wantB:
            st = cell_status(r, 311, mask15, d15, dec_s0)
            dd = ("None" if st["delta"] is None else f"{st['delta']:+d}")
            L.append(f"missing probe: m15 c=311 r={r} kind={st['kind']} "
                     f"delta={dd} dec={st['dec']} band={st['band']} "
                     f"(c312 r={r} delta={wantd:+d} dec={dci})")
            canon.append(L[-1])
    # runs + setguards
    ...
    c312set = {(r, c): (deltamap[(r, c)], int(dec_s0[r, c]))
               for (r, c) in c312rows}
    m311 = c311set == M61_C311
    m312t = c312set == tgt["c312"]
    m312d = c312set == M60_C312
    L.append(f"pair setguard c311: design_match={m311} "
             f"(m60.txt carries no c311 per-site lines)")
    canon.append(L[-1])
    L.append(f"pair setguard c312: m60txt_match={m312t} "
             f"design_match={m312d}")
    canon.append(L[-1])
    # span/dec/d guard detail for the pair (vs m60.txt + DESIGN)
    ...
    info = {"urows": urows, "srows": srows, "drows": drows,
            ...
            "decpair": dp, "s0sites": s0sites, "m15sites": m15sites}
    return L, canon, info


def task2_separator_lines(m15sites, cols0, cols15, dec_s0, masks,
                          mask15, d15, info1):
    """Task 2 receipt lines: rowgeom + runs + seats.

    info1: task1_pair_lines info (pair rows over m15 tail-Y).
    """
    L = []
    canon = []
    deltamap = _deltamap(cols15)
    ...
    srows = info1["srows"]
    drows = info1["drows"]
    c311rows = info1["c311rows"]
    c312rows = info1["c312rows"]
    rowsA = sorted(r for (r, _c) in c311rows)
    rowsB = sorted(r for (r, _c) in c312rows)
    # rowgeom: row sets + gaps + spans compared
    g = rowgeom_rows(rowsA, rowsB)
    L.append(f"rowgeom A311: rows={g['A_rows']} gaps={g['A_gaps']} "
             f"span={g['A_span']} (want {M61_GAPS[311]}/19)")
    canon.append(L[-1])
    L.append(f"rowgeom B312: rows={g['B_rows']} gaps={g['B_gaps']} "
             f"span={g['B_span']} (want {M61_GAPS[312]}/63)")
    canon.append(L[-1])
    L.append(f"rowgeom join: intersection={g['intersection']} "
             f"union_n={g['union_n']} overlap={g['overlap_range']} "
             f"nested={g['nested']} label_runs={g['label_runs']}")
    canon.append(L[-1])
    mlab = " ".join(f"{r}:{lab}" for (r, lab) in g["merged_labels"])
    L.append(f"rowgeom merged: [{mlab}]")
    canon.append(L[-1])
    # counterpart status: for each B row, kind on A col; and reverse
    for (r, _c) in c312rows:
        st = cell_status(r, 311, mask15, d15, dec_s0)
        dd = ("None" if st["delta"] is None else f"{st['delta']:+d}")
        L.append(f"counterpart B_row r={r}: on311 kind={st['kind']} "
                 f"delta={dd} dec={st['dec']} band={st['band']}")
        canon.append(L[-1])
    for (r, _c) in c311rows:
        st = cell_status(r, 312, mask15, d15, dec_s0)
        dd = ("None" if st["delta"] is None else f"{st['delta']:+d}")
        L.append(f"counterpart A_row r={r}: on312 kind={st['kind']} "
                 f"delta={dd} dec={st['dec']} band={st['band']}")
        canon.append(L[-1])
    # runs: sign runs compared + c311 candidate break loci
    for col, runs in ((311, info1["runs311"]), (312, info1["runs312"])):
        rrow = " ".join(f"{s}:{a}-{b}x{n}" for (s, a, b, n) in runs)
        L.append(f"runs c={col}: class={urows[col]['uclass']} "
                 f"nruns={len(runs)} runs=[{rrow}]")
        canon.append(L[-1])
    cand = sorted(((rowsA[i + 1] - rowsA[i], rowsA[i], rowsA[i + 1])
                   for i in range(len(rowsA) - 1)), reverse=True)
    L.append(f"runs candbreak311: gaps_desc={cand} "
             f"(where c311 runs would break if split; no verdict)")
    canon.append(L[-1])
    # seats: band/streak/carrier/701/at-peak/d distributions compared
    sd = seatdist_rows(c311rows + c312rows, dec_s0, masks, cols15)
    for col in M61_PAIR_COLS:
        b = sd[col]
        p = drows[col]
        dh = ("none" if not p["dhist"] else
              " ".join(f"{d}:{p['dhist'][d]}" for d in sorted(p["dhist"])))
        L.append(f"seats c={col}: n={b['n']} band={b['bandhist']} "
                 f"streak={b['streakhist']} carrier={b['carrierhist']} "
                 f"split701={b['split701hist']} peak={b['peak']} "
                 f"atpeak_n={b['atpeak_n']} dhist=[{dh}] na={p['na']} "
                 f"dmax={p['dmax']} dmode={p['dmode']}")
        canon.append(L[-1])
    info = {"rowgeom": g, "seatdist": sd}
    return L, canon, info


def main():
    m16d, m15d, m60t, workd, evidd = (Path(a) for a in sys.argv[1:6])
    ...
    m60b = m60t.read_bytes()
    m60sha = hashlib.sha256(m60b).hexdigest()
    print(f"input {m60t}: bytes={len(m60b)} sha256={m60sha}")
    print(f"m60.txt sha prefix={m60sha[:8]}..{m60sha[-8:]} (tabled)")
    tgt = parse_m60_pair(m60b.decode(errors="replace"))
    if tgt is None:
        print("m60.txt PAIR UNION/COLS LINES NOT FOUND: STOP, tabled.")
        return
    print(f"m60.txt target union: ncols={len(tgt['union'])} "
          f"cols={tgt['union']}")
    for col in sorted(tgt["cols"]):
        b = tgt["cols"][col]
        print(f"m60.txt target unanim c={col}: s0_n={b['s0_n']} "
              f"m15_n={b['m15_n']} pos={b['pos']} neg={b['neg']} "
              f"min={b['min']} max={b['max']} sum={b['sum']} "
              f"class={b['class']}")
    print(f"m60.txt target c312 sites: n={len(tgt['c312'])} "
          f"runs=[{tgt['runs']}]")
    for col in sorted(tgt["spans"]):
        b = tgt["spans"][col]
        print(f"m60.txt target span c={col}: m15_n={b['m15_n']} "
              f"rows={b['rowmin']}-{b['rowmax']} rowspan={b['rowspan']} "
              f"decspan={b['decspan']} dechist={b['dechist']}")
    for col in sorted(tgt["mixes"]):
        b = tgt["mixes"][col]
        print(f"m60.txt target decmix c={col}: class={b['class']} "
              f"m15_n={b['m15_n']} ndec={b['ndec']} hist={b['hist']}")
    for col in sorted(tgt["dprofs"]):
        b = tgt["dprofs"][col]
        print(f"m60.txt target dprof c={col}: class={b['class']} "
              f"m15_n={b['m15_n']} dhist={b['dhist']} na={b['na']} "
              f"dmax={b['dmax']} dmode={b['dmode']}")
    ...
    if r0 != R0_M16 or r15 != R0_M15 or f0 != FNV_S0 or f15 != FNV_M15:
        print("MISMATCH vs M17-M60 baselines: STOP, tabled.")
        return
    if not carriers_ok:
    ...
    print("== Task 1: pair reproduction (site-29 + decpair + missing) ==")
    t1, canon1, pair1 = task1_pair_lines(t0, t15, pr, pc, pl,
                                         cols0, cols15, dec_s0,
                                         masks, d0, d15, mask0, mask15, tgt)
    for ln in t1:
        print(ln)
    # pair guard: union + 311/312 margins + sets + spans + runs value-exact
    allcols = sorted(set(cols0) | set(cols15))
    oku = (allcols == tgt["union"] == M60_UNION_COLS)
    ...
    ok312 = c312set == tgt["c312"] == M60_C312
    ...
    for col in M61_PAIR_COLS:
        ...
        m = (s["n"] == wt["m15_n"] == wm["m15_n"] == wd["m15_n"]
             ...
             and s["dechist"] == wt["dechist"] == wm["hist"] == want_hist
             and wm["ndec"] == wt["decspan"]
    ...
    okd = all(pair1["decpair"][dci]["A_deltas"] == M61_DEPAIR[dci][0]
              and pair1["decpair"][dci]["B_deltas"] == M61_DEPAIR[dci][1]
              for dci in range(10))
    print(f"decpair-guard: match={okd}")
    if not (oku and okc and ok311 and ok312 and oks and okr and okd):
        print("M60 UNION/MARGIN/SETS/SPAN/RUNS/DEPAIR MISMATCH: STOP, tabled.")
        return

    print("== Task 2: separator census (rowgeom + runs + seats) ==")
    t2b, canon2, geo = task2_separator_lines(pair1["m15sites"], cols0,
                                              cols15, dec_s0, masks,
                                              mask15, d15, pair1)
    for ln in t2b:
        print(ln)
    ...
    g = geo["rowgeom"]
    print(f"bars: pair_match={okc and ok311 and ok312 and oks and okr} "
          f"decpair={[(pair1['decpair'][d]['A_n'], pair1['decpair'][d]['B_n']) for d in range(10)]} "
          f"shared_rows={g['intersection']} nested={g['nested']} "
          f"runs311={[(s, n) for (s, _a, _b, n) in pair1['runs311']]} "
          f"runs312={[(s, n) for (s, _a, _b, n) in pair1['runs312']]} "
          f"seats={dict((c, geo['seatdist'][c]) for c in M61_PAIR_COLS)}")
    ...
    _, canon1b, _pair1b = task1_pair_lines(t0b, t15b, pr, pc, pl,
    ...
    print("== PNG pair map (m15 tail-Y by pair class) ==")
    img = Image.new("RGB", (320, 224), (0, 0, 0))
    px = img.load()
    deltamap = _deltamap(cols15)
    m15sites = pair1["m15sites"]
    runs_ok = (pair1["runs311"] == M61_RUNS[311]
               and pair1["runs312"] == M61_RUNS[312])
    for (r, c) in m15sites:
        x, y = c // 2, r // 2
        if c == 311:
            col3 = (255, 255, 0)  # yellow: c311 runner-up 6
        ...
        cur = px[x, y]
        # precedence: yellow > red > green > gray > black
        pri = {(0, 0, 0): 0, (128, 128, 128): 1, (0, 255, 0): 2,
               (255, 0, 0): 3, (255, 255, 0): 4}
        if pri[col3] >= pri.get(cur, 0):
            px[x, y] = col3
    pngp = workd / "m61-pairmap.png"
    img.save(pngp)
    h5 = runs_ok
    print(f"pairmap: {pngp} bytes={pngp.stat().st_size} "
          f"(evidence iff H5 runs13; runs311={pair1['runs311']} "
          f"runs312={pair1['runs312']} h5={h5})")
