#!/usr/bin/env python3
"""E55D12 Part 2 checker: validates S1 run receipts WITHOUT judging pixels.
...
[padscript] rows (exactly one Start + one Square + five separate Downs +
two Crosses press/release in plan order, no other input), and the private
probe.log counts (5 getdir: 4 pre-C2 ok=0 reason=empty + 1 at/after C2
tick1700 ok=0 reason=empty; 0 mcread).
It CANNOT prove visual labels (Save/Load text, Load-game highlight row,
MEMORY CARD screen, footer): those are worker readings for the
orchestrator's visual verdict, recorded in REPORT.md and s1-receipts.txt,
never asserted here.

Usage: python3 local/research/E55D12/check-p2.py
...
RUN = Path('/Users/brad/dev/ssx3-work/E55D12/run/S1')
FAIL = []
...
CROSS2_TICK = 1700
CURATED = ['snap-1616t-0060.49s.png', 'snap-1808t-0067.52s.png']
...
def main():
    ...
    check('route_exact', result.get('route') ==
          ...
          result.get('route', ''))
    check('stop_tick_1800', result.get('stop_tick') == 1800, str(result.get('stop_tick')))
    check('start_square_cross_ticks',
          (result.get('start_tick'), result.get('square_tick'), result.get('cross_tick'),
           result.get('cross1_tick'), result.get('down5_tick'), result.get('cross2_tick'))
          == (636, 820, 1360, 1360, 1540, 1700),
    ...
    check('frame_targets_1620_1800',
          result.get('load_frame_tick') == 1620 and result.get('post_frame_tick') == 1800,
          str((result.get('load_frame_tick'), result.get('post_frame_tick'))))
    ...
    check('frame_proof_ge_1800', proof.get('tick', -1) >= 1800, str(proof))
    ...
    check('pad_one_start_one_square_five_downs_two_crosses',
          sorted(presses) == sorted([b'0x0008', b'0x8000', b'0x0040', b'0x0040',
                                     b'0x0040', b'0x0040', b'0x4000', b'0x0040',
                                     b'0x4000']),
          str(presses))
    check('pad_press_order_spine_plus_d5_c2',
          presses == [b'0x0008', b'0x8000', b'0x0040', b'0x0040',
                      b'0x0040', b'0x0040', b'0x4000', b'0x0040', b'0x4000'],
          str(presses))
    check('pad_nine_releases', len(releases) == 9, 'n=%d' % len(releases))
    check('pad_rows_armed_plus_press_release',
          log.count(b'[padscript]') == 19 and b'[padscript] armed n=9' in log,
    ...
    check('probe_getdir_four_before_c2',
          sorted(t for t in ticks if t < CROSS2_TICK) == [118, 122, 126, 223],
          str(ticks))
    check('probe_getdir_one_at_after_c2',
          sorted(t for t in ticks if t >= CROSS2_TICK) == [1740],
          str(ticks))
