#!/usr/bin/env python3
"""E55D12 Part 1 checker: validates the prepared route/limits/pins from the
script source WITHOUT booting, claiming a lease, or touching the fork.

It imports e55d12_boot.py (sibling in this dir) for its constants and runs:
  - route shape: first entry START at tick 636; exactly one Square at tick
    ...
    post frame (1800) after C2; stop tick = post frame.
  - caps: wall == 500 s, progress == 120 s, log == 16 MiB, scratch intent
    (frames cap) <= 2 GiB, frame-proof grace == 120 s.
  - pins: fork/runner/ISO/ELF/codegen constants equal the E55D3/E55D11 pins.
...
def main():
    ...
    check('src_claims_lease', "claim('E55D12-'" in src, '')
