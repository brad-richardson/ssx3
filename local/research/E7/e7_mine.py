#!/usr/bin/env python3
"""Mine completed E7 captures only. Standard library; no guest execution."""
import argparse
...
def packet(path):
    data=path.read_bytes(); pos=0;tags=[];regs=[]
...
def main():
    ...
    with (EVIDENCE/f'{label}-watch-series.txt').open('w') as f:
        ...
        f.write('# E7 WATCH SERIES TAIL COMPLETE\n')
    ...
    if args.phase!='a':
        ...
        if not path.exists(): path=EVIDENCE/f'{label}-events.txt'
        ...
        pp=RUN/f'{label}-join'
        if not pp.exists(): pp=EVIDENCE/f'{label}-packets'
        summary['packets']=[packet(p) for p in sorted(pp.glob('e7-copy-*.bin'))]
        with (EVIDENCE/f'{label}-burst-series.txt').open('w') as f:
            ...
            f.write('# E7 BURST SERIES TAIL COMPLETE\n')
    ...
    print('# E7 MINER TAIL COMPLETE',path)
