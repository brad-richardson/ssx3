...
import struct
import zlib
from e7_io import read_capture
...
def main():
    p=argparse.ArgumentParser();p.add_argument('phase',choices=['a','b','c','d']);a=p.parse_args();label='e7'+a.phase
    ...
    write_png(EVIDENCE/f'{label}-producer.png',w,h,prod);write_png(EVIDENCE/f'{label}-display.png',w,h,disp)
    ...
    result['boundary_present']=[r for r in dumps if 599<=r['tick']<=603]
    result['display_hash_present_matches']=[r for r in dumps if r['fnv']==result['display']['rgba_fnv32'] and 599<=r['tick']<=603]
    host=RUN/f'frames-{label}-1/upload-latest.png'
    if not host.exists():host=EVIDENCE/f'{label}-upload-latest.png'
    ...
    print(json.dumps(result,indent=2));print('# E7 FRAME TAIL COMPLETE')
