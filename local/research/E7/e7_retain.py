#!/usr/bin/env python3
"""Compress/move only closed E7-owned captures, verify hashes, retain one copy."""
import argparse
import gzip
import hashlib
import json
...
def main():
    ...
    def keep(src,dest,compress=False):
        ...
        if not dest.exists():
            if compress:
                with src.open('rb') as a,gzip.open(dest,'xb',compresslevel=1) as b:shutil.copyfileobj(a,b,1024*1024)
        ...
        rows.append(dict(original=str(src),canonical=os.path.relpath(dest,EVIDENCE),bytes=size,sha256=sha,
                         retained_bytes=dest.stat().st_size,encoding='gzip' if compress else 'identity'))
        src.unlink()
    ...
    for src in sorted((RUN/f'{label}-1').iterdir()):
        ...
        if src.suffix=='.bin':
            sha=digest(src)
            baseline=EVIDENCE.parent/'E4/e4-vram-freeze.bin.gz'
            dest=baseline if digest(baseline,True)==sha else EVIDENCE/'raw'/f'vram-{sha}.bin.gz'
            keep(src,dest,True)
    ...
    print('# E7 RETENTION TAIL COMPLETE; every removed raw verified against its canonical content hash')
