#!/usr/bin/env python3
"""N8D7M12P6M6R WSL package gate: double-read archive and members.

Adapted from P3 apk_gate.py: root n8d7m12p6m6, in-root codegen path.
Requires exactly the arm64 runner/Turnip/HAL members, unchanged Turnip/HAL
pins, a runner Build ID NEW vs N8D7M1, every replay + selected/oracle
string, and the sole arm64 CMake cache pointing at the new root's G43 and
in-root codegen with PS2X_BUILD_TEST and the five diagnostics/UI flags OFF
and shadow-parallel ON. All outputs go to the WSL root top level (outside
the four staged source roots).
"""
import hashlib
import json
...
strings = (
    # Part 3 replay entrypoint/core (brief-named)
    'PS2X_GS_REPLAY_ONDEVICE', 'PS2X_GS_REPLAY_CAPTURE', 'PS2XGSC1',
    'GB4_REPLAY_SUMMARY',
    # N8D7M1 selected/oracle path (preserved backend)
...
assert out['members']['lib/arm64-v8a/libps2EntryRunner.so']['sha'][0] != n8d7m1_runner_sha, 'runner SHA unchanged from N8D7M1'
out['runner_new_vs_n8d7m1'] = True
out['runner_new_vs_p3'] = out['members']['lib/arm64-v8a/libps2EntryRunner.so']['sha'][0] != p3_runner_sha
...
assert out['runner_build_id'] != n8d7m1_runner_build_id, 'runner Build ID unchanged from N8D7M1'
