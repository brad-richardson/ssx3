...
ap.add_argument('--offcpu', action='store_true',
                help='FS2: simpleperf record with --trace-offcpu (falls back to plain -g if refused)')
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/FS2/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/FS2/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('FS2 ')):
...
if lease.startswith('FS2 '):
    CLAIMED['v'] = True  # outer hold (or a stale FS2 lease); we own it
else:
    r = subprocess.run(['bash', LEASESH, 'claim', 'FS2'], capture_output=True, text=True, timeout=60)
    if r.returncode != 0:
...
def lease_release():
    subprocess.run(['bash', LEASESH, 'release', 'FS2'], capture_output=True, timeout=60)
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['bash', LEASESH, 'release', 'FS2'],
                           capture_output=True, timeout=60)
    except Exception:
        pass
...
env = [f'# FS2 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
assert all(not e.startswith('PS2X_PGS') or e.split('=')[0] in
           ('PS2X_PGS_PRESENT_PIPELINE', 'PS2X_PGS_SSAA', 'PS2X_PGS_HIRES_SCANOUT', 'PS2X_PGS_FRAME_CONTEXTS',
            'PS2X_PGS_FLUSH_SPLIT', 'PS2X_PGS_FS2_LEGACY')
           for e in env), \
...
def task_sched(pid):
    # FS2: tid -> [comm, cpu_ticks, processor, run_ns, wait_ns, slices].
...
def residency_snap():
    # FS2: cumulative freq residency (10 ms units per state) + gpu clock stats.
    out = {}
    for c in range(8):
...
def write_sched(tag0, tag1):
    # FS2: three-way split per thread over the cpu window (+ per-thread current cpu).
...
while True:
    ...
    if a.profile_after_tick and not profiled and tick >= a.profile_after_tick:
        ...
        # FS2: concurrent sampler (thermal/freq/gpu + per-thread schedstat) while
        ...
        # FS2: if --trace-offcpu was refused (no perf.data), fall back to plain -g
