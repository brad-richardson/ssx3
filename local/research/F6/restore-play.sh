#!/bin/bash
# F6 close-out per run: put Brad's F5 play build back exactly (brief rule):
# F5 APK 4ff81032… (two local SHA reads, installed base.apk SHA), Brad's save
# + env via I31 deploy-odin.sh with F5's play knobs (env must read a8d651a7…),
# scratch and mc0-test emptied, app stopped, lease released. No launch.
# Lease via local/tooling/odin_lease.sh (atomic; orchestrator 09-26 rule).
set -euo pipefail
...
bash "$LEASESH" claim F6 || { echo "lease held, refusing" >&2; exit 1; }
trap 'bash "$LEASESH" release F6' EXIT
...
adb -s "$S" shell "rm -rf /data/local/tmp/f6 $FILES/f6dump; rm -rf $FILES/mc0-test/*; ls -A $FILES/mc0-test | wc -l; am force-stop $PKG"
