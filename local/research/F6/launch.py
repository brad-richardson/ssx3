#!/usr/bin/env python3
"""F6 Odin launcher: VK1's launch.py (F5 lineage: play-env pin a8d651a7, mc0 pins,
transport record, cool-down done by cooldown.py first; lease/dirs repointed at F6).
F6 ABBA: variant A (1x pipelined; the F6 APK defaults to the Vulkan present) with
--apk pointing at the new APK vs Brad's F5 play APK per leg; the 4x leg is variant B.
  --variant A|B|VA|VB  A = 1x pipelined, B = 4x+hi-res pipelined (F5's legs);
                       VA/VB = the same + PS2X_PRESENT_VULKAN=1 (unused in F6: default on)
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/F6/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/F6/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('F6 ')):
...
sh(f'echo "F6 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE F6 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# F6 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
sh("echo 'LEASE_FREE F6 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
