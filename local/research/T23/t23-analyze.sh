#!/usr/bin/env bash
# T23 trace analysis on the four-press emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t23-census.txt
python3 $D/t4-sample.py $E $D/t23-samples.txt
ls -la $D/t23-census.txt $D/t23-samples.txt
date -u
