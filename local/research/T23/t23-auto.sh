#!/usr/bin/env bash
# T23 single-shot: fresh boot + FOUR single 534 ms K holds (User Prefs, Time Zone, DST, confirm Standard) + capture.
# Press1 @T+100 -> User Prefs snap @T+110; press2 right after (~T+111) -> TZ snap @~T+119; press3 right after (~T+120) -> DST snap @~T+128; press4 right after (~T+129) -> +5/+15/+30/+60/+120/+240 series.
...
fail() { echo "T23_FAIL:$1"; stamp; exit 1; }
stamp
pgrep -a Xvfb || true
setsid nohup Xvfb :99 -screen 0 1280x1024x24 >/tmp/xvfb-t23.log 2>&1 < /dev/null &
sleep 4
pgrep -a Xvfb || fail NO_XVFB
xdotool getdisplaygeometry || fail NO_DISPLAY
TS=$(date -u +%Y%m%dT%H%M%SZ)
if [ -f $E ]; then
  mv $E $D/dat/PCSX2/logs/emulog-pre-t23-$TS.txt
  ls -la $D/dat/PCSX2/logs/
fi
rm -f $D/pcsx2.pid
echo "T_BOOT_WALL:$(date -u +%s) T_BOOT_UPTIME:$(cut -d. -f1 /proc/uptime)"
nohup $D/pcsx2/build/bin/pcsx2-qt -nogui -slowboot -turbo -datapath $D/dat -logfile $D/logs/boot-t23.log -- "$D/inputs/SSX 3 (USA).iso" > $D/logs/boot-t23.stdout 2>&1 &
echo $! > $D/pcsx2.pid
echo "PID:$!"
sleep 90
...
echo "WID:$WID"
bash $D/t17-snap.sh t23-park || fail SNAP_PARK
sleep 10
...
sleep 10; bash $D/t17-snap.sh t23-prefs || fail SNAP_PREFS
...
sleep 8; bash $D/t17-snap.sh t23-tz || fail SNAP_TZ
...
sleep 8; bash $D/t17-snap.sh t23-dst || fail SNAP_DST
pgrep -f pcsx2-qt >/dev/null || fail PCSX2_DIED_PRE_PRESS4
echo "PRESS4_KEYDOWN_WALL:$(date -u +%s.%N) UPTIME:$(cut -d. -f1 /proc/uptime)"
xdotool windowfocus --sync $WID || fail FOCUS4
xdotool keydown K || fail KEYDOWN4
echo "PRESS4_HELD_WALL:$(date -u +%s.%N) UPTIME:$(cut -d. -f1 /proc/uptime)"
sleep 0.5
xdotool keyup K || fail KEYUP4
echo "PRESS4_KEYUP_WALL:$(date -u +%s.%N) UPTIME:$(cut -d. -f1 /proc/uptime)"
sleep 5; bash $D/t17-snap.sh t23-post5 || fail SNAP5
sleep 10; bash $D/t17-snap.sh t23-post15 || fail SNAP15
sleep 15; bash $D/t17-snap.sh t23-post30 || fail SNAP30
sleep 30; bash $D/t17-snap.sh t23-post60 || fail SNAP60
sleep 60; bash $D/t17-snap.sh t23-post120 || fail SNAP120
sleep 120; bash $D/t17-snap.sh t23-post240 || fail SNAP240
ls -la $D/t23-*.jpg $E
