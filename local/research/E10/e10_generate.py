#!/usr/bin/env python3
"""E10-only APFS generation with fresh admission, bounds and complete manifests."""
...
OUT=Path('/tmp/ssx3-e9-retry-codegen');TOOL=Path('/Volumes/Extreme SSD/ps2x-i10/host-recomp/ps2xRecomp/ps2_recomp')
...
def main():
    ...
    phase=sys.argv[1];assert phase in ['drop','entry'];assert not (E/f'{phase}-generation.json').exists()
    ...
    expected_csv='b9aae74782fde6ba54671cfe07a9887c0af8ba1aad9d5befc1095e8cfef146ca'
    if phase=='drop':assert sha(csv)==expected_csv
    else:assert csv.read_text().count('sub_002C5140,0x2c5140,0x2c5168,0x28')==1
    ...
    if phase=='drop':
        OUT.mkdir(exist_ok=False);(OUT/'E10-owner.json').write_text(json.dumps(dict(evidence=str(E),pid=os.getpid(),utc=rec['utc'])))
    else:assert json.loads((OUT/'E10-owner.json').read_text())['evidence']==str(E)
    ...
    print('# E10 GENERATION TAIL COMPLETE phase='+phase)
