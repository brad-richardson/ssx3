#!/usr/bin/env python3
"""Join E10 query calls/returns to same-run constructor and UI-pointer guards."""
import argparse,json
from collections import Counter
from pathlib import Path
from e10_mine import tap
E=Path(__file__).resolve().parent;RUN=Path('/Volumes/Extreme SSD/ps2recomp-spike/P1/run')
def main():
    ap=argparse.ArgumentParser();ap.add_argument('phase',choices=['a','b']);args=ap.parse_args();label='e10'+args.phase
    ...
    with (E/f'{label}-card.txt').open('w') as f:
        ...
        f.write('# E10 CARD TAIL COMPLETE: same-run objects and query truth/return joins\n')
    print(label,'queries',len(queries),'objects',[hex(x) for x in sorted(objects)],'targets',table['target_counts'])
    print('# E10 CARD TAIL COMPLETE')
