#!/usr/bin/env python3
"""N8D7M12 Part 5E1: one leased Odin OFF-repeat replay of the staged stream.
...
PKG = "com.ps2x.runner"
SCRATCH = Path("/Users/brad/dev/ssx3-work/N8D7M12P5E1")
...
result = {"brief": "N8D7M12P5E1", "install_count": 0, "launch_count": 0,
...
def run():
    ...
    if (SCRATCH / "result.json").exists() or (SCRATCH / "driver.log").exists():
        raise SystemExit("one-run guard: N8D7M12P5E1 receipt already exists")
    ...
    stamp = int(time.time())
    ppm_dir = f"{FILES}/n8d7m12p5e1-frames-{stamp}"
    hashes_path = f"{FILES}/n8d7m12p5e1-{stamp}.hashes"
        ...
        if lease_claimed:
            ...
            try:
                ...
                if current == LEASE_TAG:
                    shell(f"echo 'LEASE_FREE N8D7M12P5E1 done' > {LEASE}",
                          timeout=30)
