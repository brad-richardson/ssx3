#!/bin/bash
# I29 Simulator run: I27B sim-run.sh pattern + PS2X_SOUND=1 (launch env) so the
# sound path is exercised; bundled ps2x.env does not set PS2X_SOUND.
set -u -o pipefail
W=/Users/brad/dev/ssx3-work/I29
...
trap cleanup EXIT
while [ -z "$SLOT" ]; do
  if ! SLOT=$(python3 /Users/brad/dev/ssx3/local/tooling/p_lane_lease.py claim I29-sim 2>/dev/null); then
    SLOT=''
...
xcrun simctl terminate "$SIM" "$APP" >/dev/null 2>&1 || true
SIMCTL_CHILD_PS2X_VSYNC_RATE_LOG=1 SIMCTL_CHILD_PS2X_SOUND=1 xcrun simctl launch --console-pty "$SIM" "$APP" > "$OUT/console.log" 2>&1 &
LP=$!
