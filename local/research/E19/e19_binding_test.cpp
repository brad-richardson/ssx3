...
int run(const char *path,bool bytewise) {
    ...
    auto snapshot=reinterpret_cast<E19Snapshot>(dlsym(RTLD_DEFAULT,"e19_parser_snapshot"));
    auto mark=reinterpret_cast<E19Mark>(dlsym(RTLD_DEFAULT,"e19_parser_mark"));
    ...
    size_t cursor=0;E19ParserStats last{};
    ...
    std::printf("# E19 ACTUAL PARSER FIXTURE TAIL COMPLETE boot=0 bytesFed=%zu available=%zu mode=%s saved128=1\n",cursor,payload.size(),bytewise?"byte":"marks");
    return 0;
}
}
extern "C" int e19_binding_main(int argc,char **argv) {
    try {
        if(argc==4 && std::string(argv[1])=="threshold")
            return e19_threshold::run(argv[2],std::string(argv[3])=="byte");
        return e18_binding_main(argc,argv);
    }catch(const std::exception &e) {std::fprintf(stderr,"E19 FIXTURE ERROR: %s\n",e.what());return 2;}
}
