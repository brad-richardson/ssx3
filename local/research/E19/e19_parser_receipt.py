...
from e19_closed_events import fields
...
def receipt(directory):
    ...
    lines=data.decode().splitlines();assert lines[-1].startswith('# E19 PARSER CLOSURE ')
    assert sum(l.startswith('# E19 PARSER CLOSURE ') for l in lines)==1
