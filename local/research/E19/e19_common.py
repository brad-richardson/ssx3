...
NEWB = Path('/tmp/e19-mpeg-link/runtime')
B = Path(os.environ.get('E19_BUILD_DIR',str(B0)))
OUT = Path('/tmp/e19-no-codegen')
P = W / 'P1'
M = 1024**2
G = 1024**3
...
def sample():
    paths = list(P.glob('e19-*')) + list(P.glob('._e19-*')) + list((P/'run').glob('*e19*'))
    # The shared function trace is charged even though its name lacks E19.
