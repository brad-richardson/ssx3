#!/usr/bin/env bash
# T47 Mission 1 F2 (v2): full software-rendered front-end reference run.
# v1 (t47-frontend.sh) NO_ZC_PARKed on a true park (p1-p3 fade hop 1.36);
# v2 adds +15s settles to ZC/SP/PC/RC legs and gates static on the LAST
# TWO hops only (thresholds = t44). Outputs t47b-* (F1 t47-* untouched).
# Run with SW_FIRST=1 (F9 right after WID detect; F1 proved the toggle:
# OSD [SwitchRenderer] @emu 565.76). Bounded: ~8 min wall. Unattended.
set -x
...
gshot() {
  ...
  cp "$SNAPS/$NEW" $D/t47b-shot-$LAB.png || fail GSHOT_CP_$LAB
  ls -la $D/t47b-shot-$LAB.png
}
stamp
pkill Xvfb || true
sleep 2
...
nohup $D/pcsx2/build/bin/pcsx2-qt -nogui -slowboot -turbo -datapath $D/dat -logfile $D/logs/boot-t47b.log -- "$D/inputs/SSX 3 (USA).iso" > $D/logs/boot-t47b.stdout 2>&1 &
echo $! > $D/pcsx2.pid
echo "PID:$!"
sleep 90
...
if [ "${SW_FIRST:-0}" = "1" ]; then
  ...
  sleep 5
  snap t47b-swcheck || fail SNAP_SWCHECK
fi
...
snap t47b-start || fail SNAP_START
read SM SP <<< $(score_band $D/t47b-start.ppm)
...
for N in 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 18; do
  sleep 2
  snap t47b-a1-poll$N || fail SNAP_A1_POLL$N
  read M P <<< $(score_band $D/t47b-a1-poll$N.ppm)
  plog "A1 poll$N score mean=$M p99=$P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
  if is_title $M $P; then
    plog "A1 TITLE-DETECTED at poll$N"
    cp $D/t47b-a1-poll$N.ppm $D/t47b-title.ppm
    cp $D/t47b-a1-poll$N.jpg $D/t47b-title.jpg
    DETECTED=1
    break
  fi
done
...
sleep 3; snap t47b-post3 || fail SNAP_POST3
read M3 P3 <<< $(score_band $D/t47b-post3.ppm)
plog "post+3 titleband mean=$M3 p99=$P3 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 5; snap t47b-post8 || fail SNAP_POST8
read M8 P8 <<< $(score_band $D/t47b-post8.ppm)
plog "post+8 titleband mean=$M8 p99=$P8"
sleep 7; snap t47b-post15 || fail SNAP_POST15
read M15 P15 <<< $(score_band $D/t47b-post15.ppm)
plog "post+15 titleband mean=$M15 p99=$P15"
sleep 10; snap t47b-post25 || fail SNAP_POST25
read M25 P25 <<< $(score_band $D/t47b-post25.ppm)
plog "post+25 titleband mean=$M25 p99=$P25"
W815=$(score_whole $D/t47b-post8.ppm $D/t47b-post15.ppm)
W1525=$(score_whole $D/t47b-post15.ppm $D/t47b-post25.ppm)
plog "whole815=$W815 whole1525=$W1525"
read MM25 MP25 <<< $(score_menu $D/t47b-post25.ppm)
...
sleep 5
snap t47b-menupre || fail SNAP_MENUPRE
read MMP MMP_P <<< $(score_menu $D/t47b-menupre.ppm)
plog "MENUPRE vs-menu mean=$MMP p99=$MMP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
gshot menu
press K "MENU_CROSS"
sleep 1; snap t47b-mc-post1 || fail SNAP_MC_POST1
read M1 P1 <<< $(score_band $D/t47b-mc-post1.ppm)
read MM1 MP1 <<< $(score_menu $D/t47b-mc-post1.ppm)
read MS1 MSP1 <<< $(score_sc $D/t47b-mc-post1.ppm)
plog "MC post+1 titleband mean=$M1 p99=$P1 vs-menu mean=$MM1 p99=$MP1 vs-sc mean=$MS1 p99=$MSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 2; snap t47b-mc-post3 || fail SNAP_MC_POST3
read M3 P3 <<< $(score_band $D/t47b-mc-post3.ppm)
read MM3 MP3 <<< $(score_menu $D/t47b-mc-post3.ppm)
read MS3 MSP3 <<< $(score_sc $D/t47b-mc-post3.ppm)
plog "MC post+3 titleband mean=$M3 p99=$P3 vs-menu mean=$MM3 p99=$MP3 vs-sc mean=$MS3 p99=$MSP3"
sleep 5; snap t47b-mc-post8 || fail SNAP_MC_POST8
read M8 P8 <<< $(score_band $D/t47b-mc-post8.ppm)
read MM8 MP8 <<< $(score_menu $D/t47b-mc-post8.ppm)
read MS8 MSP8 <<< $(score_sc $D/t47b-mc-post8.ppm)
plog "MC post+8 titleband mean=$M8 p99=$P8 vs-menu mean=$MM8 p99=$MP8 vs-sc mean=$MS8 p99=$MSP8"
sleep 7; snap t47b-mc-post15 || fail SNAP_MC_POST15
read M15 P15 <<< $(score_band $D/t47b-mc-post15.ppm)
read MM15 MP15 <<< $(score_menu $D/t47b-mc-post15.ppm)
read MS15 MSP15 <<< $(score_sc $D/t47b-mc-post15.ppm)
plog "MC post+15 titleband mean=$M15 p99=$P15 vs-menu mean=$MM15 p99=$MP15 vs-sc mean=$MS15 p99=$MSP15"
H_PRE1=$(score_whole $D/t47b-menupre.ppm $D/t47b-mc-post1.ppm)
H_13=$(score_whole $D/t47b-mc-post1.ppm $D/t47b-mc-post3.ppm)
H_38=$(score_whole $D/t47b-mc-post3.ppm $D/t47b-mc-post8.ppm)
H_815=$(score_whole $D/t47b-mc-post8.ppm $D/t47b-mc-post15.ppm)
...
sleep 5
snap t47b-scpre || fail SNAP_SCPRE
read SCP SCP_P <<< $(score_sc $D/t47b-scpre.ppm)
plog "SCPRE vs-sc mean=$SCP p99=$SCP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
gshot sc
press K "ZOE_CROSS"
sleep 1; snap t47b-zc-post1 || fail SNAP_ZC_POST1
read Z1 ZP1 <<< $(score_band $D/t47b-zc-post1.ppm)
read ZS1 ZSP1 <<< $(score_sc $D/t47b-zc-post1.ppm)
read ZZ1 ZZP1 <<< $(score_zc $D/t47b-zc-post1.ppm)
plog "ZC post+1 titleband mean=$Z1 p99=$ZP1 vs-sc mean=$ZS1 p99=$ZSP1 vs-zc mean=$ZZ1 p99=$ZZP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 2; snap t47b-zc-post3 || fail SNAP_ZC_POST3
read Z3 ZP3 <<< $(score_band $D/t47b-zc-post3.ppm)
read ZS3 ZSP3 <<< $(score_sc $D/t47b-zc-post3.ppm)
read ZZ3 ZZP3 <<< $(score_zc $D/t47b-zc-post3.ppm)
plog "ZC post+3 titleband mean=$Z3 p99=$ZP3 vs-sc mean=$ZS3 p99=$ZSP3 vs-zc mean=$ZZ3 p99=$ZZP3"
sleep 5; snap t47b-zc-post8 || fail SNAP_ZC_POST8
read Z8 ZP8 <<< $(score_band $D/t47b-zc-post8.ppm)
read ZS8 ZSP8 <<< $(score_sc $D/t47b-zc-post8.ppm)
read ZZ8 ZZP8 <<< $(score_zc $D/t47b-zc-post8.ppm)
plog "ZC post+8 titleband mean=$Z8 p99=$ZP8 vs-sc mean=$ZS8 p99=$ZSP8 vs-zc mean=$ZZ8 p99=$ZZP8"
sleep 7; snap t47b-zc-post15 || fail SNAP_ZC_POST15
read Z15 ZP15 <<< $(score_band $D/t47b-zc-post15.ppm)
read ZS15 ZSP15 <<< $(score_sc $D/t47b-zc-post15.ppm)
read ZZ15 ZZP15 <<< $(score_zc $D/t47b-zc-post15.ppm)
plog "ZC post+15 titleband mean=$Z15 p99=$ZP15 vs-sc mean=$ZS15 p99=$ZSP15 vs-zc mean=$ZZ15 p99=$ZZP15"
ZH_PRE1=$(score_whole $D/t47b-scpre.ppm $D/t47b-zc-post1.ppm)
ZH_38=$(score_whole $D/t47b-zc-post3.ppm $D/t47b-zc-post8.ppm)
ZH_815=$(score_whole $D/t47b-zc-post8.ppm $D/t47b-zc-post15.ppm)
plog "ZC hops whole: pre-post1=$ZH_PRE1 p3-p8=$ZH_38 p8-p15=$ZH_815"
if is_departed $ZH_PRE1 && is_nonsc $ZS15 && is_static $ZH_38 && is_static $ZH_815; then
  plog "ZC-LIKE -> Continue Cross"
else
  fail NO_ZC_PARK
fi
sleep 5
snap t47b-ccpre || fail SNAP_CCPRE
read CZP CZP_P <<< $(score_zc $D/t47b-ccpre.ppm)
plog "CCPRE vs-zc mean=$CZP p99=$CZP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
gshot zc
press K "CONT_CROSS"
sleep 1; snap t47b-sp-post1 || fail SNAP_SP_POST1
read S1 SP1 <<< $(score_band $D/t47b-sp-post1.ppm)
read SZ1 SZP1 <<< $(score_zc $D/t47b-sp-post1.ppm)
read SS1 SSP1 <<< $(score_sp $D/t47b-sp-post1.ppm)
plog "SP post+1 titleband mean=$S1 p99=$SP1 vs-zc mean=$SZ1 p99=$SZP1 vs-sp mean=$SS1 p99=$SSP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 2; snap t47b-sp-post3 || fail SNAP_SP_POST3
read S3 SP3 <<< $(score_band $D/t47b-sp-post3.ppm)
read SZ3 SZP3 <<< $(score_zc $D/t47b-sp-post3.ppm)
read SS3 SSP3 <<< $(score_sp $D/t47b-sp-post3.ppm)
plog "SP post+3 titleband mean=$S3 p99=$SP3 vs-zc mean=$SZ3 p99=$SZP3 vs-sp mean=$SS3 p99=$SSP3"
sleep 5; snap t47b-sp-post8 || fail SNAP_SP_POST8
read S8 SP8 <<< $(score_band $D/t47b-sp-post8.ppm)
read SZ8 SZP8 <<< $(score_zc $D/t47b-sp-post8.ppm)
read SS8 SSP8 <<< $(score_sp $D/t47b-sp-post8.ppm)
plog "SP post+8 titleband mean=$S8 p99=$SP8 vs-zc mean=$SZ8 p99=$SZP8 vs-sp mean=$SS8 p99=$SSP8"
sleep 7; snap t47b-sp-post15 || fail SNAP_SP_POST15
read S15 SP15 <<< $(score_band $D/t47b-sp-post15.ppm)
read SZ15 SZP15 <<< $(score_zc $D/t47b-sp-post15.ppm)
read SS15 SSP15 <<< $(score_sp $D/t47b-sp-post15.ppm)
plog "SP post+15 titleband mean=$S15 p99=$SP15 vs-zc mean=$SZ15 p99=$SZP15 vs-sp mean=$SS15 p99=$SSP15"
SPH_PRE1=$(score_whole $D/t47b-ccpre.ppm $D/t47b-sp-post1.ppm)
SPH_38=$(score_whole $D/t47b-sp-post3.ppm $D/t47b-sp-post8.ppm)
SPH_815=$(score_whole $D/t47b-sp-post8.ppm $D/t47b-sp-post15.ppm)
...
sleep 5
snap t47b-sppre || fail SNAP_SPPRE
read SPP SPP_P <<< $(score_sp $D/t47b-sppre.ppm)
plog "SPPRE vs-sp mean=$SPP p99=$SPP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
gshot sp
press K "PEAK_CROSS"
sleep 1; snap t47b-pc-post1 || fail SNAP_PC_POST1
read P1 PP1 <<< $(score_band $D/t47b-pc-post1.ppm)
read PS1 PSP1 <<< $(score_sp $D/t47b-pc-post1.ppm)
read PX1 PXP1 <<< $(score_sm $D/t47b-pc-post1.ppm)
plog "PC post+1 titleband mean=$P1 p99=$PP1 vs-sp mean=$PS1 p99=$PSP1 vs-sm mean=$PX1 p99=$PXP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 2; snap t47b-pc-post3 || fail SNAP_PC_POST3
read P3 PP3 <<< $(score_band $D/t47b-pc-post3.ppm)
read PS3 PSP3 <<< $(score_sp $D/t47b-pc-post3.ppm)
read PX3 PXP3 <<< $(score_sm $D/t47b-pc-post3.ppm)
plog "PC post+3 titleband mean=$P3 p99=$PP3 vs-sp mean=$PS3 p99=$PSP3 vs-sm mean=$PX3 p99=$PXP3"
sleep 5; snap t47b-pc-post8 || fail SNAP_PC_POST8
read P8 PP8 <<< $(score_band $D/t47b-pc-post8.ppm)
read PS8 PSP8 <<< $(score_sp $D/t47b-pc-post8.ppm)
read PX8 PXP8 <<< $(score_sm $D/t47b-pc-post8.ppm)
plog "PC post+8 titleband mean=$P8 p99=$PP8 vs-sp mean=$PS8 p99=$PSP8 vs-sm mean=$PX8 p99=$PXP8"
sleep 7; snap t47b-pc-post15 || fail SNAP_PC_POST15
read P15 PP15 <<< $(score_band $D/t47b-pc-post15.ppm)
read PS15 PSP15 <<< $(score_sp $D/t47b-pc-post15.ppm)
read PX15 PXP15 <<< $(score_sm $D/t47b-pc-post15.ppm)
plog "PC post+15 titleband mean=$P15 p99=$PP15 vs-sp mean=$PS15 p99=$PSP15 vs-sm mean=$PX15 p99=$PXP15"
PCH_PRE1=$(score_whole $D/t47b-sppre.ppm $D/t47b-pc-post1.ppm)
PCH_38=$(score_whole $D/t47b-pc-post3.ppm $D/t47b-pc-post8.ppm)
PCH_815=$(score_whole $D/t47b-pc-post8.ppm $D/t47b-pc-post15.ppm)
...
sleep 5
snap t47b-smpre || fail SNAP_SMPRE
read SXP SXP_P <<< $(score_sm $D/t47b-smpre.ppm)
plog "SMPRE vs-sm mean=$SXP p99=$SXP_P MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
gshot sm
press K "RACE_CROSS"
sleep 1; snap t47b-rc-post1 || fail SNAP_RC_POST1
read R1 RP1 <<< $(score_band $D/t47b-rc-post1.ppm)
read RX1 RXP1 <<< $(score_sm $D/t47b-rc-post1.ppm)
read RT1 RTP1 <<< $(score_setag $D/t47b-rc-post1.ppm)
plog "RC post+1 titleband mean=$R1 p99=$RP1 vs-sm mean=$RX1 p99=$RXP1 se-tag mean=$RT1 p99=$RTP1 MENU_T+:$(( $(date -u +%s) - T_BOOT ))"
sleep 2; snap t47b-rc-post3 || fail SNAP_RC_POST3
read R3 RP3 <<< $(score_band $D/t47b-rc-post3.ppm)
read RX3 RXP3 <<< $(score_sm $D/t47b-rc-post3.ppm)
read RT3 RTP3 <<< $(score_setag $D/t47b-rc-post3.ppm)
plog "RC post+3 titleband mean=$R3 p99=$RP3 vs-sm mean=$RX3 p99=$RXP3 se-tag mean=$RT3 p99=$RTP3"
sleep 5; snap t47b-rc-post8 || fail SNAP_RC_POST8
read R8 RP8 <<< $(score_band $D/t47b-rc-post8.ppm)
read RX8 RXP8 <<< $(score_sm $D/t47b-rc-post8.ppm)
read RT8 RTP8 <<< $(score_setag $D/t47b-rc-post8.ppm)
read RS8 RSP8 <<< $(score_sp $D/t47b-rc-post8.ppm)
read RW8 RWP8 <<< $(score_se $D/t47b-rc-post8.ppm)
...
sleep 7; snap t47b-rc-post15 || fail SNAP_RC_POST15
read R15 RP15 <<< $(score_band $D/t47b-rc-post15.ppm)
read RX15 RXP15 <<< $(score_sm $D/t47b-rc-post15.ppm)
read RT15 RTP15 <<< $(score_setag $D/t47b-rc-post15.ppm)
read RS15 RSP15 <<< $(score_sp $D/t47b-rc-post15.ppm)
read RW15 RWP15 <<< $(score_se $D/t47b-rc-post15.ppm)
...
RCH_38=$(score_whole $D/t47b-rc-post3.ppm $D/t47b-rc-post8.ppm)
RCH_815=$(score_whole $D/t47b-rc-post8.ppm $D/t47b-rc-post15.ppm)
...
sleep 5
snap t47b-sepre || fail SNAP_SEPRE
read STP STP_P <<< $(score_setag $D/t47b-sepre.ppm)
...
for N in 1 2 3 4; do
  ...
  sleep 3; snap t47b-se-down$N || fail SNAP_SE_DOWN$N
  read W WP <<< $(score_se $D/t47b-se-down$N.ppm)
  read T TP <<< $(score_setag $D/t47b-se-down$N.ppm)
  plog "SE_DOWN$N vs-se mean=$W p99=$WP se-tag mean=$T p99=$TP"
done
snap t47b-se-walked || fail SNAP_SE_WALKED
gshot se-walked
press Right "SE_RIGHT1"
sleep 2; snap t47b-se-right1 || fail SNAP_SE_RIGHT1
press Left "SE_LEFT1"
sleep 2; snap t47b-se-left1 || fail SNAP_SE_LEFT1
press Left "SE_LEFT2"
sleep 2; snap t47b-se-left2 || fail SNAP_SE_LEFT2
gshot se-final
if [ "${SW_FIRST:-0}" = "1" ]; then
  ...
  press F9 "SW_TOGGLE"
  sleep 5
  snap t47b-se-sw || fail SNAP_SE_SW
  gshot se-sw
fi
ls -la $D/t47b-*.jpg $D/t47b-shot-*.png $E
