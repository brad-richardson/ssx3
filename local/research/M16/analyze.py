...
def summarise(label, path):
    ...
    out = {
        ...
        "m12_wins": [e.get("detail", "") for e in by_action.get("m12_win", [])],
        "m12_nops": [e.get("detail", "") for e in by_action.get("m12_nop", [])],
        "m12_ref2s": [e.get("detail", "") for e in by_action.get("m12_ref2", [])],
        "m13_wins": [e.get("detail", "") for e in by_action.get("m13_win", [])],
        "m13_nops": [e.get("detail", "") for e in by_action.get("m13_nop", [])],
        "m13_ref2s": [e.get("detail", "") for e in by_action.get("m13_ref2", [])],
        "m14_wins": [e.get("detail", "") for e in by_action.get("m14_win", [])],
        "m14_ref2s": [e.get("detail", "") for e in by_action.get("m14_ref2", [])],
        "m15_wins": [e.get("detail", "") for e in by_action.get("m15_win", [])],
        "m15_ref2s": [e.get("detail", "") for e in by_action.get("m15_ref2", [])],
        "m16_wins": [e.get("detail", "") for e in by_action.get("m16_win", [])],
        "m16_ref2s": [e.get("detail", "") for e in by_action.get("m16_ref2", [])],
...
def main():
    ...
    for r in results:
        ...
        if r["m11_nops"]:
            ...
            print()
        if r["m12_wins"]:
            print(f"{r['label']}: m12_win detail = {r['m12_wins'][0]}")
            print()
        if r["m12_nops"]:
            d = r["m12_nops"][0]
            print(f"{r['label']}: M12 per-shape NOP spans: "
                  f"shr_ok={kv(d, 'shr_ok', '?')} shr_draws={kv(d, 'shr_draws', '?')} "
                  f"shr_bytes={kv(d, 'shr_bytes', '?')} "
                  f"idx_ok={kv(d, 'idx_ok', '?')} idx_draws={kv(d, 'idx_draws', '?')} "
                  f"idx_bytes={kv(d, 'idx_bytes', '?')} "
                  f"text_ok={kv(d, 'text_ok', '?')} text_draws={kv(d, 'text_draws', '?')} "
                  f"text_bytes={kv(d, 'text_bytes', '?')}")
            print()
        if r["m12_ref2s"]:
            wins = {}
            for d in r["m12_ref2s"]:
                try:
                    wins[int(kv(d, "win", "-1"))] = d
                except (TypeError, ValueError):
                    pass
            print(f"{r['label']}: same-frame per-window ref2 rows (scheme mod4, "
                  f"one pristine ref per window shape)")
            print("| win | name | ref | kref | refok | n2 | gt0 | uniform2 | "
                  "xd2min | xd2max | xdmax2max | xdmean2min | xdmean2max | refhash |")
            print("| ---: | --- | :-: | ---: | :-: | ---: | ---: | ---: | ---: | "
                  "---: | ---: | ---: | ---: | --- |")
            for w in sorted(wins):
                d = wins[w]
                print(f"| {w} | {kv(d, 'name', '?')} | {kv(d, 'ref', '?')} | "
                      f"{kv(d, 'kref', '?')} | {kv(d, 'refok', '?')} | "
                      f"{kv(d, 'n2', '?')} | {kv(d, 'gt0', '?')} | "
                      f"{kv(d, 'uniform2', '?')} | {kv(d, 'xd2min', '?')} | "
                      f"{kv(d, 'xd2max', '?')} | {kv(d, 'xdmax2max', '?')} | "
                      f"{kv(d, 'xdmean2min', '?')} | {kv(d, 'xdmean2max', '?')} | "
                      f"{kv(d, 'refhash', '?')} |")
                print(f"{r['label']}: win {w} samp = {kv(d, 'samp', '?')}")
            if 0 in wins:
                try:
                    total = int(kv(wins[0], "xd2max", "0"))
                    print(f"{r['label']}: subtracted shares (total win0 xd2max={total} "
                          f"minus surviving window xd2max, same frame)")
                    print("| removed set | surviving win | surviving xd2 | "
                          "removed share (total - surviving) |")
                    print("| --- | ---: | ---: | ---: |")
                    for w, name in ((1, "shared-path (nopshr)"),
                                    (2, "indexed-path (nopidx)"),
                                    (3, "texture-epoch (notext)")):
                        s = int(kv(wins[w], "xd2max", "0")) if w in wins else None
                        share = total - s if s is not None else None
                        print(f"| {name} | {w} | {s if s is not None else 'n/a'} | "
                              f"{share if share is not None else 'n/a'} |")
                except (TypeError, ValueError):
                    pass
            print()
        if r["m13_wins"]:
            print(f"{r['label']}: m13_win detail = {r['m13_wins'][0]}")
            print()
        if r["m13_nops"]:
            print(f"{r['label']}: M13 per-draw NOP spans: n={len(r['m13_nops'])}")
            print("| shape | clk | ok | draws | bytes |")
            print("| ---: | ---: | :-: | ---: | ---: |")
            for d in r["m13_nops"]:
                print(f"| {kv(d, 'shape', '?')} | {kv(d, 'clk', '?')} | "
                      f"{kv(d, 'ok', '?')} | {kv(d, 'draws', '?')} | "
                      f"{kv(d, 'bytes', '?')} |")
            print()
        if r["m13_ref2s"]:
            shapes = {}
            for d in r["m13_ref2s"]:
                try:
                    shapes[int(kv(d, "shape", "-1"))] = d
                except (TypeError, ValueError):
                    pass
            print(f"{r['label']}: per-draw leave-one-out ref2 rows (scheme modN, "
                  f"one pristine ref per shape)")
            print("| shape | clk | name | ref | kref | refok | n2 | gt0 | uniform2 | "
                  "xd2min | xd2max | xdmax2max | xdmean2min | xdmean2max | refhash |")
            print("| ---: | ---: | --- | :-: | ---: | :-: | ---: | ---: | ---: | ---: | "
                  "---: | ---: | ---: | ---: | --- |")
            for w in sorted(shapes):
                d = shapes[w]
                print(f"| {w} | {kv(d, 'clk', '?')} | {kv(d, 'name', '?')} | "
                      f"{kv(d, 'ref', '?')} | {kv(d, 'kref', '?')} | "
                      f"{kv(d, 'refok', '?')} | {kv(d, 'n2', '?')} | "
                      f"{kv(d, 'gt0', '?')} | {kv(d, 'uniform2', '?')} | "
                      f"{kv(d, 'xd2min', '?')} | {kv(d, 'xd2max', '?')} | "
                      f"{kv(d, 'xdmax2max', '?')} | {kv(d, 'xdmean2min', '?')} | "
                      f"{kv(d, 'xdmean2max', '?')} | {kv(d, 'refhash', '?')} |")
                print(f"{r['label']}: shape {w} samp = {kv(d, 'samp', '?')}")
            if 0 in shapes:
                try:
                    total = int(kv(shapes[0], "xd2max", "0"))
                    print(f"{r['label']}: per-draw removed shares (total shape0 "
                          f"xd2max={total} minus surviving shape xd2max, same frame)")
                    print("| left-out draw clk | surviving shape | surviving xd2 | "
                          "removed share (total - surviving) |")
                    print("| ---: | ---: | ---: | ---: |")
                    for w in sorted(shapes):
                        if w == 0:
        ...
        if r["m13_ref2s"]:
            ...
            for d in r["m13_ref2s"]:
                try:
                    sh = int(kv(d, "shape", "-1"))
                    xd = int(kv(d, "xd2max", "0"))
                except (TypeError, ValueError):
                    continue
                if sh == 0:
                    tote = xd
                else:
                    scored.append((xd, sh, d))
            if tote is not None and scored:
                scored.sort(key=lambda t: tote - t[0], reverse=True)
                print(f"{r['label']}: top-10 carriers by removed share "
                      f"(total shape0 xd2max={tote}, n={len(scored)})")
                print("| rank | clk | shape | surviving xd2 | removed share | samp |")
                print("| ---: | ---: | ---: | ---: | ---: | --- |")
                for rank, (xd, sh, d) in enumerate(scored[:10], 1):
                    print(f"| {rank} | {kv(d, 'clk', '?')} | {sh} | {xd} | "
                          f"{tote - xd} | {kv(d, 'samp', '?')} |")
                print()
        if r["m14_wins"]:
            print(f"{r['label']}: m14_win detail = {r['m14_wins'][0]}")
            print()
        if r["m14_ref2s"]:
            prows = []
            for d in r["m14_ref2s"]:
                try:
                    prows.append((int(kv(d, "r", "-1")), d))
                except (TypeError, ValueError):
                    pass
            prows.sort()
            print(f"{r['label']}: per-replay moved-bytes rows (vs rendered ref, "
                  f"n={len(prows)})")
            print("| r | phase | ok | xd2 | xdmax | xdmean | hash |")
            print("| ---: | --- | :-: | ---: | ---: | ---: | --- |")
            for _, d in prows:
                print(f"| {kv(d, 'r', '?')} | {kv(d, 'phase', '?')} | "
                      f"{kv(d, 'ok', '?')} | {kv(d, 'xd2', '?')} | "
                      f"{kv(d, 'xdmax', '?')} | {kv(d, 'xdmean', '?')} | "
                      f"{kv(d, 'hash', '?')} |")
            for phase in ("pristine", "delta"):
                sub = [d for _, d in prows if kv(d, "phase", "") == phase]
                xds = []
                for d in sub:
                    try:
                        xds.append(int(kv(d, "xd2", "0")))
                    except (TypeError, ValueError):
                        pass
                hist = {}
                for x in xds:
                ...
                for d in sub:
                    h = kv(d, "hash", "?")
                    hs[h] = hs.get(h, 0) + 1
                print(f"{r['label']}: moved-bytes distribution ({phase} "
                      f"n={len(sub)}): xd2 min={min(xds) if xds else 'n/a'} "
                      f"max={max(xds) if xds else 'n/a'} distinct={len(hist)} "
                      f"hist={dict(sorted(hist.items()))} "
                      f"distinct_hashes={len(hs)}")
            print()
        if r["m15_wins"]:
            print(f"{r['label']}: m15_win detail = {r['m15_wins'][0]}")
            print()
        if r["m15_ref2s"]:
            prows = []
            for d in r["m15_ref2s"]:
                try:
                    prows.append((int(kv(d, "r", "-1")), d))
                except (TypeError, ValueError):
                    pass
            prows.sort()
            print(f"{r['label']}: per-replay moved-bytes rows (vs v0 ref, "
                  f"n={len(prows)})")
            print("| r | phase | ok | xd2 | xdmax | xdmean | hash |")
            print("| ---: | --- | :-: | ---: | ---: | ---: | --- |")
            for _, d in prows:
            ...
            print()
        if r["m16_wins"]:
            print(f"{r['label']}: m16_win detail = {r['m16_wins'][0]}")
            print()
        if r["m16_ref2s"]:
            mrows = []
            for d in r["m16_ref2s"]:
                try:
                    mrows.append((int(kv(d, "r", "-1")), d))
                except (TypeError, ValueError):
                    pass
            mrows.sort()
            byshape = {}
            for _, d in mrows:
                try:
                    ...
                    continue
                byshape.setdefault(sh, {})[(kv(d, "phase", "?"), kv(d, "rep", "?"))] = d
            print(f"{r['label']}: per-(shape,phase) a/b uniformity "
                  f"(nshapes={len(byshape)})")
            print("| shape | clk | v0 um | mid xd_a | mid xd_b | mid um | "
                  "full xd_a | full xd_b | full um |")
            print("| ---: | ---: | :-: | ---: | ---: | :-: | ---: | ---: | :-: |")
            for sh in sorted(byshape):
                ...
                fa = g.get(("full", "a"), "")
                fb = g.get(("full", "b"), "")
                clk = kv(v0 or ma or fa, "clk", "?")
                print(f"| {sh} | {clk} | {kv(v0, 'um', '?')} | "
                      f"{kv(ma, 'xd2', '?')} | {kv(mb, 'xd2', '?')} | "
                      f"{kv(ma, 'um', '?')} | {kv(fa, 'xd2', '?')} | "
                      f"{kv(fb, 'xd2', '?')} | {kv(fa, 'um', '?')} |")
            print()
            for phase, rep in (("v0", "a"), ("v0", "b"), ("mid", "a"),
                               ("mid", "b"), ("full", "a"), ("full", "b")):
                sub = [d for _, d in mrows
                       if kv(d, "phase", "") == phase and kv(d, "rep", "") == rep]
                ok = sum(1 for d in sub if kv(d, "ok", "0") == "1")
                um1 = sum(1 for d in sub if kv(d, "um", "0") == "1")
                xds = []
                for d in sub:
                    try:
                        xds.append(int(kv(d, "xd2", "0")))
                ...
                for d in sub:
                    h = kv(d, "hash", "?")
                    hs[h] = hs.get(h, 0) + 1
                print(f"{r['label']}: m16 group ({phase}{rep} n={len(sub)} "
                      f"ok={ok} um1={um1}): xd2 min={min(xds) if xds else 'n/a'} "
                      f"max={max(xds) if xds else 'n/a'} distinct={len(set(xds))} "
                      f"distinct_hashes={len(hs)}")
            bad = [(kv(d, "shape", "?"), kv(d, "phase", "?"), kv(d, "rep", "?"),
                    kv(d, "r", "?"), kv(d, "hash", "?"))
                   for _, d in mrows
                   if kv(d, "ok", "0") == "1" and kv(d, "um", "0") != "1"]
            print(f"{r['label']}: m16 non-uniform ok rows (um!=1): n={len(bad)}")
            for sh, ph, rep, rr, h in bad:
                print(f"{r['label']}: non-uniform shape={sh} phase={ph} "
                      f"rep={rep} r={rr} hash={h}")
            print(f"{r['label']}: m16 per-replay rows (n={len(mrows)})")
            print("| r | shape | clk | phase | rep | ok | xd2 | xdmax | "
                  "xdmean | hash | um |")
            print("| ---: | ---: | ---: | --- | :-: | :-: | ---: | ---: | "
                  "---: | --- | :-: |")
            for _, d in mrows:
                print(f"| {kv(d, 'r', '?')} | {kv(d, 'shape', '?')} | "
                      f"{kv(d, 'clk', '?')} | {kv(d, 'phase', '?')} | "
                      f"{kv(d, 'rep', '?')} | {kv(d, 'ok', '?')} | "
                      f"{kv(d, 'xd2', '?')} | {kv(d, 'xdmax', '?')} | "
                      f"{kv(d, 'xdmean', '?')} | {kv(d, 'hash', '?')} | "
                      f"{kv(d, 'um', '?')} |")
            print()
    for r in results:
        if r["xdiff_rows"]:
