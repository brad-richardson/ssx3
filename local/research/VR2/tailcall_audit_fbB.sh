...
set -euo pipefail
bin=$1
nm -n $bin | grep -E "VU1RecompImage.*5[fbB][0-9a-f]{4}ER14VU1" > /tmp/vb1-audit-syms.$$ || true
...
/^[0-9a-f]+ <.*VU1RecompImage.*5[fbB][0-9a-f][0-9a-f][0-9a-f][0-9a-f]ER14VU1/ {cur=$2; n++; next}
/^[0-9a-f]+ </ {cur=""; next}
cur!="" && /blr/ {blr[cur]=1}
