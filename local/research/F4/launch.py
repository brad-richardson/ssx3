#!/usr/bin/env python3
"""F4 Odin speed runs + race profile: N11 launcher adapted for the F4 fold build (fork 8559ab9).

Differences from N10: OUT text logs stay in git, binaries (PNG/perf.data) go
to scratch; mc0-test empty card (Brad's mc0 never touched); full F4 env with
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/F4/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/F4/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('F4-')):
...
sh(f'echo "F4 {time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())} {a.label}" > /data/local/tmp/mg/LEASE')
CLAIMED['v'] = True
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE F4 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# F4 {a.label}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
sh("echo 'LEASE_FREE F4 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
