#!/usr/bin/env bash
# T63 run 1 (from T58 run 3 int): cold REC boot along T48's route to the main menu, F1-save a
...
leg() {
  ...
  while [ $TRY -lt 3 ]; do
    ...
    sleep 25; wallcheck; snap t63r1-$LAB-t$TRY || fail SNAP_${LAB}
    read M P <<< $(score_ref $T4/t63r1-$LAB-t$TRY.ppm $EXP)
...
for N in $(seq 1 70); do
  sleep 3; wallcheck
  snap t63r1-a1-poll$N || fail SNAP_A1_POLL$N
  read M P <<< $(score_band $T4/t63r1-a1-poll$N.ppm)
...
snap t63r1-menu-pre || fail SNAP_MENU_PRE
read MM MP <<< $(score_ref $T4/t63r1-menu-pre.ppm $REFMENU)
...
cp "$SSD/$STATE_NEWEST" $G/t63-menu-state || fail STATE_CP
sha256sum $G/t63-menu-state | tee -a $LOG
ls -la $G/t63-menu-state
plog "EMULOG size=$(stat -c %s "$E") tw=$(grep -c '^.*tw vsync=' "$E" || true) twbase=$(grep -c '^.*twbase vsync=' "$E" || true) captw=$(grep -c 'T63_CAPTW' "$E" || true)"
sha256sum "$E" | tee -a $LOG
