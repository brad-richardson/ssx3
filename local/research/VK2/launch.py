#!/usr/bin/env python3
"""VK2 Odin launcher (VK1 copy + --stress): N12's launch.py (F5 lineage: play-env pin a8d651a7, mc0 pins,
transport record, cool-down done by cooldown.py first) plus:
  --variant A|B|VA|VB  A = 1x pipelined, B = 4x+hi-res pipelined (F5's legs);
                       VA/VB = the same + PS2X_PRESENT_VULKAN=1 (VK1 prototype)
...
DSCRAP = '/data/local/tmp/vk2'
BRAD_ENV_SHA = 'a8d651a7e55f7e29f1345f28f4ea634e06ae757f535e29439ab61c930bcc0ebd'
...
ap.add_argument('--stress', type=int, default=0,
                help='VK2: at this tick, 20 background/foreground cycles + 5 app switches, logging '
                     'fds, SurfaceFlinger layers and vk_* counters (stress.txt); screencap every 5 cycles')
ap.add_argument('--after-stress-s', type=float, default=60.0,
                help='VK2: keep running this long after the stress, then stop')
a = ap.parse_args()

OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/VK2/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/VK2/odin/{a.label}')
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE VK2 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# VK2 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
def proc_fds(pid):
    n = sh(f'ls /proc/{pid}/fd 2>/dev/null | wc -l').strip()
    if n in ('', '0'):
        n = 'run-as:' + sh(f'run-as {PKG} sh -c "ls /proc/{pid}/fd | wc -l"').strip()
    return n


def sf_layers():
    lst = sh('dumpsys SurfaceFlinger --list').splitlines()
    return sum('ps2x-game' in l for l in lst), sum(PKG in l for l in lst)


def last_line(pat):
    try:
        lines = [l for l in open(f'{OUT}/logcat.txt', errors='replace') if pat in l]
    except OSError:
        lines = []
...
def stress_snap(tag):
    global tick
    try:
        m = rate_re.findall(open(f'{OUT}/logcat.txt', errors='replace').read())
        if m:
            tick = int(m[-1][0])
    except OSError:
        pass
    pid = sh(f'pidof {PKG}').strip()
    g, allp = sf_layers()
    fds = proc_fds(pid) if pid else 'no-pid'
    focus = sh('dumpsys window | grep -m1 mCurrentFocus').strip()[-60:]
    vk = last_line('window-change stats')
    vk = vk[vk.find('vk_queued'):] if 'vk_queued' in vk else ''
    line = (f't={time.time() - T0:.1f} tick={tick} {tag} pid={pid} fds={fds} sf_ps2x_game={g} sf_pkg_layers={allp} '
            f'focus={focus!r} last_window_change: {vk}')
    with open(f'{OUT}/stress.txt', 'a') as f:
        f.write(line + '\n')
    log('STRESS ' + line[:200])


def run_stress():
    # 20 HOME -> back cycles with varied background time (0.5 s hits a fast
    # recreate with completions outstanding), then 5 switches to Settings and back.
    stress_snap('start')
    bg_waits = [0.5, 2.0, 5.0, 1.0]
    for i in range(1, 21):
        sh('input keyevent 3')
        time.sleep(bg_waits[i % len(bg_waits)])
        stress_snap(f'bg{i}')
        sh(f'am start -n {PKG}/android.app.NativeActivity')
        time.sleep(4)
        stress_snap(f'fg{i}')
        if i % 5 == 0:
            scap(f'stress-cycle{i}')
    for j in range(1, 6):
        sh('am start -a android.settings.SETTINGS')
        time.sleep(2.5)
        stress_snap(f'switch{j}-away')
        sh(f'am start -n {PKG}/android.app.NativeActivity')
        time.sleep(4)
        stress_snap(f'switch{j}-back')
    scap('stress-end')
...
while True:
    ...
    if a.stress and not stress_done and tick >= a.stress:
        stress_done = True
        log(f'STRESS start tick={tick}')
        run_stress()
        a.wall = min(a.wall, time.time() - T0 + a.after_stress_s)
        log(f'STRESS end tick={tick}; stopping by t+{a.wall:.0f}')
