#!/bin/bash
# VK2 close-out (VK1 copy): put Brad's F5 play build back exactly (brief rule): F5 APK
...
adb -s "$S" shell "rm -rf /data/local/tmp/vk2 $FILES/vk2dump; rm -rf $FILES/mc0-test/*; ls -A $FILES/mc0-test | wc -l; am force-stop $PKG"
