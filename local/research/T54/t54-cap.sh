#!/usr/bin/env bash
# T54 capture: SC-settled statefile boot, walker census whole boot (+ optional
# /tmp/t54-watch producer watch). Foreground run (T51 §T51-6: no detached boots
# on bytesize). One held ssh.
# Like T53: free-gate files are touched for the shared T50/T51 gates; the T54
# hooks themselves are ungated (statefile boot lands SC-settled).
# Env: TAG=t54a (default), WATCH_SRC= path to a watch list file to install as
# /tmp/t54-watch (absent = census only, dprod never arms).
set -x
...
WATCH_SRC=${WATCH_SRC:-}
LOG=$T4/$TAG-poll.log
fail() { echo "T54_FAIL:$1"; date -u; exit 1; }
plog() { echo "$@" | tee -a $LOG; }
[ -x $BIN ] || fail NO_BIN
...
: > $LOG
date -u
rm -f /tmp/t48-arm /tmp/t48-dump-now /tmp/t49-arm /tmp/t50-arm /tmp/t50-free /tmp/t51-arm /tmp/t51-free /tmp/t51-watch /tmp/t53-watch /tmp/t54-watch
touch /tmp/t48-arm /tmp/t50-arm /tmp/t51-arm
if [ -n "$WATCH_SRC" ]; then
  cp "$WATCH_SRC" /tmp/t54-watch || fail WATCH_STAGE
fi
cat /tmp/t54-watch 2>/dev/null || plog "NO_WATCH_FILE (census only)"
...
plog "COUNTS drec=$(grep -c '^.*drec vsync=' $E || true) drecs=$(grep -c 'drecs vsync=' $E || true) dprod=$(grep -c 'dprod vsync=' $E || true) armed=$(grep -c 'T54W_ARMED' $E || true) cap=$(grep -c 'T54W_CAP' $E || true) ctag=$(grep -c 'ctag ' $E || true)"
grep -m 3 "T54W_ARMED" $E || true
