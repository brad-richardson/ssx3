#!/usr/bin/env python3
"""F7 Odin launcher: F6's launch.py (F5 lineage: play-env pin a8d651a7, mc0 pins,
transport record, cool-down done by cooldown.py first; lease/dirs repointed at F7).
F7 matrix: variant A (1x pipelined, Vulkan present default on) on MT1's APK
(b91f827f…), knob legs via --env (PS2X_MTVU / PS2X_MTVU_LAG / PS2X_VU1_BLOCKS /
PS2X_MTVU_CENSUS_OUT plus the CPU pins on every leg).
...
OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/F7/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/F7/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('F7 ')):
...
if lease.startswith('F7 '):
    CLAIMED['v'] = True  # outer hold (or a stale F7 lease); we own it
else:
    r = subprocess.run(['bash', LEASESH, 'claim', 'F7'], capture_output=True, text=True, timeout=60)
    if r.returncode != 0:
...
def lease_release():
    subprocess.run(['bash', LEASESH, 'release', 'F7'], capture_output=True, timeout=60)
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['bash', LEASESH, 'release', 'F7'],
                           capture_output=True, timeout=60)
    except Exception:
        pass
...
env = [f'# F7 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
