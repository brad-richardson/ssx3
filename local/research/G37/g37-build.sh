#!/bin/sh
# G37: configure + build (minimum-content-threshold-wall parallel-gs-replayer for Android arm64).
# NEW build dir (all older dirs untouched). Host builds -j2.
# Recipe = G22/G26/G28/G29/G30/G31/G32/G33/G34/G35/G36 recipe (NON-sanitizer: no ANDROID_STL override, no
...
# + G37 pre-vsync() RAW-ACCESS shrunken-tag (1-word) placement hunk in gs/gs_interface.cpp
# (GSInterface::vsync, pre-call, map_vram_read->temp + single-word OR-mask
# temp[0] |= 0x00FFFFFF + RAW renderer.begin_host_vram_access()-back with
# NEITHER map_vram_write NOR end_vram_write + G37: writeback receipt; G36
# block excised in the same edit).
# Receipts: configure exit 0, build exit 0 [458/458], binary size/sha/BuildID
# in REPORT.md, G37: x3, G31: x1/x2, G30: x1/x1, G29: x1/x2, G28: x1, G26: x1.
set -e
export COPYFILE_DISABLE=1
CLONE="/Volumes/Extreme SSD/parallel-gs-g7"
BUILD="/Volumes/Extreme SSD/parallel-gs-g37-android-build"
