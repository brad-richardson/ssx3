...
def observer(env,dest):
    library=os.environ.get('E29_OBSERVER_DYLIB')
    if library:
...
def suite(label):
    # E29 CHANGE 1: the suite binary lives in a PROTECTED build tree that the
...
def validate(label,binary,modes,expect_missing=False):
    ...
    for mode in modes:
        dest=P/'e29-fixtures'/label/mode;dest.mkdir(parents=True,exist_ok=False)
        ...
        if mode=='prior':
            ...
            baseline=False  # E29 requires E18's delivered behavior at baseline
        ...
        if '# E7 SHUTDOWN' in taps:
            from e29_closed_events import tap
    ...
    print('# E29 VALIDATION TAIL COMPLETE')
    return result
