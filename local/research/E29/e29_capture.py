#!/usr/bin/env python3
"""E29 demand-watch capture driver. Fresh T13 pre-claims, caps, owned lease cleanup. One boot."""
import argparse
import datetime
import hashlib
import json
...
sys.path.insert(0, str(REPO / 'local/research/E29'))
from e29_common import sample as resource_sample, bound as resource_bound, admission, size as owned_size
from e29_validate import suite as run_suite
EVIDENCE = REPO / 'local/research/E29'
W = Path('/Volumes/Extreme SSD/ps2recomp-spike')
FORK = W / 'PS2Recomp'
RUN = W / 'P1/run'
# E29 CHANGE 5a, declared: E15-E28 all booted ONE binary at the mainline path
# and pinned it by SHA. E29 exists to boot its OWN build, so this is the one
# driver value that CANNOT be carried. The mainline pair is kept beside it and
# still pinned, so every receipt names both and the difference is explicit.
BIN = Path('/Volumes/Extreme SSD/ps2recomp-spike/e29-movie-bypass-build/ps2xRuntime/ps2EntryRunner')
TEST = Path('/Volumes/Extreme SSD/ps2recomp-spike/e29-movie-bypass-build/ps2xTest/ps2x_tests')
MAINLINE_BIN = Path('/tmp/e18-mpeg-link/runtime/ps2xRuntime/ps2EntryRunner')
MAINLINE_TEST = Path('/tmp/e18-mpeg-link/runtime/ps2xTest/ps2x_tests')
ELF = W / 'P1/cd/SLUS_207.72'
LEASE = Path('/tmp/ssx3-p-lane-lease')
WAITS = RUN / 'e29-waits.log'
...
# E29 step 4: the guest producer/source structures the E7 tap named on this exact
...
# E29 CHANGE 2: the TIERED extended watch set (BOOT-DESIGN.md, watch-set.json).
...
def preflight(label):
    ...
    if LEASE.exists():
        append_wait(f'{label.upper()} STOP {utc()} owner={LEASE.read_text().strip()!r}; E29 forbids lease wait')
        raise RuntimeError('Lease occupied; E29 tables and stops without waiting')
    ...
    # E29 CHANGE 5d: label-derived, not pinned to phase 'a'. Both boots run the
    # same binary, but a manifest hardcoded to one label would silently accept
    # the other phase's gate.
    manifest_path=EVIDENCE/f'{label}-build.json'
    assert manifest_path.exists(), 'fresh E29 probe-gate manifest required'
    ...
    # E29 CHANGE 1: PRESENCE first. e23a's driver went straight to .stat() and
    # would raise FileNotFoundError deep inside the loop; E29 opened to exactly
    ...
    if absent:
        raise RuntimeError('required binaries absent; E29 tables and stops '
...
def main():
    ...
    parser.add_argument('phase',choices=['a','b'])  # E29 CHANGE 5b: a=flag OFF, b=flag ON
    ...
    # E29 CHANGE 5c: the DEV-ONLY bypass flag. Absent by default; the driver
    # scrubs every inherited PS2X_* above, so the flag can ONLY be present
    # because it was asked for here, and it is recorded in the boot config.
    parser.add_argument('--skip-movie',action='store_true',
                        help='set PS2X_SKIP_MOVIE=1 for this boot (DEV-ONLY bypass)')
    ...
    label='e29'+args.phase
    # E29 CHANGE 5e, declared: the brief allows UP TO TWO title boots (flag-OFF
    # equivalence, then flag-ON), one at a time, and "no third boot for any
    # reason". The carried guard allowed exactly one. It becomes a per-label
    # sentinel with a HARD CAP OF TWO -- strictly a cap, never a licence: a
    # repeat of the SAME label is still refused, and a third label is refused
    # outright no matter which labels came before.
    _attempts = sorted(EVIDENCE.glob('boot-attempt-*.json'))
    assert not (EVIDENCE/f'boot-attempt-{label}.json').exists(), f'{label} already booted'
    assert len(_attempts) < 2, ('E29 permits AT MOST TWO title boots; %d already spent: %s'
                                % (len(_attempts), [a.name for a in _attempts]))
    # the second carried one-boot guard, same change: results are per label,
    # and at most two may ever exist.
    _results = sorted(EVIDENCE.glob('e29?-result.json'))
    assert not (EVIDENCE/f'{label}-result.json').exists(), f'{label} already has a result'
    assert len(_results) < 2, ('E29 permits AT MOST TWO title boots; results already: %s'
                               % [r.name for r in _results])
    assert args.report_all, 'E29 requires REPORT_ALL=1'
    ...
    for key in inherited:
        del env[key]
    # E29 CHANGE 3: PS2X_DIAG_SEMA / _S0 are already compiled into the proven
    ...
    assert not any('stdbuf' in x for x in boot_argv), 'E29 forbids stdbuf in the boot argv'
    config=dict(label=label,argv=boot_argv,cwd=str(RUN),
                ...
                skip_movie_flag=env.get('PS2X_SKIP_MOVIE'),
                skip_movie_requested=bool(args.skip_movie),
                binary_is_bypass_build=str(BIN).endswith('e29-movie-bypass-build/ps2xRuntime/ps2EntryRunner'),
    ...
    try:
        with (EVIDENCE/f'{label}-liveness.txt').open('w') as live, paths['boot_log'].open('wb') as log:
            ...
            (EVIDENCE/f'boot-attempt-{label}.json').write_text(json.dumps(dict(utc=utc(),label=label,lease_token=token,argv=config['argv']),indent=2)+'\n')
            ...
            # E29 CHANGE 4: record span-complete on EVERY path, not only when a
