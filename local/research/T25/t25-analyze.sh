#!/usr/bin/env bash
# T25 Phase 2 trace analysis on the two-press emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t25-census.txt
python3 $D/t4-sample.py $E $D/t25-samples.txt
ls -la $D/t25-census.txt $D/t25-samples.txt
date -u
