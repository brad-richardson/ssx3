#!/usr/bin/env bash
# T25 Phase 1 trace analysis on the no-input NVM-verify emulog. Read-only, streaming.
set -x
...
python3 $D/t4-census.py $E $D/t25-nvm-census.txt
python3 $D/t4-sample.py $E $D/t25-nvm-samples.txt
ls -la $D/t25-nvm-census.txt $D/t25-nvm-samples.txt
date -u
