#!/usr/bin/env python3
"""E55D14 Part 2A: ONE bounded Load-game path-observation boot (prepared, NOT run).

DO NOT RUN until the orchestrator reviews the exact script SHA and explicitly
releases Part 2B in the worker pane. Single Mac mini boot only after release.

Route (PS2X_PAD_SCRIPT_CLOCK=vsync; guest ms = tick * 100000 / 5994):
identical to E55D12's proved route (E55D12 REPORT section 2, ORCH-GATE-P2 A):
...
Card-API observation: the pinned E55D14P1B diagnostic runner carries the
ps2_e55d3_pad_card_probe.h taps (GetDir status / getdirpath sibling /
Read/pad). This script sets PS2X_PAD_CARD_PROBE=<lane>/probe.log so every
sceMcGetDir / sceMcRead / scePadRead call is recorded (getdir/getdirpath/
mcread/pad lines + 16 MiB hard cap). Fresh empty card roots only (refuses
non-empty at start). Early title/menu GetDir calls (E55D12 saw 4 at vsync
118/122/126/223, all ok=0 reason=empty) are separated from choice-triggered
calls at/after C2 tick 1700 by vsync. Each GetDir status row has a
getdirpath sibling row carrying escaped raw query, normalized query,
parent, pattern and host; the sibling shares seq and carries its own
per-family ordinal pord. Pairing is by per-family ordinal/tick/port/slot/max
only: the sibling shifts shared seq, so old global seq numbers are NOT
compared.

Stop only after tick >= 1800 AND a persisted snap PNG tagged >= 1800
(frame_unproven/OTHER if the grace expires without the proof).
Stop early on any unexpected screen (title/menu/submenu not reached,
ambiguous/unreadable frame, script/input mismatch -> OTHER).

Method (adapted from E55D12's e55d12_boot.py, read-only source):
  per-run cwd (~/dev/ssx3-work/E55D14P2/run/<label>, reserved for the released
  run only), fresh empty card roots, one mini P-lane slot held only during
  the boot, wall cap 500 s, no-tick-progress cap 120 s, frame-proof grace
  100 s (500 + 100 = 600 s worst-case active boot: never over 600 s without
  explicit orchestrator release), boot log cap 16 MiB (closed log). Kill only
  the recorded PID; release the lease even on failure. Pre-boot checks: fork
  at exact 80777cb36d5f84c1cd89122e8074afd48fe78b8e, no tracked
  modification, no unexpected untracked path (untracked .work/ build tree
  allowed), runner-dir guard vs upstream 14b1e5cb, and two matching SHA reads
  of runner/ISO/ELF/codegen against the pins below. prepare_lane() refuses
...
Usage: e55d14_boot.py --label S1          (ONE boot only, after release)
       e55d14_boot.py --self-check        (pure helper tests, no boot/lease)
"""
import argparse
import hashlib
import json
...
WORK = Path('/Users/brad/dev/ssx3-work/E55D14P2')
FORK = Path('/Users/brad/dev/ssx3-work/E55D14P1/PS2Recomp')
RUNNER = Path('/Users/brad/dev/ssx3-work/E55D14P1/PS2Recomp/.work/build/ps2xRuntime/ps2EntryRunner')
...
FORK_PIN_FULL = '80777cb36d5f84c1cd89122e8074afd48fe78b8e'
FORK_PIN_SHORT = '80777cb'
UPSTREAM_RUNNER_BASE = '14b1e5cb'
RUNNER_SHA = 'd8fa114d824a277592558425dd91357bcf2f75d0d09390a5680c41ba6002ef04'
...
def untracked_allowed(relpath):
    """Untracked paths the fork precheck tolerates: the .work/ build tree."""
    return relpath == '.work/' or relpath.startswith('.work/')


def status_errors(status_text):
    """Pure fork-cleanliness check: tracked mods and unexpected untracked.

    Takes `git status --porcelain` text; returns a list of error strings
    (empty means the tree holds only the allowed untracked .work/ tree).
    """
    errors = []
    for line in status_text.splitlines():
        ...
        if line.startswith('??'):
            ...
            if not untracked_allowed(relpath):
                errors.append('unexpected untracked path: %s' % relpath)
        else:
            errors.append('tracked modification: %s' % line)
    return errors


def precheck():
    """Verify fork pin, tree cleanliness (.work/ only), runner-dir guard, SHAs."""
    errors = []
    ...
    if status.returncode != 0:
        errors.append('fork status failed: rc=%d out=%r' % (status.returncode, status.stdout.strip()[:200]))
    else:
        errors.extend(status_errors(status.stdout))
...
def route_self_check():
    ...
    record('caps_frame_proof_grace_100', FRAME_PROOF_GRACE_S == 100,
           str(FRAME_PROOF_GRACE_S))
    record('caps_wall_plus_grace_le_600', WALL_CAP_S + FRAME_PROOF_GRACE_S <= 600,
           'wall=%d grace=%d sum=%d' % (WALL_CAP_S, FRAME_PROOF_GRACE_S,
                                        WALL_CAP_S + FRAME_PROOF_GRACE_S))

    record('precheck_allows_work_tree_only',
           status_errors('?? .work/\n') == [] and status_errors('') == [],
           'clean + .work/ pass')
    record('precheck_rejects_tracked_mod',
           status_errors('M  ps2xRuntime/src/lib/Kernel/Stubs/MemoryCard.cpp\n') != [],
           'staged tracked mod rejected')
    record('precheck_rejects_unstaged_tracked_mod',
           status_errors(' M ps2xRuntime/include/ps2_e55d3_pad_card_probe.h\n') != [],
           'unstaged tracked mod rejected')
    record('precheck_rejects_unexpected_untracked',
           status_errors('?? notes.txt\n') != [] and
           status_errors('?? .work/\n?? scratch/\n') != [],
           'stray untracked rejected alongside .work/')

    with tempfile.TemporaryDirectory(prefix='e55d14-lane-self-') as tmp:
        lane = Path(tmp) / 'S1'
    ...
    with tempfile.TemporaryDirectory(prefix='e55d14-proof-self-') as tmp:
        snap = Path(tmp) / 'snap'
...
def main():
    ...
    slot = claim('E55D14P2-' + args.label)
    while slot is None:
