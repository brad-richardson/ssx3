#!/usr/bin/env bash
# AU8 capture (from AU6 pcsx2_cap.sh): T48 route to Select Character; tag-1 + SPU2 tap in guest time.
set -x
...
rm -f $T4/pcsx2-au4.pid $T4/pcsx2-tag1.bin $T4/spu2-tap.bin
T_BOOT=$(date -u +%s)
echo "T_BOOT_WALL:$T_BOOT T_BOOT_UPTIME:$(cut -d. -f1 /proc/uptime)"
mkdir -p $DAT $T4/logs; PCSX2_AU4_CAPTURE=1 PCSX2_AU8_DIR=$T4 nohup $BIN -nogui -slowboot -turbo -datapath $DAT -logfile $T4/logs/boot-$TAG.log -- "$R/inputs/SSX 3 (USA).iso" > $T4/logs/boot-$TAG.stdout 2>&1 &
...
# Stay at Select Character for 6000 tag records (64 s of 36 kHz PCM).
for W in $(seq 1 200); do
  wallcheck
  SZ=$(stat -c %s $T4/pcsx2-tag1.bin 2>/dev/null || echo 0)
  N=$((SZ / 1572))
  plog "AU8_RECORDS=$N SC_RECORDS=$SC_RECORDS spu=$(stat -c %s $T4/spu2-tap.bin 2>/dev/null) wall=$(($(date -u +%s)-T_BOOT))"
  [ $N -ge $((SC_RECORDS + 6000)) ] && break
  sleep 2
done
[ $N -ge $((SC_RECORDS + 6000)) ] || fail TOO_SHORT
plog "AU8_CAPTURE_DONE records=$N spu=$(stat -c %s $T4/spu2-tap.bin)"
