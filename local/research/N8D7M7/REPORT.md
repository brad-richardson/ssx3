...
| --- | --- | --- |
| **W** source sparse for the original Present command as submitted | `buffers.gpu` as captured by the submitted scanout cmd holds little FBP112 content | S1 sparse (≤100) AND S2same sparse AND Gc sparse; input==oracle 448/448 on each staging; staging SHAs match each other (both sparse) and differ from Mac broad SHA. Scope limit: this proves sparseness **of the submitted command only** — missing draw execution vs late submission (draws recorded after the step-6 flush that the original Present never executed) remains unresolved while O has no execution witness |
