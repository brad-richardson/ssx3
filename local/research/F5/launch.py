#!/usr/bin/env python3
"""F5 Odin ABBA (1x pipelined vs 4x+hi-res pipelined): F4 launcher adapted for the F5
fold build (fork a3efbfe). Differences from F4: --variant A|B (run knobs pushed per
run: A = PRESENT_PIPELINE=1; B = SSAA=4 + HIRES_SCANOUT=1 + PRESENT_PIPELINE=1);
transport recorded per run (Wi-Fi serial) with disconnect counting; BACK only when
the app is not the focused window (no blind BACK over Wi-Fi — no USB dialog exists).
Usage: launch.py --label R1 --variant A [--wall 600] [--stop-tick 4500]
...
Cool-down (thermal 0, 180 s wait, thermal 0 at launch) is done by cooldown.py
BEFORE this runs; this script only records the pre-launch thermal line.
"""
...
ap.add_argument('--variant', choices=('A', 'B'), required=True,
                help='F5: A = 1x pipelined, B = 4x+hi-res pipelined')
a = ap.parse_args()

OUT = os.path.expanduser(f'/Users/brad/dev/ssx3/local/research/F5/logs/{a.label}')
SCR = os.path.expanduser(f'/Users/brad/dev/ssx3-work/F5/odin/{a.label}')
...
if not (lease.startswith('LEASE_FREE') or lease.startswith('F5-')):
...
def _cleanup():
    ...
    try:
        if CLAIMED['v']:
            subprocess.run(['adb', '-s', D, 'shell', "echo 'LEASE_FREE F5 done' > /data/local/tmp/mg/LEASE"],
                           capture_output=True, timeout=30)
    except Exception:
        pass
...
env = [f'# F5 {a.label} variant {a.variant}', 'PS2X_GS_BACKEND=parallel', 'PS2X_GS_TURNIP=1',
...
while True:
    ...
    if a.profile_after_tick and not profiled and tick >= a.profile_after_tick:
        ...
        out = sh(f'simpleperf record --call-graph fp --app {PKG} -o {DSCRAP}/perf-{a.label}.data --duration {a.profile_secs}',
...
sh("echo 'LEASE_FREE F5 done' > /data/local/tmp/mg/LEASE")
CLAIMED['v'] = False
