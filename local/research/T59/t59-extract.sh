#!/usr/bin/env bash
# T59 extract: ebw (+vaddr) + ebwlast + ebwend + caps + T57 lines + markers.
set -e
E=${1:-/home/brad/pcsx2-g7/dat-t57/PCSX2/logs/emulog.txt}
OUT=${2:-/home/brad/pcsx2-t4/t59-trace.txt}
grep -E 'ebw vsync=|ebwlast vsync=|ebwend vsync=|T58_CAP|T58_CAPLAST|vu0call vsync=|vif0op vsync=|T57_CAP|T51C_WINDOW|T51C_CAP|T48_PATHS' "$E" | sed 's/^[^]]*] //' > "$OUT" || true
